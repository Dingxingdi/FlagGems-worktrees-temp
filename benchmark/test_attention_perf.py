...
def test_perf_grouped_query_attention_gqa(is_causal):
    """Benchmark for Grouped_Query_Attention_GQA operator."""

    def gqa_kwargs(shape, dtype, device):
        # shape is (batch, num_q_heads, seq_len, head_size)
        batch, num_q_heads, seq_len, head_size = shape
        # For GQA, num_kv_heads < num_q_heads
        num_kv_heads = num_q_heads // 4  # Use 1/4 of q heads for kv
        key_shape = (batch, num_kv_heads, seq_len, head_size)
        query = torch.randn(shape, device=device, dtype=dtype)
        key = torch.randn(key_shape, device=device, dtype=dtype)
        value = torch.randn(key_shape, device=device, dtype=dtype)
        yield query, key, value, None, 0.0, is_causal

    def torch_gqa_ref(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False):
    ...
    # Define shapes for GQA: (batch, num_q_heads, seq_len, head_size)
    shapes = [
        (1, 4, 512, 64),
        (1, 8, 1024, 64),
        (1, 16, 1024, 128),
    ...
    bench.set_gems(flag_gems.ops.Grouped_Query_Attention_GQA)
    bench.run()
