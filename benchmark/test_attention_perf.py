...
# In MQA, key and value have 1 head while query can have multiple heads


@pytest.mark.Multi_Query_Attention_MQA
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_perf_multi_query_attention_mqa(dtype):
    """Benchmark for Multi-Query Attention (MQA).
    ...
    def mqa_kwargs(shape, dtype, device):
        ...
        kv_head = 1
        query = torch.randn(shape, device=device, dtype=dtype)
        key = torch.randn((batch, kv_head, seq_len, head_size), device=device, dtype=dtype)
        value = torch.randn((batch, kv_head, seq_len, head_size), device=device, dtype=dtype)
        scale = float(1.0 / math.sqrt(head_size))
        yield query, key, value, None, 0.0, False, scale

    def torch_mqa_ref(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None):
