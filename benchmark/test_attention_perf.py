...
def test_perf_group_query_attention(is_causal):
    ...
    def gqa_kwargs(shape, dtype, device):
        ...
        # num_kv_head = num_q_head // 2 for GQA
        num_kv_head = num_q_head // 2
        query = torch.randn(shape, device=device, dtype=dtype)
        ...
        # Match the pattern: yield 6 items like scaled_dot_product_attention
        # (query, key, value, attn_mask, dropout_p, is_causal)
        # For GQA, key and value have different num_heads, but we yield them anyway
        yield query, key, value, None, 0.0, is_causal

    # Reference: scaled_dot_product_attention with enable_gqa
    def torch_gqa_ref(
