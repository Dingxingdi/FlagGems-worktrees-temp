...
def torch_sdpa_backward(q, k, v, scale, is_causal, dropout_p=0.0):
    """PyTorch reference: forward + backward for timing comparison."""
    q.requires_grad_(True)
    k.requires_grad_(True)
    v.requires_grad_(True)
    out = torch.nn.functional.scaled_dot_product_attention(
        q, k, v, scale=scale, dropout_p=dropout_p, is_causal=is_causal
    )
...
def gems_sdpa_backward(q, k, v, scale, is_causal, dropout_p=0.0):
    """FlagGems implementation: forward + backward for timing comparison."""
    q.requires_grad_(True)
    k.requires_grad_(True)
    v.requires_grad_(True)
...
class SDPABackwardBenchmark(AttentionBenchmark):
    """
    benchmark for attention backward (full forward + backward)
    """
...
def test_perf_scaled_dot_product_cudnn_attention_backward(is_causal):
    """Benchmark for scaled_dot_product_cudnn_attention_backward operator.

    This tests the full forward + backward pass which triggers the backward operator.
    """
    ...
    def sdpa_backward_kwargs(shape, dtype, device):
        ...
        value = torch.randn(shape, device=device, dtype=dtype)
        scale = float(1.0 / math.sqrt(head_size))
        yield query, key, value, scale, is_causal, 0.0

    bench = SDPABackwardBenchmark(
        op_name="scaled_dot_product_cudnn_attention_backward",
        input_fn=sdpa_backward_kwargs,
        torch_op=torch_sdpa_backward,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(gems_sdpa_backward)
    bench.run()
