...
@pytest.mark.parametrize("dropout_p", [0.0])
@pytest.mark.parametrize("is_causal", [True, False])
def test_perf_scaled_dot_product_flash_attention_for_cpu(dropout_p, is_causal):
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"
    ...
    def scaled_dot_product_flash_attention_kwargs(shape, dtype, device):
        ...
        key = torch.randn(shape, device=device, dtype=dtype)
        value = torch.randn(shape, device=device, dtype=dtype)
        yield query, key, value, dropout_p, is_causal, None, None

    def torch_sdpa_flash_attention_for_cpu(
        query, key, value, dropout_p, is_causal, attn_mask, scale
    ):
        # Use standard SDPA as reference since _scaled_dot_product_flash_attention_for_cpu is CPU-only
    ...
    bench = AttentionBenchmark(
        ...
        torch_op=torch_sdpa_flash_attention_for_cpu,
        dtypes=[
            torch.float16,
            torch.bfloat16,
        ],
    )
    bench.set_gems(flag_gems.scaled_dot_product_flash_attention_for_cpu)
    bench.run()
    if flag_gems.vendor_name == "hygon":
        del os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"]
