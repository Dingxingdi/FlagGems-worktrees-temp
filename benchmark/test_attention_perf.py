...
def test_perf_scaled_dot_product_efficient_attention(dropout_p, is_causal):
    """Benchmark for _scaled_dot_product_efficient_attention"""

    def scaled_dot_product_efficient_attention_kwargs(shape, dtype, device):
        # shape: (batch, num_heads, seq_len, head_size)
        query = torch.randn(shape, device=device, dtype=dtype)
        key = torch.randn(shape, device=device, dtype=dtype)
        value = torch.randn(shape, device=device, dtype=dtype)
        scale = float(1.0 / math.sqrt(shape[-1]))
        yield query, key, value, None, True, dropout_p, is_causal, scale

    def torch_efficient_attention(
        query, key, value, attn_bias, compute_log_sumexp, dropout_p, is_causal, scale
    ):
    ...
    bench = AttentionBenchmark(
        ...
        input_fn=scaled_dot_product_efficient_attention_kwargs,
    ...
    bench.set_gems(flag_gems.ops.scaled_dot_product_efficient_attention)
    bench.run()
