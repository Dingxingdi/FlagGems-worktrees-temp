...
def flash_attention_v2_input_fn(config, dtype, device):
    """Input function for FlashAttention_V2 benchmark.

    Config: (batch, num_head, num_head_k, q_seq_len, kv_seq_len, head_size)
    Input format: (batch, num_head, seqlen, head_dim) - standard attention format
    """
    (
        batch,
        num_head,
    ...
    # Create input in (batch, num_head, seqlen, head_dim) format
    q = torch.empty(
        (batch, num_head, q_seq_len, head_size), device=device, dtype=dtype
    ).uniform_(-0.05, 0.05)
    k = torch.empty(
        (batch, num_head_k, kv_seq_len, head_size), device=device, dtype=dtype
    ).uniform_(-0.05, 0.05)
    v = torch.empty(
        (batch, num_head_k, kv_seq_len, head_size), device=device, dtype=dtype
    ).uniform_(-0.05, 0.05)

    # Yield each tensor separately to be unpacked as arguments
    yield q, k, v


def torch_flash_attention_v2(q, k, v):
...
def gems_flash_attention_v2(q, k, v):
    """GEMS implementation of FlashAttention_V2."""
    return flag_gems.FlashAttention_V2(q, k, v, 0.0, False)


class FlashAttentionV2Benchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = []
        # Test various batch/head/seq_len/head_size combinations
        configs = [
            (1, 1, 1, 128, 128, 64),
            (4, 8, 8, 512, 512, 64),
            (4, 8, 8, 1024, 1024, 128),
...
@pytest.mark.skipif(vendor_name == "hygon", reason="RuntimeError")
def test_perf_flash_attention_v2():
    bench = FlashAttentionV2Benchmark(
        op_name="FlashAttention_V2",
        input_fn=flash_attention_v2_input_fn,
        torch_op=torch_flash_attention_v2,
        dtypes=[torch.float16, torch.bfloat16],
    )
    bench.set_gems(gems_flash_attention_v2)
    bench.run()
