...
# Note: This operator passes through to PyTorch's cuDNN implementation
# Since it's not registered in FlagGems' _FULL_CONFIG, there's no custom implementation


def torch_cudnn_attention_backward(
    grad_out, query, key, value, out, logsumexp, philox_seed, philox_offset,
    attn_bias, cum_seq_q, cum_seq_k, max_q, max_k, dropout_p, is_causal
):
...
def gems_cudnn_attention_backward(
    grad_out, query, key, value, out, logsumexp, philox_seed, philox_offset,
    attn_bias, cum_seq_q, cum_seq_k, max_q, max_k, dropout_p, is_causal
):
    return flag_gems.ops._scaled_dot_product_cudnn_attention_backward(
        grad_out, query, key, value, out, logsumexp, philox_seed, philox_offset,
        attn_bias, cum_seq_q, cum_seq_k, max_q, max_k, dropout_p, is_causal
    )
...
def test_perf_cudnn_attention_backward(dtype):
    """Benchmark for _scaled_dot_product_cudnn_attention_backward operator."""
    def backward_kwargs(shape, dtype, device):
        ...
        yield grad_out, query, key, value, out, logsumexp, rest[4], rest[5], None, None, None, seq_len, seq_len, 0.0, False

    bench = AttentionBenchmark(
        op_name="cudnn_attention_backward",
        input_fn=backward_kwargs,
        torch_op=torch_cudnn_attention_backward,
        dtypes=[dtype],
    )
    bench.set_gems(gems_cudnn_attention_backward)
    bench.run()
