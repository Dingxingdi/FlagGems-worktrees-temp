...
# Benchmark for page_attention operator
@pytest.mark.page_attention
...
def test_perf_page_attention(
    batch_size, num_heads, num_kv_heads, seqlen_q, head_dim, block_size, dtype
):
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"

    max_context_length = 128
    num_blocks = (max_context_length + block_size - 1) // block_size

    def page_attention_kwargs(shape, dtype, device):
        # shape is (batch_size, num_heads, seqlen_q, head_dim)
        query = torch.randn(shape, device=device, dtype=dtype)
        ...
        block_table = torch.randint(0, num_blocks, (batch_size, num_blocks), dtype=torch.int32, device=device)
        context_lengths = torch.randint(1, max_context_length + 1, (batch_size,), dtype=torch.int32, device=device)
        yield query, key_cache, value_cache, block_table, context_lengths, max_context_length, None, False

    # For page_attention, we use SDPA as reference
    def torch_page_attention_ref(
        query, key_cache, value_cache, block_table, context_lengths, max_context_length, scale, is_causal
    ):
        try:
            # Expand paged KV to full format and use SDPA
            batch_size = query.shape[0]
            num_kv_heads = key_cache.shape[2]
            head_dim_val = key_cache.shape[3]

            # Handle different number of query heads vs kv heads (GQA)
            num_heads = query.shape[1]

            # Expand paged KV to full format
            key_full = torch.zeros(batch_size, num_kv_heads, max_context_length, head_dim_val, dtype=dtype, device=query.device)
            value_full = torch.zeros(batch_size, num_kv_heads, max_context_length, head_dim_val, dtype=dtype, device=query.device)

            for b in range(batch_size):
                ctx_len = min(context_lengths[b].item(), max_context_length)
                max_page_idx = (ctx_len + block_size - 1) // block_size
                for i in range(ctx_len):
                    page_idx = i // block_size
                    if page_idx < max_page_idx and page_idx < block_table.shape[1]:
                        block_offset = i % block_size
                        physical_block = block_table[b, page_idx].item()
                        key_full[b, :, i, :] = key_cache[physical_block, block_offset, :, :]
                        value_full[b, :, i, :] = value_cache[physical_block, block_offset, :, :]

            # For GQA, we need to expand key/value to match query heads
            if num_heads != num_kv_heads:
                # Repeat key/value across query heads
                num_groups = num_heads // num_kv_heads
                key_full = key_full.repeat_interleave(num_groups, dim=1)
                value_full = value_full.repeat_interleave(num_groups, dim=1)

            return torch.nn.functional.scaled_dot_product_attention(
                query, key_full, value_full, is_causal=is_causal
            )
        except Exception as e:
            # Return a dummy tensor if reference fails
            return torch.zeros_like(query)

    bench = AttentionBenchmark(
        op_name="page_attention",
        input_fn=page_attention_kwargs,
        torch_op=torch_page_attention_ref,
        dtypes=[dtype],
    )
    bench.set_gems(flag_gems.page_attention)
    bench.run()
