...
def test_perf_efficient_attention_backward(dropout_p, is_causal):
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"
    ...
    def sdpa_backward_kwargs(shape, dtype, device):
        ...
        # Yield the tensors and kwargs
        yield query, key, value

    def sdpa_backward_torch_op(query, key, value):
        # Forward pass
    ...
    def sdpa_backward_gems_op(query, key, value):
        # Forward pass using GEMS
        with flag_gems.use_gems():
            out = flag_gems.ops.scaled_dot_product_attention(
                query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False
            )
        # Backward pass
        grad_out = torch.randn_like(out)
    ...
    bench = AttentionBenchmark(
        ...
        torch_op=sdpa_backward_torch_op,
        dtypes=[
            torch.float16,
            torch.bfloat16,
        ],
    )
    bench.set_gems(sdpa_backward_gems_op)
    bench.run()
    if flag_gems.vendor_name == "hygon":
        del os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"]
