...
def torch_cudnn_attention_forward_ref(q, k, v, is_causal=False):
    return torch.nn.functional.scaled_dot_product_attention(
        q, k, v, is_causal=is_causal
    )


def gems_cudnn_attention_forward_ref(q, k, v, is_causal=False):
    return flag_gems.ops._cudnn_attention_forward(
        q, k, v, is_causal=is_causal
    )
...
def test_perf_cudnn_attention_forward():
    # Test with a few different shapes
    test_shapes = [
        (4, 8, 1024, 64),  # batch, num_heads, seq_len, head_size
        (4, 8, 1024, 128),
    ]
    ...
    for dtype in [torch.float16, torch.bfloat16]:
        for shape in test_shapes:
            ...
            q = torch.randn(batch, num_heads, seq_len, head_size, device="cuda", dtype=dtype)
            k = torch.randn(batch, num_heads, seq_len, head_size, device="cuda", dtype=dtype)
            v = torch.randn(batch, num_heads, seq_len, head_size, device="cuda", dtype=dtype)

            # Warmup
            for _ in range(10):
                _ = gems_cudnn_attention_forward_ref(q, k, v)
                _ = torch_cudnn_attention_forward_ref(q, k, v)
            ...
            print(f"Shape: {shape}, Dtype: {dtype}, Torch: {torch_time:.4f}ms, Gems: {gems_time:.4f}ms, Speedup: {speedup:.4f}")
