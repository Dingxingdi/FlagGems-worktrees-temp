...
class PagedAttentionBenchmark(GenericBenchmark):
    """
...
def paged_attention_input_fn(shape, dtype, device):
    """
    Generate input for paged attention benchmark.
    shape is a tuple: (batch_size, num_query_heads, num_kv_heads, query_len, kv_len, head_dim, block_size, num_blocks)
    The benchmark framework unpacks the shape tuple, so we receive each element individually.
    """
    # When called by benchmark, the shape tuple is unpacked and passed as separate arguments
    # So shape is actually a single integer (the first element of our shape tuple)
    # We need to use the shapes list directly instead

    # For this benchmark, we'll just return the test case based on index
    # Since this is complex, let's use a simpler approach and define the test case here
    raise NotImplementedError("Use direct benchmark call")


def torch_paged_attention_ref(query, key_cache, value_cache, query_lens, kv_lens, block_tables, scale):
...
def test_perf_Paged_Attention():
    """
    Direct benchmark for Paged_Attention without using the generic framework.
    """
    import time

    # Test shapes: (batch_size, num_query_heads, num_kv_heads, query_len, kv_len, head_dim, block_size, num_blocks)
    shapes = [
        (1, 4, 4, 512, 512, 128, 16, 64),
        (1, 8, 8, 1024, 1024, 128, 16, 128),
        (1, 16, 16, 2048, 2048, 128, 16, 256),
    ...
    for dtype in dtypes:
        print(f"\n=== Paged_Attention Performance Test (dtype={dtype}) ===")
        for shape in shapes:
            batch_size, num_query_heads, num_kv_heads, query_len, kv_len, head_dim, block_size, num_blocks = shape
            num_seqs = batch_size

            # Prepare inputs
            query_lens = [query_len // num_seqs] * num_seqs
            kv_lens = [kv_len // num_seqs] * num_seqs

            query = torch.randn(query_len, num_query_heads, head_dim, device=flag_gems.device, dtype=dtype)
            key_cache = torch.randn(num_blocks, block_size, num_kv_heads, head_dim, device=flag_gems.device, dtype=dtype)
            value_cache = torch.randn(num_blocks, block_size, num_kv_heads, head_dim, device=flag_gems.device, dtype=dtype)

            max_num_blocks_per_seq = (kv_len + block_size - 1) // block_size
            block_tables = torch.randint(0, num_blocks, (num_seqs, max_num_blocks_per_seq), dtype=torch.int32, device=flag_gems.device)

            scale = head_dim**-0.5

            # Warmup
            for _ in range(10):
                _ = flag_gems.ops.Paged_Attention(
            ...
            start = time.time()
            for _ in range(100):
                ref_out = torch_paged_attention_ref(
                    query, key_cache, value_cache, query_lens, kv_lens, block_tables, scale
                )
            torch.cuda.synchronize()
            torch_time = (time.time() - start) / 100 * 1000  # ms

            # Benchmark GEMS
            torch.cuda.synchronize()
            start = time.time()
            for _ in range(100):
                gems_out = flag_gems.ops.Paged_Attention(
                    query, key_cache, value_cache, query_lens, kv_lens, block_tables, scale
                )
            torch.cuda.synchronize()
            gems_time = (time.time() - start) / 100 * 1000  # ms

            speedup = torch_time / gems_time
            print(f"SUCCESS    {torch_time:.3f}    {gems_time:.3f}    {speedup:.3f}    [{shape}]")
