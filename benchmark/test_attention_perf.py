...
@pytest.mark.parametrize("dropout_p", [0.0])
@pytest.mark.parametrize("is_causal", [True, False])
def test_perf_scaled_dot_product_fused_attention_overrideable(dropout_p, is_causal):
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"

    def scaled_dot_product_fused_attention_kwargs(shape, dtype, device):
        query = torch.randn(shape, device=device, dtype=dtype)
        key = torch.randn(shape, device=device, dtype=dtype)
        value = torch.randn(shape, device=device, dtype=dtype)
        yield query, key, value, None, dropout_p, is_causal
    ...
    bench = AttentionBenchmark(
        op_name="_scaled_dot_product_fused_attention_overrideable",
        input_fn=scaled_dot_product_fused_attention_kwargs,
        torch_op=sdpa_flash,
        dtypes=[
            torch.float16,
            torch.bfloat16,
    ...
    bench.set_gems(flag_gems.ops._scaled_dot_product_fused_attention_overrideable)
    bench.run()
    if flag_gems.vendor_name == "hygon":
        del os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"]
