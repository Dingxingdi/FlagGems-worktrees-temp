...
# Special benchmark for result_type - it returns a dtype, not a tensor
# So we measure latency differently
@pytest.mark.result_type
@pytest.mark.parametrize(
    "dtype",
    FLOAT_DTYPES,
)
def test_result_type_perf(dtype):
    import flag_gems

    # Use a fixed shape for benchmarking
    shape = (1024, 1024)
    ...
    for _ in range(1000):
        _ = torch.result_type(inp1, inp2)
    torch.cuda.synchronize()
    torch_latency = (time.perf_counter() - start) * 1000 / 1000
    ...
    with flag_gems.use_gems():
        ...
        for _ in range(1000):
            _ = torch.result_type(inp1, inp2)
        torch.cuda.synchronize()
        gems_latency = (time.perf_counter() - start) * 1000 / 1000

    speedup = torch_latency / gems_latency
    print(
        f"result_type dtype={dtype} shape={shape}: torch={torch_latency:.4f}ms gems={gems_latency:.4f}ms speedup={speedup:.3f}"
    )
