...
def _masked_scale_input_fn(shape, cur_dtype, device):
    inp = torch.randn(shape, dtype=cur_dtype, device=device)
    mask = torch.randint(0, 2, shape, dtype=torch.uint8, device=device)
    scale = 2.0
    yield inp, mask, scale


class MaskedScaleBenchmark(Benchmark):
    """Benchmark for _masked_scale(self, mask, scale) operation."""

    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            for inp, mask, scale in _masked_scale_input_fn(shape, cur_dtype, self.device):
                yield inp, mask, scale
...
def test_accuracy__masked_scale_perf(dtype):
    bench = MaskedScaleBenchmark(
        op_name="_masked_scale", torch_op=torch._masked_scale, dtypes=[dtype]
    )
    bench.run()
