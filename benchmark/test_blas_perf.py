...
def scaled_grouped_mm_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    """Input function for scaled_grouped_mm benchmark."""
    from flag_gems.ops._scaled_grouped_mm import scaled_grouped_mm
    inp1 = torch.randn([b, m, k], dtype=cur_dtype, device=device)
    inp2 = torch.randn([b, k, n], dtype=cur_dtype, device=device)
    scale_a = torch.randn([b], dtype=torch.float32, device=device).abs() + 0.1
    scale_b = torch.randn([b], dtype=torch.float32, device=device).abs() + 0.1
    yield inp1, inp2, scale_a, scale_b
...
# Custom benchmark for scaled_grouped_mm


class ScaledGroupedMmBenchmark(BlasBenchmark):
    """Benchmark for scaled_grouped_mm"""

    def __init__(self, *args, input_fn, **kwargs):
        super().__init__(*args, input_fn=input_fn, **kwargs)
        self.input_fn = input_fn

    def get_input_iter(self, cur_dtype) -> Generator:
        # Use smaller batch size for scaled_grouped_mm
        for b, m, n, k in [(2, 64, 64, 64), (4, 128, 128, 128), (8, 256, 256, 256)]:
            yield from self.input_fn(b, m, n, k, cur_dtype, self.device, False)


@pytest.mark.scaled_grouped_mm
def test_perf_scaled_grouped_mm():
    """Performance benchmark for scaled_grouped_mm"""
    from flag_gems.ops._scaled_grouped_mm import scaled_grouped_mm

    bench = ScaledGroupedMmBenchmark(
        input_fn=scaled_grouped_mm_input_fn,
        op_name="scaled_grouped_mm",
        torch_op=scaled_grouped_mm,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
