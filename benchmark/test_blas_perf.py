...
def addmm__input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    # addmm_ is in-place: self = beta * self + alpha * (mat1 @ mat2)
    # self has shape (m, n), mat1 has shape (m, k), mat2 has shape (k, n)
    inp1 = torch.randn([m, k], dtype=cur_dtype, device=device)
    self_tensor = torch.randn([m, n], dtype=cur_dtype, device=device)
    if b_column_major:
        inp2 = torch.randn([n, k], dtype=cur_dtype, device=device)
        yield self_tensor, inp1, inp2.t(),
    else:
        inp2 = torch.randn([k, n], dtype=cur_dtype, device=device)
        yield self_tensor, inp1, inp2,
