...
class LinalgSolveExBenchmark(BlasBenchmark):
    """
    benchmark for linalg_solve_ex
    """
...
def test_linalg_solve_ex_benchmark():
    def linalg_solve_ex_input_fn(n, k, cur_dtype, device):
        # Create a positive definite matrix
        L = torch.randn([n, n], dtype=cur_dtype, device=device)
        A = L @ L.transpose(-2, -1) + torch.eye(n, dtype=cur_dtype, device=device)
        B = torch.randn([n, k], dtype=cur_dtype, device=device)
        yield A, B

    # Note: Only benchmark float32 as torch.linalg.solve on GPU doesn't support float16 well
    bench = LinalgSolveExBenchmark(
        input_fn=linalg_solve_ex_input_fn,
        op_name="linalg_solve_ex",
        torch_op=torch.linalg.solve,  # Use linalg.solve for baseline (different operator)
        dtypes=[torch.float32],  # Only float32 for now
    )
    bench.run()
