...
# Shapes for _scaled_mm benchmark - K must be divisible by 16
_SCALED_MM_SHAPES = [
    (128, 256, 128),
    (64, 128, 256),
    (128, 128, 256),
...
class ScaledMmBenchmark(Benchmark):
    """
    Benchmark for _scaled_mm (FP8 scaled matmul)
    """
    ...
    def set_shapes(self, shape_file_path=None):
        self.shapes = _SCALED_MM_SHAPES[:]

    def get_input_iter(self, cur_dtype) -> Generator:
        fp8_dtype = get_w8a8_block_fp8_dtype()
        if fp8_dtype is None:
            raise RuntimeError("_scaled_mm benchmark requires FP8 support")

        for m, n, k in self.shapes:
            # mat1 (self): row-major (M, K) with stride (K, 1)
            A = rand_fp8_tensor((m, k), self.device, fp8_dtype).contiguous()
            # mat2: column-major (K, N) with stride (1, K)
            # Create as (N, K) row-major, then transpose (no contiguous() - preserves stride)
            B = rand_fp8_tensor((n, k), self.device, fp8_dtype).t()
            # Scales
            scale_a = torch.rand(1, device=self.device, dtype=torch.float32)
            scale_b = torch.rand(1, device=self.device, dtype=torch.float32)

            yield A, B, scale_a, scale_b, {"out_dtype": torch.float16}
