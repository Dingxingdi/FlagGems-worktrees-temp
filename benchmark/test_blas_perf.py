...
class LinalgInvExBenchmark(Benchmark):
    """
    benchmark for linalg_inv_ex
    """

    DEFAULT_METRICS = DEFAULT_METRICS[:]
    DEFAULT_SHAPES = [16, 32, 64, 128, 256, 512, 1024]

    def __init__(self, *args, input_fn, **kwargs):
        # Force set shapes before super().__init__
        kwargs.setdefault('shapes', [16, 32, 64, 128, 256, 512, 1024])
        super().__init__(*args, **kwargs)
        self.input_fn = input_fn
    ...
    def set_shapes(self, shape_file):
        # Override to use fixed shapes instead of loading from yaml
        self.shapes = self.DEFAULT_SHAPES

    def get_input_iter(self, cur_dtype) -> Generator:
        for n in self.shapes:
            # shapes is now a list of integers, not tuples
            yield from self.input_fn(n, cur_dtype, self.device)


@pytest.mark.linalg_inv_ex
def test_linalg_inv_ex_benchmark():
    # Use basic Benchmark to avoid complex shape handling
    from benchmark.performance_utils import Benchmark

    class SimpleLinalgInvExBenchmark(Benchmark):
        DEFAULT_METRICS = DEFAULT_METRICS[:]
        DEFAULT_SHAPES = [16, 32, 64, 128, 256]

        def __init__(self, *args, input_fn, **kwargs):
            kwargs.setdefault('shapes', [16, 32, 64, 128, 256])
            super().__init__(*args, **kwargs)
            self.input_fn = input_fn
            # Force shapes after init_user_config
            self.shapes = [16, 32, 64, 128, 256]
        ...
        def get_input_iter(self, cur_dtype):
            for n in self.shapes:
                ...
                result = self.input_fn(n, cur_dtype, self.device)
                yield (result,)

    def linalg_inv_ex_input_fn(n, cur_dtype, device):
        A = torch.randn([n, n], dtype=cur_dtype, device=device) * 0.1 + torch.eye(n, dtype=cur_dtype, device=device) * 2.0
        return A

    bench = SimpleLinalgInvExBenchmark(
        input_fn=linalg_inv_ex_input_fn,
        op_name="linalg_inv_ex",
        torch_op=torch.linalg.inv_ex,
        dtypes=[torch.float32],
    )
    # Force shapes again after init_user_config
    bench.shapes = [16, 32, 64, 128, 256]
    bench.run()
