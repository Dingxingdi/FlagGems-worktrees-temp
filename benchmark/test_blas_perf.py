...
def baddbmm_input_fn_(b, m, n, k, cur_dtype, device, b_column_major):
    # For inplace operation, bias should not require grad to avoid autograd error
    inp1 = torch.randn([b, m, k], dtype=cur_dtype, device=device)

    if b_column_major:
        inp2 = torch.randn(
            [b, n, k], dtype=cur_dtype, device=device
        )
        inp2 = inp2.transpose(1, 2).contiguous()
    else:
        inp2 = torch.randn(
            [b, k, n], dtype=cur_dtype, device=device
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "baddbmm_",
            lambda bias, batch1, batch2: bias.baddbmm_(batch1, batch2),
            baddbmm_input_fn_,
            BaddbmmBenchmark,
            marks=pytest.mark.baddbmm_,
        ),
    ],
)
def test_blas_benchmark(op_name, torch_op, input_fn, bench_cls):
    if flag_gems.vendor_name == "mthreads" and op_name not in ("baddbmm", "baddbmm_"):
