...
class WeightInt4packMmBenchmark(BlasBenchmark):
    """
    ...
    def benchmark(self, *args, **kwargs):
        inp1, inp2 = args
        # Additional args: scales, zeros
        scales = torch.ones(inp2.shape[0], dtype=inp1.dtype, device=inp1.device)
        zeros = torch.zeros(inp2.shape[0], dtype=inp1.dtype, device=inp1.device)
        return flag_gems._weight_int4pack_mm_with_scales_and_zeros(inp1, inp2, scales, zeros)

    def get_tflops(self, op, *args, **kwargs):
        m, n, k = args[0].shape[0], args[1].shape[0], args[0].shape[1]
        return 2 * m * n * k


@pytest.mark.weight_int4pack_mm
def test_weight_int4pack_mm_benchmark():
    def weight_int4pack_mm_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
        inp1 = torch.randn([m, k], dtype=cur_dtype, device=device)
        if b_column_major:
            inp2 = torch.randn([n, k], dtype=cur_dtype, device=device)
            yield inp1, inp2.t()
    ...
    bench = WeightInt4packMmBenchmark(
        input_fn=weight_int4pack_mm_input_fn,
