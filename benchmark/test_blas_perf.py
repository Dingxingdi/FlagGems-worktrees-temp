...
class BlasBenchmark(Benchmark):
    ...
    def get_tflops(self, op, *args, **kwargs):
        ...
        # shape(m,n)(n,p) - sparse uses same formula as addmm
        # args: (input, mat1, mat1_meta, mat2)
        elif self.op_name == "_sparse_semi_structured_addmm":
            total_flops = (
                args[0].shape[0] * args[3].shape[1] * (args[1].shape[0] * 2 + 1)
            )
...
def sparse_semi_structured_addmm_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    # Note: b is ignored for this operation
    inp = torch.randn([m, n], dtype=cur_dtype, device=device)
    ...
    # Create meta tensor from mat1
    mat1_meta_tuple = torch._sparse_semi_structured_tile(mat1)
    mat1_meta = mat1_meta_tuple[0]
    yield inp, mat1, mat1_meta, mat2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            flag_gems._sparse_semi_structured_addmm,
            sparse_semi_structured_addmm_input_fn,
            BlasBenchmark,
            marks=pytest.mark.sparse_semi_structured_addmm,
        ),
        pytest.param(
            "bmm",
            torch.bmm,
