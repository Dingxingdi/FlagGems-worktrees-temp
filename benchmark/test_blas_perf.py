...
class LinalgSolveBenchmark(BlasBenchmark):
    """
    benchmark for linalg_solve
    """

    def set_more_shapes(self):
        # (batch, n, n, k) - for linalg_solve, k is the number of RHS
        # Using simple shapes for benchmarking
        return [
            (1, 32, 32, 1),
            (1, 64, 64, 1),
            (1, 128, 128, 1),
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            b, n, n2, k = shape
            # Create invertible matrix A
            A = torch.randn(b, n, n2, dtype=cur_dtype, device=self.device)
            # Add diagonal to ensure invertibility
            A = A + torch.eye(n, dtype=cur_dtype, device=self.device).unsqueeze(0) * n
            # Create RHS B
            B = torch.randn(b, n2 if len(shape) > 3 else n, k if len(shape) > 4 else 1,
                          dtype=cur_dtype, device=self.device)
            yield A, B

    def get_tflops(self, op, *args, **kwargs):
        # Approximate FLOPs for LU solve: O(n^3) for factorization + O(n^2*k) for solve
        A_shape = args[0].shape
        B_shape = args[1].shape
        n = A_shape[-1]
        k = B_shape[-1]
        # 2/3 n^3 for LU + 2 n^2 k for solve
        return (2 * n**3 // 3 + 2 * n**2 * k)


@pytest.mark.linalg_solve
def test_linalg_solve_benchmark():
    def linalg_solve_input_fn(b, n, n2, k, cur_dtype, device, transposed=False):
        # Create invertible matrix A
        A = torch.randn(b, n, n2, dtype=cur_dtype, device=device)
        # Add diagonal to ensure invertibility
        A = A + torch.eye(n, dtype=cur_dtype, device=device).unsqueeze(0) * n
        # Create RHS B
        B = torch.randn(b, n2, k, dtype=cur_dtype, device=device)
        yield A, B

    bench = LinalgSolveBenchmark(
        input_fn=linalg_solve_input_fn,
