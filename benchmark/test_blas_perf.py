...
def int_mm_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    inp1 = torch.randint(-128, 127, [m, k], dtype=torch.int8, device=device)
    inp2 = torch.randint(-128, 127, [k, n], dtype=torch.int8, device=device)
    yield inp1, inp2
...
class IntMMBenchmark(Benchmark):
    """
    Benchmark for int8 matrix multiplication (_int_mm).
    """
    ...
    def set_shapes(self, shape_file_path=None):
        self.shapes = INT8_MNK_SHAPES[:]
        self.shape_desc = "M, N, K"

    def get_tflops(self, op, *args, **kwargs):
        # Integer matmul: count multiply-accumulate operations
        A, B = args[0], args[1]
        m, k = A.shape
        n = B.shape[1]
        return m * n * k * 2
...
@pytest.mark.int_mm
def test_perf_int_mm():
    bench = IntMMBenchmark(
        input_fn=int_mm_input_fn,
        op_name="int_mm",
        torch_op=torch._int_mm,
        dtypes=[torch.int8],
    )
    bench.set_gems(flag_gems._int_mm)
    bench.run()
