...
def linear_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    # linear: y = x @ weight.T + bias
    # input x: (m, k)
    # weight: (n, k) - same as addmm's mat2
    # bias: (n,)
...
class LinearBenchmark(BlasBenchmark):
    """
    ...
    def get_tflops(self, op, *args, **kwargs):
        # shape(m,k)(k,n)
        # total_flops = 2*m*k*n (matmul) + m*n (bias add)
