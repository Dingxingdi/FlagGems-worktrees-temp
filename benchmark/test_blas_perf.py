...
class MatrixPowerBenchmark(BlasBenchmark):
    """
    Benchmark for linalg_matrix_power
    """

    def set_more_shapes(self):
        # Use square matrix shapes only
        return [
            (1, 32, 32, 32),
            (1, 64, 64, 64),
            (1, 128, 128, 128),
    ...
    def get_tflops(self, op, *args, **kwargs):
        # Matrix power: A^n requires n-1 matrix multiplications
        # Each multiplication: m*m*m*2 flops
        # We use n=3 for benchmark, so 2 multiplications
        m = args[0].shape[0]
        n = 2  # n-1 multiplications
        total_flops = m * m * m * 2 * n
        return total_flops
...
def matrix_power_input_fn(b, m, n, k, cur_dtype, device, b_column_major):
    # linalg_matrix_power takes a square matrix and an integer n
    # m represents the matrix dimension (m x m)
    A = torch.randn([m, m], dtype=cur_dtype, device=device)
    n = 3  # Fixed exponent for benchmark
    yield A, n
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            matrix_power_input_fn,
            MatrixPowerBenchmark,
            marks=pytest.mark.linalg_matrix_power,
        ),
    ],
)
