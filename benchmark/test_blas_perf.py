...
# SVD Benchmark
class SvdBenchmark(GenericBenchmark2DOnly):
    """
    ...
    def get_tflops(self, op, *args, **kwargs):
        # SVD complexity is roughly O(min(M,N)^2 * max(M,N))
        m, n = args[0].shape
        k = min(m, n)
        # Approximate FLOPs for Jacobi-style SVD
        return m * n * k * 2


def svd_input_fn(m, n, cur_dtype, device):
    inp = torch.randn([m, n], dtype=cur_dtype, device=device)
    yield inp, {"full_matrices": False}


@pytest.mark.linalg_svd
def test_svd_benchmark():
    # Only use float32 since torch's reference doesn't support other dtypes well
    bench = SvdBenchmark(
        input_fn=svd_input_fn,
        op_name="linalg_svd",
        torch_op=torch._linalg_svd,
