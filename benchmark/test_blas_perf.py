...
def linalg_multi_dot_input_fn(shape, cur_dtype, device):
    # shape is a tuple like (10, 20, 30, 40) representing 3 matrices
    # Create tensors: (10x20), (20x30), (30x40)
    tensors = []
    for i in range(len(shape) - 1):
        mat = torch.randn(shape[i], shape[i + 1], dtype=cur_dtype, device=device)
        tensors.append(mat)
    # Yield as a tuple containing a list - this matches the expected format
    yield (tensors,)
...
def test_linalg_multi_dot_benchmark():
    for dtype in FLOAT_DTYPES:
        for shape in MULTI_DOT_SHAPES:
            # Create tensors on GPU
            tensors = []
            for i in range(len(shape) - 1):
                mat = torch.randn(shape[i], shape[i + 1], dtype=dtype, device='cuda')
                tensors.append(mat)

            # Warmup
            for _ in range(10):
                _ = torch.linalg.multi_dot(tensors)

            import time
            # Reference timing using torch.linalg.multi_dot on GPU
            start = time.time()
            for _ in range(100):
                ref_out = torch.linalg.multi_dot(tensors)
            torch_time = (time.time() - start) / 100

            # GEMS timing - use flag_gems.use_gems() context
            import flag_gems
            ...
            speedup = torch_time / gems_time if gems_time > 0 else 0
            print(f"dtype={dtype}, shape={shape}: torch={torch_time*1000:.3f}ms, gems={gems_time*1000:.3f}ms, speedup={speedup:.3f}")
