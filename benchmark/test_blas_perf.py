...
class CholeskySolveBenchmark(BlasBenchmark):
    """
    benchmark for cholesky_solve
    """
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            n, k = shape[0], shape[1]
            yield from self.input_fn(n, k, cur_dtype, self.device)


@pytest.mark.cholesky_solve
def test_cholesky_solve_benchmark():
    def cholesky_solve_input_fn(n, k, cur_dtype, device):
        # Create a symmetric positive-definite matrix A using float32
        # Cholesky decomposition in PyTorch doesn't support float16/bfloat16
        A_float = torch.randn(n, n, dtype=torch.float32, device=device)
        A_float = A_float @ A_float.T + torch.eye(n, dtype=torch.float32, device=device) * 1e-3

        # Get Cholesky decomposition in float32, then convert to target dtype
        L_float = torch.linalg.cholesky(A_float)
        L = L_float.to(cur_dtype)

        # Create right-hand side B
        B = torch.randn(n, k, dtype=cur_dtype, device=device)

        yield B, L

    # Shapes for cholesky_solve: (n, k)
    shapes = [
        (64, 32),
        (128, 64),
        (256, 128),
    ...
    bench = CholeskySolveBenchmark(
        ...
        dtypes=[torch.float32],  # Only test float32 as PyTorch doesn't support fp16/bf16 for cholesky_solve
    )
    bench.shapes = shapes
    bench.run()
