...
class LinalgCrossBenchmark(BlasBenchmark):
    """
    benchmark for linalg_cross
    """

    # Custom shapes for linalg_cross - need dimension of size 3
    CROSS_SHAPES = [
        (1024,),  # Single vector (treated as 1D with dim=-1)
        (1024, 3),  # 1024 3D vectors
        (4096, 3),  # 4096 3D vectors
        (16384, 3),  # Large batch
        (65536, 3),  # Very large batch
    ]
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            yield from self.input_fn(shape[0], cur_dtype, self.device)


@pytest.mark.linalg_cross
def test_linalg_cross_benchmark():
    def linalg_cross_input_fn(batch_size, cur_dtype, device):
        # linalg_cross requires the cross product dimension to have size 3
        shape = (batch_size, 3)
    ...
    bench = LinalgCrossBenchmark(
        input_fn=linalg_cross_input_fn,
        op_name="linalg_cross",
        torch_op=torch.linalg.cross,
        dtypes=FLOAT_DTYPES,
    )
    bench.shapes = LinalgCrossBenchmark.CROSS_SHAPES
    bench.run()
