...
class ConvolutionBenchmark(GenericBenchmark):
    ...
    def set_more_shapes(self):
        # Return shapes that pass the prod < 1024*1024 filter
        # Use smaller shapes for testing
        return [
            # Small 2D convolutions
...
def test_perf_convolution():
    def convolution_input_fn(shape, dtype, device):
        # Handle custom shapes only
        if len(shape) == 10:
            # Custom convolution shapes: (batch, input_c, input_h, input_w, out_c, kernel_h, kernel_w, stride, padding, groups)
            (
                batch,
                input_c,
                input_h,
