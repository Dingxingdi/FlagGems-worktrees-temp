...
class CudnnConvBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        return [
            (1, 3, 32, 32, 64, 3, 3, 1, 1, 1),
            (4, 8, 32, 32, 16, 3, 3, 1, 1, 1),
            (2, 16, 64, 64, 32, 3, 3, 2, 1, 1),
            (1, 32, 128, 128, 64, 3, 3, 1, 1, 2),
            (4, 64, 64, 64, 128, 3, 3, 2, 2, 2),
        ]


@pytest.mark.cudnn_convolution
def test_perf_cudnn_convolution():
    def cudnn_conv_input_fn(shape, dtype, device):
        (
            batch,
            input_c,
            input_h,
        ...
        # Convert padding/stride to tuple if they are ints
    ...
    bench = CudnnConvBenchmark(
        input_fn=cudnn_conv_input_fn,
        op_name="cudnn_convolution",
        torch_op=torch.cudnn_convolution,
        dtypes=[
            torch.float16,
            torch.float32,
        ],
