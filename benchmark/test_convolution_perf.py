...
class ConvolutionBackwardBenchmark(GenericBenchmark):
    ...
    def set_shapes(self, shape_file_path=None):
        # Override shapes directly, ignore YAML file
        self.shapes = [
            (2, 3, 16, 16, 8, 3, 3, (1, 1), (0, 0), 1),
            (4, 8, 8, 8, 16, 3, 3, (1, 1), (0, 0), 1),
            (1, 16, 32, 32, 32, 3, 3, (1, 1), (1, 1), 1),
            (2, 3, 16, 16, 8, 3, 3, (2, 2), (0, 0), 1),
            (4, 8, 8, 8, 16, 3, 3, (2, 2), (1, 1), 1),
        ]
        self.shape_desc = "batch, input_c, input_h, input_w, out_c, kernel_h, kernel_w, stride, padding, groups"
...
def test_perf_convolution_backward():
    def convolution_backward_input_fn(shape, dtype, device):
        ...
        output = torch.nn.functional.conv2d(
            input, weight, stride=stride, padding=padding, groups=groups
        )
    ...
    bench = ConvolutionBackwardBenchmark(
        ...
        torch_op=torch.ops.aten.convolution_backward,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(lambda **kwargs: torch.ops.aten.convolution_backward(**kwargs))
    bench.run()
