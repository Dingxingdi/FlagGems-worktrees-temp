...
class ConvReluBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        return None


@pytest.mark.conv_relu
def test_perf_conv_relu():
    def conv_relu_input_fn(shape, dtype, device):
        (
            batch,
            input_c,
            input_h,
    ...
    def torch_conv_relu(input, weight, bias, stride, padding, groups):
        return torch.nn.functional.relu(
            torch.nn.functional.conv2d(input, weight, bias, stride=stride, padding=padding, groups=groups)
        )

    bench = ConvReluBenchmark(
        input_fn=conv_relu_input_fn,
        op_name="conv_relu",
        torch_op=torch_conv_relu,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(flag_gems.conv_relu)
    bench.run()
