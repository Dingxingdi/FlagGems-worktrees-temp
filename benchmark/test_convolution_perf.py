...
class ConvTranspose2DBenchmark(GenericBenchmark):
    # Custom shapes to use for conv_transpose2d benchmark
    CUSTOM_SHAPES = [
        (1, 16, 64, 64, 8, 3, 3, 1, 1, 1),
        (1, 32, 32, 32, 16, 3, 3, 2, 1, 1),
        (2, 8, 64, 64, 8, 3, 3, 1, 1, 1),
        (2, 16, 32, 32, 16, 3, 3, 2, 1, 1),
        (4, 4, 32, 32, 8, 3, 3, 1, 1, 1),
        (4, 8, 16, 16, 16, 3, 3, 2, 1, 1),
    ]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Override shapes in __init__ to ensure they are used
        self.shapes = self.CUSTOM_SHAPES

    def set_shapes(self, shape_file_path=None):
        # Override set_shapes to always use custom shapes
        self.shapes = self.CUSTOM_SHAPES
...
def test_perf_conv_transpose2d():
    def conv_transpose2d_input_fn(shape, dtype, device):
        ...
        input_shape = (batch, input_c, input_h, input_w)
        # For conv_transpose2d, weight shape is (input_c, out_c_per_group, kernel_h, kernel_w)
        weight_shape = (input_c, out_c_per_group, kernel_h, kernel_w)
        input = torch.randn(size=input_shape, device=device, dtype=dtype)
        weight = torch.randn(size=weight_shape, device=device, dtype=dtype)
