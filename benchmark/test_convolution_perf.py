...
class Depthwise2DBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        return [
            (32, 4, 8, 8, 32, 2, 2, 2, 2),
            (18, 16, 4, 4, 16, 2, 2, 2, 2),
        ]


@pytest.mark.depthwise2d
def test_perf_depthwise2d():
    def depthwise2d_input_fn(shape, dtype, device):
        # Skip shapes that don't have the expected number of dimensions
        if len(shape) != 9:
            return
        (
            batch,
        ...
        # Depthwise: groups = input_c = out_c
        input_shape = (batch, input_c, input_h, input_w)
        weight_shape = (out_c, 1, kernel_h, kernel_w)
    ...
    torch.backends.cudnn.allow_tf32 = False
    bench = Depthwise2DBenchmark(
        input_fn=depthwise2d_input_fn,
        op_name="depthwise2d",
        torch_op=torch._C._nn._conv_depthwise2d,
        dtypes=FLOAT_DTYPES,
    )
