...
def cauchy_input_fn(shape, cur_dtype, device):
    """Input function for cauchy that yields (tensor, median, sigma)"""
    x = torch.empty(shape, dtype=cur_dtype, device=device)
    yield x, 0.0, 1.0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            cauchy_input_fn,
            marks=pytest.mark.cauchy,
        ),
        pytest.param(
            "cauchy_",
            torch.Tensor.cauchy_,
            unary_input_fn,
            marks=pytest.mark.cauchy_,
        ),
    ],
)
