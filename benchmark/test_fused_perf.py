...
def test_perf_dropout_add_layernorm():
    def dropout_add_layernorm_input_fn(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        residual = torch.randn(shape, dtype=dtype, device=device)
        layer_shape = (shape[-1],)
        weight = torch.randn(layer_shape, dtype=dtype, device=device)
        bias = torch.randn(layer_shape, dtype=dtype, device=device)
        yield inp, residual, weight, bias

    def torch_op(inp, residual, weight, bias):
        layer_shape = (inp.shape[-1],)
        return torch.nn.functional.layer_norm(
            inp + residual, layer_shape, weight=weight, bias=bias
        )

    gems_op = flag_gems.dropout_add_layernorm

    bench = GenericBenchmarkExcluse1D(
        input_fn=dropout_add_layernorm_input_fn,
        op_name="dropout_add_layernorm",
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(gems_op)
