...
def test_perf_rms_norm_linear():
    def rms_norm_linear_input_fn(shape, dtype, device):
        # Input shape is (batch, seq, hidden)
        # For 2D shape, it's (batch*seq, hidden)
        if len(shape) == 2:
            batch_seq, hidden = shape
        else:
            batch_seq = shape[0]
            hidden = shape[-1]
        ...
        inp = torch.randn(shape, dtype=dtype, device=device)
        layer_shape = (hidden,)
        rms_weight = torch.randn(layer_shape, dtype=dtype, device=device)
        linear_weight = torch.randn((256, hidden), dtype=dtype, device=device)
        bias = torch.randn((256,), dtype=dtype, device=device)
        yield inp, layer_shape, rms_weight, linear_weight, bias, 1e-5

    def torch_op(x, layer_shape, rms_weight, linear_weight, bias, eps):
        # Reference: RMSNorm + Linear
        var = x.pow(2).mean(-1, keepdim=True)
        hidden_states = x * torch.rsqrt(var + eps)
        hidden_states = hidden_states * rms_weight
        return torch.nn.functional.linear(hidden_states, linear_weight, bias)

    gems_op = flag_gems.rms_norm_linear

    bench = GenericBenchmarkExcluse1D(
        input_fn=rms_norm_linear_input_fn,
