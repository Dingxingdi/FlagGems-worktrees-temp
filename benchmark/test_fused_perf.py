...
@pytest.mark.add_layer_norm
def test_perf_add_layer_norm():
    def add_layer_norm_input_fn(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        residual = torch.randn(shape, dtype=dtype, device=device)
        layer_shape = (shape[-1],)
        weight = torch.randn(layer_shape, dtype=dtype, device=device)
        bias = torch.randn(layer_shape, dtype=dtype, device=device)
        yield inp, residual, layer_shape, weight, bias, 1e-5

    def torch_op(x, residual, layer_shape, weight, bias, eps):
        x = x + residual
        mean = x.mean(-1, keepdim=True)
        var = x.var(-1, keepdim=True, unbiased=False)
        rstd = torch.rsqrt(var + eps)
        hidden_states = (x - mean) * rstd
        return weight * hidden_states + bias

    gems_op = flag_gems.add_layer_norm

    bench = GenericBenchmarkExcluse1D(
        input_fn=add_layer_norm_input_fn,
