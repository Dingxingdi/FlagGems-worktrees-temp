...
def transpose_copy_input_fn(shape, cur_dtype, device):
    inp = generate_tensor_input(shape, cur_dtype, device)
    if len(shape) > 1:
        yield inp, 0, 1  # Transpose first two dimensions
    else:
        yield inp, 0, 0  # Same dimension (trivial case)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            transpose_copy_input_fn,
            FLOAT_DTYPES,
            marks=pytest.mark.transpose_copy,
        ),
        pytest.param(
