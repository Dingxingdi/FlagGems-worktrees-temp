...
def permute_input_fn(shape, cur_dtype, device):
    inp = generate_tensor_input(shape, cur_dtype, device)
    ndim = len(shape)
    # Test different permutation dims based on tensor rank
    if ndim == 1:
        yield inp, (0,)
    elif ndim == 2:
        yield inp, (1, 0)
        ...
        yield inp, tuple(reversed(range(ndim)))
