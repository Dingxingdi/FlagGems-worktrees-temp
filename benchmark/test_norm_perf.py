...
def torch_batch_norm_no_update(inp, weight, bias, running_mean, running_var, momentum, eps):
    """Manual reference implementation of _batch_norm_no_update.

    Formula: output = weight * (input - running_mean) / sqrt(running_var + eps) + bias
    """
    ...
    return ref_out, torch.empty((0,), dtype=inp.dtype, device=inp.device), torch.empty((0,), dtype=inp.dtype, device=inp.device), torch.empty((0,), dtype=torch.uint8, device=inp.device)


@pytest.mark.batch_norm_no_update
def test_perf_batch_norm_no_update():
    bench = NormBenchmark(
        input_fn=batch_norm_no_update_input_fn,
        op_name="_batch_norm_no_update",
