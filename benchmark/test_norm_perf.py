...
# Benchmark for BatchNorm_ReLU_Dropout fused operator
def batchnorm_relu_dropout_input_fn(shape, dtype, device):
    C = shape[1]
    inp = torch.randn(shape, dtype=dtype, device=device)
    weight = torch.randn(C, dtype=dtype, device=device)
    bias = torch.randn(C, dtype=dtype, device=device)
    ...
    momentum = 0.1
    eps = 1e-5
    # Yield arguments as individual args, not as a tuple
    # Include cudnn_enabled to match torch.batch_norm signature
    yield inp, weight, bias, running_mean, running_var, training, momentum, eps, True


def batchnorm_relu_dropout_torch_ref(inp, weight, bias, running_mean, running_var, training, momentum, eps, cudnn_enabled):
    """Reference implementation using PyTorch"""
    # Apply batch norm
    out = torch.nn.functional.batch_norm(
        inp, running_mean, running_var, weight=weight, bias=bias,
        momentum=momentum, eps=eps, training=training
    )
    # Apply ReLU
    out = torch.nn.functional.relu(out)
    # Apply dropout (p=0, so essentially no dropout)
    out = torch.nn.functional.dropout(out, p=0.0, training=training)
    return out


class BatchNormReLUDropoutBenchmark(NormBenchmark):
    def set_more_shapes(self):
        return [
            # 3D shapes represented as [batch_size, channels, hidden_size]
            (16, 16, 64),
...
@pytest.mark.BatchNorm_ReLU_Dropout
def test_perf_BatchNorm_ReLU_Dropout():
    bench = BatchNormReLUDropoutBenchmark(
        input_fn=batchnorm_relu_dropout_input_fn,
        op_name="batch_norm_relu_dropout",
        torch_op=batchnorm_relu_dropout_torch_ref,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(flag_gems.ops.batch_norm_relu_dropout)
    bench.run()
