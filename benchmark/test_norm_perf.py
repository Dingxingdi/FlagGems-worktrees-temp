...
def layernorm_dropout_input_fn(shape, dtype, device):
    ...
    weight = torch.randn(layer_shape, dtype=dtype, device=device)
    bias = torch.randn(layer_shape, dtype=dtype, device=device)
    # LayerNorm+Dropout with p=0 for benchmarking (no actual dropout)
    yield inp, layer_shape, weight, bias, 0.0, 1e-5, True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "layer_norm_dropout",
            flag_gems.layer_norm_dropout,
            layernorm_dropout_input_fn,
            marks=pytest.mark.layer_norm_dropout,
        ),
    ],
)
