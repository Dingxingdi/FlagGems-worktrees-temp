...
def cudnn_batchnorm_input_fn(shape, dtype, device):
    C = shape[1]
    inp = torch.randn(shape, dtype=dtype, device=device)
    weight = torch.randn((C,), dtype=dtype, device=device)
    bias = torch.randn((C,), dtype=dtype, device=device)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            cudnn_batchnorm_input_fn,
            marks=pytest.mark.cudnn_batch_norm,
        ),
    ],
)
def test_group_and_layer_and_instance_norm_benchmark(op_name, torch_op, input_fn):
    ...
    # cudnn_batch_norm only supports float32 in PyTorch
    dtypes = [torch.float32] if op_name == "cudnn_batch_norm" else FLOAT_DTYPES
    bench = NormBenchmark(
        input_fn=input_fn, op_name=op_name, torch_op=torch_op, dtypes=dtypes
    )
    if op_name == "instance_norm":
        bench.set_gems(flag_gems.instance_norm)
    elif op_name == "cudnn_batch_norm":
