...
def layernorm_dropout_input_fn(shape, dtype, device):
    """Input function for LayerNorm_Dropout benchmark."""
    inp = torch.randn(shape, dtype=dtype, device=device)
    layer_shape = shape[1:]
    weight = torch.randn(layer_shape, dtype=dtype, device=device)
    bias = torch.randn(layer_shape, dtype=dtype, device=device)
    # yield: (input, normalized_shape, weight, bias, eps, p, train)
    yield inp, layer_shape, weight, bias, 1e-5, 0.5


def layernorm_dropout_torch_op(inp, layer_shape, weight, bias, eps, p):
    """Simulated torch operation for LayerNorm_Dropout: just LayerNorm in eval mode."""
    # In eval mode (train=False), dropout is a no-op
    return torch.layer_norm(inp, layer_shape, weight=weight, bias=bias, eps=eps)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "LayerNorm_Dropout",
            layernorm_dropout_torch_op,
            layernorm_dropout_input_fn,
            marks=pytest.mark.LayerNorm_Dropout,
        ),
        pytest.param(
            "instance_norm",
            torch.instance_norm,
...
def test_group_and_layer_and_instance_norm_benchmark(op_name, torch_op, input_fn):
    ...
    if op_name == "LayerNorm_Dropout":
        # For LayerNorm_Dropout, use train=False to benchmark LayerNorm only (without dropout)
        def gems_op_wrapper(inp, layer_shape, weight, bias, eps, p):
            return flag_gems.layer_norm_dropout(
                inp, layer_shape, weight=weight, bias=bias, eps=eps, p=p, train=False
            )
        bench.set_gems(gems_op_wrapper)
    bench.run()
    if vendor_name == "mthreads" and op_name == "instance_norm":
