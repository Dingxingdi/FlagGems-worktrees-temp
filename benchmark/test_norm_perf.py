...
def test_perf_miopen_batch_norm_backward():
    def miopen_batch_norm_backward_input_fn(shape, dtype, device):
        for forward_args in batchnorm_input_fn(shape, dtype, device):
            ...
            save_var = torch.abs(torch.randn(channels, dtype=torch.float32, device=device)) + 0.1
    ...
    def miopen_batch_norm_backward_reference(input, grad_output, weight, running_mean, running_var, save_mean, save_var, epsilon):
        ...
        return torch.ops.aten.native_batch_norm_backward(
            ...
            save_mean, save_invstd, train, epsilon, output_mask
        )
