...
def layernorm_gelu_input_fn(shape, dtype, device):
    """Input function for fused LayerNorm + GeLU operator."""
    inp = torch.randn(shape, dtype=dtype, device=device)
    layer_shape = shape[1:]
    weight = torch.randn(layer_shape, dtype=dtype, device=device)
    bias = torch.randn(layer_shape, dtype=dtype, device=device)
    eps = 1e-5
    yield inp, layer_shape, weight, bias, eps


def layernorm_gelu_ref(inp, normalized_shape, weight, bias, eps):
    """Reference implementation: LayerNorm followed by GeLU."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "layer_norm_gelu",
            layernorm_gelu_ref,
            layernorm_gelu_input_fn,
            marks=pytest.mark.layer_norm_gelu,
        ),
    ],
)
