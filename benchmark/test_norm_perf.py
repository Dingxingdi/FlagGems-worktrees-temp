...
@pytest.mark.RMSNorm
def test_perf_RMSNorm():
    """Benchmark for RMSNorm with simplified interface (no explicit weight)."""

    def RMSNorm_input_fn(shape, dtype, device):
        # RMSNorm normalizes over the last dimension
        # torch.nn.functional.rms_norm(input, normalized_shape, weight=None, eps=1e-5)
        M, N = shape
        inp = torch.randn(shape, dtype=dtype, device=device)
        weight = torch.ones(N, dtype=dtype, device=device)  # default weight
        yield inp, (N,), weight

    bench = GenericBenchmark2DOnly(
        input_fn=RMSNorm_input_fn,
        op_name="RMSNorm",
        torch_op=torch.nn.functional.rms_norm,
    )
    bench.run()
