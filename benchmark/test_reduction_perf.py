...
def adaptive_max_pool2d_input_fn(shape, dtype, device):
    ...
    output_sizes = [
        (shape[2] // 4, shape[3] // 4),  # Quarter size
        (shape[2] // 2, shape[3] // 2),  # Half size
    ]
    # Add full size output if input is larger than 4
    if shape[2] > 4 and shape[3] > 4:
        output_sizes.append((shape[2], shape[3]))
...
def test_perf_adaptive_max_pool2d():
    bench = AdaptiveMaxPool2dBenchmark(
        ...
        torch_op=torch.nn.functional.adaptive_max_pool2d,
        dtypes=FLOAT_DTYPES,
    )
...
def test_perf_adaptive_max_pool2d_backward():
    def adaptive_max_pool2d_backward_input_fn(shape, dtype, device):
        for forward_args in adaptive_max_pool2d_input_fn(shape, dtype, device):
            ...
            inp.requires_grad_(True)
            # Use FlagGems forward to produce indices compatible with FlagGems backward
            output, indices = flag_gems.adaptive_max_pool2d(inp, **params)
            grad_output = torch.randn_like(output)
            yield grad_output, inp, indices, params

    def torch_adaptive_max_pool2d_backward_wrapper(
        grad_output, input, indices, **kwargs
    ):
        # For torch baseline, we use torch forward to get compatible indices
        output = torch.nn.functional.adaptive_max_pool2d(input, **kwargs)
        grad_input = torch.autograd.grad(
            outputs=(output,), inputs=(input,), grad_outputs=(grad_output,)
        )
        return grad_input[0]
