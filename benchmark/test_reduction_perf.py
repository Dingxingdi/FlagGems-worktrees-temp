...
def adaptive_avg_pool2d_input_fn(shape, dtype, device):
    inp = generate_tensor_input(shape, dtype, device)
    # Common output sizes for typical use cases
    output_sizes = [
        (shape[-2] // 2, shape[-1] // 2),  # Half size
        (shape[-2] // 4, shape[-1] // 4),  # Quarter size
    ]
    if Config.bench_level == BenchLevel.COMPREHENSIVE:
        output_sizes.extend([
            (1, 1),  # Global average pooling
            (shape[-2], shape[-1]),  # No pooling
        ])
    for output_size in output_sizes:
        yield inp, {"output_size": output_size}


class AdaptiveAvgPool2dBenchmark(GenericBenchmark4DOnly):
    def get_input_iter(self, cur_dtype) -> Generator:
        shapes_4d = [
            (4, 3, 224, 224),  # Typical input image size
            (16, 64, 56, 56),  # Early ResNet layer output
