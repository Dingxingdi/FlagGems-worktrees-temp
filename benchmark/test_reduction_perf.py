...
def binary_cross_entropy_input_fn(shape, cur_dtype, device):
    ...
    inp = torch.sigmoid(generate_tensor_input(shape, cur_dtype, device))
    target = torch.sigmoid(generate_tensor_input(shape, cur_dtype, device))
    yield inp, target
    if Config.bench_level == BenchLevel.COMPREHENSIVE:
        yield inp, target, {"reduction": "mean"}
        yield inp, target, {"reduction": "sum"}
