...
# Adaptive max pooling 3D benchmark
def adaptive_max_pool3d_input_fn(shape, dtype, device):
    """Generate input for adaptive max pool 3D.

    Args:
        shape: Input tensor shape (N, C, D, H, W)
    """
    inp = generate_tensor_input(shape, dtype, device)

    # Common output sizes
    output_sizes = [(1, 1, 1), (2, 2, 2)]

    # Test different output sizes
    for out_size in output_sizes:
        # Skip if output size is larger than input
        if out_size[0] > shape[2] or out_size[1] > shape[3] or out_size[2] > shape[4]:
            continue
        yield inp, {"output_size": out_size}


class AdaptiveMaxPool3dBenchmark(GenericBenchmark):
    def get_input_iter(self, cur_dtype) -> Generator:
        # Typical 3D pooling shapes (N, C, D, H, W)
        shapes = [
            (1, 1, 4, 4, 4),
            (2, 3, 8, 8, 8),
            (1, 16, 16, 16, 16),
...
def test_perf_adaptive_max_pool3d():
    ...
    bench.set_gems(flag_gems.adaptive_max_pool3d)
    bench.run()
