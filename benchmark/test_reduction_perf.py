...
class AminmaxReductionBenchmark(UnaryReductionBenchmark):
    """Specialized benchmark for aminmax which requires dim as keyword argument."""
...
def test_aminmax_reduction_perf(dtype):
    bench = AminmaxReductionBenchmark(
        op_name="aminmax", torch_op=torch.aminmax, dtypes=[dtype]
    )
    bench.run()
