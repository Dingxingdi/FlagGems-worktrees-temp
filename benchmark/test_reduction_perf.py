...
def fractional_max_pool2d_input_fn(shape, dtype, device):
    ...
    if Config.bench_level == BenchLevel.COMPREHENSIVE:
        if shape[-2] > 5 and shape[-1] > 5:
            yield inp, {"kernel_size": 2, "output_size": (shape[2] // 4, shape[3] // 4)}


class FractionalMaxPool2dBenchmark(GenericBenchmark4DOnly):
    def get_input_iter(self, cur_dtype) -> Generator:
        shapes_4d = [
            (4, 3, 224, 224),
            (16, 64, 56, 56),
...
def test_perf_fractional_max_pool2d_backward():
    ...
    def torch_forward_wrapper(input, **kwargs):
        output, _ = torch.nn.functional.fractional_max_pool2d(
            input, return_indices=True, **kwargs
        )
        return output

    def torch_backward_wrapper(grad_output, input, indices, **kwargs):
        grad_input = torch.autograd.grad(
            outputs=(torch_forward_wrapper(input, **kwargs),),
            inputs=(input,),
    ...
    bench = FractionalMaxPool2dBenchmark(
        ...
        torch_op=torch_backward_wrapper,
        dtypes=FLOAT_DTYPES,
        is_backward=False,
    )
