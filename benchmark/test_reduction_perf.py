...
# ReduceMean benchmark - uses flag_gems.ReduceMean since torch.ReduceMean doesn't exist
class ReduceMeanBenchmark(Benchmark):
    """
    Benchmark for ReduceMean operator.
    """
    ...
    def get_torch_op(self):
        # Use flag_gems.ReduceMean as the torch_op for benchmarking
        return flag_gems.ReduceMean

    def get_gems_op(self):
        return flag_gems.ReduceMean


@pytest.mark.ReduceMean
def test_perf_ReduceMean():
    bench = ReduceMeanBenchmark(
        op_name="ReduceMean",
        torch_op=flag_gems.ReduceMean,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
