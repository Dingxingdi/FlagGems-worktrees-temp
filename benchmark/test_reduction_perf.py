...
def multi_margin_loss_input_fn(shape, cur_dtype, device):
    inp = generate_tensor_input(shape, cur_dtype, device)
    target = torch.randint(0, shape[-1], (shape[0],), device=device)
    yield inp, target
    if Config.bench_level == BenchLevel.COMPREHENSIVE:
        for reduction in ["mean", "sum", "none"]:
            for p in [1, 2]:
                yield inp, target, {"p": p, "margin": 1.0, "reduction": reduction}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            multi_margin_loss_input_fn,
            FLOAT_DTYPES,
            marks=pytest.mark.multi_margin_loss,
        ),
        pytest.param(
            "mse_loss",
