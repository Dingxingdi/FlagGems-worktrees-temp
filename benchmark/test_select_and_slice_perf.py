...
def scatter_reduce_inplace_input_fn_factory(reduce):
    def inner(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        dim = -1
        size_dim = shape[dim]
...
def test_perf_scatter_reduce(reduce):
    bench = TensorSelectBenchmark(
        op_name=f"scatter_reduce.{reduce}",
        torch_op=torch.scatter_reduce,
        input_fn=scatter_reduce_input_fn_factory(reduce),
        get_gbps=gather_scatter_gbps,
        dtypes=[torch.float16, torch.float32],
    )
    bench.run()
...
@pytest.mark.parametrize("reduce", ["sum", "prod", "amax", "amin"])
def test_perf_scatter_reduce_inplace(reduce):
    bench = TensorSelectBenchmark(
        op_name=f"scatter_reduce.{reduce}",
        torch_op=torch.Tensor.scatter_reduce_,
        input_fn=scatter_reduce_inplace_input_fn_factory(reduce),
        get_gbps=gather_scatter_gbps,
        dtypes=[torch.float16, torch.float32],
        is_inplace=True,
    )
