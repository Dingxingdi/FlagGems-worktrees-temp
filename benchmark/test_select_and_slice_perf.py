...
def index_reduce_gbps(bench_fn_args, latency):
    # bench_fn_args can be a tuple like (inp, dim, index, src, kwargs_dict)
    # or already unpacked as (inp, dim, index, src)
    if isinstance(bench_fn_args, tuple) and len(bench_fn_args) == 5 and isinstance(bench_fn_args[4], dict):
        inp = bench_fn_args[0]
        src = bench_fn_args[3]
        index = bench_fn_args[2]
    else:
...
def test_index_reduce_perf():
    def index_reduce_input_fn(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        dim = 0
        src_shape = list(inp.shape)
        index_len = src_shape[dim]
        index = torch.randint(0, index_len, (index_len,), device=device)
        src_shape[dim] = index_len
        src = torch.randn(src_shape, dtype=dtype, device=device)
        # Yield as separate items in a tuple, with reduce in kwargs dict
        yield (inp, dim, index, src, {"reduce": "mean", "include_self": True})

    bench = TensorSelectBenchmark(
        op_name="index_reduce",
        torch_op=torch.ops.aten.index_reduce,
        input_fn=index_reduce_input_fn,
        dtypes=[torch.float16, torch.float32],
        get_gbps=index_reduce_gbps,
    )
    bench.run()


@pytest.mark.index_reduce_
def test_index_reduce__perf():
    def index_reduce__input_fn(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        dim = 0
        src_shape = list(inp.shape)
        index_len = src_shape[dim]
    ...
    bench = TensorSelectBenchmark(
        ...
        torch_op=torch.ops.aten.index_reduce_,
        input_fn=index_reduce__input_fn,
        dtypes=[torch.float16, torch.float32],
        get_gbps=index_reduce_gbps,
    )
    bench.run()
