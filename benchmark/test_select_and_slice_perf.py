...
def _unsafe_index_put_input_fn(accumulate):
    def inner(shapes, dtype, device):
        input_shape, indices_shape, values_shape, is_bool = shapes
        inp = torch.randn(
            input_shape, dtype=dtype, device=flag_gems.device, requires_grad=False
...
class UnsafeIndexPutAccFalseBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        INDEX_PUT_SHAPE = (
            ((2**28,), ((2**16,),), (2**16,), False),
            ((32, 32), ((8,), (8,)), (8,), False),
...
def test__unsafe_index_put_acc_false_perf():
    bench = UnsafeIndexPutAccFalseBenchmark(
        op_name="_unsafe_index_put",
        torch_op=torch._unsafe_index_put,
        input_fn=_unsafe_index_put_input_fn(False),
        dtypes=FLOAT_DTYPES,
    )
    bench.run()


class UnsafeIndexPutAccTrueBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        INDEX_PUT_SHAPE = (
            ((2**28,), ((2**16,),), (2**16,), False),
            ((32, 32), ((8,), (8,)), (8,), False),
...
@pytest.mark.unsafe_index_put
def test__unsafe_index_put_acc_true_perf():
    bench = UnsafeIndexPutAccTrueBenchmark(
        op_name="_unsafe_index_put",
        torch_op=torch._unsafe_index_put,
        input_fn=_unsafe_index_put_input_fn(True),
        dtypes=[torch.float16, torch.float32],
    )
    bench.run()
