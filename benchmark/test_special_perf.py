...
def test_perf_DynamicQuantizeLinear():
    def dql_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        # Return a tuple that includes additional args for quantize_per_tensor_dynamic
        yield inp, torch.qint8, False

    bench = GenericBenchmark(
        input_fn=dql_input_fn,
