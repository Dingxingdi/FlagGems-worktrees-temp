...
class RmsNormLinearBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            linear_weight = torch.randn(O, N, dtype=cur_dtype, device=self.device)
            linear_bias = torch.randn(O, dtype=cur_dtype, device=self.device)
