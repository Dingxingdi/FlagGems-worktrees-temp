...
@pytest.mark.performance
def test_perf_segment_reduce():
    """Simple benchmark for segment_reduce using direct torch calls."""
    import time
    device = flag_gems.device
    dtype = torch.float32

    # Warm up
    for shape in SEGMENT_REDUCE_SHAPES:
        num_segments = shape[0] // 10
        data = torch.randn(shape, dtype=dtype, device=device)
        lengths = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=device)
        total_len = lengths.sum().item()
        lengths = lengths * shape[0] // total_len
        lengths[-1] = shape[0] - lengths[:-1].sum().item()
        torch.segment_reduce(data, "sum", lengths=lengths)
        torch.cuda.synchronize()

    # Benchmark
    print("\nOperator: segment_reduce  Performance Test (dtype=torch.float32)")
    for shape in SEGMENT_REDUCE_SHAPES:
        num_segments = shape[0] // 10
        data = torch.randn(shape, dtype=dtype, device=device)
        lengths = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=device)
        ...
        for _ in range(100):
            with flag_gems.use_gems():
                torch.segment_reduce(data, "sum", lengths=lengths)
        torch.cuda.synchronize()
        gems_time = (time.time() - start) / 100 * 1000

        speedup = torch_time / gems_time if gems_time > 0 else 0
        print(f"SUCCESS    {torch_time:.4f}    {gems_time:.4f}    {speedup:.4f}    {list(shape)}")
