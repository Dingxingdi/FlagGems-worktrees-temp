...
def test_perf_moe_combine():
    def moe_combine_input_fn(shape, dtype, device):
        # Convert 2D shape to 3D: (tokens, hidden) -> (tokens, 1, hidden)
        # or use default if already 3D
        shape = (shape[0], 1, shape[1]) if len(shape) == 2 else shape
        ...
        weights = torch.randn(
            num_tokens, num_experts, dtype=dtype, device=device, requires_grad=False
        )
        weights = torch.softmax(weights, dim=1)
        yield expert_outputs, weights

    def torch_op(expert_outputs, weights):
        return torch.einsum("teh,te->th", expert_outputs, weights)

    gems_op = flag_gems.moe_combine

    bench = GenericBenchmarkExcluse1D(
        input_fn=moe_combine_input_fn,
        op_name="moe_combine",
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
    )
