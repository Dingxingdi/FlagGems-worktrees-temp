...
class EmbeddingBagDenseBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for num_bags, embedding_dim, num_weights, samples_per_bag in self.shapes:
            ...
            offsets = torch.arange(
                0, num_samples + 1, samples_per_bag, dtype=torch.long, device=self.device
            )[:num_bags]
...
def test_perf_embedding_bag_dense_backward():
