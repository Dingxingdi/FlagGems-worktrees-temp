...
class FFTIRFFT2Benchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FFT_IRFFT2_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            n0, n1 = shape
            # Real FFT output has n1//2+1 complex values in the last dim
            input_real = torch.randn(n0, n1 // 2 + 1, dtype=cur_dtype, device=self.device)
            input_imag = torch.randn(n0, n1 // 2 + 1, dtype=cur_dtype, device=self.device)
            inp = torch.complex(input_real, input_imag)
            s = (n0, n1)
            yield inp, s, (-2, -1), "backward"


@pytest.mark.fft_irfft2
def test_perf_fft_irfft2():
    bench = FFTIRFFT2Benchmark(
        op_name="fft_irfft2",
        torch_op=torch.fft.irfft2,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
