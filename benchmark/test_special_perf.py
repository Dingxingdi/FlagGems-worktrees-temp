...
# Benchmark for weight-only int8 matmul
...
class WeightInt8PackMmBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            weight = torch.randint(-128, 127, (M, K), dtype=torch.int8, device=self.device)
            # Create activation matrix
            activation = torch.randn(K, N, dtype=cur_dtype, device=self.device)
            # Create scaling factors
            scales = torch.randn(N, dtype=cur_dtype, device=self.device)
            yield weight, activation, scales
...
def test_perf_weight_int8pack_mm():
    ...
    # Set gems_op to the same function to avoid running torch op as baseline
    bench.gems_op = flag_gems._weight_int8pack_mm
    bench.run()
