...
def test_perf_top_p_sampling():
    def top_p_sampling_input_fn(shape, dtype, device):
    ...
    bench = GenericBenchmark2DOnly(
        input_fn=top_p_sampling_input_fn,
        op_name="top_p_sampling",
        torch_op=flag_gems.top_p_sampling,
        dtypes=(torch.float16, torch.float32),
    )
    bench.run()
