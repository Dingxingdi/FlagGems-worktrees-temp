...
@pytest.mark.parametrize("align_corners", [False, True])
def test_perf_upsample_bicubic2d_backward(align_corners):
    """Benchmark for upsample_bicubic2d_backward - no torch comparison available."""
    def upsample_bicubic2d_backward_input_fn(shape, dtype, device):
        batch, channel, height, width = shape
        scale = 2.0
        H_out = int(height * scale)
        W_out = int(width * scale)
        grad_output = torch.ones(batch, channel, H_out, W_out, device=device, dtype=dtype)
    ...
    # Use the reference implementation from PyTorch (autograd) as comparison
    import torch.nn.functional as F

    def torch_op_ref(grad_output, output_size, input_size, align_corners, scales_h, scales_w):
        N, C, H, W = input_size
        H_out, W_out = output_size
        # Create a dummy input and run forward + backward to get gradient
        x = torch.randn(N, C, H, W, device=grad_output.device, dtype=grad_output.dtype, requires_grad=True)
        y = F.interpolate(x, size=(H_out, W_out), mode='bicubic', align_corners=align_corners)
    ...
    bench = UpsampleBenchmark(
        ...
        op_name=f"upsample_bicubic2d_backward_align_{align_corners}",
        torch_op=torch_op_ref,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
