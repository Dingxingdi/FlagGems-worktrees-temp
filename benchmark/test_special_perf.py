...
def _sym_storage_offset_input_fn(shape, cur_dtype, device):
    yield torch.randn(shape, dtype=cur_dtype, device=device)


@pytest.mark.sym_storage_offset
@pytest.mark.performance
def test_perf_sym_storage_offset():
    bench = GenericBenchmark(
        op_name="sym_storage_offset",
        torch_op=torch.ops.aten.sym_storage_offset,
        dtypes=FLOAT_DTYPES,
        input_fn=_sym_storage_offset_input_fn,
    )
    bench.run()
