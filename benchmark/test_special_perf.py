...
class EmbeddingBagBackwardBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        # shapes: (num_bags, indices_per_bag, embedding_dim, num_weights)
        self.shapes = [
            (8, 256, 128, 8192),
            (16, 512, 256, 16384),
            (32, 512, 256, 32768),
...
def embedding_bag_backward_input_fn(shape, dtype, device):
    num_bags, indices_per_bag, embedding_dim, num_weights = shape
    total_indices = num_bags * indices_per_bag

    # Create weight (embedding table)
    weight = torch.randn((num_weights, embedding_dim), device=device, dtype=dtype)

    # Create indices
    indices = torch.randint(
        0, num_weights, (total_indices,), device=device, dtype=torch.long
    )

    # Create offsets
    offsets = torch.arange(
        0, total_indices + 1, indices_per_bag, device=device, dtype=torch.long
    )
    ...
    # Yield all backward inputs
    yield grad, indices, offsets, offset2bag, bag_size, max_indices, num_weights, False, 0, False, None, -1
...
def test_perf_embedding_bag_backward():
    bench = EmbeddingBagBackwardBenchmark(
        input_fn=embedding_bag_backward_input_fn,
        op_name="_embedding_bag_backward",
        torch_op=torch.ops.aten._embedding_bag_backward,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
