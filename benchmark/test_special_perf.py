...
def fft_c2r_input_fn(shape, dtype, device):
    """Input function for FFT C2R benchmark."""
    # Complex input
    inp = torch.rand(shape, dtype=dtype, device=device)
    dim = len(shape) - 1 if len(shape) > 1 else 0
    n_fft = shape[dim]
    # dim must be a list for torch.ops.aten._fft_c2r
    yield inp, [dim], 0, n_fft


@pytest.mark.fft_c2r
def test_perf_fft_c2r():
    # FFT shapes for benchmarking
    fft_shapes = [
        (8,),
        (16,),
        (32,),
    ...
    def input_fn(shape, dtype, device):
        return fft_c2r_input_fn(shape, dtype, device)

    bench = GenericBenchmark(
        input_fn=input_fn,
        op_name="_fft_c2r",
        torch_op=torch.ops.aten._fft_c2r,
