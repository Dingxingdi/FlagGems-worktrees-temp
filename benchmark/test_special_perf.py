...
def eigvalsh_input_fn(shape, dtype, device):
    # Create a symmetric matrix for linalg_eigvalsh
    # Handle non-square matrices by using min(dim[-2], dim[-1])
    if len(shape) >= 2:
        n = min(shape[-2], shape[-1])
        # Use only the square part of the matrix
        A = torch.randn(n, n, dtype=dtype, device=device)
    else:
        # For 1D or 0D tensors, create a 2x2 matrix
...
special_operations = [
    ...
    # Matrix Eigenvalue Operations
    ("linalg_eigvalsh", torch.linalg.eigvalsh, [torch.float32], eigvalsh_input_fn),
]
