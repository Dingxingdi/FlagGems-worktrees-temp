...
class CudnnRNNBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = CUDNN_RNN_BACKWARD_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            batch, seq, feature = shape
            input_tensor = torch.randn(shape, dtype=cur_dtype, device=self.device)
            output_tensor = torch.randn(shape, dtype=cur_dtype, device=self.device)
            grad_output = torch.randn(shape, dtype=cur_dtype, device=self.device)
            yield grad_output, input_tensor, output_tensor


@pytest.mark.cudnn_rnn_backward
def test_perf_cudnn_rnn_backward():
    bench = CudnnRNNBackwardBenchmark(
        op_name="_cudnn_rnn_backward",
        torch_op=lambda go, inp, out: (go, None, None, None),
        dtypes=[torch.float32, torch.float16],
    )
    bench.set_gems(flag_gems._cudnn_rnn_backward)
    bench.run()
