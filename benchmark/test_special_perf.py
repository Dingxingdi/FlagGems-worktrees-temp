...
# broadcast_tensors benchmark shapes - pairs of shapes that can be broadcast
BROADCAST_BENCHMARK_SHAPES = [
    ((2, 3), (2, 3)),  # same shape
...
class BroadcastTensorsBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = BROADCAST_BENCHMARK_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape_a, shape_b in self.shapes:
            inp_a = torch.randn(shape_a, dtype=cur_dtype, device=self.device)
            inp_b = torch.randn(shape_b, dtype=cur_dtype, device=self.device)
            yield inp_a, inp_b


@pytest.mark.broadcast_tensors
def test_perf_broadcast_tensors():
