...
class SelectBenchmark(Benchmark):
    """
    Benchmark for select operator.
    """

    def set_shapes(self, shape_file_path=None):
        self.shapes = SELECT_SHAPES
    ...
    def get_tflops(self, op, *args, **kwargs):
        x, dim, index = args
        # Output elements = x.numel() / x.shape[dim]
        return x.numel() // x.shape[dim]


@pytest.mark.select
def test_perf_select():
    bench = SelectBenchmark(
        op_name="select",
        torch_op=torch.select,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
