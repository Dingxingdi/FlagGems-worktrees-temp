...
def test_perf_upsample_bilinear2d_aa():
    class UpsampleBilinear2dAaBenchmark(Benchmark):
        def set_shapes(self, shape_file_path=None):
            self.shapes = [
                (16, 32, 128, 128),
                (8, 16, 256, 256),
        ...
        def get_input_iter(self, cur_dtype):
            for shape in self.shapes:
                ...
                yield input, output_size, False, None, None

    bench = UpsampleBilinear2dAaBenchmark(
        op_name="upsample_bilinear2d_aa",
        torch_op=torch.ops.aten._upsample_bilinear2d_aa,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
