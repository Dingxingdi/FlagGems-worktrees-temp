...
def matrix_exp_input_fn(shape, dtype, device):
    # Generate small values to avoid overflow
    inp = (torch.rand(shape, dtype=dtype, device=device) - 0.5) * 0.5
    yield inp,


class MatrixExpBenchmark(GenericBenchmark):
    """
    Benchmark for linalg.matrix_exp operation
    """

    # Square matrix shapes for linalg_matrix_exp
    MATRIX_EXP_SHAPES = [(32, 32), (64, 64), (128, 128), (256, 256), (512, 512)]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def set_shapes(self, shape_file_path=None):
        # Override to use square matrix shapes
        self.shapes = self.MATRIX_EXP_SHAPES


@pytest.mark.linalg_matrix_exp
def test_perf_linalg_matrix_exp():
    bench = MatrixExpBenchmark(
        input_fn=matrix_exp_input_fn,
