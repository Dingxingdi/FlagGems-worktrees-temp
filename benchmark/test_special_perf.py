...
def _amp_foreach_input_fn(shape, cur_dtype, device):
    ...
    yield [inp1, inp2], found_inf, inv_scale


@pytest.mark.amp_foreach_non_finite_check_and_unscale
def test_perf_amp_foreach_non_finite_check_and_unscale():
    bench = GenericBenchmark(
        op_name="_amp_foreach_non_finite_check_and_unscale_",
        torch_op=torch._amp_foreach_non_finite_check_and_unscale_,
        dtypes=[torch.float32, torch.float16],
        input_fn=_amp_foreach_input_fn,
    )
    bench.shapes = _AMP_FOREACH_SHAPES
    bench.run()
