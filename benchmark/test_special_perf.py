...
# Efficient attention forward benchmark
EFFICIENT_ATTENTION_SHAPES = [
    (1, 4, 8, 32),
    (1, 4, 16, 32),
    (1, 8, 32, 64),
    (2, 8, 32, 64),
...
class EfficientAttentionForwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = EFFICIENT_ATTENTION_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            yield q, k, v, None, None, None, None, None, 0.0, 0, False


@pytest.mark.efficient_attention_forward
def test_perf_efficient_attention_forward():
    bench = EfficientAttentionForwardBenchmark(
        op_name="efficient_attention_forward",
        torch_op=torch.ops.aten._efficient_attention_forward,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
