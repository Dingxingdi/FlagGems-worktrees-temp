...
# sym_constrain_range_for_size benchmark
# This is a no-op operator that takes a scalar and returns nothing
# We benchmark it with simple integer inputs
def sym_constrain_range_for_size_input_fn(shape, cur_dtype, device):
    # Since this operator takes a scalar, we just yield the scalar value
    # min and max are keyword-only arguments
    yield 5, {"min": 0, "max": 10}


@pytest.mark.sym_constrain_range_for_size
@pytest.mark.performance
def test_perf_sym_constrain_range_for_size():
    bench = GenericBenchmark(
