...
def test_perf_linalg_lu_factor_ex():
    ...
    print("\nOperator: linalg_lu_factor_ex  Performance Test (dtype=torch.float32, mode=kernel)")
    for shape in shapes:
        # Generate a non-singular matrix for LU factorization
        torch.manual_seed(42)
        L = torch.tril(torch.randn(shape, dtype=torch.float32, device='cuda') * 0.5 + 0.5)
        U = torch.triu(torch.randn(shape, dtype=torch.float32, device='cuda') * 0.5 + 0.5)
        # Ensure diagonal of U is positive
        ...
        start.record()
        for _ in range(iterations):
            _ = torch.linalg.lu_factor_ex(A_ref)
        end.record()
        torch.cuda.synchronize()
        ref_elapsed = start.elapsed_time(end) / iterations

        speedup = ref_elapsed / elapsed if elapsed > 0 else 0
        print(f"SUCCESS    {ref_elapsed:.4f}    {elapsed:.4f}    {speedup:.4f}    [shape={shape}]")
