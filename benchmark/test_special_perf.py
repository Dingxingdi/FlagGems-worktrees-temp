...
MASKED_SCATTER_BACKWARD_SHAPES = [
    (4, 4),
    (8, 16),
    (16, 32),
    (32, 64),
...
class MaskedScatterBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = MASKED_SCATTER_BACKWARD_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            grad_output = torch.randn(shape, dtype=cur_dtype, device=self.device)
            mask = torch.randn(shape, dtype=cur_dtype, device=self.device) < 0.5
            numel = mask.sum().item()
            yield grad_output, mask, [numel]


@pytest.mark.masked_scatter_backward
def test_perf_masked_scatter_backward():
