...
# Shapes for unsafe_split benchmark
UNSAFE_SPLIT_SHAPES = [
    (8, 16),
    (16, 32),
    (32, 64),
...
class UnsafeSplitBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = UNSAFE_SPLIT_SHAPES
...
def unsafe_split_input_fn(shape, cur_dtype, device):
    dim = 0 if len(shape) == 1 else 1
    split_size = shape[dim] // 2 if shape[dim] > 1 else 1
...
@pytest.mark.performance
def test_perf_unsafe_split():
    bench = GenericBenchmark(
        op_name="unsafe_split",
        torch_op=torch.unsafe_split,
        dtypes=FLOAT_DTYPES,
        input_fn=unsafe_split_input_fn,
    )
    bench.run()
