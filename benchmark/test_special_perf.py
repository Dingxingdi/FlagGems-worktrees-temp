...
# FFT test shapes (small sizes for naive DFT implementation)
FFT_SHAPES = [(4, 4), (8, 8)]


class FFTIFft2Benchmark(GenericBenchmark2DOnly):
    def set_shapes(self, shape_file_path=None):
...
def torch_fft_ifft2(input):
    return torch.fft.ifft2(input)


@pytest.mark.fft_ifft2
def test_perf_fft_ifft2():
    bench = FFTIFft2Benchmark(
        input_fn=fft_ifft2_input_fn,
        op_name="fft_ifft2",
        torch_op=torch_fft_ifft2,
        dtypes=COMPLEX_DTYPES,
    )
    bench.set_gems(torch.fft.ifft2)
    bench.run()
