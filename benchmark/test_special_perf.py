...
# Embedding bag benchmark
EMBEDDING_BAG_SHAPES = [
    (10000, 128),   # (num_embeddings, embedding_dim)
    (50000, 256),
    (100000, 512),
    (500000, 256),
]
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            indices = torch.randint(
                0, num_embeddings, (num_indices,), device=self.device, dtype=torch.int64
            )
            offsets = torch.zeros(
                num_bags, device=self.device, dtype=torch.int64
            )
...
def test_perf__embedding_bag_forward_only():
    bench = EmbeddingBagBenchmark(
        op_name="_embedding_bag_forward_only",
        torch_op=torch._embedding_bag_forward_only,
        dtypes=[torch.float16, torch.float32],
    )
    bench.run()
