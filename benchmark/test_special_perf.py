...
# Benchmark for _thnn_fused_lstm_cell_backward_impl
class LSTMCellBackwardBenchmark(Benchmark):
    def get_inputs(self, batch_size, hidden_size, dtype):
        # Create input tensors
        input_gates = torch.randn(batch_size, 4 * hidden_size, dtype=dtype, device=self.device)
        hidden_gates = torch.randn(batch_size, 4 * hidden_size, dtype=dtype, device=self.device)
        cx = torch.randn(batch_size, hidden_size, dtype=dtype, device=self.device)
        input_bias = torch.zeros(4 * hidden_size, dtype=dtype, device=self.device)
        hidden_bias = torch.randn(4 * hidden_size, dtype=dtype, device=self.device)
    ...
    def get_input_iter(self, dtype):
        ...
        for hs in hidden_sizes:
            for bs in batch_sizes:
                yield self.get_inputs(bs, hs, dtype)

    def forward(self, grad_hy, grad_cy, cx, cy, workspace, has_bias):
        return self.torch_op(grad_hy, grad_cy, cx, cy, workspace, has_bias)


@pytest.mark.thnn_fused_lstm_cell_backward_impl
def test_perf_thnn_fused_lstm_cell_backward_impl():
    bench = LSTMCellBackwardBenchmark(
        op_name="_thnn_fused_lstm_cell_backward_impl",
