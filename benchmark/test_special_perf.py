...
@pytest.mark.item
def test_perf_item():
    """Benchmark item operator - extracts scalar from 1-element tensor"""

    def item_input_fn(shape, dtype, device):
        # item() only works for 1-element tensors
        yield torch.tensor([1.0], dtype=dtype, device=device)

    bench = GenericBenchmark(
        input_fn=item_input_fn,
        op_name="torch.item",
        torch_op=torch.Tensor.item,
        dtypes=FLOAT_DTYPES,
    )
