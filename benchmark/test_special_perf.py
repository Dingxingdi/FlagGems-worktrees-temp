...
def fft_input_fn(shape, dtype, device):
    inp = torch.randn(*shape, dtype=dtype, device=device)
    yield inp,


class FFTIfftBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        self.shapes = FFT_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            yield from fft_input_fn(shape, cur_dtype, self.device)


@pytest.mark.fft_ifftn
def test_perf_fft_ifftn():
    bench = FFTIfftBenchmark(
        op_name="fft_ifftn",
        torch_op=torch.ops.aten.fft_ifftn,
        dtypes=COMPLEX_DTYPES,
        input_fn=fft_input_fn,
    )
    bench.run()


class FFTFftnBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        self.shapes = FFT_SHAPES
...
def test_perf_fft_fftn():
    bench = FFTFftnBenchmark(
        op_name="fft_fftn",
        torch_op=torch.ops.aten.fft_fftn,
        dtypes=COMPLEX_DTYPES,
        input_fn=fft_input_fn,
    )
    bench.run()
