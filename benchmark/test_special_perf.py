...
class SparseCooTensorWithDimsAndTensorsBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = [
            (1000, 1000),
            (5000, 5000),
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            nnz = min(1000, shape[0] * shape[1] // 10)
            sparse_dim = 2
            # Generate random indices within bounds
            indices = torch.randint(0, shape[0], (sparse_dim, nnz), dtype=torch.long, device=self.device)
            indices[1] = torch.randint(0, shape[1], (nnz,), dtype=torch.long, device=self.device)
            # Generate random values
            values = torch.randn(nnz, dtype=cur_dtype, device=self.device)
            # Use torch.sparse_coo_tensor as reference (since _sparse_coo_tensor_with_dims_and_tensors is not available in CUDA)
            yield sparse_dim, 0, list(shape), indices, values


def sparse_coo_tensor_ref(sparse_dim, dense_dim, size, indices, values):
    """Reference implementation using torch.sparse_coo_tensor"""
    return torch.sparse_coo_tensor(indices, values, size)


@pytest.mark.sparse_coo_tensor_with_dims_and_tensors
def test_perf__sparse_coo_tensor_with_dims_and_tensors():
    bench = SparseCooTensorWithDimsAndTensorsBenchmark(
        op_name="_sparse_coo_tensor_with_dims_and_tensors",
        torch_op=sparse_coo_tensor_ref,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
