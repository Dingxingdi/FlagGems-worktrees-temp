...
def test_perf_Fused_Cross_Entropy():
    def fused_ce_input_fn(shape, dtype, device):
        M, C = shape
        logits = torch.randn(M, C, dtype=dtype, device=device)
        target = torch.randint(0, C, (M,), device=device)
        yield logits, target

    bench = GenericBenchmark2DOnly(
        input_fn=fused_ce_input_fn,
        op_name="Fused_Cross_Entropy",
        torch_op=torch.nn.functional.cross_entropy,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(lambda x, y: flag_gems.Fused_Cross_Entropy(x, y, reduction="mean")[0])
    bench.run()
