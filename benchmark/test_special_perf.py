...
class FftIhfft2Benchmark(GenericBenchmark2DOnly):
    def set_more_shapes(self):
        return FFT_IHFFT2_SHAPES


def fft_ihfft2_input_fn(shape, dtype, device):
    inp = torch.randn(shape, dtype=dtype, device=device)
    yield inp,


bench = FftIhfft2Benchmark(
    input_fn=fft_ihfft2_input_fn,
    op_name="fft_ihfft2",
    torch_op=torch.ops.aten.fft_ihfft2,
    dtypes=[torch.float32, torch.float16],
)


@pytest.mark.fft_ihfft2
def test_perf_fft_ihfft2():
    bench.run()
