...
def test_perf_Conv_BN_ReLU():
    """Benchmark for fused Conv2d + BatchNorm + ReLU operator."""
    import time
    ...
    for dtype in dtypes:
        for shape in shapes:
            ...
            weight = torch.randn(out_channels, C, 3, 3, dtype=dtype, device="cuda")
            bias = torch.randn(out_channels, dtype=dtype, device="cuda")
            running_mean = torch.zeros(out_channels, dtype=dtype, device="cuda")
            running_var = torch.ones(out_channels, dtype=dtype, device="cuda")
            bn_weight = torch.ones(out_channels, dtype=dtype, device="cuda")
            bn_bias = torch.zeros(out_channels, dtype=dtype, device="cuda")

            # Reference implementation
            def ref_impl():
                conv_out = torch.nn.functional.conv2d(input_tensor, weight, bias, padding=1)
                bn_out = torch.nn.functional.batch_norm(
                    conv_out, running_mean, running_var, bn_weight, bn_bias, training=False, eps=1e-5
                )
            ...
            def gems_impl():
                return Conv_BN_ReLU(
                    input_tensor, weight, bias, running_mean, running_var, bn_weight, bn_bias,
                    stride=1, padding=1, dilation=1, groups=1, eps=1e-5
                )
            ...
            for _ in range(n_iter):
                gems_impl()
            gems_time = (time.time() - start) / n_iter * 1000

            speedup = ref_time / gems_time

            print(f"dtype={dtype}, shape={shape}: ref={ref_time:.4f}ms, gems={gems_time:.4f}ms, speedup={speedup:.4f}x")
