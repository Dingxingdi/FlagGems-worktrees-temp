...
class RreluWithNoiseBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = RRELU_WITH_NOISE_SHAPES

    def get_input_iter(self, cur_dtype):
        lower, upper = 0.125, 1.0 / 3.0
        for shape in self.shapes:
            inp = generate_tensor_input(shape, cur_dtype, self.device)
            noise = torch.rand(shape, dtype=cur_dtype, device=self.device) * (upper - lower) + lower
            yield inp, noise, lower, upper
...
def test_perf_rrelu_with_noise_functional():
