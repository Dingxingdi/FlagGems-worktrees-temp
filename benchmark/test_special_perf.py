...
# _unsafe_index benchmark
UNSAFE_INDEX_BENCH_SHAPES = [
    (32, 64, 128),
    (64, 128, 256),
    (128, 256, 512),
    (256, 512, 1024),
...
class UnsafeIndexBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = UNSAFE_INDEX_BENCH_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            inp = torch.randn(shape, dtype=cur_dtype, device=self.device)
            # Use two index tensors
            idx1_size = 16
            idx2_size = 16
            idx1 = torch.randint(0, shape[0], (idx1_size,), device=self.device)
            idx2 = torch.randint(0, shape[1], (idx2_size,), device=self.device)
            yield inp, (idx1, idx2)


@pytest.mark.unsafe_index
def test_perf_unsafe_index():
    bench = UnsafeIndexBenchmark(
        op_name="_unsafe_index",
        torch_op=torch._unsafe_index,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
