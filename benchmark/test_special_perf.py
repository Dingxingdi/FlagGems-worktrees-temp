...
class FftIhfftBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FFT_IHFFT_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            x = torch.randn(shape, dtype=cur_dtype, device=self.device)
            yield (x,)  # Wrap in tuple as expected by benchmark framework


@pytest.mark.fft_ihfft
def test_perf_fft_ihfft():
    bench = FftIhfftBenchmark(
        op_name="fft_ihfft",
        torch_op=torch.fft.ihfft,
        dtypes=[torch.float32],
    )
    bench.run()
