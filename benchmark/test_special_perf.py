...
FUSED_MOVING_AVG_OBS_FQ_HELPER_SHAPES = [
    (2, 4),
    (4, 8),
    (16, 32),
    (32, 64),
...
class FusedMovingAvgObsFqHelperBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FUSED_MOVING_AVG_OBS_FQ_HELPER_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            self_t = torch.randn(shape, dtype=cur_dtype, device=self.device)
            observer_on = torch.ones(shape, dtype=cur_dtype, device=self.device)
            fake_quant_on = torch.ones(shape, dtype=cur_dtype, device=self.device)
            running_min = torch.tensor([0.0], dtype=torch.float32, device=self.device)
            running_max = torch.tensor([1.0], dtype=torch.float32, device=self.device)
            scale = torch.tensor([0.1], dtype=torch.float32, device=self.device)
            zero_point = torch.tensor([0.0], dtype=torch.float32, device=self.device)
            averaging_const = 0.1
            quant_min = 0
            quant_max = 127
            ch_axis = 1
...
def test_perf_fused_moving_avg_obs_fq_helper():
    bench = FusedMovingAvgObsFqHelperBenchmark(
        op_name="_fused_moving_avg_obs_fq_helper",
        torch_op=flag_gems._fused_moving_avg_obs_fq_helper,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
