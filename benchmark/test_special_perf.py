...
LSTSQ_SHAPES = [
    ...
    (200, 100, 20),  # m=200, n=100, k=20
]


class LinalgLstsqBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = LSTSQ_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            m, n, k = shape
            A = torch.randn(m, n, dtype=cur_dtype, device=self.device)
            B = torch.randn(m, k, dtype=cur_dtype, device=self.device)
            yield A, B


@pytest.mark.linalg_lstsq
def test_perf_linalg_lstsq():
    bench = LinalgLstsqBenchmark(
        op_name="linalg_lstsq",
        torch_op=torch.linalg.lstsq,
        dtypes=[torch.float32],  # PyTorch doesn't support float16 for lstsq
    )
    bench.run()
