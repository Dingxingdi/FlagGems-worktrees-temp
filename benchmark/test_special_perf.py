...
# Shapes for linalg_vander: (*, n) -> output (*, n, N)
# These are small shapes to avoid OOM
VANDER_SHAPES = [
    (4,),
    (8,),
    (16,),
...
def test_perf_linalg_vander():
    # Since linalg_vander is not in core_shapes.yaml and would use DEFAULT_SHAPES
    # which contains huge shapes causing OOM, we need to manually run the benchmark
    # with custom shapes

    import triton.testing as testing
    import flag_gems
    from benchmark.performance_utils import generate_tensor_input
    ...
    for shape in VANDER_SHAPES:
        # Generate input
        x = generate_tensor_input(shape, dtype, device)

        # Benchmark torch.linalg.vander
        latency_torch = testing.do_bench(
            lambda: torch.linalg.vander(x),
            warmup=10,
            rep=40,
            return_mode="median",
        )

        # Benchmark flag_gems.linalg_vander
        with flag_gems.use_gems():
            latency_gems = testing.do_bench(
                lambda: torch.linalg.vander(x),
                warmup=10,
                rep=40,
                return_mode="median",
        ...
        speedup = latency_torch / latency_gems
        print(f"Shape: {shape}, Torch: {latency_torch:.4f}ms, GEMS: {latency_gems:.4f}ms, Speedup: {speedup:.4f}")

        results.append({
            "shape": shape,
            "torch_latency_ms": latency_torch,
            "gems_latency_ms": latency_gems,
            "speedup": speedup,
        })
