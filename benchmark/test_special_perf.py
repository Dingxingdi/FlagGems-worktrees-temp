...
def test_perf__upsample_nearest_exact2d_backward():
    class UpsampleNearestExact2dBackwardBenchmark(Benchmark):
        def set_shapes(self, shape_file_path=None):
            self.shapes = [
                (2, 3, 16, 16),
                (4, 8, 32, 32),
        ...
        def get_input_iter(self, cur_dtype):
            for shape in self.shapes:
                N, C, H, W = shape
                out_size = [H * 2, W * 2]
                grad_output = torch.randn(N, C, out_size[0], out_size[1], dtype=cur_dtype, device=self.device)
                input_size = [N, C, H, W]
                yield grad_output, out_size, input_size, None, None

    bench = UpsampleNearestExact2dBackwardBenchmark(
        op_name="_upsample_nearest_exact2d_backward",
        torch_op=torch.ops.aten._upsample_nearest_exact2d_backward,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
