...
SEGMENT_REDUCE_BACKWARD_SHAPES = [
    (64,),
    (128,),
    (256,),
    (512,),
...
class SegmentReduceBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SEGMENT_REDUCE_BACKWARD_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            lengths = torch.tensor([segment_length] * n_segments, dtype=torch.long, device=self.device)
            # Compute forward output
            output = torch.segment_reduce(data, "sum", lengths=lengths)
            grad = torch.ones(n_segments, dtype=cur_dtype, device=self.device)
            # Use dict for keyword arguments
            yield (grad, output, data, {"reduce": "sum", "lengths": lengths})


@pytest.mark.segment_reduce_backward
def test_perf_segment_reduce_backward():
    bench = SegmentReduceBackwardBenchmark(
        op_name="_segment_reduce_backward",
