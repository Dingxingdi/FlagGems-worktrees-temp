...
def test_perf_searchsorted():
    def searchsorted_input_fn(shape, dtype, device):
        sorted_sequence = torch.sort(torch.randn(shape, dtype=dtype, device=device))[0]
        values = torch.randn(shape[0], shape[1] // 2, dtype=dtype, device=device)
        yield sorted_sequence, values

    bench = GenericBenchmark2DOnly(
        input_fn=searchsorted_input_fn,
