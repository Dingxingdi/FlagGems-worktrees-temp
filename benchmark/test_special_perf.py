...
class LinalgDetBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            # shape is a tuple like (2, 2) representing (n, n) matrix
            # Yield a tuple containing the single matrix argument
            n = shape[0]
            x = torch.randn(n, n, dtype=cur_dtype, device=self.device)
            yield (x,)  # Wrap in tuple to avoid unpacking issues
