...
class RnnTanhBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = RNN_TANH_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            # Prepare inputs
            input_tensor = torch.randn(
                batch, seq_len, input_size, dtype=cur_dtype, device=self.device
            )
            hx = torch.randn(
                1, batch, hidden_size, dtype=cur_dtype, device=self.device
            )
            # Get parameters
            params = tuple(p.to(cur_dtype).to(self.device) for p in rnn.parameters())
            yield input_tensor, hx, params, True, 1, 0.0, False, False, True


@pytest.mark.rnn_tanh
def test_perf_rnn_tanh():
    bench = RnnTanhBenchmark(
        op_name="rnn_tanh",
