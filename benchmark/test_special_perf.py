...
def weight_int4pack_mm_input_fn(shape, dtype, device):
    """Input function for _weight_int4pack_mm benchmark.

    shape is (M, K) where M is batch, K is input dim.
    Output dim N is set to K for square-ish matrices.
    """
    M, K = shape
    N = K  # Output dimension same as input for benchmarking
    qGroupSize = 32

    # Activation (M, K)
    act = torch.randn(M, K, dtype=dtype, device=device)

    # Weight in packed format (N, K/2)
    weight = torch.randint(0, 16, (N, K // 2), dtype=torch.uint8, device=device)

    # Scale (per-channel)
    scale = torch.ones(N, dtype=dtype, device=device) * 0.1

    yield act, weight, qGroupSize, scale
