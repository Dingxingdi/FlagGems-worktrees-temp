...
@pytest.mark.view
def test_perf_view():
    def view_input_fn(shape, dtype, device):
        # Create a tensor with enough elements for different view shapes
        # shape is provided by the benchmark framework
        # We use a fixed 12-element tensor and reshape to (4, 3)
        yield torch.randn(12, dtype=dtype, device=device), (4, 3)

    bench = GenericBenchmark(
        input_fn=view_input_fn,
        op_name="torch.Tensor.view",
        torch_op=torch.Tensor.view,
        dtypes=FLOAT_DTYPES,
    )
