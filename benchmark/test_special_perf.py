...
class LinearBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for batch, in_features in self.shapes:
            ...
            weight = torch.randn(out_features, in_features, dtype=cur_dtype, device=self.device)
            grad_output = torch.randn(batch, out_features, dtype=cur_dtype, device=self.device)
            yield input, grad_output, weight, (True, True, True)


@pytest.mark.linear_backward
def test_perf_linear_backward():
