...
def test_perf_multilabel_margin_loss_forward():
    # Define benchmark shapes specifically for this operator (N, C)
    shapes = [(2, 3), (16, 512), (128, 256), (512, 512)]

    for shape in shapes:
        N, C = shape
        input = torch.randn(shape, dtype=torch.float32, device='cuda')
        # Generate target with random class indices
        target = torch.zeros((N, C), dtype=torch.long, device='cuda')
        for i in range(N):
            num_classes = random.randint(1, C - 1)
            classes = torch.randperm(C)[:num_classes].sort()[0]
            target[i, :num_classes] = classes
            if num_classes < C:
                target[i, num_classes] = 0

        # Warmup
        for _ in range(10):
            _ = torch.ops.aten.multilabel_margin_loss_forward(input, target, 0)
        ...
        print(f"Shape {shape}: {elapsed:.4f} ms")
