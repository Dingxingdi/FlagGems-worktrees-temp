...
def linalg_svd_input_fn(shape, dtype, device):
    """Input function for linalg_svd benchmark."""
    m, n = shape
    A = torch.randn(m, n, dtype=dtype, device=device)
    yield A, {"full_matrices": False}


class LinalgSvdBenchmark(GenericBenchmark2DOnly):
    def set_more_shapes(self):
        # SVD shapes for benchmarking
        return [
            (4, 3),
            (8, 5),
            (16, 12),
...
def test_perf_linalg_svd():
    bench = LinalgSvdBenchmark(
        input_fn=linalg_svd_input_fn,
        op_name="linalg_svd",
        torch_op=torch.linalg.svd,
        dtypes=[torch.float32],  # SVD only supports float32 and float64
    )
    bench.run()
