...
NESTED_TENSOR_FROM_TENSOR_LIST_SHAPES = [
    ([3], [4], [2]),
    ([1], [1], [1]),
    ([5, 3], [3, 4], [2, 5]),
    ([10], [20], [30], [40]),
...
class NestedTensorFromTensorListBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = NESTED_TENSOR_FROM_TENSOR_LIST_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shapes in self.shapes:
            ...
            # Wrap in a tuple so it's passed as a single argument
            yield (tensors,)


@pytest.mark.nested_tensor_from_tensor_list
def test_perf_nested_tensor_from_tensor_list():
    bench = NestedTensorFromTensorListBenchmark(
        op_name="_nested_tensor_from_tensor_list",
        torch_op=torch._nested_tensor_from_tensor_list,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
