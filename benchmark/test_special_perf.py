...
def test_perf_fft_hfftn():
    class FFTBenchmark(GenericBenchmark):
        def set_shapes(self, shape_file_path=None):
            # FFT requires power of 2 sizes for best performance
            self.shapes = [
                (16, 64),
                (32, 128),
                (64, 256),
    ...
    bench = FFTBenchmark(
        input_fn=fft_hfftn_input_fn,
        op_name="fft_hfftn",
        torch_op=torch.fft.rfftn,
        dtypes=[torch.float32, torch.float16],  # Only float32 and float16 for FFT
    )
    bench.run()
