...
# Pad sequence benchmark configurations
# Format: (num_seqs, max_seq_len, feature_dim)
PAD_SEQUENCE_SHAPES = [
    (8, 32, 128),
    (16, 64, 256),
    (32, 128, 512),
...
class PadSequenceBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = PAD_SEQUENCE_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            num_seqs, max_seq_len, feature_dim = shape
            # Generate variable-length sequences
            # Lengths vary from 1 to max_seq_len
            lengths = torch.randint(1, max_seq_len + 1, (num_seqs,)).tolist()
            sequences = [
                torch.randn(length, feature_dim, dtype=cur_dtype, device=self.device)
                for length in lengths
            ]
            yield sequences, False, 0.0, "right"


@pytest.mark.pad_sequence
def test_perf_pad_sequence():
