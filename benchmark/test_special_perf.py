...
# FlashDecoding benchmark
FLASH_DECODING_SHAPES = [
    (1, 4, 1, 32, 32),   # (batch, heads, seq_q, head_dim, seq_kv)
    (2, 4, 1, 64, 64),
    (2, 8, 1, 64, 128),
    (4, 4, 1, 64, 256),
    (4, 8, 1, 128, 512),
...
class FlashDecodingBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FLASH_DECODING_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            batch, heads, seq_q, head_dim, seq_kv = shape
            Q = torch.randn(batch, heads, seq_q, head_dim, dtype=cur_dtype, device=self.device)
            K = torch.randn(batch, heads, seq_kv, head_dim, dtype=cur_dtype, device=self.device)
            V = torch.randn(batch, heads, seq_kv, head_dim, dtype=cur_dtype, device=self.device)
            yield Q, K, V


@pytest.mark.flash_decoding
def test_perf_flash_decoding():
    # For benchmark, use torch.nn.functional.scaled_dot_product_attention as reference
    # since FlashDecoding is a custom operator
    bench = FlashDecodingBenchmark(
        op_name="flash_decoding",
        torch_op=lambda q, k, v: torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=True),
        dtypes=[torch.float16, torch.float32],
    )
    bench.set_gems(lambda q, k, v: torch.ops.aten.flash_decoding(q, k, v))
    bench.run()
