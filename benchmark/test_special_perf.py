...
def split_input_fn(shape, dtype, device):
    # Split along the last dimension by half
    dim = len(shape) - 1
    split_size = shape[dim] // 2
    input_tensor = torch.randn(shape, dtype=dtype, device=device)
    yield input_tensor, split_size, dim


class SplitBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SPLIT_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            yield from split_input_fn(shape, cur_dtype, self.device)
