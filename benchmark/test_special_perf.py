...
def test_perf_pdist_backward():
    bench = PdistBackwardBenchmark(
        ...
        torch_ops=torch.ops.aten._pdist_backward,
        dtypes=[torch.float32],
    )
    bench.run()


# DynamicQuantizeLSTM benchmark
DYNAMIC_QUANTIZE_LSTM_SHAPES = [
    (16,),
    (32, 64),
    (64, 128),
    (128, 256),
...
class DynamicQuantizeLSTMBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = DYNAMIC_QUANTIZE_LSTM_SHAPES
...
def test_perf_DynamicQuantizeLSTM():
    bench = DynamicQuantizeLSTMBenchmark(
        op_name="DynamicQuantizeLSTM",
        torch_op=flag_gems.dynamic_quantize_lstm,
        dtypes=[torch.float32],
    )
    bench.run()
