...
def hermite_polynomial_h_input_fn(shape, dtype, device):
    x = torch.randn(shape, dtype=dtype, device=device)
    n = torch.randint(0, 10, shape, dtype=torch.long, device=device)
    yield x, n
...
special_operations = [
    ...
    # Hermite Polynomial - only float32 is supported by PyTorch reference
    (
        "special_hermite_polynomial_h",
        torch.special.hermite_polynomial_h,
        [torch.float32],
        hermite_polynomial_h_input_fn,
    ),
]
