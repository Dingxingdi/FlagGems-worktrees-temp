...
# Shapes for padded_dense_to_jagged_forward benchmark
...
class PaddedDenseToJaggedBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = PADDED_DENSE_TO_JAGGED_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            batch_size, max_seq_len, hidden_size = shape
            # Generate random sequence lengths
            seq_lengths = torch.randint(1, max_seq_len + 1, (batch_size,), device=self.device)
            # Create offsets tensor
            offsets = torch.zeros(batch_size + 1, dtype=torch.int64, device=self.device)
            offsets[1:] = torch.cumsum(seq_lengths, dim=0)
            # Create dense tensor
            dense = torch.randn(batch_size, max_seq_len, hidden_size, dtype=cur_dtype, device=self.device)
            yield dense, [offsets]


@pytest.mark.padded_dense_to_jagged_forward
def test_perf_padded_dense_to_jagged_forward():
    bench = PaddedDenseToJaggedBenchmark(
        op_name="_padded_dense_to_jagged_forward",
