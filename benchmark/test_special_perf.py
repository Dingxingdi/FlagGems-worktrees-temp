...
def test_perf_upsample_bicubic2d_aa_backward(align_corners, scale):
    def upsample_bicubic2d_aa_backward_input_fn(shape, dtype, device):
        batch, channel, height, width = shape
        output_size = (int(height * scale[0]), int(width * scale[1]))
        grad_output = torch.randn(
            (batch, channel, output_size[0], output_size[1]), device=device, dtype=dtype
        )
    ...
    bench = UpsampleBenchmark(
        ...
        torch_op=torch.ops.aten._upsample_bicubic2d_aa_backward.default,
        dtypes=dtypes,
    )
    bench.run()
