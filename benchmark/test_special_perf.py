...
def linalg_lu_op(A):
    # linalg_lu returns (P, L, U) tuple, just compute to trigger execution
    P, L, U = torch.linalg.lu(A)
    # Return L to match benchmark expectation (single tensor output)
    return L


class LinalgLuBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = LINALG_LU_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            m, n = shape
            A = torch.randn(m, n, dtype=cur_dtype, device=self.device)
            # Return as list to prevent unpacking
            yield [A]


@pytest.mark.linalg_lu
def test_perf_linalg_lu():
    bench = LinalgLuBenchmark(
        op_name="linalg_lu",
        torch_op=linalg_lu_op,
        dtypes=[torch.float32],
    )
    bench.run()
