...
def linalg_solve_ex_input_fn(shape, dtype, device):
    # shape is ignored, we use fixed shapes
    for n in [2, 4, 8, 16, 32]:
        A = torch.randn(n, n, dtype=dtype, device=device)
        A = A + torch.eye(n, dtype=dtype, device=device) * n
        B = torch.randn(n, dtype=dtype, device=device)
        yield {"A": A, "B": B},


class LinalgSolveExBenchmark(GenericBenchmark):
    def set_more_shapes(self):
        return None


@pytest.mark.linalg_solve_ex
def test_perf_linalg_solve_ex():
