...
EMBEDDING_SPMDM_SHAPES = [
    (1, 64, 128),    # (batch_size, embedding_dim, output_dim)
    (8, 128, 256),
    (32, 256, 512),
    (64, 512, 1024),
    (128, 256, 128),
...
class EmbeddingSpMDMBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            indices = torch.randint(0, num_embeddings, (batch_size,), device=self.device)
            dense = torch.randn(embedding_dim, output_dim, dtype=cur_dtype, device=self.device)
