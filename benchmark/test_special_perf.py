...
# Fused_RoPE benchmark
FUSED_ROPE_SHAPES = [
    (1, 8, 16),    # batch_size=1, seq_len=8, num_heads=16
    (2, 16, 16),
    (4, 32, 16),
    (8, 64, 16),
]
...
class FusedRoPEBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FUSED_ROPE_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        ...
        for shape in self.shapes:
            ...
            cos = freqs.cos().to(cur_dtype)
            sin = freqs.sin().to(cur_dtype)

            yield query, key, cos, sin


@pytest.mark.Fused_RoPE
def test_perf_Fused_RoPE():
    bench = FusedRoPEBenchmark(
        op_name="Fused_RoPE",
        torch_op=flag_gems.fused.apply_rotary_pos_emb,
        dtypes=[torch.float16, torch.float32],
    )
    bench.run()
