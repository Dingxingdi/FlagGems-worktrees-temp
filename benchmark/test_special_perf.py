...
class UnsqueezeCopyBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = UNSQUEEZE_COPY_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            x = torch.randn(shape, dtype=cur_dtype, device=self.device)
            # Test dim=0 for benchmark (most common use case)
            yield x, 0


@pytest.mark.unsqueeze_copy
def test_perf_unsqueeze_copy():
    bench = UnsqueezeCopyBenchmark(
        op_name="unsqueeze_copy",
        torch_op=torch.unsqueeze_copy,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
