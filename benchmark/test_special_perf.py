...
class ConvBNReLUBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = CONV_BN_RELU_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            N, C_in, H, W = shape
            C_out = min(64, C_in * 2)  # Use reasonable output channels
            kH, kW = 3, 3

            # Generate input and weight
            input = torch.randn(N, C_in, H, W, dtype=cur_dtype, device=self.device)
            weight = torch.randn(C_out, C_in, kH, kW, dtype=cur_dtype, device=self.device)

            # Return as tuple of args (input, weight) and kwargs (stride, padding)
            yield (input, weight, {"stride": 1, "padding": 1})


@pytest.mark.conv_bn_relu
def test_perf_conv_bn_relu():
    bench = ConvBNReLUBenchmark(
        op_name="conv_bn_relu",
        torch_op=flag_gems.conv_bn_relu,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
