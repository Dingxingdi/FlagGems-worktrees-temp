...
def convert_weight_to_int4pack_for_cpu_input_fn(shape, dtype, device):
    """Input function for _convert_weight_to_int4pack_for_cpu benchmark."""
    # shape can be (M, N) or (B, M, N) - take the last two as M, N
    if len(shape) >= 2:
        M, N = shape[-2], shape[-1]
    else:
        M, N = shape[0], 1
    # innerKTiles must be 2, 4, or 8 and N must be divisible by 16*innerKTiles
    innerKTiles = 2
    # Ensure N is valid
    if N % (16 * innerKTiles) != 0:
        N = ((N + 16 * innerKTiles - 1) // (16 * innerKTiles)) * (16 * innerKTiles)
    x = torch.randint(0, 255, (M, N), dtype=dtype, device=device)
    yield x, innerKTiles


@pytest.mark.convert_weight_to_int4pack_for_cpu
def test_perf_convert_weight_to_int4pack_for_cpu():
    # This operator doesn't have a PyTorch CUDA implementation,
    # so we benchmark the FlagGems implementation directly
    import time

    shapes = [(16, 64), (16, 128), (16, 256), (32, 128), (32, 256), (64, 256)]
    dtypes = [torch.uint8]

    for dtype in dtypes:
        for shape in shapes:
            M, N = shape
            # Ensure N is valid for innerKTiles=2
            innerKTiles = 2
            if N % (16 * innerKTiles) != 0:
                N = ((N + 16 * innerKTiles - 1) // (16 * innerKTiles)) * (16 * innerKTiles)

            x = torch.randint(0, 255, (M, N), dtype=dtype, device="cuda")

            # Warmup
            with flag_gems.use_gems(include=["_convert_weight_to_int4pack_for_cpu"]):
                for _ in range(10):
                    _ = torch._convert_weight_to_int4pack_for_cpu(x, innerKTiles=innerKTiles)
                ...
                gems_latency = (end - start) / num_iters * 1000  # ms
                speedup = 1.0  # No reference to compare against

                print(f"SUCCESS    N/A    {gems_latency:.3f}    {speedup:.3f}    N/A    [{shape}]")
