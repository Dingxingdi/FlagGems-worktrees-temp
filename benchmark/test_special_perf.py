...
class LinalgDetBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            # Add offset to ensure invertible matrix
...
def test_perf_linalg_det():
    bench = LinalgDetBenchmark(
        op_name="linalg_det",
        torch_op=torch.linalg.det,
        dtypes=[torch.float32],  # Only test float32 since torch.linalg.det doesn't support float16 on CUDA
    )
    bench.run()
