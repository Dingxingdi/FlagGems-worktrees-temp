...
class FusedAdamBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            param = torch.randn(shape, dtype=cur_dtype, device=self.device)
            grad = torch.randn(shape, dtype=cur_dtype, device=self.device)
            exp_avg = torch.zeros_like(param)
            exp_avg_sq = torch.zeros_like(param)
            state_step = torch.tensor([1.0], device=self.device)
            # Yield positional args and kwargs separately
            # Positional args: self, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps
            # Kwargs: lr, beta1, beta2, weight_decay, eps, amsgrad, maximize
            yield [param], [grad], [exp_avg], [exp_avg_sq], [], [
                state_step
            ], {
                "lr": 0.01,
                "beta1": 0.9,
...
def test_perf__fused_adam():
    bench = FusedAdamBenchmark(
        op_name="_fused_adam",
        torch_op=torch.ops.aten._fused_adam,
