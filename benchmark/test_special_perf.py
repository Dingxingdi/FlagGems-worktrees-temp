...
def add_softmax_input_fn(shape, cur_dtype, device):
    input1 = torch.randn(shape, dtype=cur_dtype, device=device)
    input2 = torch.randn(shape, dtype=cur_dtype, device=device)
    yield input1, input2


@pytest.mark.add_softmax
def test_perf_add_softmax():
    bench = GenericBenchmark(
        op_name="add_softmax",
        torch_op=lambda x, y: torch.softmax(x + y, dim=-1),
        dtypes=FLOAT_DTYPES,
        input_fn=add_softmax_input_fn,
        shapes=ADD_SOFTMAX_SHAPES,
    )
    bench.run()
