...
# _adaptive_avg_pool3d_backward benchmark
POOLING3D_BENCHMARK_SHAPES = [
    (1, 3, 8, 8, 8),
    (2, 4, 16, 16, 16),
    (1, 1, 4, 4, 4),
    (2, 3, 8, 12, 16),
...
def _adaptive_avg_pool3d_backward_input_fn(shape, cur_dtype, device):
    N, C, D, H, W = shape
    # Output size is half of input size for benchmark
    output_size = (D // 2, H // 2, W // 2)
    input_tensor = torch.randn(size=shape, device=device, dtype=cur_dtype)
    # Get the output first to know grad_output shape
    output = torch._adaptive_avg_pool3d(input_tensor, output_size)
    grad_output = torch.randn(size=output.shape, device=device, dtype=cur_dtype)
    yield grad_output, input_tensor


class AdaptiveAvgPool3dBackwardBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = POOLING3D_BENCHMARK_SHAPES
...
@pytest.mark.performance
def test_perf_adaptive_avg_pool3d_backward():
    bench = AdaptiveAvgPool3dBackwardBenchmark(
        ...
        dtypes=FLOAT_DTYPES,
        input_fn=_adaptive_avg_pool3d_backward_input_fn,
    )
    bench.run()
