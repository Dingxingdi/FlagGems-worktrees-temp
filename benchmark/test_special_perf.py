...
# ============================================
# _cdist_forward benchmark
# ============================================


def _cdist_input_fn(shape, cur_dtype, device):
    """Input function for cdist: yields (x1, x2, p, compute_mode) tuples"""
    n, d = shape
    m = n // 2 + 7  # Use different m for non-square case
    x1 = torch.randn(n, d, dtype=cur_dtype, device=device)
    x2 = torch.randn(m, d, dtype=cur_dtype, device=device)
    yield x1, x2, 2.0, None  # p=2.0, compute_mode=None


class CdistBenchmark(GenericBenchmark2DOnly):
    def set_more_shapes(self):
        # cdist shapes: (n, d) where n is number of points and d is dimension
        special_shapes_2d = [(128, 64), (256, 128), (512, 256), (1024, 512)]
        return special_shapes_2d


@pytest.mark.cdist_forward
@pytest.mark.performance
def test_perf_cdist_forward():
    bench = CdistBenchmark(
        op_name="cdist_forward",
        torch_op=torch.ops.aten._cdist_forward,
        dtypes=FLOAT_DTYPES,
        input_fn=_cdist_input_fn,
    )
    bench.set_gems(flag_gems._cdist_forward)
    bench.run()
