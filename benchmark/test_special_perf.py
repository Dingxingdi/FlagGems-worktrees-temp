...
# ISTFT benchmark
# Note: istft requires enough frames for overlap-add to work
# signal_length = (n_frames - 1) * hop_length + n_fft for center=True
ISTFT_SHAPES = [
    (4224, 256, 64, True),   # n_frames = 65 + 2*padding = 67
    (8512, 512, 128, True),  # n_frames = 66 + 2*padding = 68
    (17024, 512, 128, True), # n_frames = 132 + 2*padding = 134
    (4160, 256, 64, False),  # n_frames = 16
    (8448, 512, 128, False), # n_frames = 33
]
...
class ISTFTBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for signal_length, n_fft, hop_length, center in self.shapes:
            torch.manual_seed(42)
            # Create signal of appropriate length
            signal = torch.randn(signal_length, dtype=torch.float32, device=self.device)
            # Compute STFT
            stft_result = torch.stft(
                signal, n_fft=n_fft, hop_length=hop_length,
                win_length=n_fft, center=center, return_complex=True
            )
            # Use dict for kwargs to avoid bool being treated as arg
            yield (stft_result, n_fft, hop_length, n_fft, {"center": center})


@pytest.mark.istft
def test_perf_istft():
    class ISTFTBenchmarkWithGems(Benchmark):
        def set_shapes(self, shape_file_path=None):
            self.shapes = ISTFT_SHAPES
            self.gems_op = flag_gems.istft
...
# Remove the old ISTFTBenchmark and keep the updated one
# The class above is the correct implementation
