...
def test_perf_grid_sampler_3d_backward():
    # Simple benchmark using GenericBenchmark
    shapes = [(1, 3, 4, 4, 4), (2, 3, 4, 4, 4)]
    dtypes = [torch.float16, torch.float32]
    ...
    for dtype in dtypes:
        for shape in shapes:
            ...
            grid = torch.randn(N, D, H, W, 3, dtype=dtype, device=flag_gems.device)
            grad_output = torch.randn(N, C, D, H, W, dtype=dtype, device=flag_gems.device)

            # Warmup
            for _ in range(10):
                torch.ops.aten.grid_sampler_3d_backward.default(
                    grad_output, input, grid, 0, 1, False, [True, True]
                )
            ...
            print(f"grid_sampler_3d_backward: shape={shape}, dtype={dtype}, time={elapsed:.3f}ms")
