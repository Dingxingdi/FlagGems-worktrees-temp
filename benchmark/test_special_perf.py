...
# Dynamic Quantized 4-bit Matmul Benchmark
...
class DynQuantMatmul4BitBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = DYN_QUANT_MATMUL_4BIT_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            W_scale = torch.rand(N, dtype=torch.float32, device=self.device) + 0.1

            # Quantize weights to int4 (stored in int8)
            W_fp32 = torch.randn(K, N, dtype=torch.float32, device=self.device)
            int4_max = 7
            int4_min = -8
            W_quant = (
                (W_fp32 / W_scale[None, :]).round().clamp(int4_min, int4_max).to(torch.int8)
            )

            # Pack int4 into int8: 2 values per byte
            W_packed = torch.empty(K // 2, N, dtype=torch.int8, device=self.device)
            for k in range(K // 2):
                w_low = W_quant[k * 2, :]
                w_high = (
                    W_quant[k * 2 + 1, :]
                    if k * 2 + 1 < K
                    else torch.zeros(N, dtype=torch.int8, device=self.device)
                )
                W_packed[k, :] = ((w_high + 8) << 4) | (w_low + 8)

            yield A, W_packed, W_scale, cur_dtype


@pytest.mark.dyn_quant_matmul_4bit
def test_perf_dyn_quant_matmul_4bit():
    bench = DynQuantMatmul4BitBenchmark(
        op_name="dyn_quant_matmul_4bit",
        torch_op=flag_gems.dyn_quant_matmul_4bit,
