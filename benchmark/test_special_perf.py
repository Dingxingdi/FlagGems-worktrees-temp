...
def sparse_sampled_addmm_input_fn(shape, dtype, device):
    """Input function for sparse_sampled_addmm benchmark.
    shape is (M, N, K)
    """
    M, N, K = shape
    # Create a sparse CSR matrix with ~30% density
    density = 0.3
    nnz = int(M * N * density)
...
def test_sparse_sampled_addmm_perf():
    """Benchmark for sparse_sampled_addmm"""
    # Define custom shapes for sparse_sampled_addmm (M, N, K)
    sparse_shapes = [
        (16, 16, 8),
        (32, 32, 16),
        (64, 64, 32),
    ...
    class SparseSampledAddmmBenchmark(GenericBenchmarkExcluse1D):
        def set_shapes(self, shape_file_path=None):
            self.shapes = sparse_shapes

    bench = SparseSampledAddmmBenchmark(
        input_fn=sparse_sampled_addmm_input_fn,
        op_name="sparse_sampled_addmm",
        dtypes=[torch.float32],
        torch_op=torch.sparse.sampled_addmm
    )
    bench.run()
