...
def expand_input_fn(shape, cur_dtype, device):
    inp = generate_tensor_input(shape, cur_dtype, device)
    # Generate target shapes for expansion
    # Strategy: Expand any singleton dimension to 4, or keep the shape
    ...
    for i in range(len(target_shape)):
        if target_shape[i] == 1:
            ...
            expanded = True
            break
    if not expanded and len(target_shape) > 0:
        # No singleton dims, expand first non-1 dim
        for i in range(len(target_shape)):
            if target_shape[i] > 1:
                target_shape[i] = min(target_shape[i], 512)  # Cap to avoid huge sizes
                break

    yield inp, tuple(target_shape)


class ExpandBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = EXPAND_BENCHMARK_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            if not expanded and len(target_shape) > 0:
                for i in range(len(target_shape)):
                    if target_shape[i] > 1:
                        target_shape[i] = min(target_shape[i], 512)
                        break
            yield inp, tuple(target_shape)

    def get_tflops(self, op, *args, **kwargs):
        inp, target_shape = args
        return inp.numel()


@pytest.mark.expand
@pytest.mark.performance
def test_perf_expand():
    bench = ExpandBenchmark(
        op_name="expand",
        torch_op=lambda inp, size: inp.expand(size),
        dtypes=FLOAT_DTYPES,
