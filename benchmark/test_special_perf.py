...
class AsStridedScatterBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = AS_STRIDED_SCATTER_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            # Create input tensor
            inp = torch.randn(shape, dtype=cur_dtype, device=self.device)
            # Create src tensor with smaller size
            src_shape = (shape[0] - 1, shape[1] - 1)
            src = torch.randn(src_shape, dtype=cur_dtype, device=self.device)
            # Define as_strided view parameters
            size = (shape[0] - 1, shape[1] - 1)
            stride = (1, shape[1])  # row-major stride
            yield inp, src, size, stride


@pytest.mark.as_strided_scatter
def test_perf_as_strided_scatter():
    bench = AsStridedScatterBenchmark(
        op_name="as_strided_scatter",
        torch_op=torch.as_strided_scatter,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
