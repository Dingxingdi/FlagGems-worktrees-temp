...
def _reshape_alias_input_fn(shape, cur_dtype, device):
    x = torch.randn(shape, dtype=cur_dtype, device=device)
    # Flatten to 1D with stride (1,)
    numel = 1
    for dim_size in shape:
        numel *= dim_size
    new_shape = (numel,)
    new_stride = (1,)
    yield x, new_shape, new_stride


@pytest.mark.reshape_alias
@pytest.mark.performance
def test_perf_reshape_alias():
    bench = GenericBenchmark(
        op_name="reshape_alias",
        torch_op=torch.ops.aten._reshape_alias,
        dtypes=FLOAT_DTYPES,
        input_fn=_reshape_alias_input_fn,
    )
    bench.run()
