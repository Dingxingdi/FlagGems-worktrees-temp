...
# tril_indices benchmark - since it doesn't take tensor input,
# we create a custom benchmark function
TRIL_INDICES_SHAPES = [(100, 100), (500, 500), (1000, 1000)]


@pytest.mark.tril_indices
@pytest.mark.parametrize("row", [100, 500, 1000])
@pytest.mark.parametrize("col", [100, 500, 1000])
@pytest.mark.parametrize("offset", [0])
def test_perf_tril_indices(row, col, offset):
    """Benchmark for tril_indices operator."""
    # Warmup
    for _ in range(10):
        _ = torch.tril_indices(row, col, offset, device=flag_gems.device)
    ...
    start.record()
    for _ in range(100):
        _ = torch.tril_indices(row, col, offset, device="cuda")
    end.record()
    torch.cuda.synchronize()
    torch_latency = start.elapsed_time(end) / 100
    ...
    with flag_gems.use_gems():
        ...
        torch.cuda.synchronize()
        gems_latency = start.elapsed_time(end) / 100

    speedup = torch_latency / gems_latency
    print(
        f"SUCCESS    {torch_latency:.3f}    {gems_latency:.3f}    {speedup:.3f}    [{row}, {col}]"
    )
