...
# Shapes for 3D pooling benchmark (N, C, D, H, W)
...
class AdaptiveAvgPool3DBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = ADAPTIVE_AVG_POOL3D_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            N, C, D, H, W = shape
            # Use output_size = (2, 2, 2) for small shapes, (8, 8, 8) for larger shapes
            if D >= 32:
                output_size = (8, 8, 8)
            else:
                output_size = (2, 2, 2)
