...
def test_perf_pin_memory():
    def pin_memory_input_fn(shape, dtype, device):
        if dtype in FLOAT_DTYPES:
            inp = torch.randn(shape, dtype=dtype, device=device)
        else:
            inp = torch.randint(
    ...
    # Use clone as baseline since torch._pin_memory only works on CPU tensors
    bench = GenericBenchmark(
        input_fn=pin_memory_input_fn,
        op_name="torch._pin_memory",
        torch_op=lambda x: x.clone(),
        dtypes=FLOAT_DTYPES + INT_DTYPES,
    )
