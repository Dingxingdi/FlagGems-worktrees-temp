...
# Shapes for LSTM cell: (batch, hidden_size)
LSTM_SHAPES = [
    (1, 4),
    (4, 16),
    (8, 32),
...
class LSTMCellBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            cx = torch.randn(batch_size, hidden_size, dtype=cur_dtype, device=self.device)
            yield input_gates, hidden_gates, cx


@pytest.mark.thnn_fused_lstm_cell
def test_perf_thnn_fused_lstm_cell():
    bench = LSTMCellBenchmark(
        op_name="_thnn_fused_lstm_cell",
