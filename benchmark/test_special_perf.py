...
# Replication Pad 2D Backward Benchmark
REPLICATION_PAD_2D_BACKWARD_SHAPES = [
    (1, 16, 32, 32),
    (2, 32, 64, 64),
    (4, 64, 128, 128),
]
...
class ReplicationPad2DBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = REPLICATION_PAD_2D_BACKWARD_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            N, C, H, W = shape
            pad = (1, 1, 1, 1)  # pad all sides by 1
            # Create the padded dimensions
            OH = H + pad[2] + pad[3]
            OW = W + pad[0] + pad[1]
            # grad_output has the padded shape
            grad_output = torch.randn(N, C, OH, OW, dtype=cur_dtype, device=self.device)
            # input has the original shape
            input = torch.randn(N, C, H, W, dtype=cur_dtype, device=self.device)
            yield grad_output, input, pad


@pytest.mark.replication_pad2d_backward
def test_perf_replication_pad2d_backward():
