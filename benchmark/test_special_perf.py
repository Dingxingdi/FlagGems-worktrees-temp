...
# Shapes for huber_loss_backward benchmark
HUBER_LOSS_BACKWARD_SHAPES = [
    (4,),
    (16,),
    (64,),
    (128,),
...
class HuberLossBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = HUBER_LOSS_BACKWARD_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            inp = torch.randn(shape, dtype=cur_dtype, device=self.device)
            target = torch.randn(shape, dtype=cur_dtype, device=self.device)
            grad_output = torch.tensor(1.0, dtype=cur_dtype, device=self.device)
            delta = 1.0
            # Test with reduction=0 (none)
            yield grad_output, inp, target, 0, delta


@pytest.mark.huber_loss_backward
def test_perf_huber_loss_backward():
