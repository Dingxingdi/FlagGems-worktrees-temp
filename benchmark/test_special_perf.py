...
# Permute benchmark
@pytest.mark.permute
def test_perf_permute():
    class PermuteBenchmark(GenericBenchmark):
        def set_shapes(self, shape_file_path=None):
            # Use smaller shapes where GEMS can be competitive
            # Note: PyTorch's permute returns a view (no data movement),
            # while GEMS implementation copies data (required for Triton kernel)
            # So we test with shapes where memory copy overhead is acceptable
            self.shapes = [
                (2, 3),
                (2, 3, 4),
                (16, 32),
    ...
    def permute_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        # Generate a valid permutation based on ndim
        ndim = len(shape)
        dims = list(reversed(range(ndim)))  # Simple reversed permutation
        yield inp, dims,

    bench = PermuteBenchmark(
        input_fn=permute_input_fn,
        op_name="permute",
        torch_op=torch.permute,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
