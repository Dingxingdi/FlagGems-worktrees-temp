...
def meshgrid_input_fn(shape, dtype, device):
    """Input function for meshgrid benchmark."""
    x = torch.arange(shape[0], dtype=dtype, device=device)
    y = torch.arange(shape[1], dtype=dtype, device=device)
    # Yield as tuple - first two items are tensors, then kwargs dict
    yield (x, y, {"indexing": "ij"})


class MeshgridBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = MESHGRID_SHAPES_BENCH

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            yield from meshgrid_input_fn(shape, cur_dtype, self.device)


@pytest.mark.meshgrid
def test_perf_meshgrid():
    bench = MeshgridBenchmark(
        input_fn=meshgrid_input_fn,
        op_name="meshgrid",
