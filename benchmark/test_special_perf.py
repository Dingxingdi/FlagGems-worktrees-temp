...
class ListToTensorBenchmark(Benchmark):
    """
    Benchmark for _list_to_tensor operator.
    """

    def set_shapes(self, shape_file_path=None):
        # Different list sizes to benchmark
        self.shapes = [5, 100, 1000, 10000, 100000]

    def get_input_iter(self, cur_dtype):
        for size in self.shapes:
            # Generate a list of integers - wrap in tuple to pass as single argument
            input_list = list(range(size))
            yield (input_list,)


@pytest.mark.list_to_tensor
def test_perf_list_to_tensor():
    bench = ListToTensorBenchmark(
