...
# Linear+GeLU+Linear benchmark configurations
...
def linear_gelu_linear_ref(input, weight1, bias1, weight2, bias2):
    """Reference implementation for Linear+GeLU+Linear."""
...
class LinearGeLULinearBenchmark(Benchmark):
    DEFAULT_METRICS = DEFAULT_METRICS[:]

    def set_shapes(self, shape_file_path=None):
        self.shapes = LINEAR_GELU_LINEAR_SHAPES

    def get_input_iter(self, cur_dtype):
        for batch_seq, hidden_dim in self.shapes:
            intermediate_dim = hidden_dim * 4  # Typical transformer MLP ratio
            input = torch.randn(batch_seq, hidden_dim, dtype=cur_dtype, device=self.device)
            weight1 = torch.randn(hidden_dim, intermediate_dim, dtype=cur_dtype, device=self.device)
            bias1 = torch.randn(intermediate_dim, dtype=cur_dtype, device=self.device)
            weight2 = torch.randn(intermediate_dim, hidden_dim, dtype=cur_dtype, device=self.device)
            bias2 = torch.randn(hidden_dim, dtype=cur_dtype, device=self.device)
            yield input, weight1, bias1, weight2, bias2


@pytest.mark.linear_gelu_linear
def test_perf_linear_gelu_linear():
    bench = LinearGeLULinearBenchmark(
        op_name="linear_gelu_linear",
        torch_op=linear_gelu_linear_ref,
        dtypes=[torch.float16, torch.bfloat16, torch.float32],
    )
    bench.set_gems(flag_gems.linear_gelu_linear)
    bench.run()
