...
class LinalgCholeskyExBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            A = A_orig @ A_orig.t() + torch.eye(n, dtype=cur_dtype, device=self.device) * 0.1
            yield (A,)

    # Override to handle the tuple return type
    def unpack_to_args_kwargs(self, input):
        # For cholesky_ex, we need to extract just the L tensor from the result
        args = input
        kwargs = {}
        return args, kwargs
