...
def _functional_assert_async_input_fn(shape, cur_dtype, device):
    ...
    dep_token = torch.zeros(1, dtype=cur_dtype, device=device)
    # Use tuple to wrap the string message since strings are not recognized by the benchmark framework
    yield inp, ("test assert",), dep_token


@pytest.mark.functional_assert_async
@pytest.mark.performance
def test_perf_functional_assert_async():
    def op_fn(inp, msg_tuple, dep):
        # Extract the actual message from the tuple
        msg = msg_tuple[0] if isinstance(msg_tuple, tuple) else msg_tuple
        return flag_gems._functional_assert_async(inp, msg, dep)

    bench = GenericBenchmark(
        op_name="functional_assert_async",
        # Since aten::_functional_assert_async is not available on CUDA,
        # we only benchmark the flag_gems implementation (using torch_op as a fallback for latency_base)
        torch_op=op_fn,
        gems_op=op_fn,
        dtypes=FLOAT_DTYPES,
