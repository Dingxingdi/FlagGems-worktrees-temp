...
def fft_ihfftn_wrapper(x):
    """Wrapper for fft_ihfftn to work with benchmark framework."""
    return torch.fft.ihfftn(x)


class FftIhfftnBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FFT_IHFFTN_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            x = torch.randn(shape, dtype=cur_dtype, device=self.device)
            # Return as tuple to prevent unpacking
            yield (x,)


@pytest.mark.fft_ihfftn
def test_perf_fft_ihfftn():
    bench = FftIhfftnBenchmark(
        op_name="fft_ihfftn",
        torch_op=fft_ihfftn_wrapper,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
