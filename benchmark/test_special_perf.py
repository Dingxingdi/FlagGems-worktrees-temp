...
def test_perf_moe_expert_dispatch():
    def moe_expert_dispatch_input_fn(shape, dtype, device):
        # Handle both 2D and 3D+ shapes
        # For MoE dispatch, we need (num_tokens, hidden_dim)
        # The shape could be (B, N, H) or (N, H) - flatten the first dims to get num_tokens
        if len(shape) == 2:
        ...
        num_experts = 8  # Fixed number of experts

        hidden_states = torch.randn(
            num_tokens, hidden_dim, dtype=dtype, device=device, requires_grad=False
        )
        expert_ids = torch.randint(
            0, num_experts, (num_tokens,), dtype=torch.int64, device=device
        )

        yield hidden_states, expert_ids

    def torch_op(hidden_states, expert_ids):
        # Reference implementation using torch ops
        sorted_indices = torch.argsort(expert_ids, stable=True)
        return hidden_states.index_select(0, sorted_indices)

    gems_op = lambda hs, ei: flag_gems.moe_expert_dispatch(hs, ei)[0]

    bench = GenericBenchmarkExcluse1D(
        input_fn=moe_expert_dispatch_input_fn,
        op_name="moe_expert_dispatch",
        torch_op=torch_op,
        dtypes=FLOAT_DTYPES,
    )
    bench.set_gems(gems_op)
