...
@pytest.mark.conj
def test_perf__conj():
    def _conj_input_fn(shape, dtype, device):
        if dtype.is_complex:
    ...
    def torch__conj(input):
        return torch.ops.aten._conj(input)

    def gems_wrapper(input):
        return flag_gems._conj(input)

    class ConjBenchmark(GenericBenchmarkExcluse3D):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
    ...
    bench = ConjBenchmark(
        input_fn=_conj_input_fn,
        op_name="_conj",
        torch_op=torch__conj,
        dtypes=dtypes,
    )
    bench.set_gems(flag_gems._conj)
    bench.run()
