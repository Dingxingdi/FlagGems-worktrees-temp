...
# Shapes for nonzero_numpy benchmark
NONZERO_NUMPY_SHAPES = [
    (16, 16),
    (32, 32),
    (64, 64),
    (128, 128),
...
def test_perf_nonzero_numpy():
    def nonzero_numpy_input_fn(shape, dtype, device):
        # Generate tensor with roughly 50% non-zero elements
        inp = torch.randint(0, 2, shape, dtype=dtype, device=device).float()
        yield inp,

    bench = GenericBenchmark2DOnly(
        input_fn=nonzero_numpy_input_fn,
