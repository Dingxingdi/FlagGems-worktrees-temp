...
def test_perf__assert_tensor_metadata():
    def assert_tensor_metadata_input_fn(shape, dtype, device):
        inp = torch.randn(shape, dtype=dtype, device=device)
        yield inp, list(shape), list(inp.stride()), dtype

    bench = GenericBenchmark(
        input_fn=assert_tensor_metadata_input_fn,
        op_name="torch._assert_tensor_metadata",
        torch_op=torch._assert_tensor_metadata,
        dtypes=FLOAT_DTYPES,
    )
