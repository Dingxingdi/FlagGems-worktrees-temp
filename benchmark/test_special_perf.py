...
# Benchmark for _jagged_to_padded_dense_forward
JAGGED_BENCH_SHAPES = [
    (1024,),   # 512 sequences of 2 values each
    (2048,),   # 512 sequences of 4 values each
    (4096,),   # 512 sequences of 8 values each
    (8192,),   # 512 sequences of 16 values each
]


def jagged_to_padded_input_fn(shape, dtype, device):
    """Generate inputs for jagged_to_padded_dense_forward benchmark"""
    total_values = shape[0]
    num_sequences = 512
    seq_length = total_values // num_sequences

    values = torch.randn(total_values, dtype=dtype, device=device)

    # Create offsets: cumulative sum of sequence lengths
    offsets = torch.zeros(num_sequences + 1, dtype=torch.int64, device=device)
    offsets[1:] = seq_length
    offsets = torch.cumsum(offsets, dim=0)

    max_lengths = [seq_length]
    padding_value = 0.0

    yield values, [offsets], max_lengths, padding_value


@pytest.mark.jagged_to_padded_dense_forward
def test_perf_jagged_to_padded_dense_forward():
    bench = GenericBenchmark2DOnly(
        input_fn=jagged_to_padded_input_fn,
