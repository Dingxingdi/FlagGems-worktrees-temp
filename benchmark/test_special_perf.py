...
# Matrix inverse benchmark - only 2x2 and 3x3 supported
LINALG_INV_SHAPES = [(2, 2), (3, 3)]


class LinalgInvBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
...
def test_perf_linalg_inv():
    bench = LinalgInvBenchmark(
        op_name="linalg_inv",
        torch_op=lambda a: torch.linalg.inv(a),
        dtypes=[torch.float32],  # Only float32 supported by torch.linalg.inv
    )
    bench.run()
