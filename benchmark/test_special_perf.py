...
def test_perf_fft_hfft2():
    # Simple benchmark for fft_hfft2 using aten op directly
    shape = (8, 17)
    dtype = torch.float32
    ...
    with flag_gems.use_gems():
        for _ in range(10):
            _ = torch.ops.aten.fft_hfft2(x)

    # Benchmark torch (without gems)
    ref_x = x.cpu().cuda()
    torch_latency = 0
    for _ in range(10):
        ...
        start.record()
        _ = torch.fft.hfft2(ref_x)
        end.record()
        torch.cuda.synchronize()
        torch_latency += start.elapsed_time(end)
    ...
    with flag_gems.use_gems():
        for _ in range(10):
            ...
            torch.cuda.synchronize()
            gems_latency += start.elapsed_time(end)

    gems_latency /= 10
    speedup = torch_latency / gems_latency

    print(f"Shape: {shape}, dtype: {dtype}")
    print(f"Torch latency: {torch_latency:.4f} ms")
    print(f"Gems latency: {gems_latency:.4f} ms")
