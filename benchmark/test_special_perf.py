...
class GatherBlockQuantizedBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
    ...
    def get_input_iter(self, cur_dtype):
        ...
        for shape in self.shapes:
            ...
            quantized_data = torch.randint(-100, 100, shape, dtype=torch.int8, device=self.device)
            scales = torch.rand(n_blocks, dtype=torch.float32, device=self.device) * 2 + 0.5
