...
# FFT shapes for benchmark (limited due to naive implementation)
FFT_RFFT2_SHAPES = [
    (4, 4),
    (8, 8),
]
...
class FFT_RFFT2Benchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
...
def test_perf_fft_rfft2():
    def torch_fft_rfft2(x):
        return torch.fft.rfft2(x)

    bench = FFT_RFFT2Benchmark(
        op_name="fft_rfft2",
        torch_op=torch_fft_rfft2,
        dtypes=[torch.float32, torch.float16],
    )
    bench.set_gems(lambda x: torch.fft.rfft2(x))
    bench.run()
