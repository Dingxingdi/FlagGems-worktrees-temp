...
def _nested_view_from_buffer_input_fn(shape, cur_dtype, device):
    ...
    buffer = torch.randn(8, dtype=cur_dtype, device=device)
    nested_size = torch.tensor([[2, 3], [1, 2]], dtype=torch.long, device=device)
    nested_strides = torch.tensor([[3, 1], [2, 1]], dtype=torch.long, device=device)
    offsets = torch.tensor([0, 6], dtype=torch.long, device=device)
    yield buffer, nested_size, nested_strides, offsets


@pytest.mark.nested_view_from_buffer
@pytest.mark.performance
def test_perf_nested_view_from_buffer():
    bench = GenericBenchmark(
        op_name="_nested_view_from_buffer",
        torch_op=torch._nested_view_from_buffer,
        dtypes=FLOAT_DTYPES,
        input_fn=_nested_view_from_buffer_input_fn,
    )
    bench.run()
