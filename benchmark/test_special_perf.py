...
# Benchmark for nested_view_from_buffer_copy
NESTED_VIEW_FROM_BUFFER_SHAPES = [
    (1, 10),    # 1 tensor, total 10 elements
    (3, 30),    # 3 tensors, total 30 elements
    (5, 100),   # 5 tensors, total 100 elements
    (8, 200),   # 8 tensors, total 200 elements
    (10, 500),  # 10 tensors, total 500 elements
    (16, 1000), # 16 tensors, total 1000 elements
]


class NestedViewFromBufferBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = NESTED_VIEW_FROM_BUFFER_SHAPES

    def get_input_iter(self, cur_dtype):
        for num_tensors, total_size in self.shapes:
            # Generate random sizes that sum to total_size
            sizes = []
            ...
            for i in range(num_tensors):
                if i == num_tensors - 1:
                    ...
                    max_size = remaining - (num_tensors - i - 1)
                    s = torch.randint(1, max_size + 1, (1,)).item()
                    sizes.append(s)
                    remaining -= s

            # Create buffer
            buffer = torch.randn(total_size, dtype=cur_dtype, device=self.device)

            # Create nested_size tensor [num_tensors, 1]
            nested_sizes = torch.tensor([[s] for s in sizes], dtype=torch.long, device=self.device)

            # Create nested_strides tensor (all 1s)
            nested_strides = torch.ones(num_tensors, 1, dtype=torch.long, device=self.device)

            # Create offsets (cumulative sum of sizes)
            offsets = torch.tensor([0] + list(torch.cumsum(torch.tensor(sizes), dim=0)[:-1]), dtype=torch.long, device=self.device)

            yield buffer, nested_sizes, nested_strides, offsets


@pytest.mark.nested_view_from_buffer_copy
def test_perf_nested_view_from_buffer_copy():
    ...
    bench = NestedViewFromBufferBenchmark(
        ...
        torch_op=nested_view_from_buffer_copy,
        dtypes=[torch.float16, torch.float32],
    )
    # Set gems_op to the same as torch_op since we implement it ourselves
    bench.gems_op = nested_view_from_buffer_copy
    bench.run()
