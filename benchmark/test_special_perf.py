...
# Benchmark for is_non_overlapping_and_dense operator
# This operator checks if a tensor's storage is non-overlapping and dense

IS_NON_OVERLAPPING_AND_DENSE_SHAPES = [
    (1024,),
    (4, 8, 16),
    (16, 32, 64),
    (2, 4, 8, 16),
...
class IsNonOverlappingAndDenseBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = IS_NON_OVERLAPPING_AND_DENSE_SHAPES
...
def test_perf_is_non_overlapping_and_dense():
    bench = IsNonOverlappingAndDenseBenchmark(
        op_name="is_non_overlapping_and_dense",
        torch_op=torch.ops.aten.is_non_overlapping_and_dense.default,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
