...
# MoE_Gate_TopK_Routing Benchmark
class MoEGateTopKRoutingBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for config in self.shapes:
            yield from self.moe_gate_topk_routing_input_fn(config, cur_dtype, self.device)

    def moe_gate_topk_routing_input_fn(self, config, dtype, device):
...
def moe_gate_topk_routing_torch_ref(gate_logits, topk):
...
def test_perf_moe_gate_topk_routing():
    bench = MoEGateTopKRoutingBenchmark(
        ...
        torch_op=moe_gate_topk_routing_torch_ref,
        dtypes=[torch.float32, torch.float16, torch.bfloat16],
    )
    bench.set_gems(flag_gems.MoE_Gate_TopK_Routing)
    bench.run()
