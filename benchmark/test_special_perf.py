...
# Matrix rank benchmark
LINALG_MATRIX_RANK_SHAPES = [
    (4, 4),
    (8, 8),
    (16, 16),
    (32, 32),
...
class LinalgMatrixRankBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = LINALG_MATRIX_RANK_SHAPES
    ...
    def unpack_to_args_kwargs(self, input):
        # Override to handle our specific function signature
        return (input,), {}


@pytest.mark.linalg_matrix_rank
def test_perf_linalg_matrix_rank():
    ...
    # Note: float16 not supported by torch.linalg.svdvals on CUDA
    bench = LinalgMatrixRankBenchmark(
        op_name="linalg_matrix_rank",
        torch_op=torch.linalg.matrix_rank,
        gems_op=gems_linalg_matrix_rank,
        dtypes=[torch.float32],
    )
    bench.run()
