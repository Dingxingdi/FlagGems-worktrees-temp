...
class SlogdetBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = [
            (2, 2),
            (3, 3),
...
def slogdet_input_fn(shape, dtype, device):
    # Generate non-singular matrix
    A = torch.randn(shape, dtype=dtype, device=device)
    A = A + torch.eye(shape[0], dtype=dtype, device=device) * 3
    # Return as a tuple (even for single argument) to avoid iteration issues
    return (A,)


@pytest.mark.linalg_slogdet
def test_perf_linalg_slogdet():
