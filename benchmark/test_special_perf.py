...
class RnnReluBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            input_tensor = torch.randn(seq_len, batch_size, input_size, dtype=cur_dtype, device=self.device)

            rnn = torch.nn.RNN(input_size, hidden_size, 1, nonlinearity='relu')
            rnn = rnn.to(dtype=cur_dtype, device=self.device)
            params = tuple(rnn._flat_weights)
            hx = torch.randn(1, batch_size, hidden_size, dtype=cur_dtype, device=self.device)

            yield input_tensor, hx, params, True, 1, 0.0, False, False, False


@pytest.mark.rnn_relu
def test_perf_rnn_relu():
