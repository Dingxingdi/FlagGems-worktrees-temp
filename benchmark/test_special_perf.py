...
def test_perf_unique_dim():
    def unique_dim_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        yield inp, {"dim": 0, "sorted": True, "return_inverse": False, "return_counts": False},

    bench = GenericBenchmark2DOnly(
        input_fn=unique_dim_input_fn,
        op_name="unique_dim",
        torch_op=lambda x, dim, sorted, return_inverse, return_counts: torch.unique(x, dim=dim, sorted=sorted, return_inverse=return_inverse, return_counts=return_counts),
        dtypes=INT_DTYPES,
    )
    bench.run()
