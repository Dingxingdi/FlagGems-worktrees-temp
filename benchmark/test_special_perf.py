...
# Reshape benchmark
def reshape_input_fn(shape, dtype, device):
    inp = torch.randn(shape, dtype=dtype, device=device)
    target_shape = (shape[0], -1) if len(shape) > 1 else (-1,)
    yield inp, target_shape


bench = GenericBenchmark(
    input_fn=reshape_input_fn,
    op_name="torch.reshape",
    torch_op=torch.reshape,
    dtypes=FLOAT_DTYPES,
)

# Set shapes for reshape benchmark
bench.shapes = [
    (1024,),
    (2048,),
    (4096,),
