...
@pytest.mark.fft_fft2
def test_perf_fft_fft2():
    def fft_fft2_input_fn(shape, dtype, device):
        if dtype.is_complex:
            float_dtype = torch.float32 if dtype == torch.complex64 else torch.float64
            real = torch.randn(shape, dtype=float_dtype, device=device)
            imag = torch.randn(shape, dtype=float_dtype, device=device)
            ...
            raise ValueError("fft_fft2 requires complex dtype")
        yield (input_tensor,)

    def torch_fft_fft2(input):
        return torch.fft.fft2(input)

    def gems_fft_fft2(input):
        return torch.fft.fft2(input)

    class FFT_fft2Benchmark(GenericBenchmark2DOnly):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
        ...
        def set_shapes(self, shape_file_path=None):
            ...
            self.shapes = fft_fft2_shapes

        def set_more_shapes(self):
            return None

    dtypes = COMPLEX_DTYPES
    bench = FFT_fft2Benchmark(
        input_fn=fft_fft2_input_fn,
        op_name="fft_fft2",
        torch_op=torch_fft_fft2,
        dtypes=dtypes,
    )
    bench.set_gems(gems_fft_fft2)
    bench.run()
