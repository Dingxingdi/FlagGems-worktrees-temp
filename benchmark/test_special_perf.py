...
FLASH_ATTENTION_BACKWARD_SHAPES = [
    (1, 1, 16, 32),
    (1, 2, 32, 64),
    (2, 4, 64, 64),
    (1, 1, 128, 128),
...
class FlashAttentionBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = FLASH_ATTENTION_BACKWARD_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            forward_result = torch.ops.aten._flash_attention_forward(
                query, key, value, None, None, seq_len, seq_len, 0.0, False, False
            )
            out, logsumexp, philox_seed, philox_offset, _ = forward_result

            # Create grad_out
            grad_out = torch.randn_like(out)

            yield grad_out, query, key, value, out, logsumexp, None, None, seq_len, seq_len, 0.0, False, philox_seed, philox_offset
...
def test_perf_flash_attention_backward():
    bench = FlashAttentionBackwardBenchmark(
        ...
        torch_op=torch.ops.aten._flash_attention_backward,
