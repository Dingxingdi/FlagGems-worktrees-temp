...
def test_perf_fft_fftfreq():
    class FftFftFreqBenchmark(Benchmark):
        def set_shapes(self, shape_file_path=None):
            self.shapes = [
                (256, 1.0),
                (512, 1.0),
        ...
        def get_input_iter(self, cur_dtype):
            for n, d in self.shapes:
                yield (n, d, {"dtype": cur_dtype})

    bench = FftFftFreqBenchmark(
        op_name="fft_fftfreq",
        torch_op=torch.fft.fftfreq,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
