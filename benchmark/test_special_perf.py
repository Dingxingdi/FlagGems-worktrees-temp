...
class CausalConv1dBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = CAUSAL_CONV_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            weight = torch.randn(
                out_channels, in_channels, kernel_size, dtype=cur_dtype, device=self.device
            )
            yield input, weight, None, 1, 0, 1


@pytest.mark.causal_conv1d
def test_perf_causal_conv1d():
    bench = CausalConv1dBenchmark(
        op_name="causal_conv1d",
        torch_op=flag_gems.causal_conv1d,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
