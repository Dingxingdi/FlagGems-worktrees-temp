...
def test_perf_qkv_projection_fusion():
    """Benchmark QKV projection fusion: split tensor into Q, K, V"""
    # Shapes with last dimension divisible by 3
    qkv_shapes = [
        (2, 3, 63),
        (4, 8, 126),
        (1, 16, 192),
    ...
    def qkv_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        yield inp,

    bench = GenericBenchmark(
        input_fn=qkv_input_fn,
        op_name="qkv_projection_fusion",
        torch_op=lambda x: torch.chunk(x, 3, dim=-1),
        dtypes=FLOAT_DTYPES,
        shapes=qkv_shapes,
