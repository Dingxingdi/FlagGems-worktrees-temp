...
class IsCoalescedBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        # Use 2D shapes suitable for sparse tensors
        self.shapes = [
            (16, 16),
            (32, 32),
            (64, 64),
...
def test_perf_is_coalesced():
    def is_coalesced_input_fn(shape, dtype, device):
        # Create a sparse COO tensor
        size = shape[0]
        indices = torch.arange(size, dtype=torch.long, device="cpu").reshape(1, -1).repeat(2, 1)
        values = torch.arange(size, dtype=dtype, device="cpu")
        sparse = torch.sparse_coo_tensor(indices, values, shape, is_coalesced=True)
        yield sparse.to(device),

    bench = IsCoalescedBenchmark(
        input_fn=is_coalesced_input_fn,
        op_name="torch.ops.aten.is_coalesced",
