...
def test_perf_unsqueeze():
    def unsqueeze_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        dim = len(shape)  # Test inserting at the end
        yield inp, {"dim": dim},
...
def test_perf_unsqueeze_():
    def unsqueeze_input_fn(shape, dtype, device):
        inp = generate_tensor_input(shape, dtype, device)
        dim = len(shape)  # Test inserting at the end
        yield inp, {"dim": dim},

    # Use tensor method instead of torch function for in-place version
    bench = GenericBenchmark(
        input_fn=unsqueeze_input_fn,
        op_name="unsqueeze_",
        torch_op=lambda x, dim: x.unsqueeze_(dim),
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
