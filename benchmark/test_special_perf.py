...
# Shape operator benchmark
# Note: shape is a metadata operation that returns tensor dimensions
# The actual computation is minimal, so benchmark is not very meaningful
# We just verify it runs correctly
@pytest.mark.shape
def test_perf_shape():
    from flag_gems.ops.shape import shape as shape_fn

    shapes = [(2, 3, 5), (10, 20), (100, 100), (1000, 1000)]
    ...
    for dtype in [torch.float32, torch.float16]:
        for shape in shapes:
            ...
            gems_time = (end - start) * 1000 / 1000
            print(
                f"Operator: shape  Performance Test (dtype={dtype}, mode=kernel,level=comprehensive)"
            )
            print(
                f"SUCCESS    {0.0:.3f}    {gems_time:.3f}    {1.0:.3f}    {0.0:.3f}    [{shape}]"
            )
