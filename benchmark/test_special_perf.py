...
class GRUAttentionBenchmark(Benchmark):
    """Benchmark for GRU_Attention operator."""
...
def torch_gru_attention(q, k, v, scale=None):
...
def test_perf_gru_attention():
    from flag_gems.ops.GRU_Attention import gru_attention as gems_gru_attention

    bench = GRUAttentionBenchmark(
        op_name="gru_attention",
        torch_op=torch_gru_attention,
        dtypes=[torch.float32, torch.float16],
    )
    bench.run()
