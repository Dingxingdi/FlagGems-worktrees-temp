...
def test_perf_linalg_eigh():
    class LinalgEighBenchmark(GenericBenchmark2DOnly):
        def set_more_shapes(self):
            return None

    def linalg_eigh_input_fn(shape, dtype, device):
        # Create a symmetric positive definite matrix for eigenvalue decomposition
        A = torch.randn(shape, dtype=dtype, device=device)
        A = A @ A.T + torch.eye(shape[0], dtype=dtype, device=device) * 0.1
        yield A, {"UPLO": "L"}

    bench = LinalgEighBenchmark(
        input_fn=linalg_eigh_input_fn,
        op_name="linalg_eigh",
        torch_op=torch._linalg_eigh,
        dtypes=[torch.float32],
    )
    bench.run()
