...
SPARSE_SEMI_STRUCTURED_LINEAR_SHAPES = [
    (32, 128),
    (64, 256),
    (128, 512),
]
...
def sparse_semi_structured_linear_ref(input, weight, meta):
    """Reference implementation using dense matmul (meta is ignored)."""
    return torch.matmul(input, weight.t())


class SparseSemiStructuredLinearBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SPARSE_SEMI_STRUCTURED_LINEAR_SHAPES

    def get_input_iter(self, cur_dtype):
        for M, K in self.shapes:
            N = K  # Output dimension equals input dimension
            meta = torch.randint(0, 4, (K // 2, K // 4), dtype=torch.int8, device=self.device)
            input = torch.randn(M, K, dtype=cur_dtype, device=self.device)
            weight = torch.randn(N, K, dtype=cur_dtype, device=self.device)
            yield input, weight, meta


@pytest.mark.sparse_semi_structured_linear
def test_perf_sparse_semi_structured_linear():
    bench = SparseSemiStructuredLinearBenchmark(
        op_name="_sparse_semi_structured_linear",
        torch_op=sparse_semi_structured_linear_ref,
