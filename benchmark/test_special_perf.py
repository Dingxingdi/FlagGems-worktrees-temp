...
class ViewAsBenchmark(Benchmark):
    """Benchmark for view_as operator."""

    def set_shapes(self, shape_file_path=None):
        # Shapes with same number of elements but different layouts
        self.shapes = [
            (2, 3),
            (3, 2),
            (2, 2, 3),
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            numel = shape[0] * shape[1] * (shape[2] if len(shape) > 2 else 1)
            ...
            target = torch.randn(target_shape, dtype=cur_dtype, device=self.device)

            yield inp, target


@pytest.mark.view_as
def test_perf_view_as():
