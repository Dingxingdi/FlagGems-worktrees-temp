...
class LSTMBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        # LSTM shapes: (batch_size, seq_len, input_size, hidden_size)
        self.shapes = [
            (2, 4, 8, 16),
            (4, 8, 16, 32),
            (8, 16, 32, 64),
...
def lstm_input_fn(shape, dtype, device):
    batch_size, seq_len, input_size, hidden_size = shape
    ...
    h0 = torch.randn(1, batch_size, hidden_size, dtype=dtype, device=device)
    c0 = torch.randn(1, batch_size, hidden_size, dtype=dtype, device=device)

    yield input, (h0, c0), params, True, 1, 0.0, False, False, True
