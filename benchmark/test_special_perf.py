...
SLIDING_WINDOW_ATTENTION_SHAPES = [
    (1, 2, 256, 64),
    (1, 4, 512, 64),
    (2, 4, 256, 64),
    (2, 8, 512, 64),
...
class SlidingWindowAttentionBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SLIDING_WINDOW_ATTENTION_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            batch, num_heads, seq_len, head_dim = shape
            window_size = min(16, seq_len // 4)
            q = torch.randn(
            ...
            yield q, k, v, window_size


@pytest.mark.Sliding_Window_Attention
def test_perf_sliding_window_attention():
    # Use scaled_dot_product_attention as reference since PyTorch has no native Sliding_Window_Attention
    def torch_impl(q, k, v, window_size):
        # Reformat for scaled_dot_product_attention: (batch, seq, heads, dim)
        q_t = q.transpose(1, 2).contiguous()
        k_t = k.transpose(1, 2).contiguous()
        v_t = v.transpose(1, 2).contiguous()
        return torch.nn.functional.scaled_dot_product_attention(q_t, k_t, v_t).transpose(1, 2)

    bench = SlidingWindowAttentionBenchmark(
