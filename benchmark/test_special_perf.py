...
# Benchmark for _local_scalar_dense operator
# This operator extracts the first element from a tensor and returns a Python scalar


LOCAL_SCALAR_DENSE_SHAPES = [
    (1,),
    (8,),
    (64,),
    (1024,),
...
class LocalScalarDenseBenchmark(Benchmark):
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            inp = generate_tensor_input(shape, cur_dtype, self.device)
            yield inp,
...
def test_perf__local_scalar_dense():
    bench = LocalScalarDenseBenchmark(
        op_name="_local_scalar_dense",
        torch_op=torch.ops.aten._local_scalar_dense,
        dtypes=FLOAT_DTYPES + INT_DTYPES,
    )
    bench.run()
