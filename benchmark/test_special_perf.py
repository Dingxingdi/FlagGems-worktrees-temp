...
# ScatterND benchmark shapes
SCATTER_ND_BENCH_SHAPES = [
    (4, 4, 4),
    (8, 8, 8),
    (16, 16, 16),
    (32, 32, 32),
...
class ScatterNDBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SCATTER_ND_BENCH_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            all_indices = torch.randperm(num_elements, device=self.device)[:M]
            indices = torch.zeros((M, rank), dtype=torch.long, device=self.device)
            temp = all_indices
            for i in range(rank):
            ...
            values = torch.randn(M, dtype=cur_dtype, device=self.device)
            inp = torch.zeros(shape, dtype=cur_dtype, device=self.device)

            yield inp, indices, values


def scatter_nd_gbps(bench_fn_args, latency):
    inp, indices, values = bench_fn_args
    io_amount = sum([
        shape_utils.size_in_bytes(inp),
        shape_utils.size_in_bytes(indices),
        shape_utils.size_in_bytes(values),
    ])
    return io_amount * 1e-9 / (latency * 1e-3)


@pytest.mark.ScatterND
def test_scatter_nd_perf():
    bench = ScatterNDBenchmark(
        op_name="scatter_nd",
        torch_op=flag_gems.scatter_nd,
        dtypes=FLOAT_DTYPES,
        get_gbps=scatter_nd_gbps,
    )
    bench.run()
