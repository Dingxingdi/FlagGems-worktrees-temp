...
# Benchmark shapes for upsample_bilinear2d_backward
UPSAMPLE_BILINEAR_BACKWARD_SHAPES = [
    (1, 1, 4, 4),    # 2x upsample from 2x2
    (1, 1, 8, 8),    # 2x upsample from 4x4
    (1, 1, 16, 16),  # 2x upsample from 8x8
    (2, 3, 8, 8),    # Batch and channel
    (4, 8, 16, 16),  # Larger batch
]


class UpsampleBilinear2dBackwardBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = UPSAMPLE_BILINEAR_BACKWARD_SHAPES
    ...
    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            ...
            yield grad_output, output_size, input_size, False
