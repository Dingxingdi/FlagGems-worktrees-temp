...
# Benchmark for quantized_lstm operator
QUANTIZED_LSTM_SHAPES = [
    (4, 2, 8, 16, 1, False),   # seq_len, batch, input_size, hidden, num_layers, bidirectional
    (8, 4, 16, 32, 1, False),
    (16, 8, 32, 64, 1, False),
    (4, 2, 8, 16, 2, False),
    (8, 4, 16, 32, 2, False),
...
class QuantizedLSTMBenchmark(Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = QUANTIZED_LSTM_SHAPES

    def get_input_iter(self, cur_dtype):
        for seq_len, batch, input_size, hidden_size, num_layers, bidirectional in self.shapes:
            # Create input tensor
            input_tensor = torch.randn(seq_len, batch, input_size, dtype=cur_dtype, device=self.device)
            # Create hidden states
            num_directions = 2 if bidirectional else 1
            h0 = torch.zeros(num_layers * num_directions, batch, hidden_size, dtype=cur_dtype, device=self.device)
            c0 = torch.zeros(num_layers * num_directions, batch, hidden_size, dtype=cur_dtype, device=self.device)
            yield input_tensor, [h0, c0], [], True, num_layers, 0.0, False, bidirectional, False


@pytest.mark.quantized_lstm
@pytest.mark.skip(reason="Benchmark requires fix for recursion issue with flag_gems context")
def test_perf_quantized_lstm():
    bench = QuantizedLSTMBenchmark(
        op_name="quantized_lstm",
        torch_op=flag_gems.quantized_lstm,
        dtypes=[torch.float32, torch.bfloat16],
    )
    bench.run()
