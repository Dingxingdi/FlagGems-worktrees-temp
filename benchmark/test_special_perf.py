...
# KV Cache shapes for benchmarking
KV_CACHE_SHAPES = [
...
@pytest.mark.performance
def test_perf_kv_cache_update():
    """Benchmark KV_Cache_Update operator.

    Since KV_Cache_Update doesn't exist in PyTorch, we benchmark the GEMS implementation
    and report latency. For comparison, we use a simple PyTorch tensor copy as baseline.
    """
    import time
    ...
    for shape in KV_CACHE_SHAPES:
        ...
        new_k = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=device)
        new_v = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=device)

        # Warmup
        for _ in range(10):
            flag_gems.kv_cache_update(k_cache, v_cache, new_k, new_v, start_position)
        ...
        for _ in range(num_iters):
            flag_gems.kv_cache_update(k_cache, v_cache, new_k, new_v, start_position)
        torch.cuda.synchronize()
        gems_latency_ms = (time.perf_counter() - start) * 1000 / num_iters

        # Baseline: simple PyTorch copy for comparison
        k_cache2 = torch.zeros(shape, dtype=dtype, device=device)
        v_cache2 = torch.zeros(shape, dtype=dtype, device=device)
        start = time.perf_counter()
        for _ in range(num_iters):
            k_cache2[:, :, start_position:start_position + new_seq_len, :] = new_k
            v_cache2[:, :, start_position:start_position + new_seq_len, :] = new_v
        torch.cuda.synchronize()
        torch_latency_ms = (time.perf_counter() - start) * 1000 / num_iters

        speedup = torch_latency_ms / gems_latency_ms if gems_latency_ms > 0 else 0
        gbps = (new_k.numel() * 2 * dtype.itemsize) / (gems_latency_ms * 1e-3) / 1e9

        print(f"Shape: {shape}, GEMS: {gems_latency_ms:.4f}ms, PyTorch: {torch_latency_ms:.4f}ms, Speedup: {speedup:.3f}, GB/s: {gbps:.2f}")
        results.append({
            "shape": shape,
            "gems_latency_ms": gems_latency_ms,
