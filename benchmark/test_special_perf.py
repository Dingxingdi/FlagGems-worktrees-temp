...
# _embedding_bag benchmark
EMBEDDING_BAG_SHAPES = [
    (128, 32, 64, 4),   # num_embeddings, embedding_dim, num_indices, num_bags
    (512, 64, 256, 8),
    (1024, 128, 512, 16),
    (512, 64, 128, 4),
]
    ...
    def get_input_iter(self, cur_dtype):
        for num_embeddings, embedding_dim, num_indices, num_bags in self.shapes:
            ...
            indices = torch.randint(
                0, num_embeddings, (num_indices,), device=self.device, dtype=torch.long
            )
            offsets = torch.linspace(
                0, num_indices, num_bags, device=self.device, dtype=torch.long
            ).int()
            yield weight, indices, offsets, 0, False
...
def test_perf_embedding_bag():
    bench = EmbeddingBagBenchmark(
        ...
        torch_op=torch.ops.aten._embedding_bag,
        dtypes=[torch.float32, torch.bfloat16],
    )
    bench.set_gems(flag_gems._embedding_bag)
    bench.run()
