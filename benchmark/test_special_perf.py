...
# Broadcast shapes for benchmarking: (input_shape, target_shape)
BROADCAST_TO_SHAPES = [
    ((3,), (3, 3)),
    ((1, 3), (3, 3)),
    ((3,), (2, 3, 3)),
...
def broadcast_to_input_fn(shape, dtype, device):
    input_shape, target_shape = shape
    inp = torch.randn(input_shape, dtype=dtype, device=device)
    yield inp, target_shape


class BroadcastToBenchmark(GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = BROADCAST_TO_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            yield from broadcast_to_input_fn(shape, cur_dtype, self.device)


@pytest.mark.broadcast_to
def test_perf_broadcast_to():
    bench = BroadcastToBenchmark(
        input_fn=broadcast_to_input_fn,
