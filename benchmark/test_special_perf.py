...
def _pack_padded_sequence_input_fn(shape, dtype, device):
    batch_size, max_seq_len, features = shape
    # Generate random lengths sorted in descending order
    lengths = torch.randint(1, max_seq_len + 1, (batch_size,), dtype=torch.int64, device='cpu')
    lengths = lengths.sort(descending=True)[0]

    # Create padded input tensor (with padding at the end of shorter sequences)
    input_tensor = torch.randn(batch_size, max_seq_len, features, dtype=dtype, device=device)
    ...
    yield input_tensor, lengths, True


@pytest.mark.pack_padded_sequence
def test_perf_pack_padded_sequence():
    from typing import Generator

    # Custom shapes for pack_padded_sequence
    ...
    class PackPaddedSequenceBenchmark(GenericBenchmark):
        def get_input_iter(self, cur_dtype) -> Generator:
            for shape in pack_padded_sequence_shapes:
                yield from self.input_fn(shape, cur_dtype, self.device)

    bench = PackPaddedSequenceBenchmark(
        input_fn=_pack_padded_sequence_input_fn,
        op_name="_pack_padded_sequence",
        torch_op=torch._pack_padded_sequence,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
