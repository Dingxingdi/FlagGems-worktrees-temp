...
# empty_strided benchmark
def empty_strided_input_fn(shape, dtype, device):
    # Different stride configurations based on shape dimensionality
    n_dim = len(shape)
    if n_dim == 1:
        # For 1D, use simple stride
        yield {"size": shape, "stride": (1,), "dtype": dtype, "device": device},
    elif n_dim == 2:
        yield {"size": shape, "stride": (shape[1], 1), "dtype": dtype, "device": device},
        yield {"size": shape, "stride": (1, shape[0]), "dtype": dtype, "device": device},
    elif n_dim == 3:
        yield {"size": shape, "stride": (shape[1] * shape[2], shape[2], 1), "dtype": dtype, "device": device},
    else:
        # For higher dimensions, use default stride
        yield {"size": shape, "stride": tuple(1 for _ in shape), "dtype": dtype, "device": device},


@pytest.mark.empty_strided
def test_perf_empty_strided():
    from flag_gems.ops.empty_strided import empty_strided as gems_empty_strided

    bench = GenericBenchmark(
        input_fn=empty_strided_input_fn,
        op_name="empty_strided",
        torch_op=torch.empty_strided,
        gems_op=gems_empty_strided,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
