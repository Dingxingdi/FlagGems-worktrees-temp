...
def range_input_fn(shape, dtype, device):
    yield {
        "start": 0,
        "end": math.prod(shape),
        "dtype": dtype,
...
tensor_constructor_operations = [
    ...
    # range
    ("range", torch.range, range_input_fn),
    # linspace
...
def test_tensor_constructor_benchmark(op_name, torch_op, input_fn):
    ...
    # torch.range doesn't support bfloat16 on CUDA
    dtypes = None
    if op_name == "range":
        dtypes = [torch.float16, torch.float32]
    bench = GenericBenchmark(input_fn=input_fn, op_name=op_name, torch_op=torch_op, dtypes=dtypes)
    bench.run()
