...
def relu_backward_reference(grad_output, self):
    """Reference implementation for ReLU backward."""
    return torch.where(self > 0, grad_output, torch.zeros_like(grad_output))


class CudnnActivationBwdBenchmark(UnaryPointwiseBenchmark):
    def get_input_iter(self, cur_dtype: torch.dtype) -> Generator:
        for shape in self.shapes:
            inp = generate_tensor_input(shape, cur_dtype, self.device)
            grad_out = torch.randn_like(inp)
...
def test_cudnnActivateionbwd_perf():
    bench = CudnnActivationBwdBenchmark(
        op_name="cudnnActivateionbwd",
        torch_op=relu_backward_reference,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
