...
class RepeatBenchmark(UnaryPointwiseBenchmark):
    def set_more_shapes(self):
        # repeat is a data copy operation, so use moderate sizes
        special_shapes_2d = [(128, 128), (256, 256), (512, 512)]
        sp_shapes_3d = [(32, 32, 32), (64, 64, 64)]
        return special_shapes_2d + sp_shapes_3d

    def get_input_iter(self, cur_dtype) -> Generator:
        # Use fixed sizes for repeat benchmark
        base_sizes = (2, 2)
        for shape in self.shapes:
            inp = generate_tensor_input(shape, cur_dtype, self.device)
            # Extend sizes to match input tensor's dimensions
            sizes = list(base_sizes)
            for _ in range(len(sizes), len(shape)):
                sizes.append(1)
            yield inp, tuple(sizes)
...
def test_perf_repeat():
    bench = RepeatBenchmark(
        op_name="repeat",
        torch_op=lambda x, sizes: x.repeat(*sizes),
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
