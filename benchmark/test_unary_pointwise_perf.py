...
# Squeeze benchmark - use shapes with dimensions of size 1
class SqueezeBenchmark(UnaryPointwiseBenchmark):
    def set_shapes(self, shape_file_path=None):
        # Shapes with dimensions of size 1 for squeeze to work on
        self.shapes = [
            (1, 1024),
            (1024, 1),
            (1, 1024, 1024),
        ...
        self.shape_desc = ["Squeeze shapes with dim=1"] * len(self.shapes)


@pytest.mark.squeeze
def test_perf_squeeze():
    bench = SqueezeBenchmark(
        op_name="squeeze",
        torch_op=torch.squeeze,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
