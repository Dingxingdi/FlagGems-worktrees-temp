...
class PreluKernelBackwardBenchmark(Benchmark):
    def get_input_iter(self, cur_dtype) -> Generator:
        for shape in self.shapes:
            x = generate_tensor_input(shape, cur_dtype, self.device)
            # weight should match the last dimension
            if len(shape) == 0:
                w = torch.tensor(0.25, dtype=cur_dtype, device=self.device)
            else:
                w = torch.randn(shape[-1], dtype=cur_dtype, device=self.device)
            grad_output = torch.randn_like(x)
            yield grad_output, x, w


@pytest.mark.prelu_kernel_backward
def test_perf_prelu_kernel_backward():
    bench = PreluKernelBackwardBenchmark(
        op_name="_prelu_kernel_backward",
        torch_op=torch.ops.aten._prelu_kernel_backward.default,
        dtypes=FLOAT_DTYPES,
    )
    bench.run()
