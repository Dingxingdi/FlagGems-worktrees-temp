...
class SwiGLUBenchmark(GluBenchmark):
    # SwiGLU test requires even numbers
    pass


@pytest.mark.Fused_SwiGLU
def test_Fused_SwiGLU_perf():
    # Reference implementation for benchmark
    def swiglu_ref(x):
...
@pytest.mark.Fused_SwiGLU
def test_Fused_SwiGLU_backward_perf():
    # Reference implementation for benchmark
    def swiglu_ref(x):
        a, b = torch.chunk(x, 2, dim=-1)
        return a * torch.nn.functional.silu(b)
