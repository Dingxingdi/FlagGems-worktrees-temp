...
@triton.jit(do_not_specialize=["alpha", "beta", "use_gelu"])
def _addmm_activation_kernel(
    ...
    # Gelu constants for tanh approximation (must be constexpr for Triton)
    ...
    # Apply activation AFTER adding bias
    # PyTorch's _addmm_activation applies:
    #   - ReLU when use_gelu=False
    #   - GELU when use_gelu=True
    if use_gelu:
        # gelu_tanh(x) = 0.5 * x * (1 + tanh(sqrt(2/pi) * x * (1 + 0.044715 * x^2)))
        x_fp32 = accumulator.to(tl.float32)
        # Compute x^2
        x_sq = x_fp32 * x_fp32
        # Compute tanh argument: sqrt(2/pi) * x * (1 + 0.044715 * x^2)
        tanh_arg = GELU_SCALE * x_fp32 * (1.0 + GELU_COEFF * x_sq)
        # Apply tanh
        accumulator = 0.5 * x_fp32 * (1.0 + tl_extra_shim.tanh(tanh_arg))
    else:
        # Apply ReLU: max(0, x)
