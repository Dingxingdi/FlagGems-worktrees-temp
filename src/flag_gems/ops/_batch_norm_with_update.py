...
def batch_norm_with_update_forward_kernel(
    ...
    # _batch_norm_with_update always runs in training mode
...
def _batch_norm_with_update(
    ...
    # Use float32 for mean and inv_std for better numerical stability
    ...
    # running_mean and running_var must be provided for _batch_norm_with_update
    ...
    with torch_device_fn.device(input.device):
        batch_norm_with_update_forward_kernel[(feat_dim,)](
            input_3d,
            weight,
            bias,
            mean,
    ...
    # _batch_norm_with_update returns (output, save_mean, save_var, reserve)
    # save_var should be the batch variance (not inv_std)
    save_var = 1.0 / (inv_std * inv_std)
    # Reserve tensor is typically empty, used for gradient checkpointing
    reserve = torch.empty(0, device=input.device, dtype=input.dtype)

    return output.view_as(input), mean, save_var, reserve