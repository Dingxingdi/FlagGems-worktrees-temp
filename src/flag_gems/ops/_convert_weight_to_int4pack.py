...
@libentry()
@triton.jit
def _convert_weight_to_int4pack_kernel(
    input_ptr,
    output_ptr,
    M,
    N,
...
def _convert_weight_to_int4pack_triton(input: torch.Tensor, innerKTiles: int) -> torch.Tensor:
    """
    Triton implementation of _convert_weight_to_int4pack.
    """
    M, N = input.shape
    ...
    _convert_weight_to_int4pack_kernel[grid](
        input,
        output,
        M,
        N,
...
def _convert_weight_to_int4pack(input: torch.Tensor, innerKTiles: int) -> torch.Tensor:
    """
    FlagGems implementation of _convert_weight_to_int4pack.

    This operator converts a weight tensor to int4 packed format
    used by NVIDIA Tensor Cores for int4 inference.
    """
    logger.debug("GEMS _convert_weight_to_int4pack")

    M, N = input.shape

    # Get the packed flat output from Triton kernel
    packed_flat = _convert_weight_to_int4pack_triton(input, innerKTiles)
