...
def scaled_dot_product_flash_attention_for_cpu(
    ...
    """
    Implements scaled dot product attention similar to PyTorch's
    _scaled_dot_product_flash_attention_for_cpu, but optimized for GPU.

    Args:
        query: (batch, heads, seq, dim)
        key: (batch, heads, seq, dim)
        value: (batch, heads, seq, dim)
        dropout_p: dropout probability (only 0.0 supported)
    ...
    """
    logger.debug("GEMS SCALED_DOT_PRODUCT_FLASH_ATTENTION_FOR_CPU")
    # shape constraints
    ...
    # For this operator, we assume q_head_num == kv_head_num (no GQA support for simplicity)
    assert q_head_num == kv_head_num, (
        f"q_head_num {q_head_num} != kv_head_num {kv_head_num}. "
        "GQA is not supported in _scaled_dot_product_flash_attention_for_cpu."
    )
