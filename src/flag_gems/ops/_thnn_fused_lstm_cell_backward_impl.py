...
def _thnn_fused_lstm_cell_backward_impl_kernel(
    ...
    i_val = tl.load(workspace_ptr + base_offset + hidden_idx, mask=mask, other=0.5).to(tl.float32)
    f_val = tl.load(workspace_ptr + base_offset + hidden_size + hidden_idx, mask=mask, other=0.5).to(tl.float32)
    g_val = tl.load(workspace_ptr + base_offset + hidden_size * 2 + hidden_idx, mask=mask, other=0.0).to(tl.float32)
    o_val = tl.load(workspace_ptr + base_offset + hidden_size * 3 + hidden_idx, mask=mask, other=0.5).to(tl.float32)
    ...
    tl.store(grad_input_gates_ptr + base_offset + hidden_size + hidden_idx, grad_f, mask=mask)
    tl.store(grad_input_gates_ptr + base_offset + hidden_size * 2 + hidden_idx, grad_g, mask=mask)
    tl.store(grad_input_gates_ptr + base_offset + hidden_size * 3 + hidden_idx, grad_o, mask=mask)
