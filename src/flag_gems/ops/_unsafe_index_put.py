...
def generate_index_put_kernel(
    ...
    with code.indent():
        ...
        code.newline()
        # _unsafe_index_put skips bounds checking - no index_mask
...
class UnsafeIndexPutFunction:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if key in self.overloads:
            ...
            file_name = f"_unsafe_index_put_{key}.py"
...
def _unsafe_index_put(inp, indices, values, accumulate=False):
    logger.debug("GEMS UNSAFE INDEX PUT")

    out = inp.clone()
    return _unsafe_index_put_(out, indices, values, accumulate)


def _unsafe_index_put_(inp, indices, values, accumulate=False):
    logger.debug("GEMS UNSAFE INDEX PUT_")
    ...
    # _unsafe_index_put skips device and dtype safety checks
    # step 1: index preprocessing (bool expansion)
    processed_indices = []
    for idx in indices:
        if idx is None:
