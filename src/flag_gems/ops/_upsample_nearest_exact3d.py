...
def _upsample_nearest_exact3d_out(*args, **kwargs):
    ...
    in_t, out_t, out_dhw, scale_dhw = _extract_io_and_params(args, kwargs, expect_out=True)
    if out_t.ndim != 5:
        raise ValueError(
    ...
    expected_dhw = _compute_out_size(in_t.shape[-3], in_t.shape[-2], in_t.shape[-1], out_dhw, scale_dhw)
    if out_t.shape[-3:] != expected_dhw:
        raise ValueError(
            f"Provided out tensor has shape {tuple(out_t.shape)} but expected {(in_t.shape[0], in_t.shape[1]) + expected_dhw}."
        )
    if out_t.numel() == 0:
        return out_t
    return _launch_upsample_nearest_exact3d_kernel(
