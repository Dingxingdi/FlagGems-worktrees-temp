...
def bernoulli_kernel(
    ...
    # Bernoulli: output 1.0 if random < probability, else 0.0
...
def bernoulli(self, *, generator=None):
    ...
    with torch_device_fn.device(device):
        bernoulli_kernel[grid_fn](
            out, self, N, philox_seed, philox_offset
        )
    return out
...
def bernoulli_(self, *, generator=None):
    ...
    assert self.dtype in (
        ...
        torch.bfloat16,
        torch.float32,
    ), f"bernoulli_ only supports floating point dtypes, got {self.dtype}"
    device = self.device
    self = self.contiguous()
    N = self.numel()
    UNROLL = 4
    ...
    with torch_device_fn.device(device):
        bernoulli_kernel[grid_fn](
            self, self, N, philox_seed, philox_offset
        )
    return self