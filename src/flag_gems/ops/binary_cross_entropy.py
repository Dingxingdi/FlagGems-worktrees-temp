...
def _binary_cross_entropy_weighted_sum_kernel(
    x_ptr, y_ptr, w_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, eps: tl.constexpr
):
    pid = tl.program_id(axis=0)
    block_start = pid * BLOCK_SIZE
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
...
def binary_cross_entropy(
    ...
    if red == 0:
        ...
        if weight is not None:
            ...
            if weight.numel() == 1:
                ...
                weight_tensor = torch.full(
                    (n_elements,), weight.item(), device=input.device, dtype=torch.float32
                )
                _binary_cross_entropy_weighted_sum_kernel[grid](
                    input, target, weight_tensor, tmp_sum, n_elements, BLOCK_SIZE=BLOCK_SIZE, eps=eps
                )
            else:
                if weight.numel() != n_elements:
                    raise AssertionError(
                        "binary_cross_entropy: weight must have same number of elements as input and target, or be a scalar."
                    )
                _binary_cross_entropy_weighted_sum_kernel[grid](
                    input, target, weight, tmp_sum, n_elements, BLOCK_SIZE=BLOCK_SIZE, eps=eps
                )
        else:
        ...
        if red == 2:
            ...
            # mean
            if weight is not None and weight.numel() > 1:
                # Weighted mean
                weight_sum = weight.sum().to(dtype=torch.float32)
                mean_val = (tmp_sum / weight_sum).to(dtype=input.dtype)
            else:
                # Simple mean
