...
@pointwise_dynamic(
    is_tensor=[True, True, True], promotion_methods=[(0, 1, "DEFAULT")]
)
@triton.jit
...
def binary_cross_entropy_with_logits_pos_weight_kernel(x, y, pos_weight):
    ...
    x_pos = y_f32 * pw_f32 * log_1p_exp_neg_x + (1.0 - y_f32) * (x_f32 + log_1p_exp_neg_x)
...
@triton.jit
def binary_cross_entropy_with_logits_weight_pos_weight_kernel(
    x, y, weight, pos_weight
):
    x_f32 = x.to(tl.float32)
    y_f32 = y.to(tl.float32)
    w_f32 = weight.to(tl.float32)
