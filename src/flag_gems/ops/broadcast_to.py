...
# --------------------------- broadcast_to wrapper genration -----------------------------------
def parameter_for_wrapper() -> str:
    """Generate parameter declaration with type annotation for wrapper function.
    Example: in0: torch.Tensor, size: List[int], out0: torch.Tensor
    """
    parameters: List[str] = []
...
def parameter_for_wrapper_out() -> str:
    """Generate parameter declaration with type annotation for wrapper function.
    Example: in0: torch.Tensor, out0: torch.Tensor
    """
    parameters: List[str] = []
...
def generate_functional_broadcast_to_wrapper(
    wrapper_name: str,
    ...
    with code.indent():
        ...
        code.writeline("size_rank = len(size)")
        code.writeline("in0_shape = list(in0.shape)")
        code.writeline("size_shape = list(size)")
        code.newline()

        # Pad input shape with 1s on the left to match rank
        code.writeline("if (size_rank > in0_rank): ")
        with code.indent():
            code.writeline("diff = size_rank - in0_rank")
            code.writeline("ones = [1 for _ in range(diff)]")
            code.writeline("in0_shape = ones + in0_shape")
        code.newline()

        # Validate broadcast compatibility
        code.writeline("is_broadcastable = True")
        code.writeline("for i in range(size_rank):")
        with code.indent():
            code.writeline("if in0_shape[i] != 1 and in0_shape[i] != size_shape[i]:")
            with code.indent():
                code.writeline("is_broadcastable = False")
                code.writeline("break")
        code.newline()

        code.writeline("assert is_broadcastable, f'Cannot broadcast tensor of shape {in0.shape} to {size}'")
        code.newline()

        code.writeline(
            "out0 = torch.empty(size, device=in0.device, dtype=in0.dtype)"
        )
...
def generate_destination_passing_broadcast_to_wrapper(
    rank: int,
    wrapper_name: str,
    kernel_name: str,
    code: IndentedBuffer,
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            if rank > 0:
                ...
                code.writeline(
                    f"{in_shape_args}, # input shape for broadcast"
                )
...
def generate_broadcast_to_kernel(
    rank: int,
    kernel_name: str,
    code: IndentedBuffer,
) -> IndentedBuffer:
    ...
    with code.indent():
        ...
        if rank > 0:
            ...
            code.writeline(
                f"{task_space_args2}, # input shape for broadcast"
            )
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            # loads - broadcast_to semantics: if input dim is 1, broadcast (use 0 index)
            # otherwise use the output index directly (must be < input dim size due to broadcast compatibility)
            code.writeline("# loads")
            ptrs_expr: str = " + ".join(
                f"tl.where(in_s{j} == 1, 0, i{j}) * in{i}_stride{j}" for j in range(rank)
            )
...
def generate_code(
    ...
    code = generate_functional_broadcast_to_wrapper(
        wrapper_name, destination_passing_func_name, code
    )
    code = generate_destination_passing_broadcast_to_wrapper(
        rank, destination_passing_func_name, kernel_name, code
    )
    code = generate_broadcast_to_kernel(rank, kernel_name, code)
    return code


class BroadcastToFunction:
    def __init__(self):
        self.pid = os.getpid()
    ...
    def __call__(self, x, size):
        ...
        if key in self.overloads:
            ...
            code = generate_code(
                ...
                "_wrapper",
                "_wrapper_out",
                "_broadcast_to_flaggems_jit_function",
                code,
            )

            file_name = f"broadcast_to_rank_{key}.py"
        ...
        return overload(x, size)

    def arg_key(self, x, size):
        max_rank = max(x.ndim, len(size))
        return max_rank


_broadcast_to_func = BroadcastToFunction()


def broadcast_to(inp: torch.Tensor, size) -> torch.Tensor:
    logger.debug("GEMS BROADCAST_TO")

    out = _broadcast_to_func(inp, size)
    return out