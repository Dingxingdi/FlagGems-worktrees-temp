...
def conv_transpose2d_kernel(
    ...
    weight_pointer += weight_n_stride * pid_group * out_per_group_c + (
    ...
    for hwk in range(weight_height * weight_width * BLOCK_CI_COUNT):
        ...
        input_height_offset = output_height_point_value * stride_height + h * dilation_height - padding_height
        input_width_offset = output_width_point_value * stride_width + w * dilation_width - padding_width
...
class CudnnConvolutionTranspose(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, weight, padding, output_padding, stride, dilation, groups, benchmark, deterministic, allow_tf32):
        ...
        if isinstance(stride, (list, tuple)):
            stride_height, stride_width = stride[0], stride[1] if len(stride) > 1 else stride[0]
        else:
            stride_height = stride_width = stride

        # Parse padding
        if isinstance(padding, (list, tuple)):
            padding_height, padding_width = padding[0], padding[1] if len(padding) > 1 else padding[0]
        else:
        ...
        if isinstance(output_padding, (list, tuple)):
            ...
            output_padding_width = output_padding[1] if len(output_padding) > 1 else output_padding[0]
        else:
            output_padding_height = output_padding_width = output_padding

        # Parse dilation
        if isinstance(dilation, (list, tuple)):
            dilation_height, dilation_width = dilation[0], dilation[1] if len(dilation) > 1 else dilation[0]
        else:
        ...
        assert in_c == weight_c * groups, f"Incompatible input channels {in_c} and weight {weight_c} with groups {groups}"

        out_height = conv_transpose2d_output_size(
            input_height, weight_height, stride_height, padding_height, output_padding_height, dilation_height
        )
        out_width = conv_transpose2d_output_size(
            input_width, weight_width, stride_width, padding_width, output_padding_width, dilation_width
        )
        ...
        if groups == 1:
            ...
            weight_reshaped = weight.reshape(groups, out_c // groups, weight_c, weight_height, weight_width)
            weight_T = weight_reshaped.transpose(1, 2).reshape(in_c, out_c // groups, weight_height, weight_width).contiguous()
    ...
    def backward(ctx, grad_output):
        ...
        if groups != 1:
            weight_flip = weight_flip.reshape(groups, out_c // groups, weight_c, weight_height, weight_width)
            weight_flip = weight_flip.transpose(1, 2).reshape(in_c, out_c // groups, weight_height, weight_width).contiguous()
        else:
...
def cudnn_convolution_transpose(
    ...
    return CudnnConvolutionTranspose.apply(
        input, weight, padding, output_padding, stride, dilation, groups, benchmark, deterministic, allow_tf32
    )