...
def scan_part_prod_kernel(
    ...
    part_prod_via_prod = tl.reduce(inp_vals, 0, multiply_fn)
...
def scan_then_fan_col(inp, out, n_ele, dtype):
    ...
    if part_num >= 2:
        ...
        with torch_device_fn.device(inp.device):
            mul_base_prod_kernel[grid](out, partial_prod, n_ele, part_num, BLOCK_SIZE)
...
def scan_then_fan(inp, out, A, B, C, dtype):
    ...
    if part_num >= 2:
        ...
        with torch_device_fn.device(inp.device):
            mul_base_prod_abc_kernel[grid](out, partial_prod, B, C, part_num, BLOCK_SIZE)
...
def reduce_then_scan_block_prod_kernel_row(
    ...
    """The same kernel as the block prod in parallel reduce"""
    ...
    for start in range(block_offset, block_end, TILE_SIZE):
        ...
        acc *= x
    block_prod = tl.reduce(acc, 0, multiply_fn)
    tl.store(
...
def reduce_then_scan_block_scan_kernel_row(
    ...
    prefix = tl.load(
        previous_prod_ptr + pid_m * num_tiles_n + pid_n - 1, mask=pid_n > 0, other=0
    ).to(acc_dtype)
    for start in range(block_offset, block_end, TILE_SIZE):
        offsets = start + tl.arange(0, TILE_SIZE)
        mask = offsets < N
        ...
        prefix *= tl.reduce(x, 0, multiply_fn)
        tl.store(
...
def cumprod_(inp, dim=1, *, dtype=None):
    logger.debug("GEMS cumprod_")
    out = cumprod_wrapper(inp, dim, dtype)
    inp.copy_(out)
    return inp
