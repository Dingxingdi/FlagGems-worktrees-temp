...
def fft_fftshift(input: torch.Tensor, dim=None) -> torch.Tensor:
    ...
    # Compute shift amounts: (dim_size + 1) // 2 for each dimension
    shift_dict = {d: (input.shape[d] + 1) // 2 for d in shift_dims}
