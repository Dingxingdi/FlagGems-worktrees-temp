...
def _grid_sampler_2d_backward_kernel(
    ...
    grad_out_base = pid_n * stride_grad_out_n + pid_c * stride_grad_out_c + pid_oh * stride_grad_out_oh
    ...
    if interpolation_mode == 0:  # Bilinear
        ...
        if compute_grad_grid:
            ...
            input_00 = tl.load(input_ptr + addr_00, mask=mask_00, other=0.0).to(tl.float32)

            mask_x1 = (x1 >= 0) & (x1 < W)
            mask_10 = mask_x1 & mask_y0 & mask_ow
            addr_10 = input_base + y0 * stride_input_h + x1
            input_10 = tl.load(input_ptr + addr_10, mask=mask_10, other=0.0).to(tl.float32)

            mask_y1 = (y1 >= 0) & (y1 < H)
            mask_01 = mask_x0 & mask_y1 & mask_ow
            addr_01 = input_base + y1 * stride_input_h + x0
            input_01 = tl.load(input_ptr + addr_01, mask=mask_01, other=0.0).to(tl.float32)

            mask_11 = mask_x1 & mask_y1 & mask_ow
            addr_11 = input_base + y1 * stride_input_h + x1
            input_11 = tl.load(input_ptr + addr_11, mask=mask_11, other=0.0).to(tl.float32)
            ...
            grad_grid_y_ptrs = grad_grid_ptr + grad_grid_base + offs_ow * stride_grid_ow + 1
