import logging

import torch
import triton
import triton.language as tl

from flag_gems.utils import libentry
from flag_gems.utils import triton_lang_extension as tle

logger = logging.getLogger(__name__)


@libentry()
@triton.jit
def grid_sampler_3d_kernel(
    input_ptr,
    grid_ptr,
    output_ptr,
    n,
    c,
    in_d,
    in_h,
    in_w,
    out_d,
    out_h,
    out_w,
    in_strides_n,
    in_strides_c,
    in_strides_d,
    in_strides_h,
    in_strides_w,
    grid_strides_n,
    grid_strides_od,
    grid_strides_oh,
    grid_strides_ow,
    grid_stride_xyz,
    out_strides_n,
    out_strides_c,
    out_strides_d,
    out_strides_h,
    out_strides_w,
    total_out,
    interpolation_mode: tl.constexpr,
    padding_mode: tl.constexpr,
    align_corners: tl.constexpr,
):
    # pid = n * out_d * out_h * out_w + od * out_h * out_w + oh * out_w + ow
    pid = tle.program_id(0)

    # Bounds check
    if pid >= total_out:
        return

    # Decode flat pid to (n, od, oh, ow)
    n_idx = pid // (out_d * out_h * out_w)
    remainder = pid % (out_d * out_h * out_w)
    od_idx = remainder // (out_h * out_w)
    remainder = remainder % (out_h * out_w)
    oh_idx = remainder // out_w
    ow_idx = remainder % out_w

    # Load grid coordinates for this output location
    grid_offset = (
        n_idx * grid_strides_n
        + od_idx * grid_strides_od
        + oh_idx * grid_strides_oh
        + ow_idx * grid_strides_ow
    )
    gx = tl.load(grid_ptr + grid_offset + 0 * grid_stride_xyz)
    gy = tl.load(grid_ptr + grid_offset + 1 * grid_stride_xyz)
    gz = tl.load(grid_ptr + grid_offset + 2 * grid_stride_xyz)

    # Convert grid coordinates from [-1, 1] to input space
    # If align_corners=True, the corners of the input are aligned with the corners of the grid
    if align_corners:
        # x_out = (x_grid + 1) * (in_w - 1) / 2
        x = (gx + 1.0) * tl.cast((in_w - 1), tl.float32) * 0.5
        y = (gy + 1.0) * tl.cast((in_h - 1), tl.float32) * 0.5
        z = (gz + 1.0) * tl.cast((in_d - 1), tl.float32) * 0.5
    else:
        # x_out = (x_grid + 1) * in_w / 2
        x = (gx + 1.0) * tl.cast(in_w, tl.float32) * 0.5
        y = (gy + 1.0) * tl.cast(in_h, tl.float32) * 0.5
        z = (gz + 1.0) * tl.cast(in_d, tl.float32) * 0.5

    # Apply padding mode (padding_mode: 0=zeros, 1=border, 2=reflection)
    # border clamp
    x_clamped = tl.maximum(tl.minimum(x, tl.cast(in_w - 1, tl.float32)), tl.cast(0, tl.float32))
    y_clamped = tl.maximum(tl.minimum(y, tl.cast(in_h - 1, tl.float32)), tl.cast(0, tl.float32))
    z_clamped = tl.maximum(tl.minimum(z, tl.cast(in_d - 1, tl.float32)), tl.cast(0, tl.float32))

    # Compute indices based on interpolation mode
    if interpolation_mode == 1:  # NEAREST
        # Nearest neighbor: round to nearest integer
        ix = tl.cast(tl.floor(x_clamped + 0.5), tl.int32)
        iy = tl.cast(tl.floor(y_clamped + 0.5), tl.int32)
        iz = tl.cast(tl.floor(z_clamped + 0.5), tl.int32)
    else:  # BILINEAR
        # Bilinear: get floor and frac
        ix0 = tl.cast(tl.floor(x_clamped), tl.int32)
        iy0 = tl.cast(tl.floor(y_clamped), tl.int32)
        iz0 = tl.cast(tl.floor(z_clamped), tl.int32)
        fx = x_clamped - tl.floor(x_clamped)
        fy = y_clamped - tl.floor(y_clamped)
        fz = z_clamped - tl.floor(z_clamped)
        ix1 = ix0 + 1
        iy1 = iy0 + 1
        iz1 = iz0 + 1

    # Compute output offset for this (n, od, oh, ow) location
    out_offset_base = (
        n_idx * out_strides_n
        + od_idx * out_strides_d
        + oh_idx * out_strides_h
        + ow_idx * out_strides_w
    )

    # Process all channels
    for channel in range(c):
        if interpolation_mode == 1:  # NEAREST
            if padding_mode == 0:  # ZEROS
                mask_od = (iz >= 0) & (iz < in_d)
                mask_oh = (iy >= 0) & (iy < in_h)
                mask_ow = (ix >= 0) & (ix < in_w)
                mask = mask_od & mask_oh & mask_ow
                inp_offset = (
                    n_idx * in_strides_n
                    + channel * in_strides_c
                    + iz * in_strides_d
                    + iy * in_strides_h
                    + ix * in_strides_w
                )
                val = tl.load(input_ptr + inp_offset, mask=mask, other=0.0)
            else:
                # For border padding (padding_mode=1), coordinates are already clamped
                inp_offset = (
                    n_idx * in_strides_n
                    + channel * in_strides_c
                    + iz * in_strides_d
                    + iy * in_strides_h
                    + ix * in_strides_w
                )
                val = tl.load(input_ptr + inp_offset)
        else:
            # BILINEAR - Trilinear interpolation
            if padding_mode == 0:  # ZEROS
                # Compute masks for boundary conditions
                mask_000 = (iz0 >= 0) & (iz0 < in_d) & (iy0 >= 0) & (iy0 < in_h) & (ix0 >= 0) & (ix0 < in_w)
                mask_001 = (iz0 >= 0) & (iz0 < in_d) & (iy0 >= 0) & (iy0 < in_h) & (ix1 >= 0) & (ix1 < in_w)
                mask_010 = (iz0 >= 0) & (iz0 < in_d) & (iy1 >= 0) & (iy1 < in_h) & (ix0 >= 0) & (ix0 < in_w)
                mask_011 = (iz0 >= 0) & (iz0 < in_d) & (iy1 >= 0) & (iy1 < in_h) & (ix1 >= 0) & (ix1 < in_w)
                mask_100 = (iz1 >= 0) & (iz1 < in_d) & (iy0 >= 0) & (iy0 < in_h) & (ix0 >= 0) & (ix0 < in_w)
                mask_101 = (iz1 >= 0) & (iz1 < in_d) & (iy0 >= 0) & (iy0 < in_h) & (ix1 >= 0) & (ix1 < in_w)
                mask_110 = (iz1 >= 0) & (iz1 < in_d) & (iy1 >= 0) & (iy1 < in_h) & (ix0 >= 0) & (ix0 < in_w)
                mask_111 = (iz1 >= 0) & (iz1 < in_d) & (iy1 >= 0) & (iy1 < in_h) & (ix1 >= 0) & (ix1 < in_w)

                offset_000 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy0 * in_strides_h + ix0 * in_strides_w)
                offset_001 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy0 * in_strides_h + ix1 * in_strides_w)
                offset_010 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy1 * in_strides_h + ix0 * in_strides_w)
                offset_011 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy1 * in_strides_h + ix1 * in_strides_w)
                offset_100 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy0 * in_strides_h + ix0 * in_strides_w)
                offset_101 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy0 * in_strides_h + ix1 * in_strides_w)
                offset_110 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy1 * in_strides_h + ix0 * in_strides_w)
                offset_111 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy1 * in_strides_h + ix1 * in_strides_w)

                c000 = tl.load(input_ptr + offset_000, mask=mask_000, other=0.0)
                c001 = tl.load(input_ptr + offset_001, mask=mask_001, other=0.0)
                c010 = tl.load(input_ptr + offset_010, mask=mask_010, other=0.0)
                c011 = tl.load(input_ptr + offset_011, mask=mask_011, other=0.0)
                c100 = tl.load(input_ptr + offset_100, mask=mask_100, other=0.0)
                c101 = tl.load(input_ptr + offset_101, mask=mask_101, other=0.0)
                c110 = tl.load(input_ptr + offset_110, mask=mask_110, other=0.0)
                c111 = tl.load(input_ptr + offset_111, mask=mask_111, other=0.0)
            else:
                # For border padding (padding_mode=1), coordinates are already clamped
                offset_000 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy0 * in_strides_h + ix0 * in_strides_w)
                offset_001 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy0 * in_strides_h + ix1 * in_strides_w)
                offset_010 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy1 * in_strides_h + ix0 * in_strides_w)
                offset_011 = (n_idx * in_strides_n + channel * in_strides_c + iz0 * in_strides_d + iy1 * in_strides_h + ix1 * in_strides_w)
                offset_100 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy0 * in_strides_h + ix0 * in_strides_w)
                offset_101 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy0 * in_strides_h + ix1 * in_strides_w)
                offset_110 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy1 * in_strides_h + ix0 * in_strides_w)
                offset_111 = (n_idx * in_strides_n + channel * in_strides_c + iz1 * in_strides_d + iy1 * in_strides_h + ix1 * in_strides_w)

                c000 = tl.load(input_ptr + offset_000)
                c001 = tl.load(input_ptr + offset_001)
                c010 = tl.load(input_ptr + offset_010)
                c011 = tl.load(input_ptr + offset_011)
                c100 = tl.load(input_ptr + offset_100)
                c101 = tl.load(input_ptr + offset_101)
                c110 = tl.load(input_ptr + offset_110)
                c111 = tl.load(input_ptr + offset_111)

            # Trilinear interpolation
            c00 = c000 * (1.0 - fz) + c100 * fz
            c01 = c001 * (1.0 - fz) + c101 * fz
            c10 = c010 * (1.0 - fz) + c110 * fz
            c11 = c011 * (1.0 - fz) + c111 * fz

            c0 = c00 * (1.0 - fy) + c10 * fy
            c1 = c01 * (1.0 - fy) + c11 * fy

            val = c0 * (1.0 - fx) + c1 * fx

        # Store result
        out_offset = out_offset_base + channel * out_strides_c
        tl.store(output_ptr + out_offset, val)


def grid_sampler_3d(
    input,
    grid,
    interpolation_mode=0,
    padding_mode=0,
    align_corners=False,
):
    """
    Performs 3D grid sampling on input tensor.

    Args:
        input: Input tensor of shape (N, C, D, H, W)
        grid: Grid tensor of shape (N, out_D, out_H, out_W, 3) with normalized coordinates
        interpolation_mode: 0 = Bilinear, 1 = Nearest
        padding_mode: 0 = Zeros, 1 = Border, 2 = Reflection
        align_corners: If True, corners of input and grid are aligned

    Returns:
        Output tensor of shape (N, C, out_D, out_H, out_W)
    """
    logger.debug("GEMS GRID_SAMPLER_3D")

    N, C, in_d, in_h, in_w = input.shape
    out_d, out_h, out_w = grid.shape[1:4]

    output = torch.empty(
        (N, C, out_d, out_h, out_w),
        dtype=input.dtype,
        device=input.device,
    )

    if output.numel() == 0:
        return output

    total_out = N * out_d * out_h * out_w

    def grid_(meta):
        return (total_out,)

    grid_sampler_3d_kernel[grid_](
        input,
        grid,
        output,
        N,
        C,
        in_d,
        in_h,
        in_w,
        out_d,
        out_h,
        out_w,
        *input.stride(),
        *grid.stride(),
        *output.stride(),
        total_out,
        interpolation_mode,
        padding_mode,
        align_corners,
    )

    return output