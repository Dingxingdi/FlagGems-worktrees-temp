...
@libentry()
@triton.jit
def linalg_eig_kernel_2x2(A, eigenvalues_real, eigenvalues_imag, eigenvectors_real, eigenvectors_imag, n, batch_size, BLOCK_SIZE: tl.constexpr):
    """
...
def linalg_eig(A: torch.Tensor):
    ...
    *batch_dims, n, m = A.shape
    ...
    eigenvalues_real = torch.empty((*batch_dims, n), dtype=torch.float32, device=A.device)
    eigenvalues_imag = torch.empty((*batch_dims, n), dtype=torch.float32, device=A.device)
    eigenvectors_real = torch.empty((*batch_dims, n, n), dtype=torch.float32, device=A.device)
    eigenvectors_imag = torch.empty((*batch_dims, n, n), dtype=torch.float32, device=A.device)
