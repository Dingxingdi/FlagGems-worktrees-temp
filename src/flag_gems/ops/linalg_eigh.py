...
def _eig_3x3_kernel(A, eigenvalues, N, stride_a, stride_e, BLOCK_SIZE: tl.constexpr):
    ...
    e2 = tl.load(A + pid * stride_a + 6)
    f3 = tl.load(A + pid * stride_a + 7)
...
def _jacobi_eig_kernel(
    A, V, eigenvalues, N, M, max_iter, tolerance, stride_a, stride_v, stride_e, BLOCK_SIZE: tl.constexpr
):
    ...
    for i in range(0, M, BLOCK_SIZE):
        ...
        for j in range(0, M, BLOCK_SIZE):
            ...
            val = tl.where(off == off2, tl.cast(1.0, tl.float32), tl.cast(0.0, tl.float32))
        ...
        for i in range(M):
            for j in range(i + 1, M):
                ...
                if should_process:
                    ...
                    diff = a_jj - a_ii
                    denom = tl.abs(diff) + tl.sqrt(a_ij * a_ij + diff * diff)
                    tau = diff / denom
                    t = tl.where(a_ij != 0.0, tl.where(diff >= 0, 1.0 / (tl.abs(diff) + tl.sqrt(a_ij * a_ij + diff * diff)), -1.0 / (tl.abs(diff) + tl.sqrt(a_ij * a_ij + diff * diff))), 0.0)
    ...
    for i in range(M):
        for j in range(i + 1, M):
            ...
            # Swap eigenvalues
            tl.store(eigenvalues + pid * stride_e + i, tl.where(should_swap, ev_j, ev_i))
            tl.store(eigenvalues + pid * stride_e + j, tl.where(should_swap, ev_i, ev_j))

            # Swap eigenvectors columns
            for k in range(M):
                v_ki = tl.load(V + pid * stride_v + k * M + i)
                v_kj = tl.load(V + pid * stride_v + k * M + j)
                tl.store(V + pid * stride_v + k * M + i, tl.where(should_swap, v_kj, v_ki))
                tl.store(V + pid * stride_v + k * M + j, tl.where(should_swap, v_ki, v_kj))
...
def linalg_eigh(A, UPLO="L"):
    ...
    if n == 2:
        ...
        for i in range(A_flat.shape[0]):
            ...
            # First eigenvector
            if abs(d) > 1e-10:
                v1 = torch.tensor([d, ev1 - a], dtype=eigenvectors.dtype, device=eigenvectors.device)
                v2 = torch.tensor([d, ev2 - a], dtype=eigenvectors.dtype, device=eigenvectors.device)
            else:
                v1 = torch.tensor([1.0, 0.0], dtype=eigenvectors.dtype, device=eigenvectors.device)
                v2 = torch.tensor([0.0, 1.0], dtype=eigenvectors.dtype, device=eigenvectors.device)
...
def linalg_eigh_(A, UPLO="L"):
    """
    In-place version of linalg_eigh (currently not implemented).
    """
    logger.debug("GEMS LINALG_EIGH_")
    eigenvalues, eigenvectors = linalg_eigh(A, UPLO)
    A.copy_(eigenvectors)
    return eigenvalues, eigenvectors