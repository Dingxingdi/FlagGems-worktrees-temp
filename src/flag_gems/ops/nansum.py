...
def nansum_kernel_1(
    ...
    # Replace NaN with 0
    nansum_val = tl.where(_isnan(inp_val), 0.0, inp_val)
    sum_val = tl.sum(nansum_val)
    mid_ptr = mid + pid
...
def nansum(inp, dim=None, keepdim=False, *, dtype=None):
    ...
    # Route to nansum_dim when dim is specified
...
def nansum_out(inp, dim=None, keepdim=False, *, dtype=None, out):
    logger.debug("GEMS NANSUM_OUT")
    if dim is not None or keepdim:
        return nansum_dim_out(inp, dim=dim, keepdim=keepdim, dtype=dtype, out=out)
...
def nansum_dim_kernel_non_inner(
    ...
    if ONE_TILE_PER_CTA:
        ...
        # Replace NaN with 0
        inp = tl.where(_isnan(inp), 0.0, inp)
...
def nansum_dim_out(inp, dim=None, keepdim=False, *, dtype=None, out):
    logger.debug("GEMS NANSUM_DIM_OUT")
