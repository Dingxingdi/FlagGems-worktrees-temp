import logging

import torch
import triton
import triton.language as tl

from flag_gems.runtime import torch_device_fn
from flag_gems.utils import libentry

logger = logging.getLogger(__name__)


@libentry()
@triton.jit
def replication_pad1d_backward_kernel(
    grad_out_ptr,
    grad_in_ptr,
    B: tl.constexpr,
    C: tl.constexpr,
    W_in,
    W_out,
    pad_left,
    pad_right,
    grad_out_stride_n,
    grad_out_stride_c,
    grad_out_stride_w,
    grad_in_stride_n,
    grad_in_stride_c,
    grad_in_stride_w,
    BLOCK_SIZE: tl.constexpr,
):
    pid_w = tl.program_id(axis=0)
    pid_nc = tl.program_id(axis=1)

    n = pid_nc // C
    c = pid_nc % C

    off_w = pid_w * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = off_w < W_in

    # For float16/bfloat16, use float32 for accumulation
    if tl.constexpr(grad_out_ptr.dtype.element_ty == tl.float16) or tl.constexpr(
        grad_out_ptr.dtype.element_ty == tl.bfloat16
    ):
        cdtype = tl.float32
    else:
        cdtype = grad_out_ptr.dtype.element_ty

    # Base offsets
    base_out = n.to(tl.int64) * grad_out_stride_n + c.to(tl.int64) * grad_out_stride_c
    base_in = n.to(tl.int64) * grad_in_stride_n + c.to(tl.int64) * grad_in_stride_c

    w = off_w

    # Load ALL grad_output values for this NC position
    # We need grad_output positions [0, W_out-1] for left/right sums and [pad_left, pad_left+W_in-2] for middle
    # Actually, we need:
    # - positions 0..pad_left for left edge
    # - positions pad_left+1..pad_left+W_in-2 for middle (i.e., pad_left+w where w in 1..W_in-2)
    # - positions pad_left+W_in-1..W_out-1 for right edge

    # Full range needed: 0..W_out-1, which could be up to ~270 (256+7+7)
    # With BLOCK_SIZE=256, we can load up to 256 consecutive positions
    # But W_out could be larger than 256

    # Strategy: Load grad_output values using a vector of pointers
    off_w_grad_out = tl.arange(0, BLOCK_SIZE)

    # For middle positions, we need grad_output[pad_left + w]
    # Create pointer vector for this
    middle_load_offsets = (pad_left + w).to(tl.int64) * grad_out_stride_w
    middle_load_ptrs = grad_out_ptr + base_out + middle_load_offsets
    middle_mask = (w > 0) & (w < W_in - 1) & ((w + pad_left) < W_out)
    middle_val = tl.load(middle_load_ptrs, mask=middle_mask, other=0).to(cdtype)

    # For left edge (w == 0): sum grad_output[0:pad_left+1]
    # We can compute this by loading positions 0..pad_left and summing
    # Since pad_left <= 7 typically, we can unroll
    left_sum = tl.zeros([BLOCK_SIZE], dtype=cdtype)
    # Load position 0
    p0 = grad_out_ptr + base_out + 0 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (0 < W_out), tl.load(p0).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 1
    p1 = grad_out_ptr + base_out + 1 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (1 <= pad_left) & (1 < W_out), tl.load(p1).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 2
    p2 = grad_out_ptr + base_out + 2 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (2 <= pad_left) & (2 < W_out), tl.load(p2).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 3
    p3 = grad_out_ptr + base_out + 3 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (3 <= pad_left) & (3 < W_out), tl.load(p3).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 4
    p4 = grad_out_ptr + base_out + 4 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (4 <= pad_left) & (4 < W_out), tl.load(p4).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 5
    p5 = grad_out_ptr + base_out + 5 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (5 <= pad_left) & (5 < W_out), tl.load(p5).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 6
    p6 = grad_out_ptr + base_out + 6 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (6 <= pad_left) & (6 < W_out), tl.load(p6).to(cdtype), tl.zeros([], dtype=cdtype))
    # Load position 7
    p7 = grad_out_ptr + base_out + 7 * grad_out_stride_w
    left_sum = left_sum + tl.where((w == 0) & (7 <= pad_left) & (7 < W_out), tl.load(p7).to(cdtype), tl.zeros([], dtype=cdtype))

    # For right edge (w == W_in - 1): sum grad_output[pad_left + W_in - 1 : W_out]
    right_start = pad_left + W_in - 1
    right_sum = tl.zeros([BLOCK_SIZE], dtype=cdtype)
    # Load positions right_start to right_start+7 (max pad_right is 7)
    rp0 = grad_out_ptr + base_out + (right_start + 0).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 0 < W_out), tl.load(rp0).to(cdtype), tl.zeros([], dtype=cdtype))
    rp1 = grad_out_ptr + base_out + (right_start + 1).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 1 < W_out), tl.load(rp1).to(cdtype), tl.zeros([], dtype=cdtype))
    rp2 = grad_out_ptr + base_out + (right_start + 2).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 2 < W_out), tl.load(rp2).to(cdtype), tl.zeros([], dtype=cdtype))
    rp3 = grad_out_ptr + base_out + (right_start + 3).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 3 < W_out), tl.load(rp3).to(cdtype), tl.zeros([], dtype=cdtype))
    rp4 = grad_out_ptr + base_out + (right_start + 4).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 4 < W_out), tl.load(rp4).to(cdtype), tl.zeros([], dtype=cdtype))
    rp5 = grad_out_ptr + base_out + (right_start + 5).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 5 < W_out), tl.load(rp5).to(cdtype), tl.zeros([], dtype=cdtype))
    rp6 = grad_out_ptr + base_out + (right_start + 6).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 6 < W_out), tl.load(rp6).to(cdtype), tl.zeros([], dtype=cdtype))
    rp7 = grad_out_ptr + base_out + (right_start + 7).to(tl.int64) * grad_out_stride_w
    right_sum = right_sum + tl.where((w == W_in - 1) & (right_start + 7 < W_out), tl.load(rp7).to(cdtype), tl.zeros([], dtype=cdtype))

    # Combine: use left_sum for w==0, middle_val for 0<w<W_in-1, right_sum for w==W_in-1
    is_left = w == 0
    is_right = w == W_in - 1

    result = tl.where(is_left, left_sum, middle_val)
    result = tl.where(is_right, right_sum, result)

    # Handle case W_in == 1 (only one element)
    # For W_in==1: grad_input[0] = sum of ALL grad_output values
    # We can compute this as left_sum + right_sum - grad_output[pad_left] (to avoid double count)
    # Because left_sum = sum(grad_output[0:pad_left+1]), right_sum = sum(grad_output[pad_left:W_out])
    # And grad_output[pad_left] is in both, so we subtract it once
    only_one = W_in == 1
    # Load grad_output[pad_left] to subtract
    p_pad = grad_out_ptr + base_out + pad_left * grad_out_stride_w
    pad_val = tl.load(p_pad, mask=(pad_left < W_out), other=0).to(cdtype)
    single_sum = left_sum + right_sum - pad_val
    result = tl.where(only_one, single_sum, result)

    # Convert back to output dtype
    result = result.to(grad_out_ptr.dtype.element_ty)

    # Store result
    in_offsets = w.to(tl.int64) * grad_in_stride_w
    in_ptrs = grad_in_ptr + base_in + in_offsets
    tl.store(in_ptrs, result, mask=mask)


def _launch_replication_pad1d_backward_kernel(
    grad_output: torch.Tensor,
    self_tensor: torch.Tensor,
    padding,
    grad_input: torch.Tensor,
):
    if isinstance(padding, torch.Tensor):
        padding = tuple(padding.tolist())
    left, right = int(padding[0]), int(padding[1])

    dim = self_tensor.dim()
    if dim not in (2, 3):
        raise ValueError(
            "replication_pad1d_backward expects 2D (C, W) or 3D (N, C, W) input"
        )

    if dim == 3:
        N, C, W_in = self_tensor.shape
        B = N
        grad_in_s_n, grad_in_s_c, grad_in_s_w = grad_input.stride()
        grad_out_s_n, grad_out_s_c, grad_out_s_w = grad_output.stride()
    else:
        C, W_in = self_tensor.shape
        B = 1
        grad_in_s_c, grad_in_s_w = grad_input.stride()
        grad_in_s_n = 0
        if grad_output.dim() == 2:
            grad_out_s_c, grad_out_s_w = grad_output.stride()
            grad_out_s_n = 0
        elif grad_output.dim() == 3:
            grad_out_s_n, grad_out_s_c, grad_out_s_w = grad_output.stride()
        else:
            raise ValueError("grad_output has invalid dimensions")
        expected_out_shape = (C, W_in + left + right)

    W_out = W_in + left + right

    # Validate shapes
    expected_grad_output_shape = (
        (N, C, W_out) if dim == 3 else (C, W_out)
    )
    if tuple(grad_output.shape) != expected_grad_output_shape:
        raise ValueError(
            f"grad_output has incorrect shape. Expected {expected_grad_output_shape}, got {tuple(grad_output.shape)}"
        )

    if tuple(grad_input.shape) != tuple(self_tensor.shape):
        raise ValueError(
            f"grad_input has incorrect shape. Expected {tuple(self_tensor.shape)}, got {tuple(grad_input.shape)}"
        )

    grid = (triton.cdiv(W_in, 256), B * C)
    with torch_device_fn.device(grad_output.device):
        replication_pad1d_backward_kernel[grid](
            grad_output,
            grad_input,
            B,
            C,
            W_in,
            W_out,
            left,
            right,
            grad_out_s_n if dim == 3 else grad_out_s_n,
            grad_out_s_c,
            grad_out_s_w,
            grad_in_s_n if dim == 3 else grad_in_s_n,
            grad_in_s_c,
            grad_in_s_w,
            BLOCK_SIZE=256,
        )
    return grad_input


def replication_pad1d_backward(
    grad_output: torch.Tensor, self_tensor: torch.Tensor, padding
) -> torch.Tensor:
    logger.debug("GEMS REPLICATION_PAD1D_BACKWARD")
    if isinstance(padding, torch.Tensor):
        padding = tuple(padding.tolist())
    left, right = int(padding[0]), int(padding[1])

    dim = self_tensor.dim()
    if dim == 3:
        N, C, W_in = self_tensor.shape
        W_out = W_in + left + right
        grad_input = torch.empty(
            (N, C, W_in),
            device=grad_output.device,
            dtype=grad_output.dtype,
        )
    elif dim == 2:
        C, W_in = self_tensor.shape
        W_out = W_in + left + right
        grad_input = torch.empty(
            (C, W_in),
            device=grad_output.device,
            dtype=grad_output.dtype,
        )
    else:
        raise ValueError(
            "replication_pad1d_backward expects 2D (C, W) or 3D (N, C, W) input"
        )

    return _launch_replication_pad1d_backward_kernel(
        grad_output, self_tensor, (left, right), grad_input
    )


def replication_pad1d_backward_out(
    grad_output: torch.Tensor, self_tensor: torch.Tensor, padding, grad_input: torch.Tensor
) -> torch.Tensor:
    logger.debug("GEMS REPLICATION_PAD1D_BACKWARD_OUT")
    if isinstance(padding, torch.Tensor):
        padding = tuple(padding.tolist())
    left, right = int(padding[0]), int(padding[1])

    # Validate dtype/device
    if grad_input.dtype != grad_output.dtype:
        raise ValueError("grad_input dtype must match grad_output dtype")
    if grad_input.device != grad_output.device:
        raise ValueError("grad_input device must match grad_output device")

    return _launch_replication_pad1d_backward_kernel(
        grad_output, self_tensor, (left, right), grad_input
    )