...
def scatter_add_0(inp, dim, index, src, out):
...
def scatter_add_1(x, dim, index, src, out):
    ...
    if dim != x.ndim - 1:
        ...
        if dtype_convert:
            return out.copy_(x.to(src.dtype).permute(order))
        return out.copy_(x.permute(order))
    else:
        return out.copy_(x.to(src.dtype))
...
def scatter_add(x, dim, index, src):
    ...
    # Clone input for out-of-place operation
    out = x.clone()

    if equal_count == x.dim() and index.shape == src.shape and dim == x.ndim - 1:
        return scatter_add_1(x, dim, index, src, out)
    if (index.shape == src.shape and index.shape == x.shape and dim != x.ndim - 1) or (
        ...
        return scatter_add_0(x, dim, index, src, out)