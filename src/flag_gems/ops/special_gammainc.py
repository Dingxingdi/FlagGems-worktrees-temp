...
def gammainc_kernel(a_ptr, x_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    # Combine results
    result = tl.where((a_f32 > 0.0) & (x_f32 > 0.0),
...
def _launch_gammainc(out: torch.Tensor, a: torch.Tensor, x: torch.Tensor):
    ...
    assert out.numel() == a.numel() == x.numel(), "All tensors must have the same number of elements"
    ...
    gammainc_kernel[grid](a_contig, x_contig, out_contig, n_elements, BLOCK_SIZE=BLOCK_SIZE)
...
def gammainc(a: torch.Tensor, x: torch.Tensor):
    logger.debug("GEMS GAMMAINC")
    if not a.is_cuda:
...
def gammainc_out(a: torch.Tensor, x: torch.Tensor, out: torch.Tensor):
    logger.debug("GEMS GAMMAINC_OUT")
    if not (a.is_cuda and x.is_cuda and out.is_cuda):
        raise ValueError("gammainc_out: all input and output tensors must be on CUDA device")
    if not out.is_floating_point():
        raise TypeError("gammainc_out: output tensor must be a floating point type")
    if a.numel() != x.numel() or a.numel() != out.numel():
        raise ValueError("gammainc_out: input and output must have the same number of elements")
    _launch_gammainc(out, a, x)
    return out