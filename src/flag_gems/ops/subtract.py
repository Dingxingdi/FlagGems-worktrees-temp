...
# Reuse the same Triton kernels as sub operation for consistency
@pointwise_dynamic(is_tensor=[True, True, False], promotion_methods=[(0, 1, "DEFAULT")])
@triton.jit
def subtract_func(x, y, alpha):
    return x - y * alpha
...
@triton.jit
def subtract_func_tensor_scalar(x, y, alpha):
    return x - y * alpha
...
@triton.jit
def subtract_func_scalar_tensor(x, y, alpha):
    return x - y * alpha


def subtract(A, B, *, alpha=1):
    """Performs element-wise subtraction: A - alpha * B"""
    ...
    if A_is_complex or B_is_complex:
        if A_is_complex and B_is_complex:
            ...
            out_real = subtract_func(Ar, Br, alpha)
        ...
        if B.device != A.device:
            B = B.to(A.device)
        return subtract_func(A, B, alpha)
    elif isinstance(A, torch.Tensor):
        return subtract_func_tensor_scalar(A, B, alpha)
    elif isinstance(B, torch.Tensor):
        return subtract_func_scalar_tensor(A, B, alpha)
    else:
        # Both scalar
...
def subtract_(A, B, *, alpha=1):
    ...
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
        if B.device != A.device:
            B = B.to(A.device)
        return subtract_func(A, B, alpha, out0=A)
    elif isinstance(A, torch.Tensor):
        return subtract_func_tensor_scalar(A, B, alpha, out0=A)
    else:
