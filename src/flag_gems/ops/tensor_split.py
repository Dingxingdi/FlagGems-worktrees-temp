...
def split_copy_kernel(
    ...
    input_idx = pre_idx * dim_size_input * dim_prod_post + split_idx * dim_prod_post + post_idx
...
def tensor_split(
    ...
    if input.numel() == 0:
        ...
        if isinstance(indices_or_sections, int):
            ...
            n = len(indices_or_sections) + 1 if isinstance(indices_or_sections, (list, tuple)) else 1
        output_tensors = []
        for i in range(n):
            output_shape = list(input.shape)
            output_shape[dim] = 0
            output_tensors.append(torch.empty(output_shape, dtype=input.dtype, device=input.device))
    ...
    if isinstance(indices_or_sections, int):
        ...
        if len(indices) > 0:
            if indices[0] < 0 or indices[-1] > dim_size:
                raise ValueError(f"indices_or_sections must be in range [0, {dim_size}]")
    ...
    for split_size in split_sizes:
        ...
        output_tensor = torch.empty(output_shape, dtype=input.dtype, device=input.device)
