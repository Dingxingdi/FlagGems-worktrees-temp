...
def upsample_linear1d_(
    self: torch.Tensor,
    output_size,
    align_corners: bool,
    scales: float = None,
):
    """In-place version of upsample_linear1d."""
    logger.debug("GEMS UPSAMPLE LINEAR1D_ INPLACE")
    assert self.ndim == 3, "Input must be [N, C, W]"
    assert self.is_cuda
    ...
    self.resize_((N, C, W_out))
    W_out = self.shape[-1]  # Get the actual new size

    inp = self.view(N * C, W_in)
    out = self.view(N * C, W_out)
    ...
    BLOCK_SIZE = 256
    grid = (N * C, triton.cdiv(W_out, BLOCK_SIZE))
...
def upsample_linear1d_out(
    ...
    *,
    ...
    """Out-version of upsample_linear1d."""
    logger.debug("GEMS UPSAMPLE LINEAR1D OUT")
    assert self.ndim == 3, "Input must be [N, C, W]"
    assert self.is_cuda
    ...
    assert out.shape == (N, C, W_out), f"Expected out shape {(N, C, W_out)}, got {out.shape}"

    inp = self.contiguous().view(NC, W_in)
    out_view = out.contiguous().view(NC, W_out)
