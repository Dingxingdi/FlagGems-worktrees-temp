...
@triton.jit
def _var_map_kernel(X, Tmp_sum, Tmp_sum_sq, N, BLOCK_N: tl.constexpr):
    pid = tl.program_id(0)
...
def _var_fused_dim_kernel(
    ...
    tl.store(out_ptrs, safe_var.to(Out.dtype.element_ty), mask=row_mask)
...
def var(x, dim=None, *, correction=None, keepdim=False):
    ...
    if dim is None:
        logger.debug("GEMS VAR (Global Simple Map-Reduce Path)")
        N = x.numel()
        ...
        logger.warning(
            f"GEMS var: Using compatible but non-optimal path for dim={dim} (dim_compress)."
        )
