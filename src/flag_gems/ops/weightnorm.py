...
def weight_norm(v, g, dim=0):
    """Applies weight normalization to a 2D weight tensor.

    This is a simplified version that returns only the normalized tensor
    (not the norm value). For full functionality with norm retrieval, use
    weight_norm_interface.

    Args:
        v: The weight tensor with shape (..., G, ...)
        g: The gain tensor with shape (G,)
        dim: The dimension corresponding to the G groups

    Returns:
        The normalized weight tensor with the same shape as v
    """
    logger.debug("GEMS WEIGHT NORM")
    out, _ = weight_norm_interface(v, g, dim)
    return out
