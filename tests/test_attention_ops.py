...
def test_scaled_dot_product_fused_attention_overrideable(
    ...
    """Test for _scaled_dot_product_fused_attention_overrideable operator."""
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"
    device = torch_device_fn.current_device()
    q, k, v = make_input(
    ...
    torch_result = torch.nn.functional.scaled_dot_product_attention(
        ref_q, ref_k, ref_v, scale=scale, is_causal=is_causal, enable_gqa=enable_gqa
    )

    # GEMS implementation using the new operator
    gems_result = flag_gems.ops._scaled_dot_product_fused_attention_overrideable(
        q,
        k,
        v,
        attn_mask=None,
