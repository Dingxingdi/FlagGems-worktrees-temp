...
def test_scaled_dot_product_flash_attention_for_cpu_backward(
    ...
    """Test _scaled_dot_product_flash_attention_for_cpu_backward operator."""
    device = torch_device_fn.current_device()
    ...
    # Run both backward implementations
    with flag_gems.use_gems():
        # Reference: existing scaled_dot_product_attention_backward
        gq_ref, gk_ref, gv_ref = flag_gems.ops.scaled_dot_product_attention_backward(
            grad_out, query, key, value, out, logsumexp
        )

        # New: _scaled_dot_product_flash_attention_for_cpu_backward
        gq_new, gk_new, gv_new = flag_gems.ops._scaled_dot_product_flash_attention_for_cpu_backward(
            grad_out, query, key, value, out, logsumexp, 0.0, False
        )

    # Verify results match
    gems_assert_close(gq_ref, gq_new, dtype)
    gems_assert_close(gk_ref, gk_new, dtype)
    gems_assert_close(gv_ref, gv_new, dtype)
