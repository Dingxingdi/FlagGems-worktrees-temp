...
@pytest.mark.group_query_attention
@pytest.mark.parametrize(
    "batch, num_q_head, num_kv_head, seq_len, head_size",
    [
        (2, 4, 2, 512, 128),
        (2, 4, 1, 1024, 64),
        (2, 8, 2, 256, 64),
...
def test_group_query_attention(
    ...
    # query: (batch, num_q_head, seq_len, head_size)
    # key/value: (batch, num_kv_head, seq_len, head_size)
    query = torch.randn(
        batch, num_q_head, seq_len, head_size, dtype=dtype, device=device
    )
    key = torch.randn(
        batch, num_kv_head, seq_len, head_size, dtype=dtype, device=device
    )
    value = torch.randn(
        batch, num_kv_head, seq_len, head_size, dtype=dtype, device=device
    )

    # Reference: use scaled_dot_product_attention with enable_gqa=True
    # Use the torch_sdpa function which has proper handling
    scale = 1.0 / np.sqrt(head_size)
    ref_out = torch_sdpa(
        query, key, value, scale, is_causal, enable_gqa=True
    )

    # GEMS implementation
    with flag_gems.use_gems():
        gems_out = flag_gems.group_query_attention(
            query, key, value, scale=scale, is_causal=is_causal
        )

    # Allow larger tolerance for attention operations (especially bfloat16)
    # Default atol is 1e-4, use 1e-3 for float16 and 1e-2 for bfloat16
    atol = 1e-3 if dtype == torch.float16 else 1e-2
    gems_assert_close(gems_out, ref_out, dtype, atol=atol)
