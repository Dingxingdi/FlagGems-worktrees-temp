...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_cudnn_attention_forward(batch, num_heads, seq_len, head_dim, dtype):
    """Test _cudnn_attention_forward accuracy"""
    q = torch.randn(
    ...
    # GEMS implementation
    with flag_gems.use_gems():
        gems_out = flag_gems.ops._cudnn_attention_forward(
            q, k, v, is_causal=False
        )

    # Use appropriate tolerance for bfloat16 and causal tests
    atol = 2e-3 if dtype == torch.bfloat16 else 1e-3
...
@pytest.mark.parametrize("head_dim", [64, 128])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_cudnn_attention_forward_is_causal(batch, num_heads, seq_len, head_dim, dtype):
    """Test _cudnn_attention_forward with causal=True"""
    q = torch.randn(
        batch, num_heads, seq_len, head_dim, dtype=dtype, device=device
    ).requires_grad_(True)
    k = torch.randn(
    ...
    # Reference: torch.nn.functional.scaled_dot_product_attention with causal
    ref_q = to_reference(q)
    ref_k = to_reference(k)
    ref_v = to_reference(v)
    ref_out = torch.nn.functional.scaled_dot_product_attention(
