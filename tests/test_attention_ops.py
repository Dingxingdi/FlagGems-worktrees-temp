...
# Test for page_attention operator
@pytest.mark.skipif(TO_CPU, reason="Unsupported in CPU mode")
@pytest.mark.skipif(flag_gems.vendor_name == "hygon", reason="RuntimeError")
@pytest.mark.skipif(flag_gems.vendor_name == "kunlunxin", reason="RESULT TODOFIX")
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_page_attention(
    ...
    # Create context lengths (random sequence lengths up to max_context_length)
    torch.manual_seed(42)
    context_lengths = torch.randint(1, max_context_length + 1, (batch_size,), dtype=torch.int32)

    # Calculate actual number of pages needed
    max_ctx = context_lengths.max().item()
    num_actual_blocks = (max_ctx + block_size - 1) // block_size

    # Create query: (batch, num_heads, seqlen_q, head_dim)
    query = torch.randn(batch_size, num_heads, seqlen_q, head_dim, dtype=dtype, device=device)

    # Create paged key cache: (num_actual_blocks, block_size, num_kv_heads, head_dim)
    key_cache = torch.randn(num_actual_blocks, block_size, num_kv_heads, head_dim, dtype=dtype, device=device)
    value_cache = torch.randn(num_actual_blocks, block_size, num_kv_heads, head_dim, dtype=dtype, device=device)

    # Create block table: (batch, num_blocks)
    max_blocks_per_seq = num_blocks
    block_table = torch.randint(0, num_actual_blocks, (batch_size, max_blocks_per_seq), dtype=torch.int32, device=device)

    # Run page_attention
    with flag_gems.use_gems():
        res_out = flag_gems.page_attention(
            query,
            key_cache,
            value_cache,
            block_table,
    ...
    # Check that output is valid (no NaN/Inf, correct shape)
    assert res_out.shape == query.shape, f"Output shape {res_out.shape} != query shape {query.shape}"
    assert not torch.isnan(res_out).any(), "Output contains NaN"
    assert not torch.isinf(res_out).any(), "Output contains Inf"
    assert res_out.device == query.device, f"Output device {res_out.device} != query device {query.device}"
