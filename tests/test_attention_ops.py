...
def test_sdpa_math_for_mps(
    ...
    """Test _scaled_dot_product_attention_math_for_mps implementation."""
    if flag_gems.vendor_name == "mthreads":
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    # Reference: use PyTorch's math implementation
    ref_q = to_reference(q)
    ...
    # Compute reference using PyTorch's math SDPA
    if torch.__version__ >= "2.3":
        # PyTorch uses the math backend by default when flash/mem_efficient are disabled
        with torch.nn.attention.sdpa_kernel(backends=[torch.nn.attention.SDPBackend.MATH]):
            ref_out = torch.nn.functional.scaled_dot_product_attention(
                ref_q, ref_k, ref_v, scale=scale
            )
    else:
        # For older PyTorch, compute manually
        qk = torch.einsum("bhid,bhjd->bhij", ref_q * scale, ref_k)
        attn_weight = torch.softmax(qk, dim=-1)
        ref_out = torch.einsum("bhij,bhjd->bhid", attn_weight, ref_v)

    # Test our implementation
    with flag_gems.use_gems():
        gems_out = flag_gems.ops._scaled_dot_product_attention_math_for_mps(q, k, v)
