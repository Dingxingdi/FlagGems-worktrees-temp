...
# FlashAttention_V2 tests
def gems_flash_attention_forward(q, k, v, dropout_p=0.0, is_causal=False, scale=None):
    """Reference implementation using GEMS flash_attention_forward."""
    import flag_gems

    # Transpose to (batch, seqlen, num_heads, head_dim) format
    q_t = q.transpose(1, 2)
    k_t = k.transpose(1, 2)
    v_t = v.transpose(1, 2)
    ...
    with flag_gems.use_gems():
        out, lse, seed, offset, debug = flag_gems.ops.flash_attention_forward(
            q_t, k_t, v_t,
            None, None,
            q_t.shape[-3], k_t.shape[-3],
            dropout_p, is_causal, False,
            scale=scale,
        )

    # Transpose back to (batch, num_heads, seqlen, head_dim) format
    return out.transpose(1, 2)


def gems_flash_attention_v2(q, k, v, dropout_p=0.0, is_causal=False, scale=None):
    """GEMS implementation of FlashAttention_V2."""
    import flag_gems

    with flag_gems.use_gems():
        return flag_gems.FlashAttention_V2(q, k, v, dropout_p, is_causal, scale=scale)
...
@pytest.mark.FlashAttention_V2
@pytest.mark.parametrize(
    ["batch", "num_head", "q_seq_len", "kv_seq_len"],
    [(1, 1, 128, 128), (4, 8, 512, 512), (2, 4, 256, 256)],
)
@pytest.mark.parametrize("head_size", [64, 128])
@pytest.mark.parametrize("is_causal", [False, True])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_flash_attention_v2(
    batch, num_head, q_seq_len, kv_seq_len, head_size, is_causal, dtype
):
    """Test that FlashAttention_V2 produces same output as flash_attention_forward."""
    device = torch_device_fn.current_device()
    q, k, v = make_input(
        batch, num_head, num_head, q_seq_len, kv_seq_len, head_size, dtype, device
    )
    ...
    # Compare FlashAttention_V2 with flash_attention_forward
    # They should produce identical results since FlashAttention_V2 wraps flash_attention_forward
    fwd_out = gems_flash_attention_forward(q, k, v, 0.0, is_causal, scale)
    v2_out = gems_flash_attention_v2(q, k, v, 0.0, is_causal, scale)

    gems_assert_close(v2_out, fwd_out, dtype)
