...
@pytest.mark.parametrize("is_causal", [False, True])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_scaled_dot_product_flash_attention_for_cpu(
    batch,
    num_head,
    seq_len,
    head_size,
    ...
    """Test for _scaled_dot_product_flash_attention_for_cpu operator"""
    import math

    device = torch_device_fn.current_device()
    q = torch.randn(
        batch, num_head, seq_len, head_size, dtype=dtype, device=device
    ).uniform_(-0.05, 0.05)
    k = torch.randn(
        batch, num_head, seq_len, head_size, dtype=dtype, device=device
    ).uniform_(-0.05, 0.05)
    ...
    # Reference: use standard SDPA (scaled_dot_product_attention) as reference
    # Move to CPU for reference calculation
    ref_q = q.to("cpu").float()
    ref_k = k.to("cpu").float()
    ref_v = v.to("cpu").float()

    scale = 1.0 / math.sqrt(head_size)
    ref_out = torch.nn.functional.scaled_dot_product_attention(
        ref_q, ref_k, ref_v, is_causal=is_causal, scale=scale
    )

    # Also compute logsumexp manually as reference
    # Note: GEMS implementation uses exp2 internally (qk *= scale * log2(e))
    # So logsumexp = log(sum(exp(qk * scale))) = log2(sum(exp2(qk * scale * log2(e)))) / log2(e)
    # But simpler: just compute max(qk*scale) + log(sum(exp(qk*scale - max)))
    LOG2E = 1.44269504  # log2(e)
    qk = torch.einsum("bhnd,bhmd->bhnm", ref_q * scale * LOG2E, ref_k)
    if is_causal:
        mask = torch.ones(seq_len, seq_len, device=qk.device, dtype=torch.bool)
        mask = torch.triu(mask, diagonal=1)
        qk = qk.masked_fill(mask, float("-inf"))
    qk_max = qk.max(dim=-1, keepdim=True).values
    qk_exp2 = torch.pow(2.0, qk)  # exp2(qk)
    qk_sum = qk_exp2.sum(dim=-1)
    ref_lse = qk_max.squeeze(-1) + torch.log2(qk_sum)

    # GEMS implementation
    with flag_gems.use_gems():
        gems_out, gems_lse = flag_gems.scaled_dot_product_flash_attention_for_cpu(
            q, k, v, is_causal=is_causal, scale=scale
        )

    # Move reference to the same device for comparison
    ref_out_dev = ref_out.to(gems_out.device).to(dtype)
    ref_lse_dev = ref_lse.to(gems_lse.device)

    gems_assert_close(gems_out, ref_out_dev, dtype)
    # Check logsumexp - use float32 for comparison as lse is always float32
    gems_assert_close(gems_lse.float(), ref_lse_dev.float(), torch.float32, atol=1e-2)
