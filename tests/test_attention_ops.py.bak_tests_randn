...
# Multi-Query Attention (MQA) test - where KV has 1 head
...
@pytest.mark.parametrize("is_causal", [False, True])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_multi_query_attention_mqa(
    ...
    if dtype == torch.float32:
        gems_assert_close(gems_result, ref_result, dtype=dtype, atol=1e-5)
    elif dtype == torch.float16:
        gems_assert_close(gems_result, ref_result, dtype=dtype, atol=1e-3)
    else:  # bfloat16
        gems_assert_close(gems_result, ref_result, dtype=dtype, atol=2e-2)
