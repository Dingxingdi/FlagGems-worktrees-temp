...
# Multi-Head Attention MHA tests
@pytest.mark.skipif(flag_gems.vendor_name == "metax", reason="TODOFIX")
@pytest.mark.skipif(TO_CPU, reason="Unsupported in CPU mode")
@pytest.mark.multi_head_attention_mha
@pytest.mark.parametrize("batch", [1, 2])
@pytest.mark.parametrize("num_heads", [4, 8])
@pytest.mark.parametrize("seq_len", [64, 128])
@pytest.mark.parametrize("head_dim", [32, 64])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_multi_head_attention_mha(batch, num_heads, seq_len, head_dim, dtype):
    """Test multi_head_attention_mha accuracy against reference implementation"""
    device = torch_device_fn.current_device()
    ...
    # Reference: use scaled_dot_product_attention as reference
    ref_q = to_reference(q, False)
    ref_k = to_reference(k, False)
    ref_v = to_reference(v, False)
    scale = 1.0 / (head_dim**0.5)

    # Compute reference using PyTorch's SDPA
    ...
    with flag_gems.use_gems():
        gems_out = flag_gems.multi_head_attention_mha(q, k, v, scale=scale)

    # Use more lenient tolerance for attention operations
    gems_assert_close(gems_out, ref_out, dtype, atol=1e-3)
