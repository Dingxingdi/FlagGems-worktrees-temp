...
def test_efficient_attention_backward(
    ...
    """Test for _efficient_attention_backward operator.

    This test verifies that the GEMS efficient attention backward produces
    correct gradients by comparing against the PyTorch SDPA implementation.
    """
    device = torch_device_fn.current_device()
    ...
    # Forward pass - both PyTorch and GEMS
    scale = float(1.0 / np.sqrt(head_size))
    torch_result = torch_sdpa(ref_q, ref_k, ref_v, scale, is_causal=False)
    with flag_gems.use_gems():
        gems_result = flag_gems.ops.scaled_dot_product_attention(q, k, v, scale=scale)
