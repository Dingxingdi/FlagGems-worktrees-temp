...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_scaled_dot_product_cudnn_attention_backward(dtype):
    """Test for _scaled_dot_product_cudnn_attention_backward operator."""
    device = torch.device("cuda")

    # Test with typical attention shapes
    batch, num_heads, seq_len, head_dim = 2, 8, 64, 64
    q = torch.randn(batch, num_heads, seq_len, head_dim, dtype=dtype, device=device, requires_grad=True)
    k = torch.randn(batch, num_heads, seq_len, head_dim, dtype=dtype, device=device, requires_grad=True)
    v = torch.randn(batch, num_heads, seq_len, head_dim, dtype=dtype, device=device, requires_grad=True)

    # Forward pass to get required tensors
    out, logsumexp, *rest = torch.ops.aten._scaled_dot_product_cudnn_attention.default(
        q, k, v, None, True
    )
    ...
    # With FlagGems enabled - uses cuDNN implementation
    with flag_gems.use_gems():
        d_q, d_k, d_v = torch.ops.aten._scaled_dot_product_cudnn_attention_backward.default(
            grad_out, q, k, v, out, logsumexp,
            rest[4], rest[5],  # philox_seed, philox_offset
            None,  # attn_bias
            None, None,  # cum_seq_q, cum_seq_k
            seq_len, seq_len,  # max_q, max_k
            0.0, False  # dropout_p, is_causal
        )

    # Reference (without FlagGems)
    q_ref = to_reference(q)
    k_ref = to_reference(k)
    v_ref = to_reference(v)
    out_ref, logsumexp_ref, *rest_ref = torch.ops.aten._scaled_dot_product_cudnn_attention.default(
        q_ref, k_ref, v_ref, None, True
    )
    d_q_ref, d_k_ref, d_v_ref = torch.ops.aten._scaled_dot_product_cudnn_attention_backward.default(
        grad_out, q_ref, k_ref, v_ref, out_ref, logsumexp_ref,
        rest_ref[4], rest_ref[5],
        None, None, None, seq_len, seq_len, 0.0, False
    )

    # Verify gradients match
    gems_assert_close(d_q, d_q_ref, dtype)
    gems_assert_close(d_k, d_k_ref, dtype)
    gems_assert_close(d_v, d_v_ref, dtype)
