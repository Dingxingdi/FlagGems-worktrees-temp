...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_cudnn_attention_backward(batch, num_head, q_seq_len, kv_seq_len, head_size, dtype):
    """Test for scaled_dot_product_cudnn_attention_backward operator.

    This test verifies that the cuDNN attention backward operator is correctly
    registered and produces valid gradients.
    """
    if flag_gems.vendor_name == "mthreads":
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    device = torch_device_fn.current_device()
    q, k, v = make_input(
        batch, num_head, num_head, q_seq_len, kv_seq_len, head_size, dtype, device, requires_grad=True
    )
    ref_q = to_reference(q, False)
    ref_k = to_reference(k, False)
    ref_v = to_reference(v, False)
    ...
    # This will use the registered GEMS implementation
    with flag_gems.use_gems():
    ...
    assert q.grad is not None, "Query gradient should not be None"
    assert k.grad is not None, "Key gradient should not be None"
    assert v.grad is not None, "Value gradient should not None"

    assert q.grad.shape == q.shape, f"Expected query grad shape {q.shape}, got {q.grad.shape}"
    assert k.grad.shape == k.shape, f"Expected key grad shape {k.shape}, got {k.grad.shape}"
    assert v.grad.shape == v.shape, f"Expected value grad shape {v.shape}, got {v.grad.shape}"

    # Compare with PyTorch reference
    ref_out = torch.nn.functional.scaled_dot_product_attention(
        ref_q, ref_k, ref_v, scale=scale
    )
    ref_dout = to_reference(dout, False)
    ref_out.backward(ref_dout)

    # Check gradient accuracy
    gems_assert_close(q.grad, ref_q.grad, dtype)
    gems_assert_close(k.grad, ref_k.grad, dtype)
    gems_assert_close(v.grad, ref_v.grad, dtype)
