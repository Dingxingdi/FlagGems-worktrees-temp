...
def test_scaled_dot_product_efficient_attention(
    batch, num_q_head, num_kv_head, q_seq_len, kv_seq_len, head_size, dtype
):
    """Test for _scaled_dot_product_efficient_attention operator."""
    device = torch_device_fn.current_device()
    q, k, v = make_input(
    ...
    scale = float(1.0 / np.sqrt(head_size))
    ref_out = torch._scaled_dot_product_efficient_attention(
        ref_q, ref_k, ref_v, None, True, 0.0, False, scale=scale
    )

    # Call GEMS implementation directly
    gems_out = flag_gems.ops.scaled_dot_product_efficient_attention(
        q, k, v, None, True, 0.0, False, scale=scale
    )

    # Check output - main accuracy check
    gems_assert_close(gems_out[0], ref_out[0], dtype)
...
@pytest.mark.parametrize("is_causal", [False, True])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_scaled_dot_product_efficient_attention_causal(
    batch, num_q_head, num_kv_head, q_seq_len, kv_seq_len, head_size, is_causal, dtype
):
    """Test for _scaled_dot_product_efficient_attention with causal mask."""
    device = torch_device_fn.current_device()
    q, k, v = make_input(
        batch, num_q_head, num_kv_head, q_seq_len, kv_seq_len, head_size, dtype, device
    )
    ...
    scale = float(1.0 / np.sqrt(head_size))
    ref_out = torch._scaled_dot_product_efficient_attention(
        ref_q, ref_k, ref_v, None, True, 0.0, is_causal, scale=scale
    )

    # Call GEMS implementation directly
    gems_out = flag_gems.ops.scaled_dot_product_efficient_attention(
        q, k, v, None, True, 0.0, is_causal, scale=scale
    )
