...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_log_sigmoid_backward(shape, dtype):
    # log_sigmoid_backward(grad_output, self, buffer)
    # buffer = exp(-self) from log_sigmoid_forward
    # gradient = grad_output * buffer / (1 + buffer) = grad_output * (1 - sigmoid(self))
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grad_output = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    buffer = torch.exp(-inp)  # buffer from log_sigmoid_forward

    ref_inp = to_reference(inp, True)
    ref_grad_output = to_reference(grad_output, True)
    ref_buffer = torch.exp(-ref_inp)

    ref_out = torch.ops.aten.log_sigmoid_backward(ref_grad_output, ref_inp, ref_buffer)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.log_sigmoid_backward(grad_output, inp, buffer)
