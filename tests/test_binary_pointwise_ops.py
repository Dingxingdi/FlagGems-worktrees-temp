...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nextafter(shape, dtype):
    input = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    other = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_input = to_reference(input)
    ref_other = to_reference(other)
    ref_out = torch.nextafter(ref_input, ref_other)

    with flag_gems.use_gems():
        res_out = torch.nextafter(input, other)
...
def test_accuracy_nextafter_(shape, dtype):
    ...
    ref_other = to_reference(other)
    ref_out = ref_input.nextafter_(ref_other)

    with flag_gems.use_gems():
        res_out = input.nextafter_(other)
