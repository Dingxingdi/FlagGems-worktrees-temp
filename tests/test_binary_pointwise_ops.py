...
def test_accuracy_igammac(shape, dtype):
    # Use x << a to ensure series expansion converges well
    # igammac(a, x) = 1 - igamma(a, x), series converges when x < a
    # Use very conservative ranges to ensure convergence
    a = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 3 + 5  # a in [5, 8]
    x = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 0.2 + 0.02  # x in [0.02, 0.22], always << a
    ref_a = to_reference(a, True)
    ref_x = to_reference(x, True)

    ref_out = torch.igammac(ref_a, ref_x)
    with flag_gems.use_gems():
        res_out = torch.igammac(a, x)
...
def test_accuracy_igammac_(shape, dtype):
    ...
    a = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 3 + 5  # a in [5, 8]
    x = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 0.2 + 0.02  # x in [0.02, 0.22], always << a
    ref_a = to_reference(a, True).clone()
    ref_x = to_reference(x, True)

    ref_out = ref_a.igammac_(ref_x)
    with flag_gems.use_gems():
        res_out = a.igammac_(x)
