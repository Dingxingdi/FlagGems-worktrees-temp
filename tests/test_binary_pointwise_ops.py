...
# Beam Search Score operator tests
# Beam_Search_Score computes beam search scores by adding cumulative beam scores
# with log probabilities (with broadcasting)
...
def test_accuracy_Beam_Search_Score(shape, dtype):
    ...
    log_probs = torch.randn(batch_size, vocab_size, dtype=dtype, device=flag_gems.device)
    ...
    ref_beam_scores = to_reference(beam_scores, True)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Beam_Search_Score_(shape, dtype):
    if len(shape) < 2:
        pytest.skip("Beam_Search_Score requires at least 2D tensors")
    batch_size = shape[0]
    vocab_size = shape[1]
    ...
    # Reference: PyTorch broadcasting addition
    ref_log_probs = to_reference(log_probs.clone(), True)
    ref_beam_scores = to_reference(beam_scores, True)
    ref_out = ref_log_probs + ref_beam_scores.unsqueeze(-1)

    with flag_gems.use_gems():
        res_out = flag_gems.Beam_Search_Score_(log_probs, beam_scores)
