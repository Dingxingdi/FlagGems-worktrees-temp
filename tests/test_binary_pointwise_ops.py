...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_divide(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_divide_(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Avoid division by zero
    inp2 = torch.where(inp2 == 0, torch.ones_like(inp2), inp2)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("scalar", SCALARS)
def test_accuracy_divide_tensor_scalar(shape, dtype, scalar):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Avoid division by zero
    if scalar == 0:
        scalar = 1.0
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("scalar", SCALARS)
def test_accuracy_divide_scalar_tensor(shape, dtype, scalar):
    # Avoid division by zero
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.where(inp2 == 0, torch.ones_like(inp2), inp2)
...
@pytest.mark.divide
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_divide_scalar_scalar(dtype):
    # Avoid division by zero
    scalar1 = 5.0
    scalar2 = 2.0
...
def test_accuracy_divide_mode(shape, dtype, rounding_mode):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Avoid division by zero
    inp2 = torch.where(inp2 == 0, torch.ones_like(inp2), inp2)
    ...
    ref_out = torch.divide(ref_inp1, ref_inp2, rounding_mode=rounding_mode)
    with flag_gems.use_gems():
        res_out = torch.divide(inp1, inp2, rounding_mode=rounding_mode)
