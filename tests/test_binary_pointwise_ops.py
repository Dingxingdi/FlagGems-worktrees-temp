...
@pytest.mark.igamma_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
# Note: igamma only supports float32 on CUDA (float16/bfloat16 not implemented in PyTorch)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_igamma_(shape, dtype):
    # Use positive values only since igamma is only defined for a > 0 and x >= 0
