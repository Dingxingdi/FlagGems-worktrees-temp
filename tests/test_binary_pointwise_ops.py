...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_shifted_chebyshev_polynomial_u(shape, dtype):
    # x in [0, 1] range for shifted Chebyshev
    x = torch.rand(shape, dtype=dtype, device=flag_gems.device)
    # n is the degree, typically small (0-10)
    n = torch.randint(0, 10, shape, dtype=torch.long, device=flag_gems.device)

    ref_x = to_reference(x, True)
    ref_n = to_reference(n, False)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_shifted_chebyshev_polynomial_u_broadcast(shape, dtype):
    # Test broadcasting: x and n have different shapes
    x = torch.rand(shape, dtype=dtype, device=flag_gems.device)
    # n is a 1D tensor that will broadcast with x
    n = torch.randint(0, 10, (shape[-1],), dtype=torch.long, device=flag_gems.device)
