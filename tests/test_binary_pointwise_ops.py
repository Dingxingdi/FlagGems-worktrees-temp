...
def test_accuracy_complex(shape, dtype):
    # Skip bfloat16 as torch.complex doesn't support it directly
    if dtype == torch.bfloat16:
        pytest.skip("torch.complex doesn't support bfloat16 directly")

    real = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    imag = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_real = to_reference(real, True)
    ref_imag = to_reference(imag, True)
    ref_out = torch.complex(ref_real, ref_imag)

    with flag_gems.use_gems():
        res_out = torch.complex(real, imag)

    # Determine output dtype for comparison
    if dtype == torch.float16:
        output_dtype = torch.complex32
