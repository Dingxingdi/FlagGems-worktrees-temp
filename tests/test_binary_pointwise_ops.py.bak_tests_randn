...
def test_accuracy_special_hermite_polynomial_he_scalar_n(shape, dtype, n):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.inplace
@pytest.mark.special_hermite_polynomial_he_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_hermite_polynomial_he_(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randint(0, 6, shape, dtype=torch.int64, device=flag_gems.device)
    ...
    ref_out = ref_inp1.clone()
    torch.special.hermite_polynomial_he(ref_out, ref_inp2, out=ref_out)

    with flag_gems.use_gems():
        res_out = inp1.clone()
        torch.special.hermite_polynomial_he(res_out, inp2, out=res_out)
