...
def test_accuracy_fmod(shape, dtype):
    ...
    # Use larger atol for fmod due to minor precision differences from div_rz
    gems_assert_close(res_out, ref_out, dtype, atol=2.0)

    # Test tensor-scalar
    for d in inp2.flatten()[:2]:
        ref_d = to_reference(d, False)
        ref_out = torch.fmod(ref_inp1, ref_d)
        with flag_gems.use_gems():
            res_out = torch.fmod(inp1, d)
        gems_assert_close(res_out, ref_out, dtype, atol=2.0)

        ref_out = torch.fmod(ref_d, ref_inp2)
        with flag_gems.use_gems():
            res_out = torch.fmod(d, inp2)
        gems_assert_close(res_out, ref_out, dtype, atol=2.0)
