...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_laguerre_polynomial_l(shape, dtype):
    # Generate x values and n orders (n should be non-negative integers)
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    n = torch.randint(0, 10, shape, dtype=torch.int64, device=flag_gems.device)

    ref_x = to_reference(x, True)
    ref_n = n.to(ref_x.device).to(torch.int64)

    ref_out = torch.ops.aten.special_laguerre_polynomial_l(ref_x, ref_n)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.special_laguerre_polynomial_l(x, n)

    gems_assert_close(res_out, ref_out, dtype, equal_nan=True)


@pytest.mark.inplace
@pytest.mark.special_laguerre_polynomial_l_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_laguerre_polynomial_l_(shape, dtype):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    n = torch.randint(0, 10, shape, dtype=torch.int64, device=flag_gems.device)
