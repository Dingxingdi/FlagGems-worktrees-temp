...
@pytest.mark.shifted_chebyshev_polynomial_v
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_shifted_chebyshev_polynomial_v(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # n (degree) should be an integer tensor
    inp2 = torch.randint(0, 10, shape, dtype=torch.int64, device=flag_gems.device)

    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)

    ref_out = torch.special.shifted_chebyshev_polynomial_v(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.special.shifted_chebyshev_polynomial_v(inp1, inp2)

    gems_assert_close(res_out, ref_out, dtype)


# Use small integers for n (degree of polynomial)
SHIFTED_CHEBYSHEV_SCALARS = [0, 1, 2, 3, 4, 5]


@pytest.mark.shifted_chebyshev_polynomial_v
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("scalar", SHIFTED_CHEBYSHEV_SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_shifted_chebyshev_polynomial_v_tensor_scalar(scalar, shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = scalar

    ref_inp1 = to_reference(inp1, True)

    ref_out = torch.special.shifted_chebyshev_polynomial_v(ref_inp1, inp2)
    with flag_gems.use_gems():
        res_out = torch.special.shifted_chebyshev_polynomial_v(inp1, inp2)
