...
@pytest.mark.parametrize("dtype1", ALL_FLOAT_DTYPES + ALL_INT_DTYPES + BOOL_TYPES)
@pytest.mark.parametrize("dtype2", ALL_FLOAT_DTYPES + ALL_INT_DTYPES + BOOL_TYPES)
def test_accuracy_result_type(dtype1, dtype2):
    # result_type returns a dtype, not a tensor
    inp1 = torch.tensor([1, 2], dtype=dtype1, device=flag_gems.device)
    inp2 = torch.tensor([3, 4], dtype=dtype2, device=flag_gems.device)

    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.result_type(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.result_type(inp1, inp2)
