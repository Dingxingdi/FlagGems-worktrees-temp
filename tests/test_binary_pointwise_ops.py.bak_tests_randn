...
@pytest.mark.parametrize("alpha", SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_subtract(shape, alpha, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)
...
@pytest.mark.parametrize("alpha", SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_subtract_tensor_scalar(shape, scalar, alpha, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = scalar
    ref_inp1 = to_reference(inp1, True)

    ref_out = torch.subtract(ref_inp1, inp2, alpha=alpha)
    with flag_gems.use_gems():
        res_out = torch.subtract(inp1, inp2, alpha=alpha)
...
def test_accuracy_subtract_complex(shape, dtype):
    other_type = dtype if dtype.is_complex else torch.float32
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=other_type, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)
    alpha = 1.0
...
@pytest.mark.subtract
@pytest.mark.parametrize("dtype", [torch.float32, torch.int64])
def test_accuracy_subtract_scalar_scalar(dtype):
    if dtype == torch.float32:
        inp1 = float(np.float32(random.random()))
        inp2 = float(np.float32(random.random()))
        alpha = float(np.float32(random.random()))
    ...
    ref_out = torch.subtract(inp1, inp2, alpha=alpha)
    with flag_gems.use_gems():
        res_out = torch.subtract(inp1, inp2, alpha=alpha)
