...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_chebyshev_polynomial_u(shape, dtype):
    # x: float tensor, n: integer tensor
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # n values should be small integers for testing
...
@pytest.mark.special_chebyshev_polynomial_u_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_chebyshev_polynomial_u_out(shape, dtype):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    n = torch.randint(0, 10, shape, dtype=torch.long, device=flag_gems.device)
    ...
    if dtype in (torch.float16, torch.bfloat16):
        ...
        ref_out = torch.ops.aten.special_chebyshev_polynomial_u.out(
            ref_x.float(), ref_n, out=out_ref
        )
        ref_out = ref_out.to(dtype)
    else:
        out_ref = torch.empty_like(ref_x)
        ref_out = torch.ops.aten.special_chebyshev_polynomial_u.out(
            ref_x, ref_n, out=out_ref
        )

    out_act = torch.empty_like(x)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.special_chebyshev_polynomial_u.out(x, n, out=out_act)
