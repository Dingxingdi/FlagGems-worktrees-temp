...
def test_accuracy_special_chebyshev_polynomial_t(shape, dtype):
    # x in range [-1, 1] for stable Chebyshev polynomial evaluation
    x = torch.empty(shape, dtype=dtype, device=flag_gems.device).uniform_(-1, 1)
    # n is the degree, typically small positive integers
