...
def test_accuracy_bitwisexor_scalar_(shape, dtype):
    ...
    ref_out = ref_inp1
    ref_out = ref_out.bitwise_xor_(ref_inp2)
    with flag_gems.use_gems():
        res_out = inp1
        res_out = res_out.bitwise_xor_(inp2)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", INT_DTYPES + BOOL_TYPES)
def test_accuracy_bitwisexor_tensor_scalar(shape, dtype):
    if dtype in BOOL_TYPES:
        inp1 = torch.randint(0, 2, size=shape, dtype=torch.bool, device="cpu").to(
            flag_gems.device
        )
        inp2 = bool(random.randint(0, 2))
    else:
