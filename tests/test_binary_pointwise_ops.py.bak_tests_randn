...
def test_accuracy_gcd(shape, dtype):
    # Generate non-zero integers to avoid gcd(0, x) edge cases in initial testing
    inp1 = torch.randint(
        low=-0x7FFF, high=0x7FFF, size=shape, dtype=dtype, device="cpu"
    ).to(flag_gems.device)
    inp2 = torch.randint(
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_gcd_with_zero(shape, dtype):
    # Test with zeros - gcd(0, x) = x, gcd(x, 0) = x, gcd(0, 0) = 0
    inp1 = torch.randint(
