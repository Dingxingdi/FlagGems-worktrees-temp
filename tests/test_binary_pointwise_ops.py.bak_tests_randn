...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_copy(shape, dtype):
    # Test copy: out-of-place copy from src to a new tensor
    # template tensor defines output shape/stride, src provides data to copy
    src = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_src = to_reference(src)

    ref_out = torch.ops.aten.copy(ref_src, ref_src)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.copy(src, src)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_copy_(shape, dtype):
    # Test copy_: in-place copy from src to dst
    dst = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    src = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_dst = to_reference(dst)
    ref_src = to_reference(src)

    ref_out = ref_dst.copy_(ref_src)
    with flag_gems.use_gems():
        res_out = dst.copy_(src)
