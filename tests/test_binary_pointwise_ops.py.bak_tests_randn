...
# Special zeta (Hurwitz zeta function) tests
# Note: This uses a series approximation which has limited accuracy
# The tolerance is set higher to account for approximation error
@pytest.mark.special_zeta
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_zeta(shape, dtype):
    # The Hurwitz zeta function ζ(x, q) converges for x > 1 and q > 0
    # Generate positive values to ensure convergence
    # Use uniform distribution to avoid edge cases
    inp1 = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 1.5  # x in [1.5, 2.5)
    inp2 = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 1.0  # q in [1.0, 2.0)

    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)

    ref_out = torch.special.zeta(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.special.zeta(inp1, inp2)

    # Use higher tolerance for this approximation
    gems_assert_close(res_out, ref_out, dtype, equal_nan=True, atol=0.5)


@pytest.mark.special_zeta
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_zeta_scalar_tensor(shape, dtype):
    # Test with scalar x and tensor q
    scalar = 2.0  # x = 2 (Riemann zeta case)
    inp2 = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 1.0  # q in [1.0, 2.0)

    ref_inp2 = to_reference(inp2, True)

    ref_out = torch.special.zeta(scalar, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.special.zeta(scalar, inp2)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_zeta_tensor_scalar(shape, dtype):
    # Test with tensor x and scalar q
    inp1 = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 1.5  # x in [1.5, 2.5)
    scalar = 1.0  # q = 1 (Riemann zeta)

    ref_inp1 = to_reference(inp1, True)

    ref_out = torch.special.zeta(ref_inp1, scalar)
    with flag_gems.use_gems():
        res_out = torch.special.zeta(inp1, scalar)
