...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_same_size(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = inp1.clone()
    ...
    ref_out = torch.is_same_size(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.is_same_size(inp1, inp2)

    gems_assert_equal(res_out, ref_out)

    # Test with different shapes (skip for scalar tensors)
    if len(shape) > 0:
        inp3 = torch.randn((shape[0] + 1, *shape[1:]), dtype=dtype, device=flag_gems.device)
        ref_inp3 = to_reference(inp3)

        ref_out = torch.is_same_size(ref_inp1, ref_inp3)
        with flag_gems.use_gems():
            res_out = torch.is_same_size(inp1, inp3)
