...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_chebyshev_polynomial_w(shape, dtype):
    # Test tensor-tensor: chebyshev_polynomial_w(x, n) where n is integer tensor
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    n = torch.randint(0, 4, shape, dtype=torch.int64, device=flag_gems.device)

    ref_x = to_reference(x)
    ref_n = to_reference(n)

    ref_out = torch.special.chebyshev_polynomial_w(ref_x, ref_n)
    with flag_gems.use_gems():
