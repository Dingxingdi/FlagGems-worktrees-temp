...
@pytest.mark.parametrize("shapes", MULTI_DOT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_multi_dot(shapes, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 linalg_multi_dot test on tsingmicro platform")

    # Create tensors for multi_dot
    tensors = []
    ref_tensors = []
    for i in range(len(shapes) - 1):
        mat = torch.randn(shapes[i], shapes[i + 1], dtype=dtype, device=flag_gems.device)
        tensors.append(mat)
        ref_tensors.append(to_reference(mat, True))

    ref_out = torch.linalg.multi_dot(ref_tensors)
    with flag_gems.use_gems():
        res_out = torch.linalg.multi_dot(tensors)

    # Use a larger reduce_dim to account for CPU vs GPU floating point differences
    # For multi_dot, we use a fixed large value for tolerance
    # For float16/bfloat16, use larger tolerance due to GPU vs CPU precision differences
