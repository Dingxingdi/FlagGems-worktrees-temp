...
def test_accuracy_addbmm_(M, N, K, scalar, dtype):
    ...
    # Test in-place version addbmm_ - use tensor method for reference
    ref_out = ref_input.addbmm_(ref_mat1, ref_mat2, alpha=alpha, beta=beta)
    with flag_gems.use_gems():
        res_out = flag_gems.addbmm_(input_tensor, mat1, mat2, alpha=alpha, beta=beta)
...
def test_accuracy_addbmm(M, N, K, scalar, dtype):
    ...
    # Test non-in-place version addbmm
    ref_out = torch.addbmm(ref_input, ref_mat1, ref_mat2, alpha=alpha, beta=beta)
    with flag_gems.use_gems():
        res_out = flag_gems.addbmm(input_tensor, mat1, mat2, alpha=alpha, beta=beta)
