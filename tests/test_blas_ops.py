...
# Shapes for triangular solve: (n, n) matrix with (n, k) right-hand side
TRIANGULAR_SHAPES = [
    (32, 16),
]
...
@pytest.mark.parametrize("n, k", TRIANGULAR_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("upper", [True, False])
@pytest.mark.parametrize("unitriangular", [True, False])
def test_accuracy_linalg_solve_triangular(n, k, dtype, upper, unitriangular):
    # Only test left=True (the default and most common case)
    # left=False has issues with the reference implementation on CPU
    left = True

    # Create a triangular matrix A
    if upper:
        # Upper triangular: zero out elements below diagonal
        A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
        A = torch.triu(A)
        if not unitriangular:
            # Make sure diagonal is non-zero
            A.diagonal().fill_(1.5)
    else:
        # Lower triangular: zero out elements above diagonal
        A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
        A = torch.tril(A)
        if not unitriangular:
            # Make sure diagonal is non-zero
    ...
    ref_out = torch.linalg.solve_triangular(ref_A, ref_B, upper=upper, left=left, unitriangular=unitriangular)

    with flag_gems.use_gems():
        res_out = torch.linalg.solve_triangular(A, B, upper=upper, left=left, unitriangular=unitriangular)
