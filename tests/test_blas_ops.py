...
@pytest.mark.parametrize("M,N,K", SPARSE_MNK_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16])
def test_accuracy_sparse_semi_structured_addmm(M, N, K, dtype):
    if not torch.cuda.is_available():
    ...
    # Reference: dense computation using torch.addmm
    # This is what the fallback kernel computes
    ref_out = torch.addmm(input_tensor, mat1, mat2, beta=1.0, alpha=1.0)

    # Test with FlagGems (uses Triton kernel as fallback)
    with flag_gems.use_gems():
        res_out = torch._sparse_semi_structured_addmm(
            input_tensor, mat1, mat1_meta, mat2
        )

    # Compare results - should match dense computation
    gems_assert_close(res_out, ref_out, dtype, reduce_dim=K)
...
@pytest.mark.parametrize("M,N,K", SPARSE_MNK_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16])
def test_accuracy_sparse_semi_structured_addmm_with_alpha_beta(M, N, K, dtype):
    if not torch.cuda.is_available():
        pytest.skip("Test requires CUDA")
    ...
    # Reference: dense computation using torch.addmm
    ref_out = torch.addmm(input_tensor, mat1, mat2, beta=beta, alpha=alpha)
