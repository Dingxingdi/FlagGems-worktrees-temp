...
# Shapes for _scaled_mm - K must be divisible by 16 for cuBLAS compatibility
_SCALED_MM_SHAPES = [
    (128, 256, 128),
    (64, 128, 256),
    (128, 128, 256),
...
@pytest.mark.parametrize("M,N,K", _SCALED_MM_SHAPES)
def test_accuracy_scaled_mm(M, N, K):
    if not torch.cuda.is_available():
        pytest.skip("_scaled_mm test requires CUDA")
    major, _ = torch.cuda.get_device_capability()
    if major < 9:
        pytest.skip("_scaled_mm test requires H100+ (SM 9.0+)")
    device = flag_gems.device

    # Create Float8 tensors with proper layout
    # self (mat1) should be row-major: (M, K) with stride (K, 1)
    # mat2 should be column-major: (K, N) with stride (1, K)
    # We create as (N, K) and transpose to get column-major layout
    a = torch.rand(M, K, device=device).to(torch.float8_e4m3fn)
    b_orig = torch.rand(N, K, device=device).to(torch.float8_e4m3fn)
    b = b_orig.t()  # Now (K, N) column-major

    scale_a = torch.rand(1, device=device)
    scale_b = torch.rand(1, device=device)

    ref_out = torch._scaled_mm(a, b, scale_a, scale_b, out_dtype=torch.float16)
    with flag_gems.use_gems():
        res_out = torch._scaled_mm(a, b, scale_a, scale_b, out_dtype=torch.float16)

    # Float8 matmul has inherent precision loss, use looser tolerance
    gems_assert_close(res_out, ref_out, torch.float16, equal_nan=True, atol=0.1)
