...
@pytest.mark.parametrize("M, N, K", MNK_SHAPES)
@pytest.mark.parametrize("dtype", ORIG_FLOAT_DTYPES)
def test_accuracy_weight_int4pack_mm_with_scales_and_zeros(M, N, K, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 test on tsingmicro platform")
    ...
    scales = torch.ones(N, dtype=dtype, device=flag_gems.device)
    zeros = torch.zeros(N, dtype=dtype, device=flag_gems.device)

    ref_input = to_reference(input_tensor, True)
    ref_weight = to_reference(weight, True)
    ref_scales = to_reference(scales, True)
    ref_zeros = to_reference(zeros, True)

    ref_out = torch.matmul(ref_input, ref_weight.T) * ref_scales + ref_zeros
    with flag_gems.use_gems():
        res_out = flag_gems._weight_int4pack_mm_with_scales_and_zeros(
            input_tensor, weight, scales, zeros
        )
