...
@pytest.mark.parametrize("scalar", SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_baddbmm_(M, N, K, scalar, dtype):
    if flag_gems.vendor_name == "mthreads" and dtype in [torch.float16, torch.bfloat16]:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    batch = 4
    mat1 = torch.randn((batch, M, K), dtype=dtype, device=flag_gems.device)
    mat2 = torch.randn((batch, K, N), dtype=dtype, device=flag_gems.device)
    bias = torch.randn((batch, M, N), dtype=dtype, device=flag_gems.device)
    ref_bias = bias.clone()

    alpha = beta = scalar

    ref_bias_np = ref_bias.to(torch.float64)
    ref_mat1_np = mat1.to(torch.float64)
    ref_mat2_np = mat2.to(torch.float64)
    expected = torch.baddbmm(ref_bias_np, ref_mat1_np, ref_mat2_np, alpha=alpha, beta=beta)
    expected = expected.to(dtype)

    with flag_gems.use_gems():
        bias.baddbmm_(mat1, mat2, alpha=alpha, beta=beta)

    gems_assert_close(bias, expected, dtype, reduce_dim=K)
