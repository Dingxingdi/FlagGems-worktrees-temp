...
def test_accuracy_linalg_inv_ex(N, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skiping fp32 linalg_inv_ex test on tsingmicro platform")

    # Create a random invertible matrix
    # Add a large constant to ensure the matrix is well-conditioned
    A = torch.randn(N, N, dtype=dtype, device=flag_gems.device) * 0.1 + torch.eye(N, dtype=dtype, device=flag_gems.device) * 2.0
    ref_A = to_reference(A)

    ref_inv, ref_info = torch.linalg.inv_ex(ref_A)
    with flag_gems.use_gems():
        res_inv, res_info = torch.linalg.inv_ex(A)

    # Check that the inverse is correct: A @ A^-1 should be close to identity
    identity = torch.eye(N, dtype=dtype, device=flag_gems.device)
    product = A @ res_inv
    gems_assert_close(product, identity, dtype, reduce_dim=N)

    # Also check that info is zero (success)
    assert torch.all(res_info == 0)
