...
# Define shapes for scaled_grouped_mm tests
if QUICK_MODE:
    GROUPED_MNK_SHAPES = [
        (2, 16, 32),
    ]
...
@pytest.mark.parametrize("batch, M, N, K", [(b, m, n, k) for b, m, k in [(2, 16, 32), (4, 64, 128)] for n in [32, 64]])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_scaled_grouped_mm(batch, M, N, K, dtype):
    """Test for scaled_grouped_mm with batched inputs."""
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 scaled_grouped_mm test on tsingmicro platform")
    ...
    mat1 = torch.randn(batch, M, K, dtype=dtype, device=flag_gems.device)
    mat2 = torch.randn(batch, K, N, dtype=dtype, device=flag_gems.device)
    scale_a = torch.randn(batch, dtype=torch.float32, device=flag_gems.device).abs() + 0.1
    scale_b = torch.randn(batch, dtype=torch.float32, device=flag_gems.device).abs() + 0.1

    # Reference implementation - convert to reference and compute with higher precision
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)
    ref_scale_a = to_reference(scale_a, True)
    ref_scale_b = to_reference(scale_b, True)

    ref_out = torch.bmm(ref_mat1, ref_mat2)
    ref_out = ref_out * ref_scale_a.view(-1, 1, 1) * ref_scale_b.view(-1, 1, 1)

    # FlagGems implementation
    with flag_gems.use_gems():
        from flag_gems.ops._scaled_grouped_mm import scaled_grouped_mm
        res_out = scaled_grouped_mm(mat1, mat2, scale_a, scale_b)

    # Use much larger reduce_dim for float32 to account for accumulation errors
    reduce_dim = K * 50 if dtype == torch.float32 else K
    gems_assert_close(res_out, ref_out, dtype, equal_nan=True, reduce_dim=reduce_dim)


@pytest.mark.scaled_grouped_mm
@pytest.mark.parametrize("M, N, K", [(16, 32, 64), (64, 128, 256)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_scaled_grouped_mm_2d(M, N, K, dtype):
    """Test for scaled_grouped_mm with 2D (non-batched) inputs."""
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 scaled_grouped_mm test on tsingmicro platform")
    ...
    # Create non-batched inputs (treated as single group)
    mat1 = torch.randn(M, K, dtype=dtype, device=flag_gems.device)
    mat2 = torch.randn(K, N, dtype=dtype, device=flag_gems.device)
    scale_a = torch.tensor(1.5, dtype=torch.float32, device=flag_gems.device)
    scale_b = torch.tensor(2.0, dtype=torch.float32, device=flag_gems.device)
    ...
    ref_out = torch.mm(ref_mat1, ref_mat2) * ref_scale_a * ref_scale_b
