...
# Shapes for linalg_cross (cross product dimension must be 3)
# Using custom shapes since standard shapes don't always have dimension 3
CROSS_SHAPES = [
    (3,),  # Single 3D vector
    (4, 3),  # 4 3D vectors
    (16, 3),  # 16 3D vectors
    (32, 3),  # 32 3D vectors
    (1024, 3),  # Large batch
    (2, 4, 3),  # 2D batch of 3D vectors
    (4, 16, 3),  # Larger 2D batch
]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_cross(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)

    ref_out = torch.linalg.cross(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.linalg.cross(inp1, inp2)
