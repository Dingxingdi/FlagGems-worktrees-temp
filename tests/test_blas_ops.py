...
# SVD test shapes - use smaller shapes for testing
SVD_SHAPES = [
    (32, 32),
    (64, 64),
    (128, 64),
...
def test_accuracy_linalg_svd(M, N, dtype, full_matrices):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 svd test on tsingmicro platform")
    ...
    # Compute SVD using reference - convert to float32 for stability
    # then convert back to original dtype
    ref_A = A.float().cpu()
    ref_U, ref_S, ref_Vh = torch.linalg.svd(ref_A, full_matrices=full_matrices)
    ref_U = ref_U.to(A.device).to(dtype)
    ref_S = ref_S.to(A.device).to(dtype)
    ref_Vh = ref_Vh.to(A.device).to(dtype)

    # Compute SVD using FlagGems
    with flag_gems.use_gems():
        res_U, res_S, res_Vh = torch._linalg_svd(A, full_matrices=full_matrices)

    # Check singular values (most important)
    gems_assert_close(res_S, ref_S, dtype)

    # Check U and Vh if computed
    gems_assert_close(res_U, ref_U, dtype)
    gems_assert_close(res_Vh, ref_Vh, dtype)
...
@pytest.mark.parametrize("M, N", SVD_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_svd_compute_uv_false(M, N, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 svd test on tsingmicro platform")
    ...
    # Compute only singular values using reference
    ref_A = A.float().cpu()
    ref_S = torch.linalg.svdvals(ref_A)
    ref_S = ref_S.to(A.device).to(dtype)

    # Compute only singular values using FlagGems
    with flag_gems.use_gems():
        res = torch._linalg_svd(A, compute_uv=False)
        # Handle both tuple and tensor return
        res_S = res.S if hasattr(res, 'S') else res[1] if isinstance(res, tuple) else res
