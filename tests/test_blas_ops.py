...
def test_accuracy_addmm_activation(M, N, K, scalar, dtype, use_gelu):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
    ...
    ref_out1 = torch._addmm_activation(
        ref_bias1, ref_mat1, ref_mat2, alpha=alpha, beta=beta, use_gelu=use_gelu
    )
    with flag_gems.use_gems():
        res_out1 = torch._addmm_activation(
            bias1, mat1, mat2, alpha=alpha, beta=beta, use_gelu=use_gelu
        )
    ...
    ref_out2 = torch._addmm_activation(
        ref_bias2, ref_mat1, ref_mat2, alpha=alpha, beta=beta, use_gelu=use_gelu
    )
    with flag_gems.use_gems():
        res_out2 = torch._addmm_activation(
            bias2, mat1, mat2, alpha=alpha, beta=beta, use_gelu=use_gelu
        )
