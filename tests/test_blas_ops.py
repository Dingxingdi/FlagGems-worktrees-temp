...
# Shapes for linalg_matrix_power - square matrices
MATRIX_POWER_SHAPES = [
    (2, 2),
    (3, 3),
    (4, 4),
...
# Limited n values for testing to avoid extreme cases
MATRIX_POWER_N_VALUES = [-2, -1, 0, 1, 2, 3, 4]


@pytest.mark.linalg_matrix_power
@pytest.mark.parametrize("M, N", MATRIX_POWER_SHAPES)
@pytest.mark.parametrize("n", MATRIX_POWER_N_VALUES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_power(M, N, n, dtype):
    # Skip float16 with large matrices and high powers due to precision issues
    if dtype == torch.float16 and M >= 64 and n >= 3:
        pytest.skip("Float16 with large matrices and high powers has precision issues")

    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skiping fp32 linalg_matrix_power test on tsingmicro platform")
    ...
    # For negative n, need invertible matrix, so use a special construction
    if n < 0:
        # Create an invertible matrix (add identity to make it diagonally dominant)
        A = torch.randn(M, M, dtype=dtype, device=flag_gems.device)
        A = A + torch.eye(M, M, dtype=dtype, device=flag_gems.device) * M
    else:
        A = torch.randn(M, M, dtype=dtype, device=flag_gems.device)
    ...
    with flag_gems.use_gems():
        res_out = torch.linalg.matrix_power(A, n)
...
@pytest.mark.parametrize("M, N", MATRIX_POWER_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_power_batch(M, N, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skiping fp32 linalg_matrix_power test on tsingmicro platform")
    ...
    batch_size = 4
    # Test batched matrix power
    A = torch.randn(batch_size, M, M, dtype=dtype, device=flag_gems.device)
    n = 3  # Use a positive power for batched test
