...
# Shapes for linalg_vecdot: batch_size, vector_dim
VECDOT_SHAPES = [
    (3, 4),        # Simple batch
    (10, 16),      # Larger batch
    (2, 8, 16),    # 3D tensor
...
@pytest.mark.parametrize("shape", VECDOT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_vecdot(shape, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.linalg.vecdot(ref_inp1, ref_inp2, dim=-1)
    with flag_gems.use_gems():
        res_out = torch.linalg.vecdot(inp1, inp2, dim=-1)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.linalg_vecdot
@pytest.mark.parametrize("shape", [(3, 4), (2, 8, 16)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_vecdot_dim0(shape, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.linalg.vecdot(ref_inp1, ref_inp2, dim=0)
    with flag_gems.use_gems():
        res_out = torch.linalg.vecdot(inp1, inp2, dim=0)
