...
# Shapes for lu_solve: (n, n) matrices with (n, k) right-hand sides
LU_SOLVE_SHAPES = [
    (4, 2),   # 4x4 matrix, 2 RHS
    (8, 4),   # 8x8 matrix, 4 RHS
    (16, 8),  # 16x16 matrix, 8 RHS
    (32, 16), # 32x32 matrix, 16 RHS
    (64, 32), # 64x64 matrix, 32 RHS
] if not QUICK_MODE else [(8, 4)]

# Only float32 is supported since lu_factor doesn't support float16/bfloat16 on this system
LU_SOLVE_DTYPES = [torch.float32]


@pytest.mark.linalg_lu_solve
@pytest.mark.parametrize("n, k", LU_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", LU_SOLVE_DTYPES)
def test_accuracy_linalg_lu_solve(n, k, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        pytest.skip("Skip on kunlunxin")

    # Generate random invertible matrix A
    A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
    # Make it more diagonally dominant to ensure invertibility
    A = A + torch.eye(n, dtype=dtype, device=flag_gems.device) * n

    # Compute LU factorization (used by both reference and GEMS)
    LU, pivots = torch.linalg.lu_factor(A)

    # Generate random right-hand side B
    B = torch.randn(n, k, dtype=dtype, device=flag_gems.device)

    # Reference: use redispatch to bypass GEMS and compute on same device (CUDA)
    _FALLBACK_KEYSET = torch._C.DispatchKeySet(
        torch._C.DispatchKey.CompositeExplicitAutograd
    )
    ref_out = torch.ops.aten.linalg_lu_solve.default.redispatch(
        _FALLBACK_KEYSET, LU, pivots, B, left=True, adjoint=False
    )
...
@pytest.mark.parametrize("n, k", LU_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", LU_SOLVE_DTYPES)
def test_accuracy_linalg_lu_solve_batched(n, k, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        pytest.skip("Skip on kunlunxin")

    batch_size = 3

    # Generate batch of random invertible matrices
    ...
    # Generate batch of right-hand sides
