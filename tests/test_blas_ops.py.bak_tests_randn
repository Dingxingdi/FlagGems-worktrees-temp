...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linear_backward(batch, in_features, out_features, dtype):
    # Create inputs
    input_tensor = torch.randn((batch, in_features), dtype=dtype, device=flag_gems.device)
    weight = torch.randn((out_features, in_features), dtype=dtype, device=flag_gems.device)
    grad_output = torch.randn((batch, out_features), dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("out_features", LINEAR_OUT_FEATURES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linear_backward_grad_input_only(batch, in_features, out_features, dtype):
...
@pytest.mark.parametrize("out_features", LINEAR_OUT_FEATURES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linear_backward_grad_weight_only(batch, in_features, out_features, dtype):
...
@pytest.mark.parametrize("out_features", LINEAR_OUT_FEATURES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linear_backward_grad_bias_only(batch, in_features, out_features, dtype):
