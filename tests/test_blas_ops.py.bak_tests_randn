...
@pytest.mark.parametrize("M, N, K", MNK_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_matmuladd_out(M, N, K, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 matmuladd_out test on tsingmicro platform")
    ...
    ref_mat2 = to_reference(mat2, True)
    ref_bias = to_reference(bias, True)
    ref_out = torch.empty((M, N), dtype=dtype, device="cpu")

    # Reference: matmul + bias
    torch.matmul(ref_mat1, ref_mat2, out=ref_out)
    ref_out.add_(ref_bias)
    with flag_gems.use_gems():
        flag_gems.matmuladd(mat1, mat2, bias, out=out)
