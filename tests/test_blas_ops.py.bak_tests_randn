...
# Shapes for cholesky_inverse tests (square matrices)
if QUICK_MODE:
    N_SHAPES = [
        (4, 4),
        (16, 16),
...
@pytest.mark.parametrize("N", N_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("upper", [False, True])
def test_accuracy_cholesky_inverse(N, dtype, upper):
    """Test cholesky_inverse by computing inverse from Cholesky decomposition."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    # A = A @ A.T + eye * small_value to ensure positive definite
    A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
    A = A @ A.T + torch.eye(n, dtype=dtype, device=flag_gems.device) * 1e-3
    ...
    if upper:
        L_input = L.transpose(-2, -1)
    else:
        L_input = L

    ref_L = to_reference(L_input, True)

    # Compute inverse using cholesky_inverse
    ref_out = torch.cholesky_inverse(ref_L, upper=upper)
    with flag_gems.use_gems():
        res_out = torch.cholesky_inverse(L_input, upper=upper)
...
@pytest.mark.parametrize("N", N_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cholesky_inverse_batch(N, dtype):
    """Test cholesky_inverse with batch dimensions."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    A = A @ A.transpose(-2, -1) + torch.eye(n, dtype=dtype, device=flag_gems.device) * 1e-3
