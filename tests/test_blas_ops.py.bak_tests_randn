...
def test_accuracy_matmul(M, N, K, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skiping fp32 matmul test on tsingmicro platform")
    ...
    ref_out = torch.matmul(ref_mat1, ref_mat2)
    with flag_gems.use_gems():
        res_out = torch.matmul(mat1, mat2)
...
@pytest.mark.parametrize("shape", UT_SHAPES_1D)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_matmul_1d_1d(shape, dtype):
    # 1D x 1D: dot product
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1, True)
    ...
    ref_out = torch.matmul(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
        res_out = torch.matmul(inp1, inp2)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.matmul
@pytest.mark.parametrize("M, K, N", [(4, 8, 16), (16, 32, 8), (8, 16, 4)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_matmul_batched(M, K, N, dtype):
