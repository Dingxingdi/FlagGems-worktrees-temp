...
# Shapes for linalg_solve_ex testing
if QUICK_MODE:
    SOLVE_SHAPES = [
        (32, 4),  # (N, K) where N=matrix size, K=number of RHS
    ]
else:
    SOLVE_SHAPES = [
        (32, 1),
...
@pytest.mark.parametrize("N, K", SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_solve_ex(N, K, dtype):
    if flag_gems.vendor_name == "mthreads":
        pytest.skip("Skipping linalg_solve_ex test on mthreads platform")

    # Create a random positive definite matrix for stability
    # A = L @ L.T + I ensures positive definiteness
    L = torch.randn((N, N), dtype=dtype, device=flag_gems.device)
    A = L @ L.transpose(-2, -1) + torch.eye(N, dtype=dtype, device=flag_gems.device)
    B = torch.randn((N, K), dtype=dtype, device=flag_gems.device)

    ref_A = to_reference(A, True)
    ref_B = to_reference(B, True)

    ref_out = torch._linalg_solve_ex(ref_A, ref_B)
    with flag_gems.use_gems():
        res_out = torch._linalg_solve_ex(A, B)

    # Check result
    gems_assert_close(res_out.result, ref_out.result, dtype)
    # LU format may differ, skip LU comparison
    # pivots format may differ, skip pivots comparison
    # info should be zero (success)
    assert torch.all(res_out.info == 0) or torch.all(ref_out.info == 0)
