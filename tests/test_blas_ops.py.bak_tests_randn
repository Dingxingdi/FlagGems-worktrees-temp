...
from .accuracy_utils import SCALARS, UT_SHAPES_1D, gems_assert_close, gems_assert_equal, to_reference
...
# Shapes specifically for int8 matrix multiplication
INT8_MNK_SHAPES = [
    (16, 32, 64),
    (32, 64, 128),
    (64, 128, 256),
...
@pytest.mark.int_mm
@pytest.mark.parametrize("M, N, K", INT8_MNK_SHAPES)
def test_accuracy_int_mm(M, N, K):
    if not torch.cuda.is_available():
        pytest.skip("int8 matmul test requires CUDA")
    ...
    ref_out = torch._int_mm(mat1.cpu(), mat2.cpu()).to(flag_gems.device)
    with flag_gems.use_gems():
        res_out = torch._int_mm(mat1, mat2)
