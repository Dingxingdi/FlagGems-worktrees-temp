...
# cholesky_solve shapes: (n, k) for B, (n, n) for L
CHOLESKY_SOLVE_SHAPES = [(4, 2), (8, 4), (16, 8), (32, 16)]
...
def test_accuracy_cholesky_solve(n, k, dtype, upper):
    # Create a symmetric positive-definite matrix A
    # Use float32 for Cholesky decomposition since float16 is not supported
    A_float = torch.randn(n, n, dtype=torch.float32, device=flag_gems.device)
    A_float = A_float @ A_float.T + torch.eye(n, dtype=torch.float32, device=flag_gems.device) * 1e-3
    A = A_float.to(dtype)

    # Get Cholesky decomposition using float32, then convert
    L_float = torch.linalg.cholesky(A_float)
    L = L_float.to(dtype)

    # If upper=True, we need to use U = L^T
    if upper:
        L = L.T
    ...
    # Use larger tolerance for float16 and bfloat16 due to lower precision
    # Also scale by reduce_dim (n * k) since this is a matrix operation
    if dtype == torch.float16:
        ...
        atol = 1e-4
    gems_assert_close(res_out, ref_out, dtype, equal_nan=True, atol=atol)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("upper", [False, True])
def test_accuracy_cholesky_solve_batch(n, k, dtype, upper):
    # Test with batch dimensions: (batch, n, n) and (batch, n, k)
    batch = 4

    # Create batch of symmetric positive-definite matrices
    # Use float32 for Cholesky decomposition since float16 is not supported
    A_float = torch.randn(batch, n, n, dtype=torch.float32, device=flag_gems.device)
    A_float = A_float @ A_float.transpose(-2, -1) + torch.eye(n, dtype=torch.float32, device=flag_gems.device) * 1e-3
    A = A_float.to(dtype)
    ...
    # Create batch of right-hand side B
