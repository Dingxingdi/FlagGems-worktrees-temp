...
def test_accuracy_addbmm(M, N, K, scalar, dtype):
    ...
    res_out = flag_gems.addbmm(bias, mat1, mat2, beta=beta, alpha=alpha)
...
def test_accuracy_addbmm_(M, N, K, scalar, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skiping fp32 addbmm_ test on tsingmicro platform")
    ...
    mat2 = torch.randn((batch, K, N), dtype=dtype, device=flag_gems.device)
    bias = torch.randn((M, N), dtype=dtype, device=flag_gems.device)
    ref_bias = to_reference(bias, True).clone()
    ...
    ref_out = ref_bias.addbmm_(ref_mat1, ref_mat2, alpha=alpha, beta=beta)
    res_out = flag_gems.addbmm_(bias, mat1, mat2, beta=beta, alpha=alpha)
