...
@pytest.mark.parametrize("scalar", SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_addmm_(M, N, K, scalar, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
    ...
    mat1 = torch.randn((M, K), dtype=dtype, device=flag_gems.device)
    mat2 = torch.randn((K, N), dtype=dtype, device=flag_gems.device)
    # self (bias) shape should be broadcastable to (M, N)
    self = torch.randn((M, N), dtype=dtype, device=flag_gems.device)
    ref_self = to_reference(self, True)
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)

    alpha = beta = scalar

    ref_out = ref_self.addmm_(ref_mat1, ref_mat2, alpha=alpha, beta=beta)
    with flag_gems.use_gems():
        res_out = self.addmm_(mat1, mat2, alpha=alpha, beta=beta)
