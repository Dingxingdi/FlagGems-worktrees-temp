...
# triangular_solve shapes: (n, n) for A, (n, k) for b
if QUICK_MODE:
    TRIANGULAR_SOLVE_SHAPES = [
        (32, 16),  # (n, k)
    ]
else:
    # Use smaller shapes to avoid numerical precision issues with sequential algorithm
    TRIANGULAR_SOLVE_SHAPES = [
        (32, 16),
    ]
...
@pytest.mark.parametrize("n, k", TRIANGULAR_SOLVE_SHAPES)
@pytest.mark.parametrize("upper", [True])
@pytest.mark.parametrize("unitriangular", [True, False])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_triangular_solve(n, k, upper, unitriangular, dtype):
    if flag_gems.vendor_name == "tsingmicro" and dtype == torch.float32:
        pytest.skip("Skipping fp32 triangular_solve test on tsingmicro platform")

    # Create upper or lower triangular matrix A
    if upper:
        A = torch.randn(n, n, dtype=dtype, device=flag_gems.device).triu()
    else:
        A = torch.randn(n, n, dtype=dtype, device=flag_gems.device).tril()
    # Ensure diagonal is non-zero
    A.diagonal().fill_(float(torch.randn(1, dtype=dtype).abs()) + 0.1)
    ...
    ref_out = torch.triangular_solve(ref_b, ref_A, upper=upper, unitriangular=unitriangular)
    with flag_gems.use_gems():
        res_out = torch.triangular_solve(b, A, upper=upper, unitriangular=unitriangular)

    # Compare solution
    gems_assert_close(res_out.solution, ref_out.solution, dtype, reduce_dim=n)
    # Compare cloned coefficient
    gems_assert_close(res_out.cloned_coefficient, ref_out.cloned_coefficient, dtype)


# Skip batch test for now - batch dimension has issues
# @pytest.mark.triangular_solve
# @pytest.mark.parametrize("n, k", [(32, 16)])
# @pytest.mark.parametrize("dtype", [torch.float32])
# def test_accuracy_triangular_solve_batch(n, k, dtype):
#     ...
