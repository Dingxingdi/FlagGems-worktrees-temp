...
# Test shapes for _weight_int4pack_mm_for_cpu
# N must be a multiple of 32 for the reference implementation
# qGroupSize should be >= K for simplicity in testing
INT4PACK_MNK_SHAPES = [
    (1, 32, 64),
    (2, 32, 64),
    (4, 32, 64),  # Use K=64 instead of 128 for simpler testing
]
QGROUP_SIZES = [64]


@pytest.mark.weight_int4pack_mm
@pytest.mark.parametrize("M, N, K", INT4PACK_MNK_SHAPES)
@pytest.mark.parametrize("qGroupSize", QGROUP_SIZES)
def test_accuracy_weight_int4pack_mm_for_cpu(M, N, K, qGroupSize):
    """Test weight-only int4 quantization matrix multiplication."""
    # Create weight data in int4 format (stored as float16, values 0-15)
    weight = torch.randint(0, 15, (M, K), dtype=torch.float16, device=flag_gems.device)

    # Create activation in uint8 packed format (int4 packed, 2 per byte)
    act = torch.randint(0, 15, (N, K // 2), dtype=torch.uint8, device=flag_gems.device)

    # Create scale and zeros (shape: (1, N, 2))
    # We use scale=1 and zero=0 for simplicity in testing
    qscale = torch.ones((1, N, 2), dtype=torch.float16, device=flag_gems.device)
    qscale[:, :, 1] = 0  # zero = 0

    # Reference implementation
    ref_weight = weight.cpu()
    ref_act = act.cpu()
    ref_qscale = qscale.cpu()

    ref_out = torch._weight_int4pack_mm_for_cpu(
        ref_weight, ref_act, qGroupSize, ref_qscale
    )

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = torch._weight_int4pack_mm_for_cpu(
            weight, act, qGroupSize, qscale
        )

    # Compare with tolerance for float16 - use larger tolerance since quantization is involved
    # Using reduce_dim=K to handle the matrix multiplication reduction
    gems_assert_close(res_out.cpu(), ref_out, torch.float16, reduce_dim=K, atol=10.0)
