...
# Test shapes for Depthwise Separable Convolution
SHAPE_DEPTHWISE_POINTWISE = [
    # (input_shape, depthwise_kernel, pointwise_out_channels, stride, padding)
    ((1, 4, 8, 8), (4, 1, 3, 3), 8, (1, 1), (1, 1)),
    ((2, 8, 16, 16), (8, 1, 3, 3), 16, (1, 1), (1, 1)),
    ((4, 16, 8, 8), (16, 1, 3, 3), 32, (2, 2), (1, 1)),
]
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_depthwise_pointwise_conv2d(
    input_shape, depthwise_kernel, pointwise_out_channels, stride, padding, dtype
):
    """Test depthwise separable convolution (depthwise + pointwise)."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    # Create depthwise weight: (C_in, 1, KH, KW)
    depthwise_weight = torch.randn(
        depthwise_kernel, dtype=dtype, device=flag_gems.device
    )
    ref_depthwise_weight = to_reference(depthwise_weight, True)

    # Create pointwise weight: (C_out, C_in, 1, 1)
    pointwise_weight = torch.randn(
        (pointwise_out_channels, depthwise_kernel[0], 1, 1),
        dtype=dtype,
        device=flag_gems.device,
    )
    ref_pointwise_weight = to_reference(pointwise_weight, True)

    # Reference: manually compose depthwise then pointwise conv
    # Depthwise: groups = input channels
    depthwise_out = torch.nn.functional.conv2d(
        ref_inp,
        ref_depthwise_weight,
        bias=None,
        stride=stride,
        padding=padding,
        dilation=1,
        groups=input_shape[1],  # depthwise
    )

    # Pointwise: 1x1 conv
    ref_out = torch.nn.functional.conv2d(
        depthwise_out,
        ref_pointwise_weight,
        bias=None,
        stride=1,
        padding=0,
        dilation=1,
    ...
    # GEMS implementation
    res_out = flag_gems.depthwise_pointwise_conv2d(
        inp,
        depthwise_weight,
        pointwise_weight,
        depthwise_bias=None,
