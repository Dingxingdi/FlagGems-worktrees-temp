...
# Conv+GELU tests
SHAPE_CONV_GELU = [
    ((4, 16, 32, 32), (32, 16, 3, 3)),  # Basic case
    ((2, 8, 16, 16), (16, 8, 3, 3)),    # Smaller case
    ((1, 3, 8, 8), (8, 3, 3, 3)),       # Tiny case
]


@pytest.mark.conv_gelu
@pytest.mark.parametrize("shape, kernel", SHAPE_CONV_GELU)
@pytest.mark.parametrize("stride", [1, 2])
@pytest.mark.parametrize("padding", [0, 1])
@pytest.mark.parametrize("groups", [1])
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("bias", [True, False])
def test_accuracy_conv_gelu(shape, kernel, stride, padding, groups, dtype, bias):
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"

    # Adjust kernel channels based on groups
    in_channels = shape[1]
    ...
    if bias:
        bias_tensor = torch.randn([kernel[0]], dtype=dtype, device=flag_gems.device, requires_grad=True)
        bias_ref = to_reference(bias_tensor, True)
    else:
        bias_tensor = None
        bias_ref = None

    # Reference: torch.nn.functional.conv2d followed by torch.nn.functional.gelu
...
# Additional float16 tests with relaxed tolerance
@pytest.mark.conv_gelu
@pytest.mark.parametrize("shape, kernel", [SHAPE_CONV_GELU[0]])
@pytest.mark.parametrize("stride", [1])
@pytest.mark.parametrize("padding", [0])
@pytest.mark.parametrize("groups", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
@pytest.mark.parametrize("bias", [True, False])
def test_accuracy_conv_gelu_fp16(shape, kernel, stride, padding, groups, dtype, bias):
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"
    ...
    ref_out = torch.nn.functional.gelu(ref_conv).to(dtype)
    ...
    atol = 0.001
    rtol = 0.001
    torch.testing.assert_close(res_out, ref_out, atol=atol, rtol=rtol)
