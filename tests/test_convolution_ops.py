...
@pytest.mark.parametrize("padding", [0, 1])
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_conv_transpose2d(shape, kernel, stride, padding, dtype):
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    ref_inp = to_reference(inp, False)
    torch.backends.cudnn.allow_tf32 = False
    weight = torch.randn(kernel, dtype=dtype, device=flag_gems.device, requires_grad=False)

    ref_out = torch.nn.functional.conv_transpose2d(
        ref_inp, weight, bias=None, stride=stride, padding=padding
    ).to(dtype)

    res_out = flag_gems.conv_transpose2d(
        inp, weight, bias=None, stride=stride, padding=padding
    )
