...
# Conv_GELU tests - fused Conv2d + GELU activation
# Note: Some shapes may have slightly larger numerical differences due to
# the underlying conv2d implementation differences from PyTorch
SHAPE_CONV_GELU = [
    # (input_shape, weight_shape) - using shapes that work well with FlagGems conv2d
    ((4, 16, 32, 32), (32, 16, 3, 3)),
    ((2, 8, 16, 16), (16, 8, 3, 3)),
]


@pytest.mark.Conv_GELU
@pytest.mark.parametrize("input_shape, weight_shape", SHAPE_CONV_GELU)
@pytest.mark.parametrize("stride", [1, 2])
@pytest.mark.parametrize("padding", [0, 1, 2])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_Conv_GELU(input_shape, weight_shape, stride, padding, dtype):
    """Test fused Conv2d + GELU activation."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    # Reference: conv2d followed by gelu
    ref_conv = torch.nn.functional.conv2d(
        ref_inp, ref_weight, bias=None, stride=stride, padding=padding, dilation=1
    )
    ref_out = torch.nn.functional.gelu(ref_conv)

    # FlagGems implementation
    res_out = flag_gems.Conv_GELU(
        inp, weight, bias=None, stride=stride, padding=padding, dilation=1
    )
...
@pytest.mark.parametrize("padding", [0, 1])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_Conv_GELU_with_bias(input_shape, weight_shape, stride, padding, dtype):
    """Test fused Conv2d + GELU activation with bias (float32 only for accuracy)."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
...
@pytest.mark.Conv_GELU
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_Conv_GELU_groups(dtype):
    """Test fused Conv2d + GELU with groups."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"

    # Test with groups=2
    batch, in_c, out_c, h, w = 2, 16, 32, 16, 16
    groups = 2
    kernel = 3

    inp = torch.randn(batch, in_c, h, w, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    weight = torch.randn(out_c, in_c // groups, kernel, kernel, dtype=dtype, device=flag_gems.device)
    ref_weight = to_reference(weight)

    # Reference: conv2d followed by gelu
    ref_conv = torch.nn.functional.conv2d(
        ref_inp, ref_weight, bias=None, stride=1, padding=0, dilation=1, groups=groups
    )
    ref_out = torch.nn.functional.gelu(ref_conv)

    # FlagGems implementation
    res_out = flag_gems.Conv_GELU(
        inp, weight, bias=None, stride=1, padding=0, dilation=1, groups=groups
    )
