...
# Test for cudnn_convolution
SHAPE_CUDNN_CONV = [
    # shape (N, C, H, W), weight (out_c, in_c, kH, kW)
    ((1, 3, 8, 8), (64, 3, 3, 3)),  # in_c=3, groups must be 1
    ((2, 4, 16, 16), (64, 4, 3, 3)),  # in_c=4, groups can be 1,2,4
    ((4, 8, 16, 16), (16, 8, 3, 3)),  # in_c=8, groups can be 1,2,4,8
]


@pytest.mark.cudnn_convolution
@pytest.mark.parametrize("shape, weight_shape", SHAPE_CUDNN_CONV)
@pytest.mark.parametrize("stride", [(1, 1), (2, 2)])
@pytest.mark.parametrize("padding", [(0, 0), (1, 1)])
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
@pytest.mark.parametrize("groups", [1, 2])
def test_accuracy_cudnn_convolution(shape, weight_shape, stride, padding, dtype, groups):
    """Test cudnn_convolution which wraps conv2d with cuDNN parameters."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"

    # Check if groups is valid for this shape
    in_c = shape[1]
    out_c = weight_shape[0]
    if in_c % groups != 0 or out_c % groups != 0:
        pytest.skip(f"groups={groups} not valid for in_c={in_c}, out_c={out_c}")

    # Adjust weight shape for groups
    out_c, in_c, kH, kW = weight_shape
    if groups > 1:
        weight_shape = (out_c, in_c // groups, kH, kW)
    ...
    # Reference: torch.cudnn_convolution
