...
# Tests for cudnn_convolution_transpose
...
def test_accuracy_cudnn_convolution_transpose(
    ...
    output_padding_h = (stride - 1) * shape[2] - 2 * padding + dilation * (kernel[2] - 1)
    output_padding_w = (stride - 1) * shape[3] - 2 * padding + dilation * (kernel[3] - 1)
