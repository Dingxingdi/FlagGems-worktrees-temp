...
# Tests for the general convolution operator (aten::convolution)
...
@pytest.mark.parametrize("padding", [0, 1])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_convolution_2d(shape, kernel, stride, padding, dtype):
...
# Test with groups > 1
SHAPE_CONV_GENERIC_GROUPS = [
    ((2, 4, 8, 8), (4, 2, 3, 3)),
]


@pytest.mark.convolution
@pytest.mark.parametrize("shape, kernel", SHAPE_CONV_GENERIC_GROUPS)
@pytest.mark.parametrize("groups", [2, 4])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_convolution_groups(shape, kernel, groups, dtype):
    """Test grouped convolution using the general convolution interface."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)
    # For grouped conv, weight shape is (C_out, C_in/groups, kH, kW)
    weight = torch.randn(kernel[0], kernel[1] // groups, kernel[2], kernel[3], dtype=dtype, device=flag_gems.device)
    ref_weight = to_reference(weight, True)
...
def test_accuracy_convolution_groups(shape, kernel, groups, dtype):
    """Test grouped convolution using the general convolution interface."""
    # For grouped conv: input channels must be divisible by groups
    # input shape: (N, C_in, H, W), weight shape: (C_out, C_in/groups, kH, kW)
    # With groups=2, C_in=8, kernel[1]=4: C_in/groups = 4
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)
    # For grouped conv, weight shape is (C_out, C_in/groups, kH, kW)
    weight = torch.randn(kernel[0], kernel[1], kernel[2], kernel[3], dtype=dtype, device=flag_gems.device)
    ref_weight = to_reference(weight, True)
