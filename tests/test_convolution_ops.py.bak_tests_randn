...
# Tests for _convolution (low-level convolution primitive)
# Keep test shapes simpler to avoid precision issues with float16
SHAPE_CONVOLUTION = [
    ((1, 2, 5, 5), (1, 2, 3, 3)),
    ((2, 3, 9, 9), (1, 3, 3, 3)),
]
...
def test_accuracy__convolution(
    shape, kernel, stride, padding, dtype, dilation, bias, groups
):
    """Test _convolution accuracy against PyTorch reference implementation."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"

    # Check channel compatibility
    if shape[1] != groups * kernel[1]:
        pytest.skip("Incompatible input and weight channels for groups")
    ...
    if bias is True:
        bias_tensor = torch.randn([weight.shape[0]], dtype=dtype, device=flag_gems.device)
    ...
    # Reference: torch._convolution(input, weight, bias, stride, padding, dilation, transposed, output_padding, groups, benchmark, deterministic, cudnn_enabled)
    ...
    # Our implementation
    res_out = flag_gems._convolution(
        inp,
        weight,
        bias_tensor,
        [stride, stride],
...
# Test 1D convolution via _convolution
...
def test_accuracy__convolution_1d(shape, kernel, stride, padding, dtype):
    """Test _convolution for 1D convolutions."""
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    # Reference: torch._convolution for 1D
    ref_out = torch._convolution(
        ref_inp,
        ref_weight,
        None,  # bias
