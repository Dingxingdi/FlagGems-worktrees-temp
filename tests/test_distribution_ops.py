...
@pytest.mark.parametrize("shape", DISTRIBUTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_exponential(shape, dtype):
    x = torch.empty(size=shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res = torch.ops.aten.exponential(x)
    assert res.min() > 0
...
@pytest.mark.parametrize("shape", DISTRIBUTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_exponential_with_lambd(shape, dtype):
    x = torch.empty(size=shape, dtype=dtype, device=flag_gems.device)
    lambd = 2.5
    mean_tol = 0.05
    var_tol = 0.05
    with flag_gems.use_gems():
        res = torch.ops.aten.exponential(x, lambd)
    x_res = to_reference(res)
