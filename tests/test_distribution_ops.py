...
def test_accuracy_cauchy_(shape, dtype):
    ...
    # Cauchy distribution has no finite moments, so we just check that
    # the tensor is filled with values and no errors occur
...
@pytest.mark.parametrize("shape", DISTRIBUTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cauchy_with_params(shape, dtype):
    median = 5.0
    sigma = 2.0
    x = torch.empty(size=shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        x.cauchy_(median=median, sigma=sigma)
    # The median of a Cauchy distribution is the location parameter
    # We check that the distribution is roughly centered around median
    # Use quantiles as Cauchy has no mean
    x_sorted, _ = torch.sort(x.flatten())
    n = x_sorted.numel()
    # Check that median is roughly in the middle
    q25 = x_sorted[int(n * 0.25)]
    q75 = x_sorted[int(n * 0.75)]
    # For standard Cauchy, quantiles should be symmetric around median
