...
@pytest.mark.parametrize("shape", DISTRIBUTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_log_normal(shape, dtype):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(42)
        torch.mlu.manual_seed_all(42)
    if flag_gems.vendor_name in ["metax", "iluvatar", "kunlunxin"]:
    ...
    std = 2.0
    with flag_gems.use_gems():
        res_out = torch.ops.aten.log_normal(inp, mean, std)
    # Log-normal distribution always produces positive values
...
def test_accuracy_log_normal_(shape, dtype):
    ...
    mean = 1.0
    std = 2.0
    res_out = torch.empty(size=shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res_out.log_normal_(mean, std)
    # Log-normal distribution always produces positive values
    assert (res_out > 0).all()
    # Verify output shape matches input shape
    assert res_out.shape == shape
