...
def test_accuracy_log_normal_(shape, dtype):
    ...
    mean = 1.0
    std = 2.0
    # For LogNormal(mean, std): E[X] = exp(mean + std^2/2), Var[X] = (exp(std^2) - 1) * exp(2*mean + std^2)
    expected_mean = torch.exp(torch.tensor(mean + std**2 / 2)).item()
    expected_var = (torch.exp(torch.tensor(std**2)) - 1) * torch.exp(
        torch.tensor(2 * mean + std**2)
    )
    expected_std = torch.sqrt(expected_var).item()
    # Use 20% tolerance for mean, 25% for std (log-normal has high variance)
    mean_tol = 0.20 * expected_mean
    std_tol = 0.25 * expected_std
    with flag_gems.use_gems():
        x.log_normal_(mean=mean, std=std)
    x_res = to_reference(x)
    mean_res = torch.mean(x_res.to(torch.float32)).item()
    std_res = torch.std(x_res.to(torch.float32)).item()
    assert abs(mean_res - expected_mean) < mean_tol
    assert abs(std_res - expected_std) < std_tol
