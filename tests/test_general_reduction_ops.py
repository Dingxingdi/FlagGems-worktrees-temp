...
NANSUM_SHAPES = REDUCTION_SHAPES + [(1, 8192), (32, 50257)]


def _generate_tensor_with_nans(shape, dtype, device, nan_ratio=0.1):
    """Generate a tensor with random NaN values."""
...
@pytest.mark.nansum
@pytest.mark.parametrize("shape", NANSUM_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nansum_without_dim(shape, dtype):
    inp = _generate_tensor_with_nans(shape, dtype, flag_gems.device, nan_ratio=0.1)
    ref_inp = to_reference(inp, True)
    ...
    gems_assert_close(res_out, ref_out, dtype, reduce_dim=inp.numel(), equal_nan=True)
...
def test_accuracy_nansum_dim(shape, dim, keepdim, dtype):
    ...
    if dim == []:
        _dim = inp.numel()
    gems_assert_close(res_out, ref_out, dtype, reduce_dim=_dim, equal_nan=True)
...
@pytest.mark.parametrize("keepdim, dim", KEEPDIM_DIM)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nansum_dim_out(shape, dim, keepdim, dtype):
    inp = _generate_tensor_with_nans(shape, dtype, flag_gems.device, nan_ratio=0.1)
    ref_inp = to_reference(inp, True)
    ...
    out = torch.empty((1,), dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res_result = torch.nansum(inp, dim=dim, keepdim=keepdim, out=out)
    ...
    if dim == []:
        _dim = inp.numel()
    gems_assert_close(res_result, ref_result, dtype, reduce_dim=_dim, equal_nan=True)
    gems_assert_close(out, ref_result, dtype, reduce_dim=_dim, equal_nan=True)
