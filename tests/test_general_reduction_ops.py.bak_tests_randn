...
# ReduceMean tests - uses the same Triton kernels as mean
@pytest.mark.ReduceMean
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_ReduceMean_without_dim(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    ref_out = torch.mean(ref_inp)
    with flag_gems.use_gems():
        res_out = flag_gems.ReduceMean(inp)
...
@pytest.mark.parametrize("keepdim, dim", KEEPDIM_DIM)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_ReduceMean_dim(shape, dim, keepdim, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    ref_out = torch.mean(ref_inp, dim=dim, keepdim=keepdim)
    with flag_gems.use_gems():
        res_out = flag_gems.ReduceMean(inp, dim=dim, keepdim=keepdim)
