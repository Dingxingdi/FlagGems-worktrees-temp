...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy_batch_norm_no_update(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    running_var = torch.abs(torch.randn(size=(C,), dtype=dtype, device=flag_gems.device)) + 0.1

    eps = 1e-5

    # Compute reference manually: output = weight * (input - running_mean) / sqrt(running_var + eps) + bias
    # For non-affine case, weight=1 and bias=0
    # Use float32 for computation to match the kernel's precision
    w = weight.to(torch.float32) if weight is not None else torch.ones(C, dtype=torch.float32, device=flag_gems.device)
    b = bias.to(torch.float32) if bias is not None else torch.zeros(C, dtype=torch.float32, device=flag_gems.device)
    ...
    with flag_gems.use_gems():
        res_out, res_save_mean, res_save_var, res_reserved = torch.ops.aten._batch_norm_no_update(
            inp,
            weight,
            bias,
            running_mean,
