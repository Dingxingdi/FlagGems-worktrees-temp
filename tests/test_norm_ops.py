...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy_native_batch_norm_legit_training(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    # Reference: compute on GPU using PyTorch's native implementation
    ref_inp = inp.clone()
    ...
    ref_out, ref_mean, ref_var = torch._native_batch_norm_legit(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    with flag_gems.use_gems():
        res_out, res_mean, res_var = torch._native_batch_norm_legit(
            inp,
            weight,
            bias,
            running_mean,
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy__native_batch_norm_legit_inference(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    # Initialize running stats that will be used in inference mode
    ...
    gems_assert_close(res_out, ref_out, dtype)
    # In inference mode, running_mean and running_var should NOT be updated
    gems_assert_close(running_mean, ref_running_mean, dtype)
    gems_assert_close(running_var, ref_running_var, dtype)
    # In inference mode, mean and var outputs should be empty tensors
    assert res_mean.shape == ref_mean.shape == torch.Size([0])
    assert res_var.shape == ref_var.shape == torch.Size([0])
