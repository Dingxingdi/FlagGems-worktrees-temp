...
# LayerNorm + GeLU fused operator tests
...
@pytest.mark.parametrize("wb_none", [False, True])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_layer_norm_gelu(shape, dtype, wb_none):
    """Test fused LayerNorm + GeLU operator.

    This is a common pattern in Transformer architectures.
    The reference is computed by composing torch.layer_norm and torch.gelu.
    """
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    # Reference: LayerNorm followed by GeLU
    ...
    ref_out = torch.nn.functional.gelu(ref_norm)
