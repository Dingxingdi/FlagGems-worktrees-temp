...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnn_batch_norm(shape, dtype):
    C = shape[1]
    inp = torch.randn(size=shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_out, ref_save_mean, ref_save_var, _ = torch.cudnn_batch_norm(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    # Call FlagGems implementation directly to avoid recursion with use_gems()
    res_out, res_save_mean, res_save_var, _ = flag_gems.cudnn_batch_norm(
        inp,
        weight,
        bias,
        running_mean,
    ...
    gems_assert_close(res_save_var, ref_save_var, dtype)
    gems_assert_close(running_mean, ref_running_mean, dtype)
    gems_assert_close(running_var, ref_running_var, dtype)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnn_batch_norm_inference(shape, dtype):
    C = shape[1]
    inp = torch.randn(size=shape, dtype=dtype, device=flag_gems.device)
    weight = torch.randn(size=(C,), dtype=dtype, device=flag_gems.device)
    bias = torch.randn(size=(C,), dtype=dtype, device=flag_gems.device)

    # Fixed running mean/var for inference
    ...
    ref_out, _, _, _ = torch.cudnn_batch_norm(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    # Call FlagGems implementation directly to avoid recursion with use_gems()
    res_out, _, _, _ = flag_gems.cudnn_batch_norm(
        inp,
        weight,
        bias,
        running_mean,
