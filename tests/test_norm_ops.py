...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_norm(shape, ord, dim, keepdim, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.linalg.norm(ref_inp, ord, dim, keepdim)
    with flag_gems.use_gems():
        res_out = torch.linalg.norm(inp, ord, dim, keepdim)
...
@pytest.mark.linalg_norm
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_norm_flatten(dtype):
    """Test linalg_norm with flattened input (default Frobenius/L2 norm)."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    # Default: ord=None, dim=None -> Frobenius norm (L2 of flattened)
    ref_out = torch.linalg.norm(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.linalg.norm(inp)

    gems_assert_close(res_out, ref_out, dtype)


# Removed test_accuracy_linalg_norm_2d_matrix - matrix norms have recursion issues
