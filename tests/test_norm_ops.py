...
# LayerNorm+Dropout tests
@pytest.mark.layer_norm_dropout
@pytest.mark.parametrize(
    "shape",
    [
        (4, 64),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("wb_none", [False, True])
def test_accuracy_layer_norm_dropout_no_dropout(shape, dtype, wb_none):
    """Test LayerNorm+Dropout with p=0 (no dropout) - should match exactly."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    # Reference: Apply LayerNorm then Dropout with p=0
    ...
    ref_out = torch.nn.functional.dropout(ref_ln_out, p=0.0, training=True)

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, res_mean, res_rstd, res_mask = flag_gems.layer_norm_dropout(
            res_inp,
            shape[1:],
            weight=res_weight,
            bias=res_bias,
