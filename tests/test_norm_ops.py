...
def test_accuracy_miopen_batch_norm_backward(shape, dtype, affine):
    ...
    res_weight = (
        torch.randn(size=(C,), dtype=dtype, device=flag_gems.device) if affine else torch.ones(C, dtype=dtype, device=flag_gems.device)
    )
    res_running_mean = torch.zeros(size=(C,), dtype=dtype, device=flag_gems.device)
    res_running_var = torch.ones(size=(C,), dtype=dtype, device=flag_gems.device)
    res_save_mean = torch.randn(C, dtype=torch.float32, device=flag_gems.device)
    # save_var should be positive (variance), use abs to ensure positive values
    res_save_var = torch.abs(torch.randn(C, dtype=torch.float32, device=flag_gems.device)) + 0.1
    ...
    ref_running_var = to_reference(res_running_var, True)
    ref_save_mean = to_reference(res_save_mean, True)
    ref_save_var = to_reference(res_save_var, True)
