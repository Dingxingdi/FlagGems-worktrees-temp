...
# Dropout_Add_LayerNorm tests
@pytest.mark.dropout_add_layernorm
@pytest.mark.parametrize(
    "shape",
    [
        (16, 64),
...
@pytest.mark.parametrize("wb_none", [False, True])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dropout_add_layernorm(shape, dtype, wb_none):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)

    # Create input and residual tensors
    res_input = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    res_residual = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    if wb_none:
        res_weight = None
        res_bias = None
    else:
        res_weight = torch.ones(shape[1:], dtype=dtype, device=flag_gems.device)
        res_bias = torch.zeros(shape[1:], dtype=dtype, device=flag_gems.device)
    eps = 1e-5

    ref_input = to_reference(res_input, True)
    ref_residual = to_reference(res_residual, True)
    ref_weight = to_reference(res_weight, True) if res_weight is not None else None
    ref_bias = to_reference(res_bias, True) if res_bias is not None else None

    # Reference: input + residual -> layer_norm
    def reference_dropout_add_layernorm(input, residual, weight, bias, eps):
        added = input + residual
        normalized = torch.nn.functional.layer_norm(
            added, input.shape[1:], weight=weight, bias=bias, eps=eps
        )
        return normalized

    ref_out = reference_dropout_add_layernorm(
        ref_input, ref_residual, ref_weight, ref_bias, eps
    )
    with flag_gems.use_gems():
        res_out = flag_gems.dropout_add_layernorm(
            res_input, res_residual, res_weight, res_bias, dropout_p=0.0, eps=eps
        )
