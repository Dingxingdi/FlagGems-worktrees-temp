...
# Test for BatchNorm_ReLU_Dropout fused operator
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy_BatchNorm_ReLU_Dropout(shape, dtype, affine):
    """Test for fused BatchNorm + ReLU + Dropout operator"""
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    ...
    eps = 1e-5
    momentum = 0.1
    p = 0.0  # No dropout for accuracy test (we test the BN+ReLU part)
    ...
    # Reference: manually chain BatchNorm -> ReLU (dropout disabled with p=0)
    ref_out = torch.nn.functional.batch_norm(
        ref_inp,
        ref_running_mean,
        ref_running_var,
    ...
    # dropout with p=0 returns identity

    with flag_gems.use_gems():
        res_out = flag_gems.ops.batch_norm_relu_dropout(
            inp,
            weight=weight,
            bias=bias,
            running_mean=running_mean,
