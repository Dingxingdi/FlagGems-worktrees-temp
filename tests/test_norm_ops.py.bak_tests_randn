...
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy__native_batch_norm_legit_functional(
    shape, dtype, training, affine
):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.ops.aten._native_batch_norm_legit_functional(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    with flag_gems.use_gems():
        res_out = torch.ops.aten._native_batch_norm_legit_functional(
            inp,
            weight,
            bias,
            running_mean,
    ...
    # Check all 5 outputs
    # output, running_mean_out, running_var_out have same dtype as input
    # save_mean and save_var are always float32 in PyTorch
    gems_assert_close(res_out[0], ref_out[0], dtype)  # output
    gems_assert_close(res_out[1], ref_out[1], torch.float32)  # save_mean
    gems_assert_close(res_out[2], ref_out[2], torch.float32)  # save_var
    gems_assert_close(res_out[3], ref_out[3], dtype)  # running_mean_out
    gems_assert_close(res_out[4], ref_out[4], dtype)  # running_var_out
