...
@pytest.mark.parametrize("shape", REDUCTION_SHAPES[:2])  # Skip large shapes for now
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_rms_norm_linear(shape, dtype):
    """
    Test for fused RMSNorm + Linear operation.
    This is a common pattern in LLM architectures.
    """
    N = shape[1]
    # Output dimension for the linear layer
    K = 256 if not QUICK_MODE else 64
    layer_shape = [N]

    # Create inputs
    inp = torch.randn(shape[:2], dtype=dtype, device=flag_gems.device)
    rms_weight = torch.randn(layer_shape, dtype=dtype, device=flag_gems.device)
    linear_weight = torch.randn(K, N, dtype=dtype, device=flag_gems.device)
    bias = torch.randn(K, dtype=dtype, device=flag_gems.device)
    eps = 1e-5

    ref_inp = to_reference(inp)
    ref_rms_weight = to_reference(rms_weight)
    ref_linear_weight = to_reference(linear_weight)
    ref_bias = to_reference(bias)

    # Reference: compute RMSNorm then Linear
    def _torch_rms_norm_linear(x, rms_weight, linear_weight, bias, eps):
        # RMSNorm
        upcast_x = x.to(torch.float32)
        variance = upcast_x.pow(2).mean(-1, keepdim=True)
        hidden_states = upcast_x * torch.rsqrt(variance + eps).to(torch.float32)
        hidden_states = hidden_states.to(x.dtype)
        hidden_states = hidden_states * rms_weight
        # Linear
        return torch.nn.functional.linear(hidden_states, linear_weight, bias)

    ref_out = _torch_rms_norm_linear(
        ref_inp, ref_rms_weight, ref_linear_weight, ref_bias, eps
    )
    res_out = flag_gems.rms_norm_linear(
        inp, list(layer_shape), rms_weight, linear_weight, bias, eps
    )
...
@pytest.mark.rms_norm_linear
@pytest.mark.parametrize("shape", REDUCTION_SHAPES[:2])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_rms_norm_linear_no_bias(shape, dtype):
    """
    Test for fused RMSNorm + Linear operation without bias.
    """
    N = shape[1]
    K = 256 if not QUICK_MODE else 64
    layer_shape = [N]
    ...
    # Reference: compute RMSNorm then Linear (no bias)
    def _torch_rms_norm_linear_no_bias(x, rms_weight, linear_weight, eps):
        # RMSNorm
        upcast_x = x.to(torch.float32)
        variance = upcast_x.pow(2).mean(-1, keepdim=True)
        hidden_states = upcast_x * torch.rsqrt(variance + eps).to(torch.float32)
        ...
        hidden_states = hidden_states * rms_weight
        # Linear (no bias)
        return torch.nn.functional.linear(hidden_states, linear_weight, None)

    ref_out = _torch_rms_norm_linear_no_bias(
        ref_inp, ref_rms_weight, ref_linear_weight, eps
    )
    res_out = flag_gems.rms_norm_linear(
        inp, list(layer_shape), rms_weight, linear_weight, None, eps
    )
