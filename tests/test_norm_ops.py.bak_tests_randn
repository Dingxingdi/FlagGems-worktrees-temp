...
# Test for BatchNorm + ReLU + Dropout fused operator
# Note: We use p=0 to disable dropout in test, as dropout is random and cannot be
# compared element-wise. The test verifies the BatchNorm + ReLU fusion correctness.
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy_batch_norm_relu_dropout(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    eps = 1e-5
    p = 0  # Disable dropout for deterministic testing
    ...
    # Reference: compose BatchNorm -> ReLU (dropout disabled with p=0)
    # Note: F.batch_norm defaults to training=False (inference mode)
    ref_bn_out = torch.nn.functional.batch_norm(
        ref_inp,
        ref_running_mean,
        ref_running_var,
        weight=ref_weight,
        bias=ref_bias,
        eps=eps,
        training=False,  # Use inference mode for reference
    )
    ref_out = torch.nn.functional.relu(ref_bn_out)

    # Our implementation - use training=False to match reference
    with flag_gems.use_gems():
        res_out, res_mean, res_inv_std, res_mask = flag_gems.batch_norm_relu_dropout(
            inp,
            weight=weight,
            bias=bias,
            running_mean=running_mean,
            running_var=running_var,
            training=False,  # Use inference mode
            eps=eps,
            p=p,
            train=False,  # Disable dropout training
        )
