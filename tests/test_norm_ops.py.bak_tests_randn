...
def test_accuracy__batch_norm_with_update(shape, dtype, affine):
    ...
    ref_weight = to_reference(weight, True)
    ref_bias = to_reference(bias, True)
    ref_running_mean = to_reference(running_mean, True).clone()
    ref_running_var = to_reference(running_var, True).clone()

    ref_out, ref_save_mean, ref_save_var, ref_reserve = torch.ops.aten._batch_norm_with_update(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    res_out, res_save_mean, res_save_var, res_reserve = torch.ops.aten._batch_norm_with_update(
        inp,
        weight,
        bias,
        running_mean,
    ...
    # Check output
    gems_assert_close(res_out, ref_out, dtype)
    # Check running_mean and running_var were updated correctly
    gems_assert_close(running_mean, ref_running_mean, dtype)
    gems_assert_close(running_var, ref_running_var, dtype)
