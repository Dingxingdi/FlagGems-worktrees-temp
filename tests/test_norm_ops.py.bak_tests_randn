...
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_add_layer_norm(shape, dtype):
    N = shape[1]
    layer_shape = [N]
    inp = torch.randn(shape[:2], dtype=dtype, device=flag_gems.device)
    residual = torch.randn(shape[:2], dtype=dtype, device=flag_gems.device)
    ...
    def _torch_add_layer_norm(x, residual, weight, bias, eps):
        x = x + residual
    ...
    ref_out, ref_new_residual = _torch_add_layer_norm(
        ref_inp,
        ref_residual,
        weight=ref_weight,
        bias=ref_bias,
    ...
    res_out, res_new_residual = flag_gems.add_layer_norm(
        inp, residual, list(layer_shape), weight=weight, bias=bias, eps=eps
    )
