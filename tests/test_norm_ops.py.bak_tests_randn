...
def test_accuracy_batch_norm_with_update_functional(shape, dtype, affine):
    ...
    running_mean = torch.zeros(size=(C,), dtype=torch.float32, device=flag_gems.device)
    running_var = torch.ones(size=(C,), dtype=torch.float32, device=flag_gems.device)

    eps = 1e-5
    momentum = 0.1

    ref_inp = to_reference(inp, True)
    ref_weight = to_reference(weight, True) if weight is not None else None
    ...
    # ref_out is (output, mean, invstd, reserve, running_mean_out, running_var_out)
    # res_out is the same
    gems_assert_close(res_out[0], ref_out[0], dtype)
    # running_mean_out and running_var_out are float32
    gems_assert_close(res_out[4], ref_out[4], torch.float32)
    gems_assert_close(res_out[5], ref_out[5], torch.float32)
