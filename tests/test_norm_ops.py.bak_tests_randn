...
# Matrix norm shapes: (m, n) matrices and (*, m, n) batches
MATRIX_NORM_SHAPES = (
    [(2, 3)] if QUICK_MODE else [(4, 4), (16, 16), (32, 32), (64, 64), (128, 128), (1024, 1024)]
)
MATRIX_NORM_BATCH_SHAPES = (
    [(2, 2, 3)] if QUICK_MODE else [(2, 16, 16), (4, 32, 32), (2, 128, 128)]
)


@pytest.mark.linalg_matrix_norm
@pytest.mark.parametrize("shape", MATRIX_NORM_SHAPES)
@pytest.mark.parametrize("keepdim", [False, True])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_norm(shape, keepdim, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.linalg.matrix_norm(ref_inp, "fro", keepdim=keepdim)
    with flag_gems.use_gems():
        res_out = torch.linalg.matrix_norm(inp, "fro", keepdim=keepdim)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.linalg_matrix_norm
@pytest.mark.parametrize("shape", MATRIX_NORM_BATCH_SHAPES)
@pytest.mark.parametrize("keepdim", [False, True])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_norm_batch(shape, keepdim, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
