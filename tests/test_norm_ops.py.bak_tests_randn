...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy__native_batch_norm_legit_no_training(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    gems_assert_close(res_out, ref_out, dtype)
    # running_mean and running_var should not change in no_training mode
    gems_assert_close(running_mean, ref_running_mean, dtype)
    gems_assert_close(running_var, ref_running_var, dtype)
    # mean and var returned are empty tensors in no_training mode
    assert res_mean.numel() == 0
    assert res_var.numel() == 0
