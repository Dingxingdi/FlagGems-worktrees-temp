...
MULTI_MARGIN_LOSS_SHAPES = [(2, 8), (4, 16), (16, 32)]
...
@pytest.mark.parametrize("weight", [True, False])
@pytest.mark.parametrize("shape", MULTI_MARGIN_LOSS_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_multi_margin_loss(shape, dtype, weight, p, reduction):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
        np.random.seed(0)
    ...
    if weight:
        weight_tensor = torch.randn(C, dtype=dtype, device=flag_gems.device)
    else:
        weight_tensor = None

    ref_inp = to_reference(inp, True)
    ref_target = to_reference(target)
    ref_weight = to_reference(weight_tensor, True)

    ref_out = torch.nn.functional.multi_margin_loss(
        ref_inp, ref_target, p=p, margin=1.0, weight=ref_weight, reduction=reduction
    )
    with flag_gems.use_gems():
        res_out = torch.nn.functional.multi_margin_loss(
            inp, target, p=p, margin=1.0, weight=weight_tensor, reduction=reduction
        )
    ...
    with flag_gems.use_gems():
        (res_in_grad,) = torch.autograd.grad(res_out, inp, out_grad)
    gems_assert_close(res_in_grad, ref_in_grad, dtype, reduce_dim=C, equal_nan=True)
