...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nanmedian(shape, dtype):
    # Use fixed seed for reproducibility
    torch.manual_seed(12345)
    # Create input with some NaN values
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Set some elements to NaN using a fixed pattern
    mask = torch.rand(shape, device=flag_gems.device) < 0.3
    inp[mask] = float('nan')

    ref_inp = to_reference(inp)

    ref_out = torch.nanmedian(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.nanmedian(inp)

    # Use gems_assert_close with more lenient tolerance for nanmedian
    # due to potential differences in sorting algorithms
...
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nanmedian_large(shape, dtype):
    # Use fixed seed for reproducibility
    torch.manual_seed(12345)
    # Test with larger shapes
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Set some elements to NaN using a fixed pattern
    mask = torch.rand(shape, device=flag_gems.device) < 0.2
    inp[mask] = float('nan')
...
@pytest.mark.nanmedian
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nanmedian_all_nan(dtype):
    ...
    # Both should be NaN
    assert torch.isnan(res_out) and torch.isnan(ref_out)
