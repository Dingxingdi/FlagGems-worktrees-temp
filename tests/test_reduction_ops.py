...
# index_reduce tests
# Note: Currently only prod reduction is implemented for 2D tensors with dim=0
REDUCE_OPS_TESTED = ["prod"]


@pytest.mark.index_reduce
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dim", [0])  # Only test dim=0 for now
@pytest.mark.parametrize("reduce", REDUCE_OPS_TESTED)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_index_reduce(shape, dim, reduce, dtype):
    if len(shape) > 2:
        pytest.skip("index_reduce only supports 1D and 2D tensors for now")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_src = to_reference(src)
    ref_index = to_reference(index)
    ref_out = torch.index_reduce(ref_inp, dim, ref_index, ref_src, reduce)

    with flag_gems.use_gems():
        res_out = torch.index_reduce(inp, dim, index, src, reduce)
...
@pytest.mark.parametrize("reduce", REDUCE_OPS_TESTED)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_index_reduce_(shape, dim, reduce, dtype):
    if len(shape) > 2:
        pytest.skip("index_reduce_ only supports 1D and 2D tensors for now")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_src = to_reference(src)
    ref_index = to_reference(index)
    ref_inp.index_reduce_(dim, ref_index, ref_src, reduce)

    with flag_gems.use_gems():
        inp.index_reduce_(dim, index, src, reduce)
