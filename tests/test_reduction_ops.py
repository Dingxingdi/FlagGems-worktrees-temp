...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cumprod_float(shape, dtype):
    dim = 1 if shape == REDUCTION_SHAPES[-1] else -1
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Avoid 0 values which cause numerical issues in cumprod (0 * x = 0)
    inp = torch.where(inp == 0, inp + 0.1, inp)
    ref_inp = to_reference(inp, True)
...
@pytest.mark.cumprod
@pytest.mark.parametrize("shape", [(2, 32)] if QUICK_MODE else [(1, 2), (4096, 256)])
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_cumprod_int(shape, dtype):
    dim = -1
    inp = torch.randint(1, 4, shape, device=flag_gems.device).to(dtype)
    ref_inp = to_reference(inp)
    ...
    # we should use ref's output type, since cumprod of int dtype results in int64
    check_dtype = (
        dtype
        if flag_gems.vendor_name == "cambricon" or flag_gems.vendor_name == "enflame"
        else (ref_out.dtype if dtype in INT_DTYPES else dtype)
