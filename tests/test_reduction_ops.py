...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_adaptive_max_pool3d(shape, output_size, dtype):
    # Skip if output size is larger than input size in any dimension
    if output_size[0] > shape[2] or output_size[1] > shape[3] or output_size[2] > shape[4]:
        pytest.skip("Output size larger than input size")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    ref_out = torch.nn.functional.adaptive_max_pool3d(ref_inp, output_size)
    res_out = flag_gems.adaptive_max_pool3d(inp, output_size)
