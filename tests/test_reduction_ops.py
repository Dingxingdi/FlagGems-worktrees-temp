...
@pytest.mark.parametrize("shape, kernel_size, output_size", FRACTIONAL_MAXPOOL2D_CONFIGS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fractional_max_pool2d(shape, kernel_size, output_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ref_inp = to_reference(inp, True)
...
@pytest.mark.parametrize("shape, kernel_size, output_size", FRACTIONAL_MAXPOOL2D_CONFIGS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fractional_max_pool2d_backward(shape, kernel_size, output_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ref_inp = to_reference(inp, upcast=True)
