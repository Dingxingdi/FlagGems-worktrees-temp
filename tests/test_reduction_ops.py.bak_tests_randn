...
@pytest.mark.parametrize("shape", CUMSPROD_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cumprod_(shape, dtype):
    dim = 1 if shape == REDUCTION_SHAPES[-1] else -1
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)
...
@pytest.mark.parametrize("shape", CUMSPROD_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cumprod_inplace(shape, dtype):
    """Test in-place cumprod_"""
    dim = 1 if shape == REDUCTION_SHAPES[-1] else -1
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    ref_out = ref_inp.clone()
    ref_out.cumprod_(dim=dim)
    with flag_gems.use_gems():
        res_out = inp.clone()
        res_out.cumprod_(dim=dim)
