...
ADAPTIVE_MAX_POOL2D_CONFIGS = [
    # Classic cases with different output sizes
    ((4, 3, 32, 32), (4, 4)),
    ((4, 3, 32, 32), (8, 8)),
    ((4, 3, 32, 32), (16, 16)),
    ((2, 8, 16, 16), (4, 4)),
...
@pytest.mark.parametrize("shape, output_size", ADAPTIVE_MAX_POOL2D_CONFIGS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
...
def test_accuracy_adaptive_max_pool2d_backward(shape, output_size, dtype):
    ...
    res_out, res_indices = flag_gems.adaptive_max_pool2d(
        inp,
        output_size=output_size,
    )
    out_grad = torch.randn_like(res_out, device=flag_gems.device)
    ref_grad = to_reference(out_grad, upcast=True)
    (ref_in_grad,) = torch.autograd.grad(ref_out, ref_inp, ref_grad)
    res_in_grad = flag_gems.adaptive_max_pool2d_backward(
        out_grad,
        inp,
        res_indices,
        output_size=output_size,
