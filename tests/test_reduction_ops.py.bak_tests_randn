...
ADAPTIVE_AVG_POOL2D_CONFIGS = [
    # Typical use case: 4x4 -> 2x2
    ((4, 3, 32, 32), (2, 2)),
    ...
    # Typical CNN case
    ((1, 64, 56, 56), (7, 7)),
    # Channel first, small spatial
    ((1, 3, 4, 4), (2, 2)),
]


@pytest.mark.adaptive_avg_pool2d
@pytest.mark.parametrize("shape, output_size", ADAPTIVE_AVG_POOL2D_CONFIGS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_adaptive_avg_pool2d(shape, output_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch._adaptive_avg_pool2d(ref_inp, output_size)
    with flag_gems.use_gems():
        res_out = torch._adaptive_avg_pool2d(inp, output_size)
