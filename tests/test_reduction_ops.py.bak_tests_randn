...
def test_accuracy_binary_cross_entropy(shape, dtype, reduction):
    ...
    # BCE requires input and target to be between 0 and 1 (as probabilities)
    # Use sigmoid to ensure valid inputs
    inp = torch.sigmoid(torch.randn(shape, dtype=dtype, device=flag_gems.device))
    target = torch.sigmoid(torch.randn(shape, dtype=dtype, device=flag_gems.device))
    ...
    # For sum reduction, float32 precision can cause
    # larger relative differences due to atomic add precision loss.
    # Use a higher rtol for float32 sum case.
    if dtype == torch.float32 and reduction == "sum":
        torch.testing.assert_close(res_out, ref_out.to(dtype), atol=1.0, rtol=1e-5, equal_nan=True)
    else:
        gems_assert_close(res_out, ref_out, dtype, equal_nan=True)
