...
@pytest.mark.parametrize("keepdim", [True, False])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy_mode(shape, dim, keepdim, dtype):
    if dtype in INT_DTYPES:
        # For integers, use a smaller range to ensure duplicates for mode
        inp = torch.randint(0, 8, size=shape, device=flag_gems.device).to(dtype)
    else:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.mode(ref_inp, dim=dim, keepdim=keepdim)
    with flag_gems.use_gems():
        res_out = torch.mode(inp, dim=dim, keepdim=keepdim)

    # Mode returns (values, indices) tuple
    gems_assert_equal(res_out[0], ref_out[0])
    gems_assert_equal(res_out[1], ref_out[1])
