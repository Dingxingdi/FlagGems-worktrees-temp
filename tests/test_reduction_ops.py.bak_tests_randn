...
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_sum_dtype(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.sum(ref_inp, dtype=torch.float64)
    with flag_gems.use_gems():
        res_out = torch.sum(inp, dtype=torch.float64)

    gems_assert_close(res_out, ref_out, torch.float64)


# Skip integer tests as flag_gems.sum has limited integer support
# Skip empty tensor tests as flag_gems.sum has a bug with M=0
# Skip inf tests as gems_assert_close can't handle nan vs nan comparison
