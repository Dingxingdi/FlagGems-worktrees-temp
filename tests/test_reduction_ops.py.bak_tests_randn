...
NONZERO_STATIC_SHAPES = [(2, 32)] if QUICK_MODE else REDUCTION_SHAPES + [(2637,)]


@pytest.mark.nonzero_static
@pytest.mark.parametrize("shape", NONZERO_STATIC_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES + [torch.bool])
def test_accuracy_nonzero_static(shape, dtype):
    if dtype == torch.bool:
        inp = torch.randint(0, 2, shape, dtype=torch.int, device=flag_gems.device).to(
            dtype
        )
    ...
    # Test various size and fill_value combinations
    size = reduce(lambda x, y: x * y, shape)  # Use total elements as size
    ref_out = torch.nonzero_static(ref_inp, size=size)
    with flag_gems.use_gems():
        res_out = torch.nonzero_static(inp, size=size)
...
@pytest.mark.parametrize("shape", NONZERO_STATIC_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES + [torch.bool])
def test_accuracy_nonzero_static_with_fill_value(shape, dtype):
    if dtype == torch.bool:
        inp = torch.randint(0, 2, shape, dtype=torch.int, device=flag_gems.device).to(
            dtype
        )
    ...
    # Test with custom fill_value and size
    size = reduce(lambda x, y: x * y, shape) + 5  # Larger than actual nonzero count
    fill_value = 0
    ref_out = torch.nonzero_static(ref_inp, size=size, fill_value=fill_value)
    with flag_gems.use_gems():
        res_out = torch.nonzero_static(inp, size=size, fill_value=fill_value)
...
@pytest.mark.parametrize("shape", NONZERO_STATIC_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES + [torch.bool])
def test_accuracy_nonzero_static_size_smaller(shape, dtype):
    if dtype == torch.bool:
        inp = torch.randint(0, 2, shape, dtype=torch.int, device=flag_gems.device).to(
            dtype
        )
    ...
    # Test with size smaller than actual nonzero count
    size = 2
    ref_out = torch.nonzero_static(ref_inp, size=size)
    with flag_gems.use_gems():
        res_out = torch.nonzero_static(inp, size=size)
