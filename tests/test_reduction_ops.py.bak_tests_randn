...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test__unsafe_index_put_acc_false(input_shape, indices_shape, values_shape, is_bool, dtype):
    accumulate = False
    inp = torch.randn(
        input_shape, dtype=dtype, device=flag_gems.device, requires_grad=False
    )
    ...
    with flag_gems.use_gems():
        out = torch._unsafe_index_put(inp, indices, values, accumulate)
    gems_assert_close(out, ref_out, dtype)
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test__unsafe_index_put_acc_true(input_shape, indices_shape, values_shape, is_bool, dtype):
    init_seed(0)
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(24)
        torch.mlu.manual_seed_all(24)
