...
def test_accuracy_index_fill(shape, dim, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    index_max = inp.size(dim)
    # Use random subset of indices
    from math import floor

    index_len = max(1, floor(index_max * 0.5))
    index = torch.randint(0, index_max, [index_len], device=flag_gems.device)
    value = torch.tensor(99.0, dtype=dtype)
