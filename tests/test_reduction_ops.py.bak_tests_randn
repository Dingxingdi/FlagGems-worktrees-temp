...
@pytest.mark.parametrize("shape", REDUCTION_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_std_mean_without_dim(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    ref_std, ref_mean = torch.std_mean(ref_inp)
    with flag_gems.use_gems():
        res_std, res_mean = torch.std_mean(inp)

    gems_assert_close(res_std, ref_std, dtype)
    gems_assert_close(res_mean, ref_mean, dtype)
...
@pytest.mark.parametrize("keepdim", [True] if QUICK_MODE else [True, False])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_std_mean_dim(shape, dim, correction, keepdim, dtype):
    if shape[0] == 1:  # TODO: res is inf, while ref is nan
        shape = (2, 2)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)
    ...
    with flag_gems.use_gems():
        res_std, res_mean = torch.std_mean(
            inp, dim, correction=correction, keepdim=keepdim
        )
