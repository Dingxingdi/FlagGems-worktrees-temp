...
@pytest.mark.parametrize("step", [1, 2])
def test_accuracy_slice(shape, dtype, start, end, step):
    ...
    res_out = flag_gems.ops.slice(inp, dim=dim, start=start, end=end, step=step)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_slice_dim0(shape, dtype):
    dim = 0
    start = 1
    end = shape[dim] - 1
    ...
    res_out = flag_gems.ops.slice(inp, dim=dim, start=start, end=end)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_slice_negative_indices(shape, dtype):
    ...
    res_out = flag_gems.ops.slice(inp, dim=1, start=-8, end=-2)
