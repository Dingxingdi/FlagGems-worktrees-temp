...
# Test for logsumexp operator
LOGSUMEXP_SHAPES = (
    [(REDUCTION_SHAPES[0], 1)]
    if QUICK_MODE
    else [(1, 32), (4096, 256), (200, 40999, 3), (32, 50257), (1, 8192)]
)
LOGSUMEXP_DIMS = [1] if QUICK_MODE else [0, 1, -1]


@pytest.mark.logsumexp
@pytest.mark.parametrize("shape", LOGSUMEXP_SHAPES)
@pytest.mark.parametrize("dim", LOGSUMEXP_DIMS)
@pytest.mark.parametrize("keepdim", [True, False])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
...
# Test extreme inputs for logsumexp
@pytest.mark.logsumexp
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_logsumexp_extreme_inputs(dtype):
    # Test with all zeros
    shape = (32, 64)
    inp = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.logsumexp(ref_inp, dim=1)
    with flag_gems.use_gems():
        res_out = torch.logsumexp(inp, dim=1)

    gems_assert_close(res_out, ref_out, dtype)

    # Test with -inf (should handle correctly)
    inp = torch.full(shape, -float("inf"), dtype=dtype, device=flag_gems.device)
    inp[:, 0] = 0  # Set one value to 0
    ref_inp = to_reference(inp)
