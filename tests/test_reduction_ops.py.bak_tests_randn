...
# index_reduce tests
REDUCE_OPS = ["mean", "prod", "amax", "amin"]


@pytest.mark.index_reduce
@pytest.mark.parametrize("reduce", REDUCE_OPS)
@pytest.mark.parametrize("shape", [(4, 8), (8, 4), (4, 4, 4)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_index_reduce(reduce, shape, dtype):
    # Test 1D index_reduce (dim is the reduce dimension)
    dim = 0
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    src_shape = list(inp.shape)
    index_len = src_shape[dim]
    index = torch.randint(0, index_len, (index_len,), device=flag_gems.device)
    src_shape[dim] = index_len
    src = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.ops.aten.index_reduce(
        ref_inp, dim, ref_index, ref_src, reduce, include_self=True
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.index_reduce(inp, dim, index, src, reduce, include_self=True)
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_index_reduce_dim1(reduce, shape, dtype):
    # Test with dim=1
    dim = 1
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("shape", [(4, 8), (8, 4)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_index_reduce_(reduce, shape, dtype):
    dim = 0
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    gems_assert_close(inp, ref_inp, dtype=dtype)
