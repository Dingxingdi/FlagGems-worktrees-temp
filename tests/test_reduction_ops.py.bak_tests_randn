...
def test_accuracy_scatter_reduce(src_shape, inp_shape, dim, dtype, reduce):
    init_seed(0)
    inp = torch.randn(inp_shape, dtype=dtype, device=flag_gems.device)
    src = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
    size_dim = min(src_shape[dim], inp_shape[dim])
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
@pytest.mark.parametrize("reduce", ["sum", "prod", "amax", "amin"])
def test_accuracy_scatter_reduce_inplace(src_shape, inp_shape, dim, dtype, reduce):
    init_seed(0)
    inp = torch.randn(inp_shape, dtype=dtype, device=flag_gems.device)
    src = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
    size_dim = min(src_shape[dim], inp_shape[dim])
    ...
    ref_src = to_reference(src, upcast=True)
    ref_out = ref_inp.clone()
    ref_out.scatter_reduce_(dim, ref_index, ref_src, reduce)
    with flag_gems.use_gems():
        res_out = inp.clone()
        res_out.scatter_reduce_(dim, index, src, reduce)
