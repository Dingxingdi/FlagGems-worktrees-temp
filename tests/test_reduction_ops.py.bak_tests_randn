...
@pytest.mark.parametrize("shape", [(1, 2), (64, 128)])
@pytest.mark.parametrize("dim", DIM_LIST)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_index_select_backward(shape, dim, dtype):
    # index_select_backward schema:
    ...
    # Backward: index_select_backward
    # grad has same shape as forward output
    grad = torch.ones_like(ref_out)
    ref_grad_input = torch.ops.aten.index_select_backward(
        grad, list(inp.shape), dim, ref_index
    )

    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.index_select_backward(
            grad, list(inp.shape), dim, index
        )

    gems_assert_equal(res_grad_input, ref_grad_input)
