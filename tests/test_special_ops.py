...
# _adaptive_avg_pool3d_backward tests
POOLING3D_SHAPES = [
    (1, 3, 8, 8, 8),
    (2, 4, 16, 16, 16),
    (1, 1, 4, 4, 4),
...
@pytest.mark.parametrize("shape", POOLING3D_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("output_size", [(2, 2, 2), (4, 4, 4), (1, 1, 1)])
def test_accuracy__adaptive_avg_pool3d_backward(shape, dtype, output_size):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ref_inp = to_reference(inp)

    # Forward pass to get output
    ref_out = torch._adaptive_avg_pool3d(ref_inp, output_size)
    act_out = torch._adaptive_avg_pool3d(inp, output_size)

    # Backward pass
    grad_output = torch.ones_like(ref_out)
    ref_inp_grad = torch.ops.aten._adaptive_avg_pool3d_backward(grad_output, ref_inp)
    act_inp_grad = torch.ops.aten._adaptive_avg_pool3d_backward(grad_output, inp)

    gems_assert_close(act_inp_grad, ref_inp_grad, dtype)
