...
def test_accuracy_cslt_sparse_mm(shape, dtype):
    """Test for _cslt_sparse_mm operator.

    Note: This operator wraps cuSPARSELt which cannot be reimplemented in Triton.
    The test verifies that the wrapper function works correctly and produces
    valid output.
    """
    M, N = shape
    K = N  # Square matrix for simplicity
    ...
    # Compress A using cuSPARSELt
    compressed_A = torch._cslt_compress(A)

    # Call the flag_gems implementation (which wraps torch._cslt_sparse_mm)
    res_out = flag_gems.ops._cslt_sparse_mm(compressed_A, B)

    # Verify the output has the correct shape
    assert res_out.shape == (M, N), f"Expected shape ({M}, {N}), got {res_out.shape}"
    assert res_out.dtype == dtype, f"Expected dtype {dtype}, got {res_out.dtype}"

    # Verify the output is not all zeros (basic sanity check)
    assert not torch.all(res_out == 0), "Output should not be all zeros"

    # Verify numerical stability - compute using regular matmul as reference
    # Note: Due to structured sparsity, results will differ, but we check no NaN/Inf
    assert torch.isfinite(res_out).all(), "Output contains NaN or Inf values"
