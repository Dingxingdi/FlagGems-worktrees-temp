...
# Note: Cholesky only supports float32 and float64 on CPU, so we only test float32
CHOLESKY_DTYPES = [torch.float32]


@pytest.mark.cholesky
@pytest.mark.parametrize("shape", UT_SHAPES_2D)
...
def test_accuracy_cholesky(shape, dtype):
    ...
    A = A @ A.transpose(-2, -1) + torch.eye(n, dtype=dtype, device=flag_gems.device) * 0.1

    ref_inp = to_reference(A)
...
def test_accuracy_cholesky_(shape, dtype):
    if flag_gems.vendor_name == "mthreads":
        pytest.skip("Skipping cholesky_ test on mthreads platform")
    ...
    # Use linalg.cholesky which supports more dtypes
    ref_out = torch.linalg.cholesky(ref_A)
    # In-place version
    torch.linalg.cholesky(ref_A, out=ref_A)
    with flag_gems.use_gems():
        res_out = A.cholesky_()
