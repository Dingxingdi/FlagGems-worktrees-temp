...
# Test shapes for weight-only int8 matmul
WEIGHT_INT8PACK_MM_SHAPES = [(16, 32), (32, 64), (64, 128), (128, 256), (256, 512)]


@pytest.mark.weight_int8pack_mm
@pytest.mark.parametrize("shape", WEIGHT_INT8PACK_MM_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_weight_int8pack_mm(shape, dtype):
    """Test weight-only int8 matrix multiplication"""
    M, K = shape
    N = shape[1]  # Use square output for simplicity

    # Create int8 weight matrix
    weight = torch.randint(-128, 127, (M, K), dtype=torch.int8, device=flag_gems.device)

    # Create activation matrix
    activation = torch.randn(K, N, dtype=dtype, device=flag_gems.device)

    # Create scaling factors
    scales = torch.randn(N, dtype=dtype, device=flag_gems.device)

    # Compute reference using float32
    weight_fp32 = weight.to(torch.float32)
    activation_fp32 = activation.to(torch.float32)
    ref_output = torch.mm(weight_fp32, activation_fp32)
    ref_output = ref_output * scales

    # Compute using FlagGems
    with flag_gems.use_gems():
        res_output = torch._weight_int8pack_mm(weight, activation, scales)

    # Compare with relaxed tolerance for float16/bfloat16
    # The int8->float conversion and accumulation can introduce extra numerical error
    atol = 1e-2 if dtype in (torch.float16, torch.bfloat16) else 1e-4
    gems_assert_close(res_output, ref_output.to(dtype), dtype, atol=atol)
