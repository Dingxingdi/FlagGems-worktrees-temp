...
@pytest.mark.parametrize("shape", _cdist_shapes)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("p", [1.0, 2.0])
def test_accuracy_cdist_forward_2d(shape, dtype, p):
    """Test _cdist_forward with 2D tensors (n, d) and (m, d)"""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    n, d = shape
    m = n // 2 + 7  # Use different m for non-square case

    x1 = torch.randn(n, d, dtype=dtype, device=flag_gems.device)
    x2 = torch.randn(m, d, dtype=dtype, device=flag_gems.device)

    ref_x1 = to_reference(x1)
    ref_x2 = to_reference(x2)

    # PyTorch's cdist doesn't support float16/bfloat16 on CUDA, so we compute manually
    if dtype in (torch.float16, torch.bfloat16):
        # Manual reference: compute p-norm distance manually
        diff = ref_x1.unsqueeze(1) - ref_x2.unsqueeze(0)  # (n, m, d)
        if p == 1.0:
            ref_out = torch.sum(torch.abs(diff), dim=2)
        else:  # p == 2.0
            ref_out = torch.sqrt(torch.sum(diff ** 2, dim=2))
    else:
        ref_out = torch.ops.aten._cdist_forward(ref_x1, ref_x2, p, None)

    with flag_gems.use_gems():
        res_out = torch.ops.aten._cdist_forward(x1, x2, p, None)

    gems_assert_close(res_out, ref_out, dtype)


# Test p=3.0 only for float32 (bfloat16 and float16 have precision issues or no native support)
@pytest.mark.cdist_forward
@pytest.mark.parametrize("shape", _cdist_shapes[:3])  # Use smaller shapes
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("p", [3.0])
def test_accuracy_cdist_forward_2d_p3(shape, dtype, p):
    """Test _cdist_forward with p=3.0 for float32"""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
...
@pytest.mark.cdist_forward
@pytest.mark.parametrize("shape", _cdist_shapes[:3])  # Use smaller shapes for 3D
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("p", [1.0, 2.0])
def test_accuracy_cdist_forward_3d(shape, dtype, p):
    """Test _cdist_forward with 3D tensors (batch, n, d) and (batch, m, d)"""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
    ...
    x1 = torch.randn(batch, n, d, dtype=dtype, device=flag_gems.device)
    x2 = torch.randn(batch, m, d, dtype=dtype, device=flag_gems.device)
    ...
    if dtype in (torch.float16, torch.bfloat16):
        # Manual reference: compute p-norm distance manually
        diff = ref_x1.unsqueeze(2) - ref_x2.unsqueeze(1)  # (batch, n, m, d)
        if p == 1.0:
            ref_out = torch.sum(torch.abs(diff), dim=3)
        else:  # p == 2.0
            ref_out = torch.sqrt(torch.sum(diff ** 2, dim=3))
    else:
        ref_out = torch.ops.aten._cdist_forward(ref_x1, ref_x2, p, None)
