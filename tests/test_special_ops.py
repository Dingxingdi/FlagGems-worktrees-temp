...
@pytest.mark.parametrize("shape", [(2, 3), (4, 5, 6), (7, 8, 9, 10)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1, -1])
def test_accuracy_unsqueeze(shape, dtype, dim):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1, -1])
def test_accuracy_unsqueeze_(shape, dtype, dim):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
...
def test_accuracy_unsqueeze_copy(shape, dtype, dim):
    ...
    with flag_gems.use_gems():
        res_out = torch.ops.aten.unsqueeze_copy(inp, dim)
