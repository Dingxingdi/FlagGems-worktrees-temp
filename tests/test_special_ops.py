...
# slogdet test shapes - square matrices
SLOGDET_SHAPES = [(2, 2)]
SLOGDET_BATCH_SHAPES = [(2, 2, 2)]
# slogdet only supports float32 and bfloat16 in PyTorch
...
def test_accuracy_linalg_slogdet(shape, dtype):
    # Generate non-singular matrix (add diagonal to ensure invertible)
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    A = A + torch.eye(shape[0], dtype=dtype, device=flag_gems.device) * 3

    ref_A = to_reference(A)

    ref_out = torch._linalg_slogdet(ref_A)
    with flag_gems.use_gems():
        res_out = torch._linalg_slogdet(A)

    # Compare sign and logabsdet (LU and pivots are not computed in our implementation)
    gems_assert_close(res_out.sign, ref_out.sign, dtype, atol=1e-1)
    gems_assert_close(res_out.logabsdet, ref_out.logabsdet, dtype, atol=1e-1)


@pytest.mark.linalg_slogdet
@pytest.mark.parametrize("shape", SLOGDET_BATCH_SHAPES)
@pytest.mark.parametrize("dtype", SLOGDET_DTYPES)
def test_accuracy_linalg_slogdet_batch(shape, dtype):
    # Generate non-singular matrices (add diagonal to ensure invertible)
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Add identity to each batch matrix to ensure invertibility
    for i in range(shape[0]):
        A[i] = A[i] + torch.eye(shape[1], dtype=dtype, device=flag_gems.device) * 3
