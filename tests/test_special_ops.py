...
UNBIND_COPY_SHAPES = (
    [(2, 19, 7)]
    if QUICK_MODE
    else [(1,), (3, 4, 5), (16, 8, 32), (4, 32, 128), (2, 16, 8, 64)]
)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_unbind_copy(shape, dtype):
    if len(shape) == 0:
        pytest.skip("Scalar tensor has no dimension to unbind")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    ...
    for i in range(len(ref_out)):
        gems_assert_close(res_out[i], ref_out[i], dtype)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1])
def test_accuracy_unbind_copy_dim(shape, dtype, dim):
    if len(shape) == 0:
        pytest.skip("Scalar tensor has no dimension to unbind")
    if dim >= len(shape):
    ...
    ref_out = torch.unbind_copy(ref_inp, dim=dim)
    with flag_gems.use_gems():
        res_out = torch.unbind_copy(inp, dim=dim)
