...
# Test for masked_scatter_backward
MASKED_SCATTER_BACKWARD_SHAPES = [(4, 4), (8, 16), (16, 32), (32, 64)]


@pytest.mark.masked_scatter_backward
@pytest.mark.parametrize("shape", MASKED_SCATTER_BACKWARD_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
...
def test_accuracy_masked_scatter_backward(shape, dtype, threshold):
    ...
    # The output size is the number of True elements in the mask
    numel = mask.sum().item()

    ref_grad_output = to_reference(grad_output)
    ref_mask = to_reference(mask)
    ref_out = torch.ops.aten.masked_scatter_backward.default(
        ref_grad_output, ref_mask, [numel]
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.masked_scatter_backward.default(
            grad_output, mask, [numel]
        )
