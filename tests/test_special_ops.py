...
# Shapes for SVD testing - matrix dimensions
SVD_SHAPES = [
    (4, 3),
    (3, 4),
    (8, 5),
...
def test_accuracy_linalg_svd(shape, dtype, full_matrices):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_U, res_S, res_Vh = torch.linalg.svd(A, full_matrices=full_matrices)

    # Compare singular values (most important)
    gems_assert_close(res_S, ref_S, dtype)

    # Compare U and Vh (may differ in sign due to SVD non-uniqueness)
    # We verify by checking A = U @ diag(S) @ Vh
    # When full_matrices=True, U has shape (m, m) and Vh has shape (n, n)
    # We only use the first k columns/rows
    res_reconstructed = res_U[:, :k] @ torch.diag(res_S) @ res_Vh[:k, :]
    ref_reconstructed = ref_U[:, :k] @ torch.diag(ref_S) @ ref_Vh[:k, :]
    gems_assert_close(res_reconstructed, ref_reconstructed, dtype)
