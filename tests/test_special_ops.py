...
@pytest.mark.parametrize("shape", DQL_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_DynamicQuantizeLinear(shape, dtype):
    """Test DynamicQuantizeLinear accuracy against PyTorch implementation."""
    ...
    # Note: PyTorch's quantize_per_tensor_dynamic only supports float32
    ref_out = torch.quantize_per_tensor_dynamic(ref_inp, torch.qint8, False)
    ref_output = ref_out.int_repr()
    ref_scale = ref_out.q_scale()
    ref_zero_point = ref_out.q_zero_point()

    with flag_gems.use_gems():
        res_output, res_scale, res_zero_point = flag_gems.DynamicQuantizeLinear(inp)

    # Check scale matches (should be exact)
    assert torch.allclose(res_scale.cpu(), torch.tensor(ref_scale), rtol=1e-5, atol=1e-5)

    # Check zero_point matches (should be exact)
    assert int(res_zero_point.cpu().item()) == ref_zero_point

    # Check output - allow max diff of 1 due to rounding differences
    diff = (res_output.cpu().int() - ref_output.int().cpu()).abs()
    assert diff.max().item() <= 1, f"Max diff: {diff.max().item()}"
