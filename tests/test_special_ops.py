...
# Reference implementation for dynamic quantized 4-bit matmul
def native_dyn_quant_matmul_4bit(A, W, W_scale):
    """Native PyTorch implementation for reference."""
    M, K = A.shape
    N = W_scale.shape[0]

    # Dequantize W from int4 to float32
    # W is stored as (K//2, N) in int8, packed with 2 int4 values per byte
    K_packed = W.shape[0]
    W_dequant = torch.empty((K, N), dtype=torch.float32, device=W.device)

    # Unpack int4 from int8
    for k in range(K_packed):
        w_packed = W[k, :]
        # Lower 4 bits (even indices)
        w_low = (w_packed & 0xF).to(torch.int8) - 8
        # Upper 4 bits (odd indices)
        w_high = ((w_packed >> 4) & 0xF).to(torch.int8) - 8

        W_dequant[k * 2, :] = w_low.to(torch.float32) * W_scale
        if k * 2 + 1 < K:
            W_dequant[k * 2 + 1, :] = w_high.to(torch.float32) * W_scale

    # Perform matmul
    return torch.matmul(A.to(torch.float32), W_dequant)


DYN_QUANT_MATMUL_4BIT_SHAPES = [
    (16, 32),
    (32, 64),
    (64, 128),
    (128, 256),
...
@pytest.mark.parametrize("shape", DYN_QUANT_MATMUL_4BIT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_dyn_quant_matmul_4bit(shape, dtype):
    M, K = shape
    N = 64  # Output dimension

    # Create random activation A
    A = torch.randn(M, K, dtype=dtype, device=flag_gems.device)

    # Create random scale for quantization
    # Scale is per-output channel (N,)
    W_scale = torch.rand(N, dtype=torch.float32, device=flag_gems.device) + 0.1

    # Quantize weights to int4 (stored in int8)
    # Generate random float weights first
    W_fp32 = torch.randn(K, N, dtype=torch.float32, device=flag_gems.device)

    # Quantize to int4: scale to [-8, 7] range
    int4_max = 7
    int4_min = -8
    W_quant = (W_fp32 / W_scale[None, :]).round().clamp(int4_min, int4_max).to(torch.int8)

    # Pack int4 into int8: 2 values per byte
    # Output shape: (K//2, N)
    W_packed = torch.empty(K // 2, N, dtype=torch.int8, device=flag_gems.device)
    for k in range(K // 2):
        w_low = W_quant[k * 2, :]
        w_high = W_quant[k * 2 + 1, :] if k * 2 + 1 < K else torch.zeros(N, dtype=torch.int8, device=flag_gems.device)
        # Pack: lower 4 bits = low, upper 4 bits = high
        W_packed[k, :] = ((w_high + 8) << 4) | (w_low + 8)

    # Reference implementation
    ref_A = to_reference(A)
    ref_W_packed = to_reference(W_packed)
    ref_W_scale = to_reference(W_scale)

    ref_out = native_dyn_quant_matmul_4bit(ref_A, ref_W_packed, ref_W_scale)

    # FlagGems implementation
    with flag_gems.use_gems():
        res_out = flag_gems.dyn_quant_matmul_4bit(A, W_packed, W_scale, output_dtype=dtype)

    # Compare with relaxed tolerance for quantization
    # Use rtol=0.1, atol=0.1 for quantization operations
    res_fp32 = res_out.to(torch.float32)
    ref_fp32 = ref_out.to(dtype).to(torch.float32)
    torch.testing.assert_close(res_fp32, ref_fp32, rtol=0.1, atol=0.1)
