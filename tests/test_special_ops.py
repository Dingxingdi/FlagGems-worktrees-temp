...
# Define shapes specifically for unique_dim (2D tensors for row/column uniqueness)
UNIQUE_DIM_SHAPES = [(4, 4)]


@pytest.mark.unique_dim
@pytest.mark.parametrize("shape", UNIQUE_DIM_SHAPES)
@pytest.mark.parametrize("dtype", [torch.int32])
@pytest.mark.parametrize("dim", [0])
def test_accuracy_unique_dim_basic(shape, dtype, dim):
    """Basic test for unique_dim - just output, no inverse/counts"""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_out, _, _ = flag_gems.unique_dim(inp, dim, True, False, False)
    ref_out, _, _ = flag_gems.unique_dim(ref_inp, dim, True, False, False)
...
@pytest.mark.parametrize("dtype", [torch.int32])
@pytest.mark.parametrize("dim", [0])
def test_accuracy_unique_dim_inverse(shape, dtype, dim):
    """Test unique_dim with return_inverse"""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_out, res_inverse, _ = flag_gems.unique_dim(inp, dim, True, True, False)
    ref_out, ref_inverse, _ = flag_gems.unique_dim(ref_inp, dim, True, True, False)
    assert res_out.shape == ref_out.shape
    gems_assert_equal(res_out, ref_out)
...
@pytest.mark.parametrize("dtype", [torch.int32])
@pytest.mark.parametrize("dim", [0])
def test_accuracy_unique_dim_counts(shape, dtype, dim):
    """Test unique_dim with return_counts"""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_out, _, res_counts = flag_gems.unique_dim(inp, dim, True, False, True)
    ref_out, _, ref_counts = flag_gems.unique_dim(ref_inp, dim, True, False, True)
    assert res_out.shape == ref_out.shape
    gems_assert_equal(res_counts, ref_counts)
