...
# Test for nested_view_from_buffer_copy
@pytest.mark.nested_view_from_buffer_copy
@pytest.mark.parametrize("num_tensors", [1, 3, 5, 8])
@pytest.mark.parametrize("max_size", [1, 5, 10, 20])
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_nested_view_from_buffer_copy(num_tensors, max_size, dtype):
    """Test nested_view_from_buffer_copy operator"""
    # Generate random sizes for each sub-tensor
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    sizes = [torch.randint(1, max_size + 1, (1,)).item() for _ in range(num_tensors)]
    total_size = sum(sizes)

    # Create buffer
    buffer = torch.randn(total_size, dtype=dtype, device=flag_gems.device)

    # Create nested_size tensor [num_tensors, 1]
    nested_sizes = torch.tensor([[s] for s in sizes], dtype=torch.long, device=flag_gems.device)

    # Create nested_strides tensor (all 1s)
    nested_strides = torch.ones(num_tensors, 1, dtype=torch.long, device=flag_gems.device)

    # Create offsets (cumulative sum of sizes)
    offsets = torch.tensor([0] + list(torch.cumsum(torch.tensor(sizes), dim=0)[:-1]), dtype=torch.long, device=flag_gems.device)

    # Get reference result - manually move to CPU to avoid segfault
    ref_buffer = buffer.cpu()
    ref_nested_sizes = nested_sizes.cpu()
    ref_nested_strides = nested_strides.cpu()
    ref_offsets = offsets.cpu()

    ref_out = torch._nested_view_from_buffer_copy(ref_buffer, ref_nested_sizes, ref_nested_strides, ref_offsets)

    # Our result - call the FlagGems implementation directly
    from flag_gems.ops._nested_view_from_buffer_copy import nested_view_from_buffer_copy
    res_out = nested_view_from_buffer_copy(buffer, nested_sizes, nested_strides, offsets)

    # Compare values - move ref to GPU for comparison
    gems_assert_close(res_out.values(), ref_out.values().to(buffer.device), dtype)
