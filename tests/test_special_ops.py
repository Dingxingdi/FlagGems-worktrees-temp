...
# Add_Softmax operator tests
# Test shapes suitable for add + softmax (testing reduction along last dimension)
ADD_SOFTMAX_SHAPES = [(2, 19, 7), (1024, 512), (16, 128, 64)]


@pytest.mark.add_softmax
@pytest.mark.parametrize("shape", ADD_SOFTMAX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [-1, 0, 1])
def test_accuracy_add_softmax(shape, dtype, dim):
    """Test Add_Softmax: softmax(input1 + input2, dim)"""
    if dim >= len(shape):
        pytest.skip(f"dim {dim} is out of range for shape {shape}")
    ...
    # Reference: compute softmax(add(input1, input2), dim)
    ref_out = torch.softmax(ref_input1 + ref_input2, dim=dim)

    with flag_gems.use_gems():
        res_out = flag_gems.add_softmax(input1, input2, dim=dim)
