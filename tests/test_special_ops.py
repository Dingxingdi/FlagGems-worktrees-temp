...
def test_accuracy_select(shape, dtype):
    """Test select operator accuracy against PyTorch reference"""
    # Test all dimensions
    for dim in range(len(shape)):
        dim_size = shape[dim]
        # Test various indices including first, middle, last, and negative
        indices_to_test = [0, dim_size // 2, dim_size - 1, -1, -dim_size]
        for index in indices_to_test:
            inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
            ref_inp = to_reference(inp)
