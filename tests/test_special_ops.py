...
# Argsort operator tests
# Note: argsort only supports up to 3D tensors reliably due to radix sort limitations
ARGSORT_SHAPES = [
    (), (1,), (1024, 1024), (20, 320, 15)
]

@pytest.mark.argsort
@pytest.mark.parametrize("shape", ARGSORT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("dim", [-1, 0])
@pytest.mark.parametrize("descending", [True, False])
def test_accuracy_argsort(shape, dtype, dim, descending):
    if len(shape) <= abs(dim):
        pytest.skip(f"Dimension {dim} is out of bounds for shape {shape}")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.argsort(ref_inp, dim=dim, descending=descending)
    with flag_gems.use_gems():
        res_out = torch.argsort(inp, dim=dim, descending=descending)

    gems_assert_equal(res_out, ref_out)


@pytest.mark.argsort
@pytest.mark.parametrize("shape", [(128, 256), (16, 1024)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_argsort_large(shape, dtype):
    """Test argsort with larger shapes for performance validation."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.argsort(ref_inp, dim=-1, descending=False)
    with flag_gems.use_gems():
        res_out = torch.argsort(inp, dim=-1, descending=False)
...
@pytest.mark.argsort
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_argsort_1d(dtype):
    """Test argsort with 1D tensor."""
    shape = (1024,)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.argsort(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.argsort(inp)
