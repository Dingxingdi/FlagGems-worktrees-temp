...
MOE_COMBINE_DTYPES = [torch.float16, torch.bfloat16, torch.float32]


@pytest.mark.moe_combine
@pytest.mark.parametrize("shape", MOE_COMBINE_SHAPES)
@pytest.mark.parametrize("dtype", MOE_COMBINE_DTYPES)
def test_accuracy_moe_combine(shape, dtype):
    """Test moe_combine operator: weighted sum of expert outputs."""
    num_tokens, num_experts, hidden_size = shape
    # Create expert outputs
    expert_outputs = torch.randn(
        num_tokens, num_experts, hidden_size, dtype=dtype, device=flag_gems.device
    )
    # Create weights (softmax-normalized across experts for each token)
    weights = torch.randn(num_tokens, num_experts, dtype=dtype, device=flag_gems.device)
    weights = torch.softmax(weights, dim=1)

    ref_expert_outputs = to_reference(expert_outputs)
    ref_weights = to_reference(weights)

    # Reference implementation
    ref_out = torch.einsum("teh,te->th", ref_expert_outputs, ref_weights)

    with flag_gems.use_gems():
        res_out = flag_gems.moe_combine(expert_outputs, weights)
