...
# Test shapes for _resize_output_
_RESIZE_SHAPES = [
    ((4,), (8,)),  # 1D: expand
    ((4,), (2,)),  # 1D: shrink
    ((6,), (2, 3)),  # 1D to 2D
    ((6,), (3, 2)),  # 1D to 2D (different layout)
]


def _reference_resize_output(src_tensor, size):
    """Reference implementation of _resize_output_ using PyTorch operations that work on CPU."""
    import numpy as np

    # Convert to float32 first if bfloat16 (numpy doesn't support bfloat16)
    if src_tensor.dtype == torch.bfloat16:
        src_tensor = src_tensor.to(torch.float32)

    # Convert to numpy for easier manipulation
    src_np = src_tensor.cpu().numpy().flatten()
    ...
    # Create output array
    out_np = np.zeros(new_numel, dtype=src_np.dtype)

    # Copy the minimum of src and dst elements
    min_numel = min(len(src_np), new_numel)
    out_np[:min_numel] = src_np[:min_numel]

    # Reshape to target size
    result = torch.from_numpy(out_np.reshape(size))
...
@pytest.mark.parametrize("shapes", _RESIZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__resize_output_(shapes, dtype):
    src_shape, dst_shape = shapes
    # Create a simple test tensor with known values
    src = torch.arange(src_shape[0], dtype=dtype, device=flag_gems.device).clone()
    if len(src_shape) > 1:
        src = src.reshape(src_shape)

    # Reference: compute using a simple CPU-based implementation
    ref_src = src.cpu()
    ref_out = _reference_resize_output(ref_src, list(dst_shape))

    with flag_gems.use_gems():
        res_out = torch.ops.aten._resize_output(src, list(dst_shape), flag_gems.device)
