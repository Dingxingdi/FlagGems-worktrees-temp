...
# Test for is_coalesced operator
@pytest.mark.is_coalesced
@pytest.mark.parametrize("shape", [(3, 3), (5, 5), (10, 10)])
def test_accuracy_is_coalesced(shape):
    # Test coalesced sparse tensor
    size = shape[0]
    indices = torch.arange(size, dtype=torch.long).reshape(1, -1).repeat(2, 1)
    values = torch.arange(size, dtype=torch.float32)
    coalesced = torch.sparse_coo_tensor(indices, values, shape, is_coalesced=True)

    with flag_gems.use_gems():
        res_coalesced = torch.ops.aten.is_coalesced(coalesced)
    ref_coalesced = coalesced.is_coalesced()
    assert res_coalesced == ref_coalesced, f"Expected {ref_coalesced}, got {res_coalesced}"

    # Test uncoalesced sparse tensor (duplicate indices)
    indices_uncoalesced = torch.tensor(
        [[0, 0, 1], [0, 1, 0]], dtype=torch.long
    )
    values_uncoalesced = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32)
    uncoalesced = torch.sparse_coo_tensor(
        indices_uncoalesced, values_uncoalesced, shape
    )

    with flag_gems.use_gems():
        res_uncoalesced = torch.ops.aten.is_coalesced(uncoalesced)
    ref_uncoalesced = uncoalesced.is_coalesced()
    assert res_uncoalesced == ref_uncoalesced, f"Expected {ref_uncoalesced}, got {res_uncoalesced}"
