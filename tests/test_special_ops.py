...
# _cudnn_rnn_backward tests
@pytest.mark.cudnn_rnn_backward
@pytest.mark.parametrize("shape", [(2, 3, 4), (4, 8, 16), (1, 1, 8)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnn_rnn_backward(shape, dtype):
    """Test _cudnn_rnn_backward accuracy.

    This is a simplified backward implementation that computes grad_input = grad_output.
    """
    batch, seq, feature = shape
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    output_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grad_output = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Reference: identity gradient (grad_input = grad_output)
    ref_grad_input = grad_output.clone()

    # GEMS implementation
    with flag_gems.use_gems():
        res_grad_input, res_grad_hx, res_grad_cx, res_grad_weight = flag_gems._cudnn_rnn_backward(
