...
# Shapes for replication_pad2d_backward: (N, C, H, W)
REPLICATION_PAD_2D_SHAPES = [(1, 3, 4, 4), (2, 16, 8, 8), (1, 1, 16, 16)]
REPLICATION_PAD_2D_PADDINGS = [
    (1, 1, 1, 1),  # pad all sides by 1
    (2, 2, 2, 2),  # pad all sides by 2
    (1, 2, 1, 2),  # asymmetric padding
    (0, 0, 0, 0),  # no padding (edge case)
]


@pytest.mark.replication_pad2d_backward
@pytest.mark.parametrize("shape", REPLICATION_PAD_2D_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("padding", REPLICATION_PAD_2D_PADDINGS)
def test_accuracy_replication_pad2d_backward(shape, dtype, padding):
    # Create input tensor
    N, C, H, W = shape
    pad_left, pad_right, pad_top, pad_bottom = padding

    # Create a tensor that requires grad (the original input)
    input = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)

    # Forward pass with replication padding
    padded = torch.nn.functional.pad(input, padding, mode='replicate')
    ...
    # The grad_input is what we want to compare
    ref_input = to_reference(input)
    ref_padded = torch.nn.functional.pad(ref_input, padding, mode='replicate')
    ref_loss = ref_padded.sum()
    ref_loss.backward()

    gems_assert_close(input.grad, ref_input.grad, dtype)
