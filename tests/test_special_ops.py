...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_thnn_fused_lstm_cell_backward_impl(shape, dtype):
    """Test accuracy for _thnn_fused_lstm_cell_backward_impl."""
    batch_size, hidden_size = shape
    input_size = hidden_size  # Use hidden_size as input_size for simplicity

    # Create input tensors
    input_gates = torch.randn(batch_size, 4 * hidden_size, dtype=dtype, device=flag_gems.device)
    hidden_gates = torch.randn(batch_size, 4 * hidden_size, dtype=dtype, device=flag_gems.device)
    ...
    # Reference inputs
    ref_input_gates = to_reference(input_gates)
    ref_hidden_gates = to_reference(hidden_gates)
    ref_cx = to_reference(cx)
    ref_input_bias = to_reference(input_bias)
    ref_hidden_bias = to_reference(hidden_bias)
    ...
    ref_grad_hy = to_reference(grad_hy)
    ref_grad_cy = to_reference(grad_cy)
    ...
    # Compare outputs - check shape and dtype match (exact match not required due to approximation)
