...
# Shapes for eigenvalue tests - only 2x2 for now
EIG_SHAPES = [(2, 2)]
EIG_DTYPES = [torch.float32]


@pytest.mark.parametrize("shape", EIG_SHAPES)
@pytest.mark.parametrize("dtype", EIG_DTYPES)
def test_accuracy_linalg_eigvals(shape, dtype):
    n, m = shape
    assert n == m, "Eigenvalue computation requires square matrices"

    # Use fixed seed for reproducibility
    torch.manual_seed(42)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Reference - compute on CPU with same dtype, then move to device
    ref_inp_cpu = inp.to('cpu')
    ref_out = torch.linalg.eigvals(ref_inp_cpu).to(flag_gems.device)

    with flag_gems.use_gems():
        res_out = torch.ops.aten._linalg_eigvals(inp)

    # Compare complex results with loose tolerance
    torch.testing.assert_close(res_out, ref_out, atol=1e-3, rtol=1e-2)
