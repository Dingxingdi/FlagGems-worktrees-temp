...
# Shapes for linalg_lstsq tests
LSTSQ_SHAPES = [
    (10, 5, 3),   # m=10, n=5, k=3
    (20, 10, 4),  # m=20, n=10, k=4
    (50, 20, 5),  # m=50, n=20, k=5
    (100, 50, 10),  # m=100, n=50, k=10
]


@pytest.mark.linalg_lstsq
@pytest.mark.parametrize("shape", LSTSQ_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])  # Only test float32 for accuracy (PyTorch doesn't support float16/bfloat16)
def test_accuracy_linalg_lstsq(shape, dtype):
    """Test linalg_lstsq accuracy against PyTorch reference."""
    m, n, k = shape

    # Create random input matrices
    # A: (m, n), B: (m, k)
    A = torch.randn(m, n, dtype=dtype, device=flag_gems.device)
    B = torch.randn(m, k, dtype=dtype, device=flag_gems.device)

    # Reference result using PyTorch on GPU
    with torch.no_grad():
        ref_out = torch.linalg.lstsq(A, B)

    # GEMS result
    with flag_gems.use_gems():
        res_out = torch.linalg.lstsq(A, B)

    # Compare solution
    if ref_out.solution.dtype != res_out.solution.dtype:
        ref_out_solution = ref_out.solution.to(res_out.solution.dtype)
    else:
        ref_out_solution = ref_out.solution
    gems_assert_close(res_out.solution, ref_out_solution, dtype)

    # Compare residuals (may be None or empty)
    if ref_out.residuals is not None and ref_out.residuals.numel() > 0:
        if res_out.residuals is not None and res_out.residuals.numel() > 0:
            gems_assert_close(res_out.residuals, ref_out.residuals, dtype)

    # Compare rank
    gems_assert_equal(res_out.rank, ref_out.rank)

    # Compare singular values
    if ref_out.singular_values is not None and ref_out.singular_values.numel() > 0:
        if res_out.singular_values is not None and res_out.singular_values.numel() > 0:
            gems_assert_close(res_out.singular_values, ref_out.singular_values, dtype)


@pytest.mark.linalg_lstsq
@pytest.mark.parametrize("shape", LSTSQ_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])  # Test float16/bfloat16 runs without error
def test_linalg_lstsq_fp16(shape, dtype):
    """Test linalg_lstsq runs without error for float16/bfloat16."""
    m, n, k = shape
    ...
    # GEMS result - should run without error
    with flag_gems.use_gems():
        res_out = torch.linalg.lstsq(A, B)

    # Just verify it runs and returns a valid result
    assert res_out.solution.shape == (n, k)
    assert res_out.solution.dtype == dtype
