...
# Test for sym_storage_offset
@pytest.mark.sym_storage_offset
@pytest.mark.parametrize("shape", [(10,), (10, 20), (10, 20, 30)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_sym_storage_offset(shape, dtype):
    """Test sym_storage_offset accuracy."""
    # Create a tensor
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Get storage offset from reference
    ref_out = torch.ops.aten.sym_storage_offset.default(ref_inp)

    # Get storage offset with gems
    with flag_gems.use_gems():
        res_out = torch.ops.aten.sym_storage_offset.default(inp)

    # Compare results (both should be integers)
    assert ref_out == res_out, f"Storage offset mismatch: ref={ref_out}, res={res_out}"


@pytest.mark.sym_storage_offset
def test_accuracy_sym_storage_offset_slice():
    """Test sym_storage_offset with tensor slices."""
    # Create a tensor and slice it
    inp = torch.randn(100, dtype=torch.float32, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Test different slice offsets
    for offset in [0, 10, 25, 50, 75]:
        sliced = inp[offset:]
        ref_sliced = ref_inp[offset:]

        ref_out = torch.ops.aten.sym_storage_offset.default(ref_sliced)
        with flag_gems.use_gems():
            res_out = torch.ops.aten.sym_storage_offset.default(sliced)

        assert ref_out == res_out, f"Storage offset mismatch at offset {offset}: ref={ref_out}, res={res_out}"
