...
# Matrix pseudoinverse test using Newton-Schulz iteration
@pytest.mark.linalg_pinv
@pytest.mark.parametrize("shape", [(16, 16), (32, 32), (64, 64), (16, 32), (32, 16)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_pinv(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.linalg.pinv(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.linalg.pinv(inp)
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_pinv_batch(shape, dtype):
    # Test batched input (3D tensor)
    batch_size = 4
    inp = torch.randn(batch_size, shape[0], shape[1], dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
