...
def test_upsample_nearest2d_backward(dtype, shape, scale):
    # Create input and compute forward upsampled output
    input = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_i = to_reference(input).to(torch.float32)
    ...
    # Create grad_output (gradient from output) - using ones for simplicity
    grad_output = torch.ones_like(res_out)
    ref_grad_output = torch.ones_like(ref_out)

    # Backward pass
    input_size = (shape[0], shape[1], shape[2], shape[3])  # N, C, H, W
    ref_grad_input = torch.ops.aten.upsample_nearest2d_backward.default(
        ref_grad_output, output_size, input_size
    )
    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.upsample_nearest2d_backward.default(
            grad_output, output_size, input_size
        )

    gems_assert_close(res_grad_input, ref_grad_input.to(dtype), dtype)
