...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("factor", [2, 3])
def test_accuracy__upsample_nearest_exact2d_backward(shape, dtype, factor):
    N, C, H, W = shape
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)

    # Compute forward upsampling
    out_size = [H * factor, W * factor]
    ref_out = torch.ops.aten._upsample_nearest_exact2d(ref_x, out_size, None)
    out_gems = torch.ops.aten._upsample_nearest_exact2d(x, out_size, None)

    # Backward pass - gradient of ones
    grad_output = torch.ones_like(ref_out)
    grad_output_gems = torch.ones_like(out_gems)

    input_size = [N, C, H, W]
    ref_out_backward = torch.ops.aten._upsample_nearest_exact2d_backward(
        grad_output, out_size, input_size, None, None
    )
    with flag_gems.use_gems():
        res_out_backward = torch.ops.aten._upsample_nearest_exact2d_backward(
            grad_output_gems, out_size, input_size, None, None
        )

    gems_assert_close(res_out_backward, ref_out_backward, dtype)
