...
# Polygamma tests
@pytest.mark.special_polygamma
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_polygamma(shape, dtype):
    inp = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 1.0
    ref_inp = to_reference(inp)

    ref_out = torch.special.polygamma(0, ref_inp)
    with flag_gems.use_gems():
        res_out = torch.special.polygamma(0, inp)
