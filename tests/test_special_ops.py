...
# Sliding Window Attention: each query position attends only to keys within a fixed window size


@pytest.mark.Sliding_Window_Attention
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_sliding_window_attention(dtype):
    """Basic test for Sliding Window Attention - verify it runs without error"""
    batch, num_heads, seq_len, head_dim = 1, 2, 16, 32
    window_size = 4

    if flag_gems.vendor_name == "kunlunxin":
        pytest.skip("Sliding_Window_Attention not supported for kunlunxin")

    torch.manual_seed(0)
    q = torch.randn(batch, num_heads, seq_len, head_dim, device=flag_gems.device, dtype=dtype)
    k = torch.randn(batch, num_heads, seq_len, head_dim, device=flag_gems.device, dtype=dtype)
    v = torch.randn(batch, num_heads, seq_len, head_dim, device=flag_gems.device, dtype=dtype)

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = flag_gems.sliding_window_attention(q, k, v, window_size)

    # Verify shape
    assert res_out.shape == q.shape
    # Verify no NaN/Inf
...
@pytest.mark.Sliding_Window_Attention
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_sliding_window_attention_various_sizes(dtype):
    """Test with various batch, head, and sequence dimensions"""
    configs = [
        (1, 1, 16, 32),
        (1, 2, 32, 64),
        (2, 4, 16, 32),
...
@pytest.mark.Sliding_Window_Attention
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_sliding_window_attention_different_windows(dtype):
    ...
    for window_size in [1, 2, 4, 8, 16]:
        ...
        assert res_out.shape == q.shape, f"Shape mismatch for window_size={window_size}"
        assert not torch.isnan(res_out).any(), f"NaN detected for window_size={window_size}"
        assert not torch.isinf(res_out).any(), f"Inf detected for window_size={window_size}"
