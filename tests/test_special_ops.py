...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fused_dropout(shape, p, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)

    # Skip small shapes where random test is unstable
    if shape == (1,):
        pytest.skip("Skip single element shape for random test")
    ...
    ref_out, ref_mask = torch._fused_dropout(ref_inp, p)
    with flag_gems.use_gems():
        res_out, res_mask = torch._fused_dropout(res_inp, p)

    # Check mask dtype is uint8
    assert res_mask.dtype == torch.uint8, f"Expected mask dtype uint8, got {res_mask.dtype}"

    # Check output values
    res_out_cpu = to_reference(res_out)
    scale = 1.0 / one_minus_p

    # For elements where mask is True (1), output should be input * scale
    # For elements where mask is False (0), output should be 0
    valid_mask = res_mask == 1
    zero_mask = res_mask == 0

    # Check zero positions
    assert torch.all(res_out_cpu[zero_mask] == 0), "Output should be 0 where mask is 0"

    # Check scaled positions - use dtype-appropriate tolerance
    expected_scaled = ref_inp[valid_mask] * scale
