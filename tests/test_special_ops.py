...
# Shapes for pad_sequence tests: list of (sequence_length, feature_dim)
PAD_SEQUENCE_SHAPES = [
    [(5, 32), (8, 32), (3, 32)],
    [(16, 128), (32, 128), (8, 128)],
    [(1, 1024), (4, 1024), (2, 1024)],
    [(10, 320, 15), (20, 320, 15), (5, 320, 15)],
    [(8, 128, 64), (16, 128, 64), (4, 128, 64)],
    # Edge case: single sequence
    [(1, 32)],
]

PAD_SEQUENCE_BATCH_FIRST = [True, False]
PAD_SEQUENCE_PADDING_SIDE = ["right", "left"]
PAD_SEQUENCE_PADDING_VALUE = [0.0, -1.0, 1.5]


@pytest.mark.pad_sequence
@pytest.mark.parametrize("shapes", PAD_SEQUENCE_SHAPES)
@pytest.mark.parametrize("batch_first", PAD_SEQUENCE_BATCH_FIRST)
@pytest.mark.parametrize("padding_side", PAD_SEQUENCE_PADDING_SIDE)
@pytest.mark.parametrize("padding_value", PAD_SEQUENCE_PADDING_VALUE)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_pad_sequence(shapes, batch_first, padding_side, padding_value, dtype):
    # Create sequences with different lengths
    inp = [torch.randn(s, dtype=dtype, device=flag_gems.device) for s in shapes]
    ref_inp = [to_reference(seq) for seq in inp]

    # Reference implementation
    ref_out = torch.nn.utils.rnn.pad_sequence(
        ref_inp, batch_first=batch_first, padding_value=padding_value, padding_side=padding_side
    )

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = torch.nn.utils.rnn.pad_sequence(
            inp, batch_first=batch_first, padding_value=padding_value, padding_side=padding_side
        )
...
@pytest.mark.parametrize("shapes", PAD_SEQUENCE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_pad_sequence_default(shapes, dtype):
    """Test with default parameters (batch_first=False, padding_value=0.0, padding_side='right')"""
    inp = [torch.randn(s, dtype=dtype, device=flag_gems.device) for s in shapes]
    ref_inp = [to_reference(seq) for seq in inp]

    ref_out = torch.nn.utils.rnn.pad_sequence(ref_inp)

    with flag_gems.use_gems():
        res_out = torch.nn.utils.rnn.pad_sequence(inp)
