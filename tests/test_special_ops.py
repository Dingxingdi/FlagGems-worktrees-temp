...
# Test for fft_rfftfreq
@pytest.mark.fft_rfftfreq
@pytest.mark.parametrize("n", [1, 2, 4, 5, 8, 16, 17, 32, 64, 100, 256, 512, 1024])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fft_rfftfreq(n, dtype):
    ref_out = torch.fft.rfftfreq(n, dtype=dtype)
    with flag_gems.use_gems():
        res_out = torch.fft.rfftfreq(n, dtype=dtype)
...
@pytest.mark.parametrize("d", [0.1, 0.5, 1.0, 2.0, 10.0])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fft_rfftfreq_with_d(n, d, dtype):
    ref_out = torch.fft.rfftfreq(n, d=d, dtype=dtype)
    with flag_gems.use_gems():
        res_out = torch.fft.rfftfreq(n, d=d, dtype=dtype)
