...
def test_accuracy_linalg_eigvals(shape, dtype):
    ...
    def assert_eigenvalues_close(res, ref, dtype):
        ...
        res_sorted = sorted(
            [(x.real, x.imag) for x in res.flatten()],
            key=lambda x: (x[0], x[1])
        )
        ref_sorted = sorted(
            [(x.real, x.imag) for x in ref.flatten()],
            key=lambda x: (x[0], x[1])
        )
        assert len(res_sorted) == len(ref_sorted)
        for (res_r, res_i), (ref_r, ref_i) in zip(res_sorted, ref_sorted):
            if dtype == torch.float32:
                assert abs(res_r - ref_r) < 1e-4, f"Real part mismatch: {res_r} vs {ref_r}"
                assert abs(res_i - ref_i) < 1e-4, f"Imag part mismatch: {res_i} vs {ref_i}"
            else:
                assert abs(res_r - ref_r) < 1e-10, f"Real part mismatch: {res_r} vs {ref_r}"
                assert abs(res_i - ref_i) < 1e-10, f"Imag part mismatch: {res_i} vs {ref_i}"

    assert_eigenvalues_close(res_out, ref_out, dtype)


@pytest.mark.linalg_eigvals
@pytest.mark.skip(reason="Triton kernel does not support complex64/complex128 input directly")
