...
@pytest.mark.fft_irfft2
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_irfft2(shape, dtype):
    # Generate complex input (from rfft2 of a real tensor)
    # For irfft2, input is complex with shape (..., N0, N1//2+1) for output shape (..., N0, N1)
    n0, n1 = shape
    # Real FFT output has n1//2+1 complex values in the last dim
    input_real = torch.randn(n0, n1 // 2 + 1, dtype=dtype, device=flag_gems.device)
    input_imag = torch.randn(n0, n1 // 2 + 1, dtype=dtype, device=flag_gems.device)
    inp = torch.complex(input_real, input_imag)

    # Compute output size
    s = (n0, n1)

    ref_inp = to_reference(inp)
    ref_out = torch.fft.irfft2(ref_inp, s=s)
    with flag_gems.use_gems():
        res_out = torch.fft.irfft2(inp, s=s)
