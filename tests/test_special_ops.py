...
@pytest.mark.parametrize("num_embeddings", [128] if TO_CPU else [512])
@pytest.mark.parametrize("embedding_dim", [32] if TO_CPU else [64])
@pytest.mark.parametrize("num_indices", [16] if TO_CPU else [64])
@pytest.mark.parametrize("num_bags", [2] if TO_CPU else [4])
@pytest.mark.parametrize("mode", [0, 1])  # Mode 2 (MAX) has issues, only test SUM and MEAN
@pytest.mark.parametrize("include_last_offset", [True, False])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__embedding_bag(
    num_embeddings, embedding_dim, num_indices, num_bags, mode, include_last_offset, dtype
):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    weight = torch.randn(
        (num_embeddings, embedding_dim), device=flag_gems.device, dtype=dtype
    )

    # Create indices - ensure they're valid (within range)
    indices = torch.randint(
        0, num_embeddings, (num_indices,), device=flag_gems.device, dtype=torch.long
    )

    # Create offsets
    # We need to carefully generate offsets to ensure we have exactly num_bags bags
    if include_last_offset:
        # Offsets includes the last offset, so length is num_bags + 1
        # Offsets partition the indices into num_bags bags
        offsets = torch.linspace(0, num_indices, num_bags + 1, device=flag_gems.device, dtype=torch.long)
        offsets = offsets.int()
    else:
        # Offsets does not include the last offset, so length is num_bags
        # Last bag goes from offsets[-1] to num_indices
        offsets = torch.linspace(0, num_indices, num_bags, device=flag_gems.device, dtype=torch.long)
        offsets = offsets.int()
    ...
    ref_out, ref_offset2bag, ref_bag_size, ref_max_indices = torch._embedding_bag(
        ref_weight, ref_indices, ref_offsets,
        mode=mode, include_last_offset=include_last_offset
    )

    with flag_gems.use_gems():
        res_out, res_offset2bag, res_bag_size, res_max_indices = torch._embedding_bag(
    ...
    # Compare outputs
    gems_assert_close(res_out, ref_out, dtype)
    gems_assert_equal(res_bag_size, ref_bag_size)

    # Only compare offset2bag for non-empty bags
    if res_offset2bag.numel() > 0:
        gems_assert_equal(res_offset2bag, ref_offset2bag)
