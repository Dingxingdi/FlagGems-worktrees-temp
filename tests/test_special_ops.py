...
# Note: lu_factor only supports float32 on CUDA in this environment
...
def test_accuracy_lu_unpack(shape, dtype):
    ...
    gems_assert_close(P, ref_P, dtype)
    gems_assert_close(L, ref_L, dtype)
    gems_assert_close(U, ref_U, dtype)

    # Verify P @ L @ U == A
    PLU = torch.matmul(torch.matmul(P, L), U)
    gems_assert_close(PLU, A, dtype)


@pytest.mark.lu_unpack
@pytest.mark.parametrize("shape", LU_SHAPES)
@pytest.mark.parametrize("dtype", LU_DTYPES)
def test_accuracy_lu_unpack_unpack_data_false(shape, dtype):
    ...
    ref_P, ref_L, ref_U = torch.ops.aten.lu_unpack(ref_LU, ref_pivots, unpack_data=False)
...
@pytest.mark.parametrize("shape", LU_SHAPES)
@pytest.mark.parametrize("dtype", LU_DTYPES)
def test_accuracy_lu_unpack_unpack_pivots_false(shape, dtype):
    ...
    ref_P, ref_L, ref_U = torch.ops.aten.lu_unpack(ref_LU, ref_pivots, unpack_pivots=False)
...
@pytest.mark.parametrize("shape", LU_SHAPES)
@pytest.mark.parametrize("dtype", LU_DTYPES)
def test_accuracy_lu_unpack_batched(shape, dtype):
