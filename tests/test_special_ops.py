...
# Define shapes for sym_numel testing
SYML_NUMEL_SHAPES = [
    (10,),
    (2, 3),
    (1, 2, 3),
...
@pytest.mark.parametrize("shape", SYML_NUMEL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_sym_numel(shape, dtype):
    """Test sym_numel operator returns correct number of elements."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
