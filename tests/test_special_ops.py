...
# Shapes for attention backward tests - typical transformer attention shapes
ATTENTION_SHAPES = [
    (1, 2, 128, 64),   # (batch, heads, seq_len, head_dim)
    (2, 4, 256, 64),
    (4, 8, 512, 128),
]
...
def test_accuracy_scaled_dot_product_efficient_attention_backward(shape, dtype):
    """Test _scaled_dot_product_efficient_attention_backward accuracy."""
    B, H, S, D = shape
    # Skip float32 on small heads for numerical stability
    if dtype == torch.float32 and D > 64:
        pytest.skip("float32 with large head_dim may have numerical issues")
    ...
    # Forward pass to get out, logsumexp, philox_seed, philox_offset
    ref_out, ref_lse, ref_seed, ref_offset = torch.ops.aten._scaled_dot_product_efficient_attention(
        ref_query, ref_key, ref_value, None, True, 0.0, False
    )
    with flag_gems.use_gems():
        res_out, res_lse, res_seed, res_offset = torch.ops.aten._scaled_dot_product_efficient_attention(
            query, key, value, None, True, 0.0, False
        )
    ...
    # Backward pass - call flag_gems function directly to avoid recursion
    ref_result = torch.ops.aten._scaled_dot_product_efficient_attention_backward(
        ref_grad_out,
        ref_query,
        ref_key,
        ref_value,
    ...
    # Call FlagGems implementation directly
    res_result = flag_gems.scaled_dot_product_efficient_attention_backward(
        grad_out,
        query,
        key,
        value,
    ...
    # Compare results (grad_query, grad_key, grad_value, grad_attn_bias)
    res_grad_query, res_grad_key, res_grad_value, res_grad_attn_bias = res_result
    ref_grad_query, ref_grad_key, ref_grad_value, ref_grad_attn_bias = ref_result

    gems_assert_close(res_grad_query, ref_grad_query, dtype)
    gems_assert_close(res_grad_key, ref_grad_key, dtype)
    gems_assert_close(res_grad_value, ref_grad_value, dtype)
