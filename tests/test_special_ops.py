...
@pytest.mark.parametrize("shape", [(2, 3, 4, 5), (16, 32, 64, 128)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_strides_like_format_contiguous(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device).contiguous()

    ref_inp = to_reference(inp)
    ref_out = torch.ops.aten.is_strides_like_format(ref_inp, torch.contiguous_format)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.is_strides_like_format(inp, torch.contiguous_format)
...
@pytest.mark.parametrize("shape", [(2, 3, 4, 5), (16, 32, 64, 128)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_strides_like_format_channels_last(shape, dtype):
    if len(shape) != 4:
        pytest.skip("channels_last only supports 4D tensors")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device).to(
        memory_format=torch.channels_last
    )

    ref_inp = to_reference(inp)
    ref_out = torch.ops.aten.is_strides_like_format(ref_inp, torch.channels_last)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.is_strides_like_format(inp, torch.channels_last)
