...
def test_accuracy_linalg_eigvalsh(shape, dtype):
...
@pytest.mark.parametrize("shape", EIGVALSH_SHAPES)
@pytest.mark.parametrize("dtype", EIGVALSH_DTYPES)
def test_accuracy_linalg_eigvalsh_batch(shape, dtype):
    ...
    # Make each matrix in the batch symmetric
    A = (A + A.mT) / 2
...
@pytest.mark.parametrize("dtype", EIGVALSH_DTYPES)
def test_accuracy_linalg_eigvalsh_uplo(shape, dtype):
    # Test with different UPLO options
    n = shape[0]
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Make it symmetric
    A = (A + A.mT) / 2
    ...
    # Test with UPLO='U' (upper triangular)
