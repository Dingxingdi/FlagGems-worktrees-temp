...
# Shapes with dimensions of size 1 for squeeze tests
SQUEEZE_SHAPES = [(1,), (1, 1024), (1024, 1), (1, 1, 1024), (2, 1, 4, 1, 8)]
...
def test_accuracy_squeeze_dim(shape, dtype):
    ...
    # Test squeezing a specific dimension that has size 1
    dim = shape.index(1) if 1 in shape else 0
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.squeeze(ref_inp, dim)
    with flag_gems.use_gems():
        res_out = torch.squeeze(inp, dim)
