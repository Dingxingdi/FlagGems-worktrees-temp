...
# Quantized GRU test
# Note: The quantized_gru operator implementation wraps a standard GRU for simplicity.
# The aten::quantized_gru schema requires CellParamsBase which is a PyTorch internal type,
# making direct testing via torch.ops.aten.quantized_gru.input difficult.
# This test verifies the FlagGems implementation directly.


@pytest.mark.quantized_gru
@pytest.mark.parametrize("shape", [(3, 3, 10), (2, 5, 16)])
@pytest.mark.parametrize("hidden_size", [16, 20])
@pytest.mark.parametrize("num_layers", [1, 2])
@pytest.mark.parametrize("bidirectional", [False, True])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_quantized_gru(shape, hidden_size, num_layers, bidirectional, dtype):
    """Test quantized_gru runs and produces valid output."""
    from flag_gems.ops.quantized_gru import quantized_gru_impl
    ...
    # Call the FlagGems implementation directly
    res_output, res_hx_out = quantized_gru_impl(
        input_tensor, hx, None, True, num_layers, 0.0, False, bidirectional, True
    )

    # Verify output shape
    assert res_output.shape == (batch_size, seq_len, hidden_size * num_directions), \
        f"Expected output shape {(batch_size, seq_len, hidden_size * num_directions)}, got {res_output.shape}"
    assert res_hx_out.shape == (num_layers * num_directions, batch_size, hidden_size), \
        f"Expected hx shape {(num_layers * num_directions, batch_size, hidden_size)}, got {res_hx_out.shape}"

    # Verify output is valid (not NaN or Inf)
    assert not torch.isnan(res_output).any(), "Output contains NaN"
    assert not torch.isinf(res_output).any(), "Output contains Inf"
    assert not torch.isnan(res_hx_out).any(), "Hidden state contains NaN"
    assert not torch.isinf(res_hx_out).any(), "Hidden state contains Inf"
