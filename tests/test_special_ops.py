...
# _fft_r2c tests - real-to-complex FFT
# Only test 1D shapes for now (2D support requires more work)
FFT_SHAPES_1D = [(8,), (16,), (32,), (64,), (128,)]


@pytest.mark.fft_r2c
@pytest.mark.parametrize("shape", FFT_SHAPES_1D)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_r2c(shape, dtype):
    # FFT requires power-of-2 sizes along the FFT dimension
    # Use the last dimension for FFT
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # PyTorch reference using torch.fft.rfft (equivalent to _fft_r2c with default params)
    ref_out = torch.fft.rfft(x, dim=-1)

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = torch._fft_r2c(x, dim=[-1], normalization=0, onesided=True)

    # Compare - convert ref to same dtype as result for comparison
    res, ref = res_out.cpu(), ref_out.to(res_out.dtype).cpu()
    torch.testing.assert_close(res, ref, atol=1e-3, rtol=1e-2)
