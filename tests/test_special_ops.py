...
# sym_constrain_range_for_size tests
@pytest.mark.sym_constrain_range_for_size
@pytest.mark.parametrize("size", [1, 5, 10, 100, 1024])
@pytest.mark.parametrize("min_val", [None, 0])
@pytest.mark.parametrize("max_val", [None, 10, 1000])
def test_accuracy_sym_constrain_range_for_size(size, min_val, max_val):
    # This is a no-op operator used for symbolic shape validation in torch.compile
    # It validates that a symbolic size is within a certain range
    # In practice, it does nothing - it's a side-effect-only operation

    # Skip invalid combinations where size is outside the valid range
    if min_val is not None and size < min_val:
        pytest.skip("Invalid size < min")
    if max_val is not None and size > max_val:
        pytest.skip("Invalid size > max")
    ...
    # Both should return None (void return type)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_sym_constrain_range_for_size_tensor(shape, dtype):
    # Test with tensor input (scalar tensor)
    if flag_gems.vendor_name == "ascend":
        size_tensor = torch.tensor(shape[0], dtype=dtype).to(device=flag_gems.device)
    else:
        size_tensor = torch.tensor(shape[0], dtype=dtype, device=flag_gems.device)

    ref_result = torch.sym_constrain_range_for_size(size_tensor, min=0, max=10000)
    with flag_gems.use_gems():
        gems_result = torch.sym_constrain_range_for_size(size_tensor, min=0, max=10000)
