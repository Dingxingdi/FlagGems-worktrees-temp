...
@pytest.mark.fft_fftfreq
@pytest.mark.parametrize("n", [1, 2, 4, 5, 8, 16, 17, 64, 128, 256])
@pytest.mark.parametrize("d", [0.1, 0.5, 1.0, 2.0, 10.0])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fft_fftfreq(n, d, dtype):
    ref_out = torch.fft.fftfreq(n, d=d, dtype=dtype)
    with flag_gems.use_gems():
        res_out = torch.fft.fftfreq(n, d=d, dtype=dtype)
