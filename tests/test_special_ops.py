...
# Shapes for linalg_eigh (square matrices)
...
def test_accuracy_linalg_eigh(shape, dtype):
    """Test linalg_eigh accuracy for small matrices."""
    inp = make_symmetric_matrix(shape, dtype, flag_gems.device)

    # Compute reference using torch on GPU
    ref_out = torch.linalg.eigh(inp)
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_eigh_batch(shape, dtype):
    """Test linalg_eigh accuracy for batched matrices."""
    inp = make_symmetric_matrix(shape, dtype, flag_gems.device)
