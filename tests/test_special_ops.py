...
# Shapes for linalg_solve testing
LINEAR_SOLVE_SHAPES = [
    (32, 32),
    (64, 64),
    (128, 128),
...
@pytest.mark.parametrize("shape", LINEAR_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
def test_accuracy_linalg_solve(shape, dtype):
    n, k = shape
...
@pytest.mark.parametrize("shape", LINEAR_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
def test_accuracy_linalg_solve_vector(shape, dtype):
    n, _ = shape
    # Test with single RHS vector (shape n)
    A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
    A = A + torch.eye(n, dtype=dtype, device=flag_gems.device) * n
