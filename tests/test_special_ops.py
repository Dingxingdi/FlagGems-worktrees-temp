...
# FFT 2D tests
FFT_SHAPES = [(4, 4), (8, 8), (16, 16), (8, 16), (16, 8)]


@pytest.mark.fft_fft2
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.complex64])
def test_accuracy_fft_fft2(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fft2(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.fft2(inp)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.complex64])
def test_accuracy_fft_fft2_with_norm(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    for norm in ["forward", "backward", "ortho"]:
        ref_out = torch.fft.fft2(ref_inp, norm=norm)
        with flag_gems.use_gems():
            res_out = torch.fft.fft2(inp, norm=norm)
