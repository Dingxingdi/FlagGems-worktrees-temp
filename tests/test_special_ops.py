...
def test_accuracy__unsafe_masked_index_put_accumulate(shape, dtype):
    ...
    indices = torch.randint(0, max(shape[-1], 1), shape, dtype=torch.long, device=flag_gems.device)

    # Create values tensor
    values = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_out = torch._unsafe_masked_index_put_accumulate(ref_inp, mask, (indices,), values)
    with flag_gems.use_gems():
        res_out = torch._unsafe_masked_index_put_accumulate(inp, mask, (indices,), values)
