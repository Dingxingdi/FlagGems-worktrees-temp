...
# FFT test shapes
FFT_SHAPES = [(16,), (32,), (64,), (128,), (16, 16), (32, 32), (8, 16), (4, 8, 16)]


@pytest.mark.fft_ifftn
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifftn(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.fft_ifftn(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.fft_ifftn(inp)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_fftn(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.fft_fftn(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.fft_fftn(inp)
