...
# Test shapes for cross entropy: (batch_size, num_classes)
FUSED_CROSS_ENTROPY_SHAPES = [(4, 10), (16, 100), (32, 1000), (64, 50257)]


@pytest.mark.Fused_Cross_Entropy
@pytest.mark.parametrize("shape", FUSED_CROSS_ENTROPY_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Fused_Cross_Entropy(shape, dtype):
    """Test Fused Cross Entropy with various shapes and dtypes"""
    torch.manual_seed(0)
    M, C = shape
    logits = torch.randn(M, C, dtype=dtype, device=flag_gems.device)
    target = torch.randint(0, C, (M,), device=flag_gems.device)

    ref_logits = to_reference(logits)
    ref_target = to_reference(target)

    # Reference: PyTorch cross entropy
    ref_out = torch.nn.functional.cross_entropy(ref_logits, ref_target, reduction="mean")

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, _ = flag_gems.Fused_Cross_Entropy(logits, target, reduction="mean")

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.Fused_Cross_Entropy
@pytest.mark.parametrize("shape", FUSED_CROSS_ENTROPY_SHAPES[:2])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Fused_Cross_Entropy_none(shape, dtype):
    """Test Fused Cross Entropy with reduction='none'"""
    torch.manual_seed(0)
    M, C = shape
    logits = torch.randn(M, C, dtype=dtype, device=flag_gems.device)
    target = torch.randint(0, C, (M,), device=flag_gems.device)
    ...
    # Reference: PyTorch cross entropy
    ref_out = torch.nn.functional.cross_entropy(ref_logits, ref_target, reduction="none")

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, _ = flag_gems.Fused_Cross_Entropy(logits, target, reduction="none")
...
@pytest.mark.parametrize("shape", FUSED_CROSS_ENTROPY_SHAPES[:2])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Fused_Cross_Entropy_sum(shape, dtype):
    """Test Fused Cross Entropy with reduction='sum'"""
    torch.manual_seed(0)
    M, C = shape
    logits = torch.randn(M, C, dtype=dtype, device=flag_gems.device)
    target = torch.randint(0, C, (M,), device=flag_gems.device)
    ...
    # Reference: PyTorch cross entropy
    ref_out = torch.nn.functional.cross_entropy(ref_logits, ref_target, reduction="sum")

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, _ = flag_gems.Fused_Cross_Entropy(logits, target, reduction="sum")

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.Fused_Cross_Entropy
@pytest.mark.skip(reason="weight handling needs further investigation")
def test_accuracy_Fused_Cross_Entropy_with_weight():
    """Test Fused Cross Entropy with class weights"""
    torch.manual_seed(0)
    M, C = 4, 10
    logits = torch.randn(M, C, dtype=torch.float32, device=flag_gems.device)
    target = torch.tensor([0, 1, 2, 3], device=flag_gems.device)
    weight = torch.randn(C, dtype=torch.float32, device=flag_gems.device)

    ref_logits = to_reference(logits)
    ref_target = to_reference(target)
    ref_weight = to_reference(weight)

    # Reference: PyTorch cross entropy with weight
    ref_out = torch.nn.functional.cross_entropy(ref_logits, ref_target, weight=ref_weight, reduction="mean")

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, _ = flag_gems.Fused_Cross_Entropy(logits, target, weight=weight, reduction="mean")
