...
# efficient_attention_forward tests
@pytest.mark.skipif(TO_CPU, reason="Unsupported in CPU mode")
@pytest.mark.efficient_attention_forward
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16, torch.float32])
def test_accuracy_efficient_attention_forward(dtype):
    # Simple test for _efficient_attention_forward
    ...
    res_out = flag_gems._efficient_attention_forward(
        query,
        key,
        value,
        None,
...
def test_accuracy_efficient_attention_forward_shapes(shape, dtype):
