...
# Import flag_gems.testing for custom tolerance
...
def rms_norm_linear_tolerance(dtype):
    ...
    if dtype == torch.float16:
        ...
        return 1e-1  # Large tolerance for bfloat16 due to fusion precision issues
    else:
...
def test_accuracy_rms_norm_linear(shape, dtype):
    ...
    B, N = shape
    O = N * 2  # output features

    # Create input tensors
    x = torch.randn(B, N, dtype=dtype, device=flag_gems.device)
    rms_weight = torch.randn(N, dtype=dtype, device=flag_gems.device)
    linear_weight = torch.randn(O, N, dtype=dtype, device=flag_gems.device)
    linear_bias = torch.randn(O, dtype=dtype, device=flag_gems.device)
    ...
    ref_linear_bias = to_reference(linear_bias)

    # First RMSNorm
    ref_rms_out = torch.rms_norm(ref_x, [N], ref_rms_weight)
    # Then Linear
    ref_out = torch.nn.functional.linear(ref_rms_out, ref_linear_weight, ref_linear_bias)
    ...
    flag_gems.testing.assert_close(res_out, ref_out, dtype, atol=atol)
...
@pytest.mark.rms_norm_linear
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rms_norm_linear_3d_input(dtype):
    """Test fused RMSNorm + Linear with 3D input (batch, seq, hidden)"""
    B, S, N = 4, 8, 128
    O = 256  # output features
