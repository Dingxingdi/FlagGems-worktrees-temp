...
def get_diagonal_copy_shape_and_dims():
    shapes = SPECIAL_SHAPES
    result = []
...
@pytest.mark.parametrize("shape, dim1, dim2", get_diagonal_copy_shape_and_dims())
@pytest.mark.parametrize("offset", [-1, 0, 1])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diagonal_copy(shape, dtype, dim1, dim2, offset):
    if flag_gems.vendor_name == "mthreads":
        torch.manual_seed(123)
        torch.musa.manual_seed_all(123)
