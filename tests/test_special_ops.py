...
# FFT/IFFT operator tests
COMPLEX_DTYPES = [torch.complex64, torch.complex128]
FFT_SHAPES_TRITON = [8, 16, 32, 64]  # Power of 2 sizes for Triton implementation
FFT_SHAPES_LARGE = [128, 256]  # Larger sizes for PyTorch fallback


@pytest.mark.fft_ifft
@pytest.mark.parametrize("size", FFT_SHAPES_TRITON)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft(size, dtype):
    """Test fft_ifft with small power-of-2 sizes (Triton implementation)"""
    shape = (size,)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    ref_out = torch.fft.ifft(ref_input)
    with flag_gems.use_gems():
        res_out = torch.fft.ifft(input_tensor)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.fft_ifft
@pytest.mark.parametrize("size", FFT_SHAPES_LARGE)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft_large(size, dtype):
    """Test fft_ifft with larger sizes (PyTorch fallback)"""
    shape = (size,)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)
...
@pytest.mark.fft_ifft
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft_2d(dtype):
    """Test fft_ifft with 2D tensor"""
    shape = (4, 8)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    ref_out = torch.fft.ifft(ref_input, dim=1)
    with flag_gems.use_gems():
        res_out = torch.fft.ifft(input_tensor, dim=1)
...
@pytest.mark.fft_ifft
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft_with_n(dtype):
    """Test fft_ifft with explicit n parameter (same as input size)"""
    shape = (32,)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    # Test with n equal to input size (no padding)
    ref_out = torch.fft.ifft(ref_input, n=32)
    with flag_gems.use_gems():
        res_out = torch.fft.ifft(input_tensor, n=32)
...
@pytest.mark.fft_ifft
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft_norm(dtype):
    """Test fft_ifft with different normalization modes"""
    shape = (32,)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    for norm in ["backward", "forward", "ortho"]:
        ref_out = torch.fft.ifft(ref_input, norm=norm)
        with flag_gems.use_gems():
            res_out = torch.fft.ifft(input_tensor, norm=norm)

        gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.inplace
@pytest.mark.fft_ifft_
@pytest.mark.parametrize("size", FFT_SHAPES_TRITON)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_ifft_(size, dtype):
    """Test in-place fft_ifft_"""
    shape = (size,)
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor.clone())

    ref_out = ref_input.fft_ifft_()
    with flag_gems.use_gems():
        res_out = input_tensor.fft_ifft_()
