...
# Test for diagonal_scatter
DIAGONAL_SCATTER_SHAPES = [(4, 4), (3, 5), (5, 3), (2, 4, 6), (2, 3, 4, 5)]


@pytest.mark.diagonal_scatter
@pytest.mark.parametrize("shape", DIAGONAL_SCATTER_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diagonal_scatter(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Get diagonal shape for src tensor
...
@pytest.mark.parametrize("shape", DIAGONAL_SCATTER_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diagonal_scatter_offset(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.parametrize("shape", DIAGONAL_SCATTER_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diagonal_scatter_negative_offset(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
