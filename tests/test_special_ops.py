...
@pytest.mark.parametrize("shape", UNSAFE_SPLIT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1, -1])
def test_accuracy_unsafe_split(shape, dtype, dim):
    """Test unsafe_split operator with various shapes and dimensions."""
    # Normalize dim
    dim = dim % len(shape)
    ...
    # Test with different split sizes
    split_size = shape[dim] // 2 if shape[dim] > 1 else 1

    ref_out = torch.unsafe_split(ref_inp, split_size, dim=dim)
    with flag_gems.use_gems():
        res_out = torch.unsafe_split(inp, split_size, dim=dim)

    # Compare the list of tensors
    assert len(res_out) == len(ref_out), "Number of split tensors mismatch"
    for i, (res, ref) in enumerate(zip(res_out, ref_out)):
        gems_assert_close(res, ref, dtype)


@pytest.mark.unsafe_split
@pytest.mark.parametrize("shape", [(8, 16), (4, 8, 16)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("split_size", [1, 2, 4, 8])
def test_accuracy_unsafe_split_various_sizes(shape, dtype, split_size):
    """Test unsafe_split with various split sizes."""
    # Use dim 1 for 2D, dim 2 for 3D
