...
# Embedding bag shapes
EMBEDDING_BAG_SHAPES = [
    (10, 32),   # (num_embeddings, embedding_dim)
    (100, 64),
    (1000, 128),
    (5000, 256),
]

EMBEDDING_BAG_INDICES_SHAPES = [
    (50,),   # num_indices
    (100,),
    (500,),
    (1000,),
...
@pytest.mark.embedding_bag_forward
@pytest.mark.parametrize("weight_shape", EMBEDDING_BAG_SHAPES)
@pytest.mark.parametrize("num_bags", [2, 4, 8])
@pytest.mark.parametrize("mode", [0, 1])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__embedding_bag_forward_only(weight_shape, num_bags, mode, dtype):
    """Test _embedding_bag_forward_only accuracy"""
    ...
    # Create num_bags + 1 offsets
    # Each bag starts at offsets[i] and ends at offsets[i+1]
    # The indices are drawn from [0, num_embeddings)
    max_indices_per_bag = 20
    num_indices = num_bags * max_indices_per_bag

    # Create indices
    indices = torch.randint(0, num_embeddings, (num_indices,), device=flag_gems.device)
    ref_indices = to_reference(indices)

    # Create offsets (for include_last_offset=False, we have num_bags starting positions)
    # The last bag goes to the end
    offsets = torch.zeros(num_bags + 1, dtype=torch.int64, device=flag_gems.device)
    for i in range(num_bags):
        offsets[i] = i * max_indices_per_bag
    offsets[num_bags] = num_indices
    ref_offsets = to_reference(offsets)

    # Reference
    ref_out, ref_offset2bag, ref_bag_size, ref_max_indices = torch._embedding_bag_forward_only(
        ref_weight, ref_indices, offsets[:-1].clone(), mode=mode, include_last_offset=False
    )

    # GEMS
    with flag_gems.use_gems():
        res_out, res_offset2bag, res_bag_size, res_max_indices = torch._embedding_bag_forward_only(
            weight, indices, offsets[:-1].clone(), mode=mode, include_last_offset=False
        )

    # Compare output (the main result)
    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.embedding_bag_forward
@pytest.mark.parametrize("weight_shape", [(100, 64)])
@pytest.mark.parametrize("num_bags", [2, 4])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__embedding_bag_forward_only_with_padding(
    weight_shape, num_bags, dtype
):
    """Test _embedding_bag_forward_only with padding_idx"""
    num_embeddings, embedding_dim = weight_shape
    torch.manual_seed(42)
    ...
    # Create indices including some that will be padding_idx
    padding_idx = 5
    indices = torch.randint(0, num_embeddings, (num_indices,), device=flag_gems.device)
    # Replace some indices with padding_idx
    indices[::3] = padding_idx
    ref_indices = to_reference(indices)
    ...
    ref_out, _, _, _ = torch._embedding_bag_forward_only(
        ref_weight, ref_indices, offsets[:-1].clone(),
        mode=0, padding_idx=padding_idx, include_last_offset=False
    )

    with flag_gems.use_gems():
        res_out, _, _, _ = torch._embedding_bag_forward_only(
            weight, indices, offsets[:-1].clone(),
            mode=0, padding_idx=padding_idx, include_last_offset=False
        )
