...
# FFT ihfft tests
FFT_SHAPES = [(8,), (16,), (32,), (64,), (128,), (256,), (512,), (1024,), (8, 16), (16, 32)]


@pytest.mark.fft_ihfft
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfft(shape, dtype):
    """Test fft_ihfft accuracy for real input"""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.ihfft(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfft(inp)
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfft_norm(shape, dtype):
    """Test fft_ihfft with different normalization modes"""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    for norm in [None, "forward", "backward", "ortho"]:
        ref_out = torch.fft.ihfft(ref_inp, norm=norm)
        with flag_gems.use_gems():
            res_out = torch.fft.ihfft(inp, norm=norm)
...
@pytest.mark.fft_ihfft
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfft_n_param(dtype):
    """Test fft_ihfft with n parameter (padding and truncation)"""
    # Test padding
    inp = torch.randn(8, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.ihfft(ref_inp, n=16)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfft(inp, n=16)
    ...
    ref_out = torch.fft.ihfft(ref_inp, n=8)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfft(inp, n=8)
...
@pytest.mark.fft_ihfft
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfft_dim(dtype):
    """Test fft_ihfft with different dim parameter"""
    # Test 1D with default dim (last dimension)
    inp = torch.randn(8, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.ihfft(ref_inp)  # dim=-1 by default
    with flag_gems.use_gems():
        res_out = torch.fft.ihfft(inp)
    ...
    # Test 1D with explicit dim=0
    ref_out = torch.fft.ihfft(ref_inp, dim=0)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfft(inp, dim=0)
