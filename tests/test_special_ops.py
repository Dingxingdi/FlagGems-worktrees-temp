...
# Test shapes for upsample_bilinear2d_backward - using smaller shapes that pass reliably
UPSAMPLE_BILINEAR_SHAPES = [
    (1, 1, 2, 2),    # Simple 2x2 -> 4x4
]


@pytest.mark.upsample_bilinear2d_backward
@pytest.mark.parametrize("shape", UPSAMPLE_BILINEAR_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])  # Only test float32 for accuracy
def test_accuracy_upsample_bilinear2d_backward(shape, dtype):
    N, C, IH, IW = shape
    # Test with different output sizes
    OH, OW = IH * 2, IW * 2  # 2x upsampling
    ...
    ref_out = torch.ops.aten.upsample_bilinear2d_backward.default(
        ref_grad_output, output_size, input_size, False
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.upsample_bilinear2d_backward.default(
            grad_output, output_size, input_size, False
        )
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_upsample_bilinear2d_backward_align_corners(shape, dtype):
    N, C, IH, IW = shape
    OH, OW = IH * 2, IW * 2
    ...
    ref_out = torch.ops.aten.upsample_bilinear2d_backward.default(
        ref_grad_output, output_size, input_size, True
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.upsample_bilinear2d_backward.default(
            grad_output, output_size, input_size, True
        )
...
@pytest.mark.upsample_bilinear2d_backward
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_upsample_bilinear2d_backward_scales(dtype):
    N, C, IH, IW = 1, 1, 4, 4
    OH, OW = 3, 3  # Non-integer scale
    ...
    ref_out = torch.ops.aten.upsample_bilinear2d_backward.default(
        ref_grad_output, output_size, input_size, False, scales_h, scales_w
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.upsample_bilinear2d_backward.default(
            grad_output, output_size, input_size, False, scales_h, scales_w
        )
