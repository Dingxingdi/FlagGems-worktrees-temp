...
# SVD shapes for testing - matrix shapes (m, n) with m >= n for proper SVD
SVD_SHAPES = [
    (4, 3),
    (8, 6),
    (16, 12),
...
def test_accuracy_svd(shape, dtype):
    ...
    # Test default (some=True, compute_uv=True)
    ref_U, ref_S, ref_V = torch.svd(ref_A)
    with flag_gems.use_gems():
        res_U, res_S, res_V = torch.svd(A)

    # SVD result may have different signs, so we check the reconstruction
    # A = U @ diag(S) @ V^T
    ref_recon = torch.matmul(torch.matmul(ref_U, torch.diag(ref_S)), ref_V.t())
    res_recon = torch.matmul(torch.matmul(res_U, torch.diag(res_S)), res_V.t())
    gems_assert_close(res_recon, ref_recon, dtype)

    # Also check singular values match
...
@pytest.mark.parametrize("shape", SVD_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
def test_accuracy_svd_some_false(shape, dtype):
    m, n = shape
    A = torch.randn((m, n), dtype=dtype, device=flag_gems.device)
    ref_A = to_reference(A)

    # Test some=False (full SVD)
    ref_U, ref_S, ref_V = torch.svd(ref_A, some=False)
    with flag_gems.use_gems():
        res_U, res_S, res_V = torch.svd(A, some=False)

    # Verify shapes
    assert res_U.shape == (m, m)
    assert res_V.shape == (n, n)

    # Check reconstruction - need to use only first k columns of U
    k = ref_S.shape[0]
    ref_recon = ref_U[:, :k] @ torch.diag(ref_S) @ ref_V.t()
    res_recon = res_U[:, :k] @ torch.diag(res_S) @ res_V.t()
    gems_assert_close(res_recon, ref_recon, dtype)
...
@pytest.mark.parametrize("shape", SVD_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
def test_accuracy_svd_compute_uv_false(shape, dtype):
    m, n = shape
    A = torch.randn((m, n), dtype=dtype, device=flag_gems.device)
    ref_A = to_reference(A)

    # Test compute_uv=False (only singular values)
    ref_U, ref_S, ref_V = torch.svd(ref_A, compute_uv=False)
    with flag_gems.use_gems():
        res_U, res_S, res_V = torch.svd(A, compute_uv=False)

    # U and V should be zero-filled when compute_uv=False
