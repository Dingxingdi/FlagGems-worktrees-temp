...
# Shapes for testing _unsafe_view (reshape operations)
# Each tuple is (input_shape, target_shape) with same total elements
VIEW_SHAPES = [
    ((2, 3), (3, 2)),
    ((2, 3), (6,)),
    ((2, 3), (1, 6)),
...
@pytest.mark.parametrize("shapes", VIEW_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__unsafe_view(shapes, dtype):
    input_shape, target_shape = shapes
    inp = torch.randn(input_shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Use reshape as reference since both our implementation and reshape create copies
    ref_out = ref_inp.reshape(target_shape)
    with flag_gems.use_gems():
        res_out = torch.ops.aten._unsafe_view(inp, target_shape)
