...
# RNN_RELU tests
RNN_SHAPES = [(4, 2, 8), (8, 4, 16)] if not QUICK_MODE else [(4, 2, 8)]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rnn_relu(seq_len, batch_size, input_size, hidden_size, dtype):
    ...
    ref_params = tuple(p.to(dtype=dtype, device=flag_gems.device) for p in params)

    # Run PyTorch reference
    ref_out = torch.rnn_relu(ref_input, ref_hx, ref_params, True, 1, 0.0, False, False, False)

    # Run FlagGems implementation
    with flag_gems.use_gems():
        res_out = torch.rnn_relu(input_tensor, hx, params, True, 1, 0.0, False, False, False)
...
@pytest.mark.parametrize("seq_len", [4, 8])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rnn_relu_batch_first(seq_len, batch_size, input_size, hidden_size, dtype):
    """Test rnn_relu with batch_first=True"""
    # Create input tensor: (batch, seq_len, input_size)
    input_tensor = torch.randn(batch_size, seq_len, input_size, dtype=dtype, device=flag_gems.device)
    ...
    # Run PyTorch reference
    ref_out = torch.rnn_relu(ref_input, ref_hx, ref_params, True, 1, 0.0, False, False, True)

    # Run FlagGems implementation
    with flag_gems.use_gems():
        res_out = torch.rnn_relu(input_tensor, hx, params, True, 1, 0.0, False, False, True)
