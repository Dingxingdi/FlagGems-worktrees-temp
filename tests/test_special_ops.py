...
@pytest.mark.parametrize("input_shape,target_shape", BROADCAST_TO_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy_broadcast_to(input_shape, target_shape, dtype):
    if dtype in INT_DTYPES:
        inp = torch.randint(0, 10, input_shape, dtype=dtype, device=flag_gems.device)
    else:
