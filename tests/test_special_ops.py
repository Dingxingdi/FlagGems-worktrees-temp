...
# Causal Convolution Test
# Causal convolution ensures output[t] only depends on input[0:t+1]


def causal_conv1d_reference(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    """
    Reference implementation of causal convolution using PyTorch.
    Causal convolution applies padding only to the left (beginning) of the input.
    """
    if input.dim() == 2:
    ...
    # For causal convolution, left padding = dilation * (kernel_size - 1) + padding
    ...
    output = torch.nn.functional.conv1d(
        input, weight, bias=bias, stride=stride, padding=0, dilation=dilation, groups=groups
    )
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_causal_conv1d(shape, dtype):
    if dtype == torch.float64:
        pytest.skip("float64 not supported for this test")
    ...
    input = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    weight = torch.randn(
        out_channels, in_channels // 1, kernel_size, dtype=dtype, device=flag_gems.device
    )
    ...
    ref_out = causal_conv1d_reference(ref_input, ref_weight, bias=None, stride=1, padding=0, dilation=1)

    # FlagGems implementation - directly call flag_gems.causal_conv1d
    with flag_gems.use_gems():
        res_out = flag_gems.causal_conv1d(input, weight, bias=None, stride=1, padding=0, dilation=1)
...
@pytest.mark.parametrize("shape", CAUSAL_CONV_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_causal_conv1d_with_bias(shape, dtype):
    if dtype == torch.float64:
        pytest.skip("float64 not supported for this test")
    ...
    ref_out = causal_conv1d_reference(ref_input, ref_weight, bias=ref_bias, stride=1, padding=0, dilation=1)

    # FlagGems implementation - directly call flag_gems.causal_conv1d
    # Note: Using larger atol for bias tests due to precision differences in conv implementations
    with flag_gems.use_gems():
        res_out = flag_gems.causal_conv1d(input, weight, bias=bias, stride=1, padding=0, dilation=1)
...
@pytest.mark.parametrize("shape", CAUSAL_CONV_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_causal_conv1d_with_padding(shape, dtype):
    if dtype == torch.float64:
        pytest.skip("float64 not supported for this test")
    ...
    # Reference implementation with padding
    ref_input = to_reference(input)
    ref_weight = to_reference(weight)

    ref_out = causal_conv1d_reference(ref_input, ref_weight, bias=None, stride=1, padding=2, dilation=1)

    # FlagGems implementation - directly call flag_gems.causal_conv1d
    with flag_gems.use_gems():
        res_out = flag_gems.causal_conv1d(input, weight, bias=None, stride=1, padding=2, dilation=1)
