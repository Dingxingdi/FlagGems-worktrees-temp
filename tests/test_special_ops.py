...
# Test shapes for transpose_ (needs at least 2 dimensions)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_(shape, dtype):
    ...
    assert res_out.shape == ref_out_copy.shape, f"Shape mismatch: {res_out.shape} vs {ref_out_copy.shape}"
    # Check that stride is correctly modified
    assert res_out.stride() == ref_out_copy.stride(), f"Stride mismatch: {res_out.stride()} vs {ref_out_copy.stride()}"
    # Check that data pointer is unchanged (in-place)
    assert res_out.data_ptr() == inp.data_ptr(), "transpose_ should be in-place"
    # Check data equality
    gems_assert_equal(res_out, ref_out_copy)


@pytest.mark.transpose_
@pytest.mark.parametrize("shape", TRANSPOSE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_dim_middle(shape, dtype):
...
@pytest.mark.parametrize("shape", TRANSPOSE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_negative_dim(shape, dtype):
...
def test_accuracy_transpose_int(shape, dtype):
