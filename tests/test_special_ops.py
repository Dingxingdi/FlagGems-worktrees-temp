...
def test_accuracy_istft(n_fft, hop_length, center):
    # Generate test signal
    signal_length = 8192
    torch.manual_seed(42)

    signal = torch.randn(signal_length, dtype=torch.float32, device=flag_gems.device)
    ref_signal = to_reference(signal)

    # Compute STFT
    n_fft_val = n_fft
    win_length = n_fft_val
    stft_result = torch.stft(
        signal, n_fft=n_fft_val, hop_length=hop_length,
        win_length=win_length, center=center, return_complex=True
    )
    ref_stft_result = torch.stft(
        ref_signal, n_fft=n_fft_val, hop_length=hop_length,
        win_length=win_length, center=center, return_complex=True
    )

    # Reference istft using torch
    ref_out = torch.istft(
        ref_stft_result, n_fft=n_fft_val, hop_length=hop_length,
        win_length=win_length, center=center
    )

    # GEMS istft using flag_gems.istft directly
    res_out = flag_gems.istft(
        stft_result, n_fft=n_fft_val, hop_length=hop_length,
        win_length=win_length, center=center
    )
