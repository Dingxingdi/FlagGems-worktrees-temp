...
# Test for FlashDecoding operator
# FlashDecoding is optimized attention for decoding phase in LLM inference
FLASH_DECODING_SHAPES = [
    (1, 4, 1, 32),   # (batch, heads, seq_q, head_dim) - typical decode with seq_kv=32
    (2, 4, 1, 64),   # larger head_dim
    (2, 8, 1, 128),  # more heads
    (4, 4, 1, 64),   # larger batch
]


@pytest.mark.flash_decoding
@pytest.mark.parametrize("shape", FLASH_DECODING_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16, torch.float32])
def test_accuracy_flash_decoding(shape, dtype):
    """Test FlashDecoding operator against PyTorch scaled_dot_product_attention reference."""
    batch, heads, seq_q, head_dim = shape
    seq_kv = 32  # KV cache length for decoding

    # Create Q, K, V tensors
    # Q: (batch, heads, seq_q, head_dim) - during decoding seq_q is typically 1
    # K, V: (batch, heads, seq_kv, head_dim)
    Q = torch.randn(batch, heads, seq_q, head_dim, dtype=dtype, device=flag_gems.device)
    K = torch.randn(batch, heads, seq_kv, head_dim, dtype=dtype, device=flag_gems.device)
    V = torch.randn(batch, heads, seq_kv, head_dim, dtype=dtype, device=flag_gems.device)
    ...
    # Compute reference using PyTorch SDPA
    ref_out = torch.nn.functional.scaled_dot_product_attention(
        ref_Q, ref_K, ref_V, is_causal=True
    )

    # Compute using FlagGems FlashDecoding
    with flag_gems.use_gems():
        res_out = torch.ops.aten.flash_decoding(Q, K, V)
