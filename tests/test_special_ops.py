...
@pytest.mark.fft_ihfft2
@pytest.mark.parametrize("shape", FFT_IHFFT2_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_ihfft2(shape, dtype):
    if dtype == torch.float16 and not torch.cuda.is_bf16_supported():
        pytest.skip("bf16 not supported")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.ihfft2(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.fft_ihfft2(inp)

    # FFT outputs are complex
    gems_assert_close(res_out.real, ref_out.real, dtype)
    gems_assert_close(res_out.imag, ref_out.imag, dtype)


@pytest.mark.inplace
@pytest.mark.fft_ihfft2_
@pytest.mark.parametrize("shape", FFT_IHFFT2_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_ihfft2_(shape, dtype):
    if dtype == torch.float16 and not torch.cuda.is_bf16_supported():
        pytest.skip("bf16 not supported")
