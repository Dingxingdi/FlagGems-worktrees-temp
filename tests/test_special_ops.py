...
# fft_rfftn tests
FFT_SHAPES = [(4, 8), (16, 32), (8, 16, 32)]
# Note: PyTorch FFT does not support bfloat16
FFT_DTYPES = [torch.float16, torch.float32]


@pytest.mark.fft_rfftn
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_rfftn(shape, dtype):
    """Test fft_rfftn accuracy against PyTorch reference."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.rfftn(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.rfftn(inp)

    # FFT outputs complex dtype, use output dtype for comparison
    out_dtype = res_out.dtype
    if out_dtype in [torch.complex32, torch.complex64, torch.complex128]:
        if dtype == torch.float32:
            gems_assert_close(res_out, ref_out, out_dtype, atol=1e-4)
        elif dtype == torch.float16:
            gems_assert_close(res_out, ref_out, out_dtype, atol=1e-3)
        else:  # bfloat16
            gems_assert_close(res_out, ref_out, out_dtype, atol=2e-2)
    else:
        gems_assert_close(res_out, ref_out, dtype)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_rfftn_with_s(shape, dtype):
    """Test fft_rfftn with s parameter."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Use s parameter to specify output size
    s = tuple(d // 2 for d in shape)
    ref_out = torch.fft.rfftn(ref_inp, s=s)
    with flag_gems.use_gems():
        res_out = torch.fft.rfftn(inp, s=s)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_rfftn_with_norm(shape, dtype):
    """Test fft_rfftn with norm parameter."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    for norm in ["forward", "backward", "ortho"]:
        ref_out = torch.fft.rfftn(ref_inp, norm=norm)
        with flag_gems.use_gems():
            res_out = torch.fft.rfftn(inp, norm=norm)
