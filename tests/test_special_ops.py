...
# Matrix shapes for linalg_matrix_exp tests
# Note: Due to numerical precision limits of Taylor series implementation,
# we test with smaller matrices using float32 dtype
MATRIX_SHAPES = [(2, 2), (4, 4), (8, 8)]
...
def test_accuracy_linalg_matrix_exp(shape, dtype):
    # Generate a random square matrix with small values for numerical stability
    # Use uniform distribution with small range to avoid overflow
...
@pytest.mark.parametrize("dtype", MATRIX_DTYPES)
def test_accuracy_linalg_matrix_exp_batched(shape, dtype):
    # Test with batch dimension
    batch_size = 4
    inp = (torch.rand(batch_size, *shape, dtype=dtype, device=flag_gems.device) - 0.5) * 0.5
    ref_inp = to_reference(inp)
