...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_hermite_polynomial_h(shape, dtype):
    # x: input value, n: degree of the polynomial
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # n should be non-negative integers
    n = torch.randint(0, 10, shape, dtype=torch.long, device=flag_gems.device)

    ref_x = to_reference(x)
    ref_n = to_reference(n)

    ref_out = torch.special.hermite_polynomial_h(ref_x, ref_n)
    with flag_gems.use_gems():
        res_out = torch.special.hermite_polynomial_h(x, n)
