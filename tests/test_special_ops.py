...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nested_view_from_buffer(dtype):
    # Test _nested_view_from_buffer which creates NestedTensors from flat buffers
    # Create a simple nested tensor with 2 variable-length sequences
    # First sequence: shape [2, 3], second sequence: shape [1, 2]
    # Total elements needed: 2*3 + 1*2 = 8 elements

    buffer = torch.randn(8, dtype=dtype, device=flag_gems.device)

    # nested_size: (num_tensors, num_dims) = (2, 2)
    nested_size = torch.tensor([[2, 3], [1, 2]], dtype=torch.long, device=flag_gems.device)

    # nested_strides for each sub-tensor
    nested_strides = torch.tensor([[3, 1], [2, 1]], dtype=torch.long, device=flag_gems.device)

    # offsets: starting position for each sub-tensor in the buffer
    offsets = torch.tensor([0, 6], dtype=torch.long, device=flag_gems.device)

    # Call the implementation through flag_gems
    with flag_gems.use_gems():
        res_out = torch._nested_view_from_buffer(buffer, nested_size, nested_strides, offsets)

    # Verify it's a nested tensor
    assert res_out.is_nested, "Output should be a nested tensor"
