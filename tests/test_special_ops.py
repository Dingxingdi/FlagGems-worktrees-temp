...
# unfold_copy shapes - 3D tensors for testing
UNFOLD_SHAPES = [
    (2, 4, 8),
    (4, 8, 16),
    (8, 16, 32),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", UNFOLD_DIMS)
@pytest.mark.parametrize("size", [1, 2])
@pytest.mark.parametrize("step", [1, 2])
def test_accuracy_unfold_copy(shape, dtype, dim, size, step):
    # Skip invalid combinations where size > dim size
    if size > shape[dim]:
        pytest.skip("size > dimension size")

    # Skip invalid combinations where L (number of windows) would be 0
    L = (shape[dim] - size) // step + 1
    if L <= 0:
        pytest.skip("no windows to extract")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.unfold_copy(ref_inp, dim, size, step)
    with flag_gems.use_gems():
        res_out = torch.unfold_copy(inp, dim, size, step)
