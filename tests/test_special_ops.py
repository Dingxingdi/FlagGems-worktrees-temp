...
def test_accuracy_linalg_tensorinv_ind2(shape, dtype):
    ...
    if dtype == torch.float16:
        ...
        A_flat = A_flat @ A_flat.T + torch.eye(m, dtype=torch.float32, device=flag_gems.device) * 0.1
        ...
        A_flat = A.reshape(m, n)
        A_flat = A_flat @ A_flat.T + torch.eye(m, dtype=dtype, device=flag_gems.device) * 0.1
...
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_linalg_tensorinv_ind1(shape, dtype):
