...
# FFT test shapes - use smaller sizes for testing (naive FFT is slow)
FFT_SHAPES = [(4, 4), (8, 8)]
...
def test_accuracy_fft_rfft2(shape, dtype):
    # FFT produces complex output from real input
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.rfft2(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.rfft2(inp)

    # Output is complex, so use complex dtype for comparison
    out_dtype = torch.complex64 if dtype == torch.float32 else torch.complex32
    # Use looser tolerance for float16 (FFT is numerically sensitive)
    atol = 1e-2 if dtype == torch.float16 else 1e-4
    gems_assert_close(res_out, ref_out, out_dtype, equal_nan=True, atol=atol)
