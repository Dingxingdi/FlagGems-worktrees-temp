...
def test_accuracy_smooth_l1_loss_backward(shape, dtype, reduction, beta):
    ...
    target_t = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_grad_output = to_reference(grad_output)
    ref_self = to_reference(self_t)
    ref_target = to_reference(target_t)

    ref_out = torch.ops.aten.smooth_l1_loss_backward(
        ref_grad_output, ref_self, ref_target, reduction, beta
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.smooth_l1_loss_backward(
            grad_output, self_t, target_t, reduction, beta
        )
