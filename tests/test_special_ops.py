...
# FFT test shapes - using power-of-2 friendly sizes for FFT
# For hfft2: output size = 2*(input_size-1), so we need 2*(n-1) = power of 2
# That means n-1 should be power of 2: n = 2^k + 1
# Example: n=9 gives output=16, n=17 gives output=32, n=33 gives output=64
FFT_SHAPES = [(1, 9), (2, 17), (1, 33), (2, 65), (1, 129)] if not QUICK_MODE else [(2, 17)]
FFT_SHAPES_F32 = [(1, 8), (2, 16), (4, 32), (1, 64), (2, 128)] if not QUICK_MODE else [(2, 16)]


@pytest.mark.fft_hfft2
@pytest.mark.parametrize("shape", FFT_SHAPES_F32)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_hfft2(shape, dtype):
    """Test fft_hfft2 accuracy against PyTorch implementation.

    fft_hfft2 computes the 2D Hermitian FFT - the inverse of ihfft2.
    It takes a real input and returns a real output (time domain signal).
    """
    # Generate input - real-valued half-Hermitian spectrum
    # For output size n, half-spectrum has n//2 + 1 points
    # But hfft2 on real input interprets it differently
    # For direct hfft2 on real input, output is 2*(n-1) where n is input size
    n_input = shape[-1]
    ...
    # Compute hfft2
    ref_out = torch.fft.hfft2(ref_x)
    with flag_gems.use_gems():
        res_out = torch.fft.hfft2(x)
...
@pytest.mark.parametrize("shape", FFT_SHAPES_F32)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_hfft2_with_s(shape, dtype):
    """Test fft_hfft2 with explicit output size s parameter."""
    n_input = shape[-1]
    # Specify output size
    s = (shape[0], n_input)
    ...
    ref_out = torch.fft.hfft2(ref_x, s=s)
    with flag_gems.use_gems():
        res_out = torch.fft.hfft2(x, s=s)
