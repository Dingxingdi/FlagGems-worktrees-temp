...
# Fused_RoPE tests
ROPE_SHAPES = [(1, 8, 16), (2, 4, 8), (4, 2, 4)]
ROPE_DTYPES = [torch.float16, torch.float32]


@pytest.mark.Fused_RoPE
@pytest.mark.parametrize("shape", ROPE_SHAPES)
@pytest.mark.parametrize("dtype", ROPE_DTYPES)
def test_accuracy_Fused_RoPE(shape, dtype):
    batch_size, seq_len, num_heads = shape
    head_dim = 64
    num_kv_heads = 2
    max_seq_len = 128
    ...
    # Reference implementation using apply_rotary_pos_emb
    # apply_rotary_pos_emb expects 4D input when position_ids is None
    # format: (batch * seq_len, num_heads, head_dim) -> need to keep 4D
    ref_q_out, ref_k_out = flag_gems.fused.apply_rotary_pos_emb(
        query, key, cos, sin
    )

    # Test our implementation
    res_q_out, res_k_out = flag_gems.Fused_RoPE(query, key, cos, sin)

    gems_assert_close(res_q_out, ref_q_out, dtype)
    gems_assert_close(res_k_out, ref_k_out, dtype)


@pytest.mark.Fused_RoPE
@pytest.mark.parametrize("shape", ROPE_SHAPES)
@pytest.mark.parametrize("dtype", ROPE_DTYPES)
def test_accuracy_Fused_RoPE_with_position_ids(shape, dtype):
    batch_size, seq_len, num_heads = shape
    head_dim = 64
    num_kv_heads = 2
    max_seq_len = 128
    ...
    # Test our implementation
    res_q_out, res_k_out = flag_gems.Fused_RoPE(query, key, cos, sin, position_ids=position_ids)
