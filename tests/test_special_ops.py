...
@pytest.mark.parametrize("shape", [(1,), (1024, 1024), (20, 320, 15), (16, 128, 64, 1280), (16, 7, 57, 32, 29)])
@pytest.mark.parametrize("dtype", [torch.int32, torch.float32])
def test_accuracy_unique_consecutive(shape, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_out = torch.unique_consecutive(inp)
    ref_out = torch.unique_consecutive(ref_inp)
    gems_assert_equal(res_out, ref_out)


# Skip return_inverse and dim tests for now - requires additional implementation
