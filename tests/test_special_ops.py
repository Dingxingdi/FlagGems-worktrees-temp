...
def generate_embedding_bag_inputs(num_bags, embedding_dim, num_weights, num_samples, dtype, device):
    ...
    indices = torch.randint(0, num_weights, (num_samples,), dtype=torch.long, device=device)
    ...
    while sum(bag_sizes) > num_samples:
        ...
        if sum(bag_sizes) <= num_samples:
            break
        bag_sizes[-1] = sum(bag_sizes[:len(bag_sizes)-1]) - num_samples + bag_sizes[-1]
...
def test_accuracy__embedding_bag_dense_backward(num_bags, embedding_dim, dtype):
    ...
    # Create inputs
    weight = torch.randn(num_weights, embedding_dim, dtype=dtype, device=device, requires_grad=True)
    indices = torch.randint(0, num_weights, (num_samples,), dtype=torch.long, device=device)
...
@pytest.mark.parametrize("embedding_dim", [16, 32])
@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
def test_accuracy__embedding_bag_dense_backward_with_weights(num_bags, embedding_dim, dtype):
...
@pytest.mark.parametrize("embedding_dim", [16, 32])
@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
def test_accuracy__embedding_bag_dense_backward_mode_sum(num_bags, embedding_dim, dtype):
