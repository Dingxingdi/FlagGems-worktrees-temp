...
@pytest.mark.parametrize("shape", [(2, 4), (16, 32), (128, 256)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fused_adam(shape, dtype):
    """Test fused_adam operator accuracy"""
    # Create parameter and optimizer state tensors
    params = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grads = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avgs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    exp_avg_sqs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    max_exp_avg_sqs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    state_steps = torch.ones(1, dtype=torch.long, device=flag_gems.device)

    # Create reference copies
    ref_params = params.clone()
    ref_grads = grads.clone()
    ref_exp_avgs = exp_avgs.clone()
    ref_exp_avg_sqs = exp_avg_sqs.clone()
    ref_max_exp_avg_sqs = max_exp_avg_sqs.clone()
    ref_state_steps = state_steps.clone()

    # Run reference implementation (manual Adam algorithm)
    # Note: PyTorch's _fused_adam_ has a bug that produces NaN, so we use manual computation
    beta1, beta2, lr, eps = 0.9, 0.999, 0.01, 1e-8
    ref_exp_avgs_new = beta1 * ref_exp_avgs + (1 - beta1) * ref_grads
    ref_exp_avg_sqs_new = beta2 * ref_exp_avg_sqs + (1 - beta2) * ref_grads * ref_grads
    ref_denom = torch.sqrt(ref_exp_avg_sqs_new) + eps
    ref_update = lr * ref_exp_avgs_new / ref_denom
    ref_params_new = ref_params - ref_update

    # Run GEMS implementation
    with flag_gems.use_gems():
        res_out = torch._fused_adam_(
            (params,),
            (grads,),
            (exp_avgs,),
            (exp_avg_sqs,),
    ...
    # Compare updated params with manual Adam result
    gems_assert_close(params, ref_params_new, dtype)


@pytest.mark.fused_adam
@pytest.mark.parametrize("shape", [(2, 4), (16, 32)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fused_adamamsgrad(shape, dtype):
    """Test fused_adam operator with amsgrad=True"""
    # Create parameter and optimizer state tensors
    params = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grads = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avgs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    ...
    # Run reference implementation (manual Adam algorithm with AMSGrad)
    beta1, beta2, lr, eps = 0.9, 0.999, 0.01, 1e-8
    grad = ref_grads
    ref_exp_avgs_new = beta1 * ref_exp_avgs + (1 - beta1) * grad
    ref_exp_avg_sqs_new = beta2 * ref_exp_avg_sqs + (1 - beta2) * grad * grad
    ref_max_exp_avg_sqs_new = torch.maximum(ref_max_exp_avg_sqs, ref_exp_avg_sqs_new)
    ref_denom = torch.sqrt(ref_max_exp_avg_sqs_new) + eps
    ref_update = lr * ref_exp_avgs_new / ref_denom
    ref_params_new = ref_params - ref_update
...
@pytest.mark.parametrize("shape", [(2, 4), (16, 32)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fused_adammaximize(shape, dtype):
    """Test fused_adam operator with maximize=True"""
    # Create parameter and optimizer state tensors
    params = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grads = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avgs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    ...
    # Run reference implementation (manual Adam algorithm with maximize)
    beta1, beta2, lr, eps = 0.9, 0.999, 0.01, 1e-8
    grad = -ref_grads  # maximize means negate the gradient
    ref_exp_avgs_new = beta1 * ref_exp_avgs + (1 - beta1) * grad
    ref_exp_avg_sqs_new = beta2 * ref_exp_avg_sqs + (1 - beta2) * grad * grad
    ref_denom = torch.sqrt(ref_exp_avg_sqs_new) + eps
    ref_update = lr * ref_exp_avgs_new / ref_denom
...
@pytest.mark.parametrize("shape", [(2, 4), (16, 32)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fused_adamweight_decay(shape, dtype):
    """Test fused_adam operator with weight decay (AdamW style)"""
    # Create parameter and optimizer state tensors
    params = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grads = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avgs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    ...
    # Run reference implementation (manual Adam algorithm with weight decay)
    beta1, beta2, lr, eps, weight_decay = 0.9, 0.999, 0.01, 1e-8, 0.01
    grad = ref_grads
    ref_exp_avgs_new = beta1 * ref_exp_avgs + (1 - beta1) * grad
    ref_exp_avg_sqs_new = beta2 * ref_exp_avg_sqs + (1 - beta2) * grad * grad
    ref_denom = torch.sqrt(ref_exp_avg_sqs_new) + eps
    ...
    ref_params_new = ref_params - lr * weight_decay * ref_params - ref_update
