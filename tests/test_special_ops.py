...
# Reference implementation of MoE Load Balance Loss
def reference_moe_load_balance_loss(gate_logits):
    """
    Reference implementation of MoE load balance loss.

    The loss encourages uniform usage of experts by penalizing
    imbalanced expert loads.

    Formula: loss = num_experts * sum((expert_load / num_tokens)^2)
    """
    ...
    # Compute expert loads (sum of probabilities for each expert)
...
MOE_LOAD_BALANCE_LOSS_SHAPES = [
    ...
    (64, 32),
    (128, 64),
] if not QUICK_MODE else [(16, 8), (32, 16)]
...
def test_accuracy_moe_load_balance_loss(shape, dtype):
    ...
    ref_out = reference_moe_load_balance_loss(ref_gate_logits)
    with flag_gems.use_gems():
        res_out = flag_gems.moe_load_balance_loss(gate_logits)
