...
@pytest.mark.size
@pytest.mark.parametrize("shape", [(2, 3), (16, 32, 64), (8, 4, 2, 16)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_size(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_out = torch.ops.aten.size(inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.size(inp)

    assert ref_out == res_out, f"Shape mismatch: {ref_out} vs {res_out}"
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1, -1])
def test_accuracy_size_dim(shape, dtype, dim):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_out = torch.ops.aten.size(inp, dim)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.size(inp, dim)

    assert ref_out == res_out, f"Size at dim {dim} mismatch: {ref_out} vs {res_out}"
