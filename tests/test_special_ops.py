...
# Define shapes for transpose_copy testing
TRANSPOSE_SHAPES = (
    (32, 64),
    (64, 32),
    (2, 3, 4),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_copy(shape, dtype):
    # Test transposing last two dimensions
    dim0 = len(shape) - 2
    dim1 = len(shape) - 1
    ...
    ref_out = torch.transpose_copy(ref_inp, dim0, dim1)
    with flag_gems.use_gems():
        res_out = torch.transpose_copy(inp, dim0, dim1)
...
@pytest.mark.parametrize("shape", TRANSPOSE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_copy_dim0_dim1(shape, dtype):
...
@pytest.mark.transpose_copy
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_copy_3d_random_dims(dtype):
    # Test random dimension pairs for 3D tensors
    shape = (4, 8, 16)
    for dim0, dim1 in [(0, 1), (0, 2), (1, 2)]:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
        ref_inp = to_reference(inp)
...
@pytest.mark.transpose_copy
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_transpose_copy_negative_dims(dtype):
    # Test negative dimension indices
    shape = (4, 8, 16)
    dim0, dim1 = -2, -1  # Last two dimensions
