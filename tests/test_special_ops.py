...
# FFT hfft test: takes half-Hermitian complex input, returns real output
# Input shapes should produce even-length outputs (n = 2 * (input_len - 1))
FFT_HFFT_SHAPES = [(8,), (16,), (32,), (64,), (128,), (256,), (8, 16), (32, 64)]


@pytest.mark.fft_hfft
@pytest.mark.parametrize("shape", FFT_HFFT_SHAPES)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_fft_hfft(shape, dtype):
    """Test fft_hfft accuracy against PyTorch implementation."""
    # Create half-Hermitian input: X[k] = conj(X[N-k]) for k > 0
    # For hfft, input length L produces output length N = 2*(L-1)
    # The input must be Hermitian-symmetric for the output to be real
    input_len = shape[-1]
    batch_shape = shape[:-1] if len(shape) > 1 else ()

    # Create base real signal and compute its FFT to get valid Hermitian spectrum
    # Then take only the first half (N/2+1 elements) as input to hfft
    output_len = 2 * (input_len - 1)

    # Generate random real signal
    if batch_shape:
        full_shape = batch_shape + (output_len,)
    else:
        full_shape = (output_len,)

    real_signal = torch.randn(full_shape, dtype=torch.float32, device=device)
    # Compute FFT and take first half
    fft_result = torch.fft.rfft(real_signal)
    # Use first input_len elements as half-Hermitian input
    if batch_shape:
        input_shape = batch_shape + (input_len,)
    else:
        input_shape = (input_len,)
    half_hermitian = fft_result[..., :input_len]

    # Convert to the target dtype
    if dtype == torch.complex64:
        input_tensor = half_hermitian.to(torch.complex64)
    else:
        input_tensor = half_hermitian.to(torch.complex128)

    ref_input = to_reference(input_tensor)

    ref_out = torch.fft.hfft(ref_input)
    with flag_gems.use_gems():
        res_out = torch.fft.hfft(input_tensor)

    # hfft returns real output, not complex
    # For complex64 input -> float32 output, for complex128 input -> float64 output
...
def test_accuracy_fft_hfft_norm(shape, dtype, norm):
    """Test fft_hfft with different normalization modes."""
    input_len = shape[-1]
    batch_shape = shape[:-1] if len(shape) > 1 else ()
    output_len = 2 * (input_len - 1)
    ...
    ref_out = torch.fft.hfft(ref_input, norm=norm)
    with flag_gems.use_gems():
        res_out = torch.fft.hfft(input_tensor, norm=norm)
