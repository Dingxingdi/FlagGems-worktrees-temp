...
def test_accuracy_bernoulli_(shape, dtype):
    ...
    ref_inp = inp.clone()
    with flag_gems.use_gems():
        res_out = inp.bernoulli_()
    # Check outputs are only 0 or 1
    assert (res_out == 0).logical_or(res_out == 1).all()
    # Check shape is preserved
    assert res_out.shape == shape
    # In-place should return self
