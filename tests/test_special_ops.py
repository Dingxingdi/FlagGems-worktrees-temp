...
def scatter_nd_reference(input, indices, values, accumulate=False):
    """
    ...
    M = indices_np.shape[0]
    N = values_np.shape[1] if values_np.ndim > 1 else 1

    for m in range(M):
        idx = tuple(indices_np[m])
        if N == 1:
            if accumulate:
                input_np[idx] += values_np[m, 0]
            else:
                input_np[idx] = values_np[m, 0]
        else:
            for n in range(N):
                full_idx = idx + (n,)
                if accumulate:
                    input_np[full_idx] += values_np[m, n]
                else:
                    input_np[full_idx] = values_np[m, n]

    result = torch.from_numpy(input_np).to(input.device)
...
@pytest.mark.inplace
@pytest.mark.ScatterND_
@pytest.mark.parametrize("shape", SCATTER_ND_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_scatter_nd_(shape, dtype):
    M = 8
    rank = len(shape)

    indices = torch.randint(0, min(shape), (M, rank), dtype=torch.long, device=device)
    values = torch.randn(M, dtype=dtype, device=device)

    inp = torch.zeros(shape, dtype=dtype, device=device)

    ref_inp = inp.clone()
    ref_out = scatter_nd_reference(ref_inp, indices, values, accumulate=False)
    with flag_gems.use_gems():
        res_out = flag_gems.scatter_nd_(inp, indices, values)
