...
# FFT test shapes - powers of 2 are optimal for FFT
FFT_SHAPES = [
    (8,),
    (16,),
    (32,),
...
# Complex dtypes for FFT (only test dtypes supported by gems_assert_close)
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_fft_fft_real_input(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fft(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.fft(inp)

    # FFT output is complex, use the output dtype for comparison
...
def test_accuracy_fft_fft_complex_input(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
