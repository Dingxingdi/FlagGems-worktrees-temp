...
def test_accuracy_unsqueeze_copy(shape, dtype, dim):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)

    # Normalize dim for reference
    ndim = len(shape)
    ref_dim = dim if dim >= 0 else dim + ndim + 1
    ref_out = torch.unsqueeze_copy(ref_x, ref_dim)

    with flag_gems.use_gems():
        res_out = torch.unsqueeze_copy(x, dim)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.unsqueeze_copy_
@pytest.mark.parametrize("shape", [(2, 3), (128, 256)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, 1, 2, -1])
def test_accuracy_unsqueeze_copy_(shape, dtype, dim):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x.clone())
    ...
    ndim = len(shape)
    ref_dim = dim if dim >= 0 else dim + ndim + 1
    ref_out = ref_x.unsqueeze_copy_(ref_dim)

    with flag_gems.use_gems():
        res_out = x.unsqueeze_copy_(dim)
