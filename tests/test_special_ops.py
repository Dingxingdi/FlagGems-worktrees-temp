...
# _reshape_alias tests
@pytest.mark.reshape_alias
@pytest.mark.parametrize("shape", [(2, 3), (128, 256), (512, 512)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__reshape_alias(shape, dtype):
    # Test basic _reshape_alias functionality
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)

    # Reshape to a different shape with compatible size
    new_shape = (shape[0] * shape[1], 1)
    new_stride = (1, 1)

    ref_out = torch.ops.aten._reshape_alias(ref_x, new_shape, new_stride)
    with flag_gems.use_gems():
        act_out = torch.ops.aten._reshape_alias(x, new_shape, new_stride)

    # Check shape
    assert act_out.shape == ref_out.shape, f"Shape mismatch: {act_out.shape} vs {ref_out.shape}"
    # Check stride
    assert act_out.stride() == ref_out.stride(), f"Stride mismatch: {act_out.stride()} vs {ref_out.stride()}"
    # Check content
    gems_assert_close(act_out, ref_out, dtype)

    # Verify it returns a view (shares storage)
    assert act_out.untyped_storage().data_ptr() == x.untyped_storage().data_ptr(), \
        "Result should share storage with input"


@pytest.mark.reshape_alias
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__reshape_alias_with_different_strides(dtype):
    # Test with non-trivial strides
    x = torch.randn(2, 3, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)

    # Transpose: shape (3, 2), stride (1, 3)
    new_shape = (3, 2)
    new_stride = (1, 3)
    ...
    # Check shape
    assert act_out.shape == ref_out.shape
    # Check stride
    assert act_out.stride() == ref_out.stride()
    # Check content
    gems_assert_close(act_out, ref_out, dtype)
