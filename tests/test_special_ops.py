...
# convolution_backward tests
CONV_SHAPES = [
    (2, 3, 16, 16),  # Batch, In Channels, Height, Width
    (4, 8, 8, 8),
    (1, 16, 32, 32),
]
...
def test_accuracy_convolution_backward(shape, dtype, stride, padding):
    # Create input tensors for a convolution
    batch, in_c, h, w = shape
    out_c = 8
    kernel_size = 3
    groups = 1

    # Forward pass: create input and weight
    input = torch.randn(batch, in_c, h, w, dtype=dtype, device=flag_gems.device)
    weight = torch.randn(out_c, in_c // groups, kernel_size, kernel_size, dtype=dtype, device=flag_gems.device)

    # Run forward convolution to get output
    output = torch.nn.functional.conv2d(input, weight, stride=stride, padding=padding, groups=groups)
    ...
    ref_grad_input, ref_grad_weight, ref_grad_bias = torch.ops.aten.convolution_backward(
        ref_grad_output,
        ref_input,
        ref_weight,
        bias_sizes=None,
    ...
    # GEMS implementation
    with flag_gems.use_gems():
        res_grad_input, res_grad_weight, res_grad_bias = torch.ops.aten.convolution_backward(
            grad_output,
            input,
            weight,
            bias_sizes=None,
    ...
    # Use larger tolerance for float16 due to float32 accumulation in kernels
    if dtype == torch.float16:
        gems_assert_close(res_grad_input, ref_grad_input, dtype, atol=0.02)
        gems_assert_close(res_grad_weight, ref_grad_weight, dtype, atol=0.02)
    else:
        gems_assert_close(res_grad_input, ref_grad_input, dtype)
        gems_assert_close(res_grad_weight, ref_grad_weight, dtype)
