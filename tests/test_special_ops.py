...
def test_accuracy__conj(shape, dtype):
    if dtype == torch.complex32:
        real = torch.randn(shape, dtype=torch.float16, device=device)
        imag = torch.randn(shape, dtype=torch.float16, device=device)
    else:
        real = torch.randn(shape, dtype=torch.float32, device=device)
        imag = torch.randn(shape, dtype=torch.float32, device=device)
    input = torch.complex(real, imag)
    ...
    ref_input = to_reference(input)
    ref_out = torch.ops.aten._conj(ref_input)
    with flag_gems.use_gems():
        res_out = torch.ops.aten._conj(input)
