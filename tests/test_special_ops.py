...
# Test for split with integer split_size
@pytest.mark.split
@pytest.mark.parametrize(
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_split(shape, dim, split_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.split(ref_inp, split_size, dim)
    with flag_gems.use_gems():
        res_out = torch.split(inp, split_size, dim)

    assert len(res_out) == len(ref_out)
    for res_tensor, ref_tensor in zip(res_out, ref_out):
        gems_assert_equal(res_tensor, ref_tensor)


# Test for split with list of split_sizes
@pytest.mark.split
@pytest.mark.parametrize(
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_split_with_sizes(shape, dim, split_sizes, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.split(ref_inp, split_sizes, dim)
    with flag_gems.use_gems():
        res_out = torch.split(inp, split_sizes, dim)
...
@pytest.mark.parametrize(
    "shape,dim,split_size",
    [
        ((10, 32), 0, 3),  # Not evenly divisible
        ((10, 32), 1, 10),  # Single split
    ],
)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_split_edge_cases(shape, dim, split_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
