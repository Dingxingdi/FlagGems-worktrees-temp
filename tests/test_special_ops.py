...
# broadcast_tensors test shapes - pairs/tuples of shapes that can be broadcast together
BROADCAST_SHAPES = [
    ((2, 3), (2, 3)),  # same shape
    ((2, 1), (2, 3)),  # broadcast dim 1
    ((1, 3), (2, 3)),  # broadcast dim 0
    ((2, 1, 1), (2, 3, 4)),  # multi-dim broadcast
    ((1,), (2, 3)),  # scalar to 2d
    ((1, 1), (4, 3)),  # all dims broadcast
    ((1, 3, 4), (2, 3, 4)),  # complex broadcast
]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_broadcast_tensors(shapes, dtype):
    """Test broadcast_tensors with pairs of shapes that can be broadcast."""
    ...
    ref_inp_a = to_reference(inp_a)
    ref_inp_b = to_reference(inp_b)

    ref_out = torch.broadcast_tensors(ref_inp_a, ref_inp_b)
    with flag_gems.use_gems():
        res_out = torch.broadcast_tensors(inp_a, inp_b)

    assert len(res_out) == len(ref_out), f"Length mismatch: {len(res_out)} vs {len(ref_out)}"

    for i, (res, ref) in enumerate(zip(res_out, ref_out)):
        res_cpu = to_reference(res)
        ref_cpu = to_reference(ref)
        gems_assert_close(res_cpu, ref_cpu, dtype)


@pytest.mark.broadcast_tensors
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_broadcast_tensors_three_inputs(dtype):
    """Test broadcast_tensors with three inputs."""
    shape_a = (2, 1)
    shape_b = (1, 3)
    shape_c = (2, 3)
    ...
    inp_a = torch.randn(shape_a, dtype=dtype, device=flag_gems.device)
    inp_b = torch.randn(shape_b, dtype=dtype, device=flag_gems.device)
    inp_c = torch.randn(shape_c, dtype=dtype, device=flag_gems.device)

    ref_inp_a = to_reference(inp_a)
    ref_inp_b = to_reference(inp_b)
    ref_inp_c = to_reference(inp_c)

    ref_out = torch.broadcast_tensors(ref_inp_a, ref_inp_b, ref_inp_c)
    with flag_gems.use_gems():
        res_out = torch.broadcast_tensors(inp_a, inp_b, inp_c)
...
@pytest.mark.broadcast_tensors
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_broadcast_tensors_single_input(dtype):
    """Test broadcast_tensors with a single input."""
    shape = (2, 3)

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.broadcast_tensors(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.broadcast_tensors(inp)

    assert len(res_out) == 1, f"Expected length 1, got {len(res_out)}"
    assert len(ref_out) == 1, f"Expected ref length 1, got {len(ref_out)}"

    res_cpu = to_reference(res_out[0])
    ref_cpu = to_reference(ref_out[0])
    gems_assert_close(res_cpu, ref_cpu, dtype)
