...
# LDL Factorization test
# For LDL factorization, we need symmetric positive definite matrices
# We create them by generating random matrices and adding a diagonal offset
@pytest.mark.linalg_ldl_factor
@pytest.mark.parametrize("shape", [(4, 4), (8, 8), (16, 16), (32, 32)])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
@pytest.mark.parametrize("hermitian", [False, True])
def test_accuracy_linalg_ldl_factor(shape, dtype, hermitian):
    ...
    gems_assert_close(res_LD, ref_LD, dtype)
