...
# Reduction shapes for testing special_log_softmax
...
def test_accuracy_special_log_softmax(shape, dtype, dim):
    """Test special_log_softmax operator accuracy.

    special_log_softmax is mathematically equivalent to log_softmax,
    which computes log(softmax(x)) along the specified dimension.
    """
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, True)

    # Use torch.special.log_softmax as the reference
    ref_out = torch.special.log_softmax(ref_inp, dim=dim)
    with flag_gems.use_gems():
        res_out = torch.special.log_softmax(inp, dim=dim)
