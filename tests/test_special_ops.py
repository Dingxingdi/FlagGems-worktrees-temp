...
# Matmul_Layer_Norm fused operation tests
@pytest.mark.matmul_layernorm
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_matmul_layernorm(shape, dtype):
    ...
    atol = 0.01 if dtype == torch.float16 else (0.05 if dtype == torch.bfloat16 else 1e-4)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_matmul_layernorm_no_bias(shape, dtype):
    # Test without bias
    K = shape[-1]
    N = K
