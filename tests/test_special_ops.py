...
# Test for _local_scalar_dense operator
# This operator extracts the first element from a tensor and returns it as a Python scalar


@pytest.mark.local_scalar_dense
@pytest.mark.parametrize("shape", [(1,), (3,), (2, 3), (1, 2, 3), (1, 2, 3, 4)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy__local_scalar_dense(shape, dtype):
    # Create input tensor with known first element value
    # Use a fixed value (42 or 42.0) as the first element
    if dtype in FLOAT_DTYPES:
    ...
    # For scalar comparison, we just check equality
    assert res_out == ref_out, f"Expected {ref_out}, got {res_out}"


@pytest.mark.local_scalar_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy__local_scalar_dense_scalar_tensor(dtype):
    # Test with scalar tensor (0-d)
    if dtype in FLOAT_DTYPES:
        inp = torch.tensor(42.0, dtype=dtype, device=flag_gems.device)
    else:
        inp = torch.tensor(42, dtype=dtype, device=flag_gems.device)
