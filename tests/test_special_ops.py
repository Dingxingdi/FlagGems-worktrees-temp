...
# Test for mkldnn_rnn_layer operator
@pytest.mark.mkldnn_rnn_layer
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_mkldnn_rnn_layer_lstm(dtype):
    """Test mkldnn_rnn_layer for LSTM mode (mode=2), single layer, non-bidirectional."""
    ...
    hx = torch.zeros(num_layers, batch_size, hidden_size, dtype=dtype, device=flag_gems.device)
    cx = torch.zeros(num_layers, batch_size, hidden_size, dtype=dtype, device=flag_gems.device)

    # Create weights (LSTM uses 4 * hidden_size for each weight matrix)
    weight0 = torch.randn(4 * hidden_size, input_size, dtype=dtype, device=flag_gems.device)
    weight1 = torch.randn(4 * hidden_size, hidden_size, dtype=dtype, device=flag_gems.device)
    weight2 = torch.empty(0, dtype=dtype, device=flag_gems.device)
    weight3 = torch.empty(0, dtype=dtype, device=flag_gems.device)

    # Reference implementation using PyTorch - run on CPU then move to GPU for comparison
    # Note: mkldnn_rnn_layer on CPU works for float32 but may not work for all dtypes
    ref_input_cpu = input_tensor.cpu()
    ref_hx_cpu = hx.cpu()
    ref_cx_cpu = cx.cpu()
    ref_weight0_cpu = weight0.cpu()
    ref_weight1_cpu = weight1.cpu()

    ref_output_cpu, ref_hy_cpu, ref_cy_cpu, _ = torch.mkldnn_rnn_layer(
        ref_input_cpu, ref_weight0_cpu, ref_weight1_cpu, weight2.cpu(), weight3.cpu(),
        ref_hx_cpu, ref_cx_cpu,
        reverse=False,
        batch_sizes=[],
        mode=2,  # LSTM
    ...
    # Move reference to GPU for comparison
    ref_output = ref_output_cpu.to(flag_gems.device)
    ref_hy = ref_hy_cpu.to(flag_gems.device)
    ref_cy = ref_cy_cpu.to(flag_gems.device)

    # Test with FlagGems
    with flag_gems.use_gems():
        res_output, res_hy, res_cy, _ = flag_gems.mkldnn_rnn_layer(
            input_tensor, weight0, weight1, weight2, weight3,
            hx, cx,
            reverse=False,
            batch_sizes=[],
            mode=2,
    ...
    gems_assert_close(res_hy, ref_hy, dtype)
    gems_assert_close(res_cy, ref_cy, dtype)
