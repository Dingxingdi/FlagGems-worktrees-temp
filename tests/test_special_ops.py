...
def test_accuracy__make_dep_token(dtype):
    ...
    Note: _make_dep_token is a special operator that doesn't have CUDA implementation
    in PyTorch. FlagGems provides a CUDA implementation, but the device depends on
    whether flag_gems.use_gems() properly redirects the operator.
    """
    with flag_gems.use_gems():
        res_out = torch.ops.aten._make_dep_token(dtype=dtype)

    # Check shape is scalar (0-dimensional)
...
@pytest.mark.make_dep_token
def test_accuracy__make_dep_token_default():
    """Test _make_dep_token with default parameters."""
    with flag_gems.use_gems():
        res_out = torch.ops.aten._make_dep_token()

    # Default dtype should be float32
    assert res_out.dtype == torch.float32, f"Expected default dtype float32, got {res_out.dtype}"
...
@pytest.mark.make_dep_token
def test_accuracy__make_dep_token_gems_impl():
    """Test _make_dep_token with FlagGems implementation directly."""
    # Test direct call to FlagGems implementation
    res_out = flag_gems.ops._make_dep_token()

    # Check shape is scalar (0-dimensional)
    assert res_out.shape == torch.Size([]), f"Expected scalar tensor, got shape {res_out.shape}"

    # Check dtype is float32 by default
    assert res_out.dtype == torch.float32, f"Expected default dtype float32, got {res_out.dtype}"
