...
# _grouped_mm is a grouped matrix multiplication operator
# Since torch._grouped_mm doesn't exist in PyTorch 2.6.0, we test against torch.matmul
GROUPED_MM_SHAPES = [
    (16, 32),
    (32, 64),
    (64, 128),
...
@pytest.mark.parametrize("shape", GROUPED_MM_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__grouped_mm(shape, dtype):
    M, K = shape
    ...
    with flag_gems.use_gems():
        res_out = flag_gems._grouped_mm(a, b)
