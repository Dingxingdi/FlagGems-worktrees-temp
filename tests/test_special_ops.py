...
# IM2COL operator tests
IM2COL_SHAPES = [(3, 8, 8), (1, 3, 16, 16), (16, 64, 64), (32, 128, 128)]
IM2COL_DTYPES = [torch.float32, torch.float16, torch.bfloat16]
IM2COL_CONFIGS = [
    ((3, 3), (1, 1), (1, 1), (1, 1)),
    ((3, 3), (1, 1), (0, 0), (2, 2)),
    ((5, 4), (2, 2), (2, 1), (1, 2)),
...
@pytest.mark.parametrize("dtype", IM2COL_DTYPES)
@pytest.mark.parametrize(
    "kernel_size, dilation, padding, stride", IM2COL_CONFIGS
)
def test_accuracy_im2col(shape, dtype, kernel_size, dilation, padding, stride):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)
