...
def test_accuracy_permute(shape, dtype):
    # Generate all possible permutations for the given shape
    ndim = len(shape)
    ...
    # Test a subset of permutations for efficiency
    all_perms = list(permutations(range(ndim)))
    # For higher dimensions, only test a few permutations
    if ndim > 3:
        test_perms = [all_perms[0], all_perms[len(all_perms) // 2], all_perms[-1]]
    else:
        test_perms = all_perms

    for dims in test_perms:
