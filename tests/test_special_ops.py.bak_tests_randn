...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_moe_gate_topk_routing(num_tokens, num_experts, topk, dtype):
    if topk > num_experts:
    ...
    ref_gate_logits = to_reference(gate_logits)
    ...
    res_topk_weights, res_topk_indices = flag_gems.MoE_Gate_TopK_Routing(
        gate_logits, topk
    )

    # Compare indices - convert both to same type for comparison
    ref_topk_indices = ref_topk_indices.to(res_topk_indices.dtype)
    gems_assert_equal(res_topk_indices, ref_topk_indices)

    # Compare weights (both should be float32 now)
    # Use larger tolerance for float16/bfloat16 due to numerical precision
    if dtype == torch.float32:
        gems_assert_close(res_topk_weights, ref_topk_weights, torch.float32)
    elif dtype == torch.float16:
        gems_assert_close(res_topk_weights, ref_topk_weights, torch.float32, atol=5e-4)
    else:  # bfloat16
        gems_assert_close(res_topk_weights, ref_topk_weights, torch.float32, atol=2e-3)
