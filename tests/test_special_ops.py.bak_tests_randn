...
# Define shapes for view_as tests
VIEW_AS_SHAPES = [(2, 3), (3, 2), (2, 2, 3), (3, 2, 2), (1, 6), (6, 1), (3, 4), (4, 3), (2, 3, 4), (4, 3, 2)]


@pytest.mark.view_as
@pytest.mark.parametrize("shape", VIEW_AS_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_view_as(shape, dtype):
    """Test view_as operator accuracy"""
    # Create input tensor with random values
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Create target shape with same number of elements
    numel = inp.numel()
    # Use a different shape with same number of elements
    if len(shape) == 2:
        target_shape = (shape[1], shape[0])
    elif len(shape) == 3:
        target_shape = (shape[2], shape[1], shape[0])
    else:
        target_shape = (numel,)
    ...
    ref_out = torch.ops.aten.view_as(ref_inp, ref_target)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.view_as(inp, target)

    # Check that shapes match
    assert res_out.shape == ref_out.shape
    # Check that it's a view (shares storage)
...
@pytest.mark.view_as
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_view_as_flatten(dtype):
    """Test view_as operator with flatten operation"""
    shape = (2, 3, 4)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Target shape: flatten to 1D
    target_shape = (24,)
    target = torch.randn(target_shape, dtype=dtype, device=flag_gems.device)
    ref_target = to_reference(target)
