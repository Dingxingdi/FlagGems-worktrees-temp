...
# Reshape shapes for testing: (input_shape, target_shape)
RESHAPE_SHAPES = (
    [(2, 19, 7)]
    if QUICK_MODE
    else [
...
@pytest.mark.reshape
@pytest.mark.parametrize("shapes", RESHAPE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_reshape(shapes, dtype):
    input_shape, target_shape = shapes
    inp = torch.randn(input_shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.reshape(ref_inp, target_shape)
    with flag_gems.use_gems():
        res_out = torch.reshape(inp, target_shape)
...
@pytest.mark.reshape
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_reshape_infer_dim(dtype):
    # Test with -1 dimension
    inp = torch.randn((2, 6), dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # -1 should be inferred as 4
    ref_out = torch.reshape(ref_inp, (3, -1))
    with flag_gems.use_gems():
        res_out = torch.reshape(inp, (3, -1))

    assert res_out.shape == (3, 4), f"Expected shape (3, 4), got {res_out.shape}"
    gems_assert_close(res_out, ref_out, dtype)


# Test reshape from non-contiguous tensor (requires copy)
@pytest.mark.reshape
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_reshape_non_contiguous(dtype):
    # Create a non-contiguous tensor by slicing
    inp = torch.randn((4, 6), dtype=dtype, device=flag_gems.device)
    inp = inp[:, ::2]  # Now it's non-contiguous
    assert not inp.is_contiguous()

    ref_inp = to_reference(inp)
    ref_out = torch.reshape(ref_inp, (2, 6))
    with flag_gems.use_gems():
        res_out = torch.reshape(inp, (2, 6))
