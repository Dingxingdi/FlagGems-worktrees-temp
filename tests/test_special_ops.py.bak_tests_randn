...
EMBEDDING_BAG_BAG_SIZES = [(2, 5), (3, 7), (1, 10)] if not QUICK_MODE else [(2, 5)]
EMBEDDING_BAG_EMBEDDING_DIMS = [32, 64] if not QUICK_MODE else [32]
EMBEDDING_BAG_NUM_WEIGHTS = [1024, 2048] if not QUICK_MODE else [1024]
EMBEDDING_BAG_MODES = [0, 1, 2]


@pytest.mark.embedding_bag_backward
@pytest.mark.parametrize("bag_sizes", EMBEDDING_BAG_BAG_SIZES)
@pytest.mark.parametrize("embedding_dim", EMBEDDING_BAG_EMBEDDING_DIMS)
@pytest.mark.parametrize("num_weights", EMBEDDING_BAG_NUM_WEIGHTS)
@pytest.mark.parametrize("mode", EMBEDDING_BAG_MODES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__embedding_bag_backward(
    bag_sizes, embedding_dim, num_weights, mode, dtype
):
    """Test _embedding_bag_backward accuracy against PyTorch implementation."""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    # Create embedding bag forward pass to get inputs for backward
    num_bags, indices_per_bag = bag_sizes
    total_indices = num_bags * indices_per_bag
    ...
    # Create indices (random embedding indices)
    indices = torch.randint(
        0, num_weights, (total_indices,), device=flag_gems.device, dtype=torch.long
    )

    # Create offsets (starting positions for each bag plus final offset)
    offsets = torch.arange(
        0, total_indices + 1, indices_per_bag, device=flag_gems.device, dtype=torch.long
    )

    # Run forward to get all backward inputs
    out, offset2bag, bag_size, max_indices = torch.ops.aten._embedding_bag(
    ...
    # Note: PyTorch's embedding_bag_backward for max mode (mode=2) doesn't support bfloat16 on CUDA
    try:
        ref_out = torch.ops.aten._embedding_bag_backward(
            grad,
            indices,
            offsets,
            offset2bag,
        ...
        if "not implemented for 'BFloat16'" in str(e) and mode == 2:
            pytest.skip(f"PyTorch reference doesn't support max mode with bfloat16: {e}")
        raise

    # Compute flag_gems backward
    with flag_gems.use_gems():
        res_out = torch.ops.aten._embedding_bag_backward(
            grad,
            indices,
            offsets,
            offset2bag,
