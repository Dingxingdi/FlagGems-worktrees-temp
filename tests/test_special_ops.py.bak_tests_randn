...
# Define matrix shapes for Cholesky (square matrices)
...
def test_accuracy_linalg_cholesky_ex(shape, dtype):
    ...
    # Compare L matrices
    gems_assert_close(res_out.L, ref_out.L, dtype)

    # Compare info (should be 0 for positive definite matrix)
    gems_assert_equal(res_out.info, ref_out.info)
