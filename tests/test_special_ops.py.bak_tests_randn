...
def test_accuracy_shape(shape):
    ...
    # Get shape using our implementation
    res_out = shape_fn(inp)

    # Reference: use tensor.shape
    ref_out = torch.tensor(list(inp.shape), dtype=torch.int64, device=flag_gems.device)

    gems_assert_equal(res_out, ref_out)


@pytest.mark.shape
def test_accuracy_shape_scalar():
    from flag_gems.ops.shape import shape as shape_fn

    # Test scalar tensor (0-D)
    inp = torch.tensor(3.14, device=flag_gems.device)

    # Get shape using our implementation
    res_out = shape_fn(inp)

    # Reference: scalar tensor has empty shape
    ref_out = torch.tensor([], dtype=torch.int64, device=flag_gems.device)
