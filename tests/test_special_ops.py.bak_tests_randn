...
@pytest.mark.parametrize("M, K, N", WEIGHT_INT4PACK_MM_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_weight_int4pack_mm(M, K, N, dtype):
    """Test _weight_int4pack_mm accuracy against manual reference."""
    if dtype == torch.bfloat16 and not torch.cuda.is_bf16_supported():
        pytest.skip("bfloat16 not supported")

    qGroupSize = 32  # quantization group size

    # Create activation (M, K)
    act = torch.randn(M, K, dtype=dtype, device=flag_gems.device)

    # Create int4 weight in packed format (N, K/2)
    # Each byte stores 2 int4 values (0-15)
    weight = torch.randint(0, 16, (N, K // 2), dtype=torch.uint8, device=flag_gems.device)

    # Create scale (per-channel)
    scale = torch.ones(N, dtype=dtype, device=flag_gems.device) * 0.1

    # Reference: manually compute the expected output
    ...
    # 2. Dequantize: weight_fp = weight_int4 * scale
    weight_dequant = weight_unpacked * scale.to(torch.float32).view(N, 1)

    # 3. Matmul: output = act @ weight_dequant.T (weight is stored as (N, K), need to transpose)
    ref_out = torch.matmul(act.to(torch.float32), weight_dequant.T).to(dtype)

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = torch._weight_int4pack_mm(act, weight, qGroupSize, scale)

    # Compare results
    # Use wider tolerance for int4 quantization
