...
# Test for linalg_tensorsolve
# For linalg.tensorsolve(A, B, dims=None):
# - A shape: (*B.shape[:B.ndim], m, n) where prod(B.shape[:B.ndim]) == prod(A.shape[B.ndim:])
# - B shape: (*B.shape[:B.ndim])
# - X shape: A.shape[B.ndim:]
# - Solves tensordot(A, X, dims=X.ndim) = B


@pytest.mark.linalg_tensorsolve
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_tensorsolve_simple(dtype):
...
@pytest.mark.linalg_tensorsolve
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_tensorsolve_3d(dtype):
