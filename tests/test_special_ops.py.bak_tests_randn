...
# FFT C2C tests
# Use small power-of-2 sizes for testing (DFT-based implementation is O(N^2))
FFT_C2C_SHAPES = [
    (8,),
    (16,),
    (32,),
...
@pytest.mark.fft_c2c
@pytest.mark.parametrize("shape", FFT_C2C_SHAPES)
@pytest.mark.parametrize("dtype", [torch.complex64])
def test_accuracy_fft_c2c_1d(shape, dtype):
    """Test 1D FFT C2C accuracy"""
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    dim = [0]
    # Test all normalizations: 0=none, 1=forward, 2=backward
    for normalization in [0, 1, 2]:
        for forward in [True, False]:
            ref_out = torch._fft_c2c(ref_input, dim, normalization, forward)
            with flag_gems.use_gems():
                res_out = torch._fft_c2c(input_tensor, dim, normalization, forward)

            # Use more lenient tolerance for FFT
            gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.fft_c2c
@pytest.mark.parametrize("shape", FFT_C2C_2D_SHAPES)
@pytest.mark.parametrize("dtype", [torch.complex64])
@pytest.mark.parametrize("dim", [[0], [1]])
def test_accuracy_fft_c2c_2d(shape, dtype, dim):
    """Test 2D FFT C2C accuracy"""
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_input = to_reference(input_tensor)

    # Test with normalization=2 (backward) which is most common
    normalization = 2
    for forward in [True, False]:
        ref_out = torch._fft_c2c(ref_input, dim, normalization, forward)
        with flag_gems.use_gems():
