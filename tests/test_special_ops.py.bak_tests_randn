...
# FFT irfft tests
# Test shapes for FFT (power of 2 for our implementation)
FFT_SHAPES = [
    (16,),
    (32,),
    (64,),
...
FFT_DTYPES = [torch.complex64, torch.complex128]


@pytest.mark.fft_irfft
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_irfft(shape, dtype):
    # Use power of 2 for FFT size along our implementation
    n_fft = 64  # Power of 2
    dim = -1

    # Create real input first, then use rfft to get the correct half-Hermitian format
    input_shape = list(shape)
    input_shape[dim] = n_fft

    # Map complex dtype to real dtype
    real_dtype = torch.float32 if dtype == torch.complex64 else torch.float64

    # Create real input
    real_input = torch.randn(input_shape, dtype=real_dtype, device=flag_gems.device)

    # Get half-Hermitian complex input via rfft
    inp = torch.fft.rfft(real_input, dim=dim)

    ref_inp = to_reference(inp)

    ref_out = torch.fft.irfft(ref_inp, n=n_fft, dim=dim)
    with flag_gems.use_gems():
        res_out = torch.fft.irfft(inp, n=n_fft, dim=dim)

    # Output is real, use the real dtype for comparison
    if dtype == torch.complex64:
        gems_assert_close(res_out, ref_out, real_dtype, atol=1e-2)
    else:
        gems_assert_close(res_out, ref_out, real_dtype)


@pytest.mark.fft_irfft
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_irfft_2d(shape, dtype):
    # Test with different dim parameter
    n_fft = 64
    dim = 0

    # Create real input first
    input_shape = list(shape)
    input_shape[dim] = n_fft
