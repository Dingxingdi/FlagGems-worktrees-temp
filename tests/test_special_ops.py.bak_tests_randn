...
# Matrix shapes for linalg_matrix_rank tests
LINALG_MATRIX_SHAPES = [
    (4, 4),
    (8, 8),
    (16, 16),
    (32, 32),
...
@pytest.mark.parametrize("shape", LINALG_MATRIX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_rank(shape, dtype):
    # linalg_matrix_rank supports float, double, cfloat, cdouble
    # Note: float16 is not supported by torch.linalg.svdvals on CUDA
    if dtype not in (torch.float32, torch.float64):
        pytest.skip("linalg_matrix_rank: torch.linalg.svdvals does not support float16 on CUDA")

    if flag_gems.vendor_name == "kunlunxin" and dtype == torch.bfloat16:
        pytest.skip("kunlunxin bfloat16 precision issue")

    # Create input tensor - need at least 2D
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.linalg.matrix_rank(ref_inp)
    # Import the FlagGems implementation directly since the aten dispatch has issues
    from flag_gems.ops.linalg_matrix_rank import linalg_matrix_rank as gems_linalg_matrix_rank
    with flag_gems.use_gems():
        # Monkey-patch to ensure our implementation is used
        torch.linalg.matrix_rank = gems_linalg_matrix_rank
        res_out = torch.linalg.matrix_rank(inp)
...
@pytest.mark.parametrize("shape", LINALG_MATRIX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_matrix_rank_with_tol(shape, dtype):
    # Note: float16 is not supported by torch.linalg.svdvals on CUDA
    if dtype not in (torch.float32, torch.float64):
        pytest.skip("linalg_matrix_rank: torch.linalg.svdvals does not support float16 on CUDA")
    ...
    tol = 0.1
    ref_out = torch.linalg.matrix_rank(ref_inp, tol=tol)
    # Import the FlagGems implementation directly since the aten dispatch has issues
    from flag_gems.ops.linalg_matrix_rank import linalg_matrix_rank as gems_linalg_matrix_rank
    with flag_gems.use_gems():
        # Monkey-patch to ensure our implementation is used
        torch.linalg.matrix_rank = gems_linalg_matrix_rank
        res_out = torch.linalg.matrix_rank(inp, tol=tol)
