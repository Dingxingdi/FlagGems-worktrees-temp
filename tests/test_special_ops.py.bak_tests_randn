...
# Define view test shapes - source shapes that can be reshaped to different target shapes
VIEW_SHAPES = [(2, 6), (3, 4), (2, 3, 4), (4, 6), (2, 2, 3)]
VIEW_TARGET_SHAPES = [(6, 2), (4, 3), (2, 12), (3, 4), (4, 3, 2)]


@pytest.mark.view
@pytest.mark.parametrize("src_shape,target_shape", zip(VIEW_SHAPES, VIEW_TARGET_SHAPES))
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_view(src_shape, target_shape, dtype):
    # Create input tensor with enough elements for target shape
    num_elements = 1
    for dim in target_shape:
        num_elements *= dim
    src_shape = (num_elements // 6, 6) if num_elements > 6 else (1, num_elements)

    inp = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = ref_inp.view(*target_shape)
    with flag_gems.use_gems():
        res_out = inp.view(*target_shape)
