...
def test_accuracy_lstm(batch_size, seq_len, input_size, hidden_size, num_layers, dtype):
    """Test LSTM accuracy against PyTorch reference."""
    torch.manual_seed(42)
    torch.cuda.manual_seed_all(42)

    # Create LSTM layer - move to device with correct dtype
    lstm = torch.nn.LSTM(
    ...
    # Create input
    input = torch.randn(batch_size, seq_len, input_size, device=flag_gems.device, dtype=dtype)

    # Get weights and reshape for aten.lstm (they're already on device and correct dtype)
    weights = lstm.all_weights[0]
    params = tuple([w.contiguous().flatten() for w in weights])

    # Initial hidden states
    h0 = torch.randn(num_layers, batch_size, hidden_size, device=flag_gems.device, dtype=dtype)
    c0 = torch.randn(num_layers, batch_size, hidden_size, device=flag_gems.device, dtype=dtype)

    # Reference (PyTorch) - use positional args
    ref_input = to_reference(input)
    ref_h0 = to_reference(h0)
    ref_c0 = to_reference(c0)
    # Move params to reference (CPU) with correct dtype
    ref_params = tuple([p.cpu().to(dtype=ref_input.dtype, device=ref_input.device) for p in params])
    ref_out, ref_hn, ref_cn = torch.lstm(
        ref_input, (ref_h0, ref_c0), ref_params, True, 1, 0.0, False, False, True
    )

    # GEMS implementation - use positional args
    with flag_gems.use_gems():
        res_out, res_hn, res_cn = torch.lstm(
            input, (h0, c0), params, True, 1, 0.0, False, False, True
        )

    gems_assert_close(res_out, ref_out, dtype)
    gems_assert_close(res_hn, ref_hn, dtype)
    gems_assert_close(res_cn, ref_cn, dtype)
