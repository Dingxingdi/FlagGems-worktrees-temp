...
def test_accuracy_fft_c2r(shape, dtype):
    # Create complex input - use simple rand instead of randn
    inp = torch.rand(shape, dtype=dtype, device=flag_gems.device)

    # Get the FFT dimension - use positive index
    dim = len(shape) - 1 if len(shape) > 1 else 0
    n_fft = shape[dim]

    # Output dtype is real (float32 for complex64, float64 for complex128)
    output_dtype = torch.float32 if dtype == torch.complex64 else torch.float64

    # Use torch.ops.aten to compute reference directly (avoid any interception)
    ref_out = torch.ops.aten._fft_c2r(inp, [dim], 0, n_fft)

    # Compute result using FlagGems
    with flag_gems.use_gems():
        res_out = torch.ops.aten._fft_c2r(inp, [dim], 0, n_fft)

    # Compare output dtypes (which are real, not complex)
...
def test_accuracy_fft_c2r_normalization(shape, dtype, normalization):
    ...
    # Use torch.ops.aten to compute reference directly
    ref_out = torch.ops.aten._fft_c2r(inp, [dim], normalization, n_fft)

    # Compute result using FlagGems
    with flag_gems.use_gems():
        res_out = torch.ops.aten._fft_c2r(inp, [dim], normalization, n_fft)
