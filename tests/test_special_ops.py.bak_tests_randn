...
@pytest.mark.parametrize("params", TRIU_INDICES_PARAMS)
def test_accuracy_triu_indices(params):
    row, col, offset = params
    ref_out = torch.triu_indices(row, col, offset)
    with flag_gems.use_gems():
        res_out = torch.triu_indices(row, col, offset)
...
def test_accuracy_triu_indices_dtype(dtype):
    row, col, offset = 5, 5, 0
    ref_out = torch.triu_indices(row, col, offset, dtype=dtype)
    with flag_gems.use_gems():
        res_out = torch.triu_indices(row, col, offset, dtype=dtype)
