...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("factor", [2, 3])
def test_accuracy_upsample_nearest_exact3d(shape, dtype, factor):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)
    out_size = [shape[2] * factor, shape[3] * factor, shape[4] * factor]
    ref_out = torch.ops.aten._upsample_nearest_exact3d(ref_x, out_size, None, None, None)
    with flag_gems.use_gems():
        res_out = torch.ops.aten._upsample_nearest_exact3d(x, out_size, None, None, None)
    gems_assert_close(res_out, ref_out, dtype)
