...
SPLIT_WITH_SIZES_DIMS = [0, 1, -1]


@pytest.mark.unsafe_split_with_sizes
@pytest.mark.parametrize("shape", SPLIT_WITH_SIZES_SHAPES)
@pytest.mark.parametrize("dim", SPLIT_WITH_SIZES_DIMS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_unsafe_split_with_sizes(shape, dim, dtype):
    # Normalize dim to handle negative indices
    ndim = len(shape)
    dim = dim % ndim if ndim > 0 else 0
    # Create split sizes that sum to the dimension size
    dim_size = shape[dim]
    num_splits = 3
    split_sizes = [dim_size // num_splits] * num_splits
    # Adjust last split to match exact size
    split_sizes[-1] = dim_size - sum(split_sizes[:-1])

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.unsafe_split_with_sizes(ref_inp, split_sizes, dim)
    with flag_gems.use_gems():
        res_out = torch.unsafe_split_with_sizes(inp, split_sizes, dim)
...
@pytest.mark.unsafe_split_with_sizes
@pytest.mark.parametrize("shape", [(10,), (12, 4), (24, 8, 4)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_unsafe_split_with_sizes_default_dim(shape, dtype):
    ...
    ref_out = torch.unsafe_split_with_sizes(ref_inp, split_sizes)
    with flag_gems.use_gems():
        res_out = torch.unsafe_split_with_sizes(inp, split_sizes)
