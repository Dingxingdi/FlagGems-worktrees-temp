...
# Diff operator supports up to 4D tensors
SPECIAL_SHAPES_4D = (
    [(2, 19, 7)]
    if QUICK_MODE
    else [(1,), (1024, 1024), (20, 320, 15), (16, 128, 64, 60)]
)
...
@pytest.mark.diff
@pytest.mark.parametrize("shape", SPECIAL_SHAPES_4D)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy_diff(shape, dtype):
    ...
    if dtype in FLOAT_DTYPES:
        ...
        inp = torch.randint(-10, 10, shape, dtype=dtype, device="cpu").to(
            flag_gems.device
        )
    ref_inp = to_reference(inp)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES_4D)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy_diff_dim(shape, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    for d in range(inp.ndim):
        ref_out = torch.diff(ref_inp, dim=d)
        with flag_gems.use_gems():
            res_out = torch.diff(inp, dim=d)
        gems_assert_close(res_out, ref_out, dtype)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES_4D)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diff_n(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_diff_prepend_append(shape, dtype):
    ...
    # Test with prepend
    if inp.ndim >= 1:
        prepend = torch.randn(shape[:-1] + (2,), dtype=dtype, device=flag_gems.device)
    ...
    # Test with append
    append = torch.randn(shape[:-1] + (3,), dtype=dtype, device=flag_gems.device)
