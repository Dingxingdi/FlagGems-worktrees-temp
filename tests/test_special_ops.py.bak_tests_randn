...
def test_accuracy_linalg_lu(shape, dtype):
    if dtype == torch.float16 and shape[0] > 32:
        pytest.skip("Skipping large shape for float16")
    ...
    k = min(m, n)
    assert P.shape == (m, m), f"P shape mismatch: {P.shape} vs {(m, m)}"
    assert L.shape == (m, k), f"L shape mismatch: {L.shape} vs {(m, k)}"
    assert U.shape == (k, n), f"U shape mismatch: {U.shape} vs {(k, n)}"

    # Verify P is a permutation matrix (rows sum to 1)
    ones = torch.ones(m, dtype=dtype, device=flag_gems.device)
    assert torch.allclose(P.sum(dim=1), ones, atol=1e-3), "P is not a permutation matrix"


@pytest.mark.linalg_lu
@pytest.mark.parametrize("shape", LU_BATCH_SHAPES)
@pytest.mark.parametrize("dtype", LU_DTYPES)
def test_accuracy_linalg_lu_batch(shape, dtype):
    if dtype == torch.float16 and shape[1] > 64:
        pytest.skip("Skipping large batch shape for float16")
    ...
    # Verify output shapes
    k = min(m, n)
    assert P.shape == (batch, m, m), f"P shape mismatch: {P.shape} vs {(batch, m, m)}"
    assert L.shape == (batch, m, k), f"L shape mismatch: {L.shape} vs {(batch, m, k)}"
    assert U.shape == (batch, k, n), f"U shape mismatch: {U.shape} vs {(batch, k, n)}"
