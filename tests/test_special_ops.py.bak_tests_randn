...
# MoE_Expert_Dispatch tests
@pytest.mark.moe_expert_dispatch
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_moe_expert_dispatch(num_tokens, hidden_dim, num_experts, dtype):
    """Test moe_expert_dispatch reorders tokens correctly based on expert_ids."""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    # Create input tensor
    hidden_states = torch.randn(
        (num_tokens, hidden_dim), dtype=dtype, device=flag_gems.device
    )

    # Create expert_ids - randomly assign each token to an expert
    expert_ids = torch.randint(0, num_experts, (num_tokens,), device=flag_gems.device)

    # Reference implementation: use torch.argsort to get sorted indices
    ref_sorted_indices = torch.argsort(expert_ids, stable=True)
    ref_output = hidden_states.index_select(0, ref_sorted_indices)

    # Test with FlagGems
    with flag_gems.use_gems():
        res_output, res_sorted_indices = flag_gems.moe_expert_dispatch(
            hidden_states.clone(), expert_ids
        )

    # Verify output is correctly reordered
    gems_assert_close(res_output, ref_output, dtype)

    # Verify sorted indices match
    gems_assert_equal(res_sorted_indices, ref_sorted_indices)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_moe_expert_dispatch_(num_tokens, hidden_dim, num_experts, dtype):
    """Test moe_expert_dispatch_ in-place reordering."""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
    ...
    ref_sorted_indices = torch.argsort(expert_ids, stable=True)
    ref_output = hidden_states_copy.index_select(0, ref_sorted_indices)

    # Test in-place version with FlagGems
    with flag_gems.use_gems():
        res_output = flag_gems.moe_expert_dispatch_(hidden_states.clone(), expert_ids)
