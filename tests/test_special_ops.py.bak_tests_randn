...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_lift_fresh(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    ref_out = torch.ops.aten.lift_fresh(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.lift_fresh(inp)
    # lift_fresh returns the same tensor object
    assert res_out is inp, "lift_fresh should return the same tensor object"
    gems_assert_equal(res_out, ref_out)
