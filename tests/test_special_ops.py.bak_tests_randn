...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_thnn_fused_lstm_cell(shape, dtype):
    ...
    atol = 1e-2 if dtype == torch.bfloat16 else (1.5e-3 if dtype == torch.float16 else 1e-4)
    gems_assert_close(res_hy, ref_hy, dtype, atol=atol)
    gems_assert_close(res_cy, ref_cy, dtype, atol=atol)


@pytest.mark.thnn_fused_lstm_cell
@pytest.mark.parametrize("shape", LSTM_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_thnn_fused_lstm_cell_with_bias(shape, dtype):
