...
# pairwise_distance shapes
PAIRWISE_DISTANCE_SHAPES = [(2, 32), (4, 16), (8, 64)]


@pytest.mark.pairwise_distance
@pytest.mark.parametrize("shape", PAIRWISE_DISTANCE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_pairwise_distance(shape, dtype):
    ...
    ref_out = torch.nn.functional.pairwise_distance(ref_x1, ref_x2)
    with flag_gems.use_gems():
        res_out = torch.nn.functional.pairwise_distance(x1, x2)
...
def test_accuracy_pairwise_distance_p(shape, dtype, p):
    # Skip p=1.0 due to precision issues in computation
    if p == 1.0:
        pytest.skip("p=1.0 has precision issues")
    x1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    x2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.nn.functional.pairwise_distance(ref_x1, ref_x2, p=p)
    with flag_gems.use_gems():
        res_out = torch.nn.functional.pairwise_distance(x1, x2, p=p)
...
def test_accuracy_pairwise_distance_keepdim(shape, dtype, keepdim):
    x1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    x2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.nn.functional.pairwise_distance(ref_x1, ref_x2, keepdim=keepdim)
    with flag_gems.use_gems():
        res_out = torch.nn.functional.pairwise_distance(x1, x2, keepdim=keepdim)
