...
# Square matrix shapes for linalg_eigh
LINALG_EIGH_SHAPES = [
    (4, 4),
    (8, 8),
    (16, 16),
...
@pytest.mark.parametrize("shape", LINALG_EIGH_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_eigh(shape, dtype):
    """Test linalg_eigh accuracy against torch implementation."""
    # Create a symmetric positive definite matrix
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    A = A @ A.T + torch.eye(shape[0], dtype=dtype, device=flag_gems.device) * 0.1

    ref_A = to_reference(A)

    ref_out = torch._linalg_eigh(ref_A, "L")
    with flag_gems.use_gems():
        res_out = torch._linalg_eigh(A, "L")

    # Compare eigenvalues
    gems_assert_close(res_out.eigenvalues, ref_out.eigenvalues, dtype)
    # Compare eigenvectors
    gems_assert_close(res_out.eigenvectors, ref_out.eigenvectors, dtype)


@pytest.mark.linalg_eigh
@pytest.mark.parametrize("shape", LINALG_EIGH_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_eigh_uplo(shape, dtype):
    """Test linalg_eigh with UPLO='U'."""
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    A = A @ A.T + torch.eye(shape[0], dtype=dtype, device=flag_gems.device) * 0.1

    ref_A = to_reference(A)

    ref_out = torch._linalg_eigh(ref_A, "U")
    with flag_gems.use_gems():
        res_out = torch._linalg_eigh(A, "U")
...
@pytest.mark.parametrize("shape", LINALG_EIGH_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_eigh_compute_v_false(shape, dtype):
    """Test linalg_eigh with compute_v=False."""
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    A = A @ A.T + torch.eye(shape[0], dtype=dtype, device=flag_gems.device) * 0.1

    ref_A = to_reference(A)

    ref_out = torch._linalg_eigh(ref_A, "L", compute_v=False)
    with flag_gems.use_gems():
        res_out = torch._linalg_eigh(A, "L", compute_v=False)
