...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_resize_as(shape, dtype):
    ...
    for target_shape in target_shapes:
        ...
        ref_template = to_reference(template)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_resize_as_(shape, dtype):
    # resize_as_ requires same number of elements
    # Create target shape with same number of elements but different shape
    numel = 1
    for s in shape:
        numel *= s
    ...
    for target_shape in target_shapes:
        ...
        ref_out = ref_inp.resize_as_(template)
        with flag_gems.use_gems():
            res_out = inp.resize_as_(template)
