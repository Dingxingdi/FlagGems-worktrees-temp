...
TRIL_INDICES_OFFSETS = [-2, -1, 0, 1, 2]


@pytest.mark.tril_indices
@pytest.mark.parametrize("row,col", TRIL_INDICES_SHAPES)
@pytest.mark.parametrize("offset", TRIL_INDICES_OFFSETS)
def test_accuracy_tril_indices(row, col, offset):
    """Test tril_indices accuracy against PyTorch reference."""
    ref_out = torch.tril_indices(row, col, offset, device=flag_gems.device)
    with flag_gems.use_gems():
        res_out = torch.tril_indices(row, col, offset, device=flag_gems.device)
...
def test_accuracy_tril_indices_dtypes(dtype):
    """Test tril_indices with different dtypes."""
    row, col, offset = 5, 5, 0
    ref_out = torch.tril_indices(row, col, offset, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res_out = torch.tril_indices(row, col, offset, dtype=dtype, device=flag_gems.device)
