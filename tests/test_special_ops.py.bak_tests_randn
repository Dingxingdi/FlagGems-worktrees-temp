...
# Test for _fused_adamw_ operator
@pytest.mark.fused_adamw
@pytest.mark.parametrize("shape", [(1024,), (2048,), (4096,)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__fused_adamw_(shape, dtype):
    # Create input tensors for AdamW optimization
    ...
    state_steps = torch.zeros(shape, dtype=torch.long, device=flag_gems.device)

    # Reference implementation using manual AdamW computation
    ref_params = params.clone()
    ref_grads = grads.clone()
    ref_exp_avgs = exp_avgs.clone()
    ref_exp_avg_sqs = exp_avg_sqs.clone()

    # PyTorch parameters for AdamW
    lr = 0.001
    beta1 = 0.9
    beta2 = 0.999
    weight_decay = 0.01
    ...
    # Manual AdamW implementation (reference)
    step = 1  # First step
    bias_correction1 = 1.0 - beta1 ** step
    bias_correction2 = 1.0 - beta2 ** step

    # Update exp_avgs: m = beta1 * m + (1 - beta1) * grad
    ref_exp_avgs = beta1 * ref_exp_avgs + (1.0 - beta1) * ref_grads

    # Update exp_avg_sqs: v = beta2 * v + (1 - beta2) * grad^2
    ref_exp_avg_sqs = beta2 * ref_exp_avg_sqs + (1.0 - beta2) * ref_grads * ref_grads

    # Compute the update
    step_size = lr / bias_correction1
    denom = torch.sqrt(ref_exp_avg_sqs / bias_correction2) + eps

    # AdamW with decoupled weight decay
    update = (ref_exp_avgs / denom) + (weight_decay * ref_params)
    ref_params = ref_params - step_size * update
    ...
    with flag_gems.use_gems():
        torch._fused_adamw_(
            ...
            eps=eps, amsgrad=False, maximize=False
        )

    # Compare results
    gems_assert_close(params, ref_params, dtype)
    gems_assert_close(exp_avgs, ref_exp_avgs, dtype)
    gems_assert_close(exp_avg_sqs, ref_exp_avg_sqs, dtype)


@pytest.mark.fused_adamw
@pytest.mark.parametrize("shape", [(1024,), (2048,)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__fused_adamw_amsgrad(shape, dtype):
    # Test with amsgrad=True
    params = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grads = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avgs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    exp_avg_sqs = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    ...
    # Reference implementation (manual AdamW with amsgrad)
    ref_params = params.clone()
    ref_grads = grads.clone()
    ref_exp_avgs = exp_avgs.clone()
    ref_exp_avg_sqs = exp_avg_sqs.clone()
    ...
    # Update max_exp_avg_sqs for amsgrad
    ref_max_exp_avg_sqs = torch.maximum(ref_max_exp_avg_sqs, ref_exp_avg_sqs)

    step_size = lr / bias_correction1
    denom = torch.sqrt(ref_max_exp_avg_sqs / bias_correction2) + eps
    ...
    with flag_gems.use_gems():
        torch._fused_adamw_(
            ...
            (max_exp_avg_sqs,), (state_steps,),
            lr=lr, beta1=beta1, beta2=beta2, weight_decay=weight_decay,
            eps=eps, amsgrad=True, maximize=False
        )
    ...
    gems_assert_close(exp_avgs, ref_exp_avgs, dtype)
    gems_assert_close(exp_avg_sqs, ref_exp_avg_sqs, dtype)
    gems_assert_close(max_exp_avg_sqs, ref_max_exp_avg_sqs, dtype)


@pytest.mark.fused_adamw
@pytest.mark.parametrize("shape", [(1024,), (2048,)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__fused_adamw_maximize(shape, dtype):
    ...
    # Reference implementation (manual AdamW with maximize)
    ref_params = params.clone()
    ref_grads = grads.clone()
    ref_exp_avgs = exp_avgs.clone()
    ref_exp_avg_sqs = exp_avg_sqs.clone()
    ...
    # Manual AdamW with maximize=True (negate gradient)
    step = 1
    bias_correction1 = 1.0 - beta1 ** step
    bias_correction2 = 1.0 - beta2 ** step

    # With maximize=True, grad is negated
    neg_grad = -ref_grads
    ref_exp_avgs = beta1 * ref_exp_avgs + (1.0 - beta1) * neg_grad
    ref_exp_avg_sqs = beta2 * ref_exp_avg_sqs + (1.0 - beta2) * neg_grad * neg_grad
    ...
    with flag_gems.use_gems():
        torch._fused_adamw_(
            ...
            (max_exp_avg_sqs,), (state_steps,),
            lr=lr, beta1=beta1, beta2=beta2, weight_decay=weight_decay,
            eps=eps, amsgrad=False, maximize=True
        )
