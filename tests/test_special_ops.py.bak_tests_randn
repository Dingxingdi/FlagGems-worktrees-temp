...
@pytest.mark.parametrize("shape", UPSAMPLE_SHAPES_3D)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_grid_sampler_3d(dtype, shape, align_corners, padding_mode, interpolation_mode):
    # input: (N, C, D, H, W)
    # grid: (N, OD, OH, OW, 3) where 3 is (x, y, z) coordinates
    N, C, D, H, W = shape
    OD, OH, OW = D // 2, H // 2, W // 2  # Output size is smaller
    ...
    ref_out = torch.grid_sampler_3d(
        ref_input, grid, interpolation_mode, padding_mode, align_corners
    )
    with flag_gems.use_gems():
        res_out = torch.grid_sampler_3d(
            input, grid, interpolation_mode, padding_mode, align_corners
        )
    # Use dtype-appropriate tolerance due to numerical precision in trilinear interpolation
    # float16: 2e-3 atol, float32: 1e-4 atol, bfloat16: 4e-2 atol
