...
# Test for _cudnn_rnn operator
# This operator wraps cuDNN's RNN implementation
# Testing via the torch.ops.aten interface with flag_gems enabled

RNN_SHAPES = [
    (2, 4, 4, 8, 1),  # batch, seq_len, input_size, hidden_size, num_layers
    (1, 8, 10, 20, 2),
]


@pytest.mark.cudnn_rnn
@pytest.mark.parametrize("batch, seq_len, input_size, hidden_size, num_layers", RNN_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_cudnn_rnn(batch, seq_len, input_size, hidden_size, num_layers, dtype):
    """Test _cudnn_rnn accuracy"""
    if dtype == torch.float16 and torch.cuda.get_device_capability()[0] < 7:
        pytest.skip("float16 requires sm70+")

    device = flag_gems.device

    # Create LSTM module and move to device
    lstm = torch.nn.LSTM(
    ...
    # Flatten parameters for efficient cuDNN usage
    lstm.flatten_parameters()

    # Create input
    input_tensor = torch.randn(batch, seq_len, input_size, device=device, dtype=dtype)

    # Reference: using same LSTM on same device
    ...
    ref_lstm.load_state_dict(lstm.state_dict())

    with torch.no_grad():
        ref_output, ref_hidden = ref_lstm(input_tensor)

    # Test: with flag_gems
    with flag_gems.use_gems():
        # Enable flag_gems which registers operators
        # Note: _cudnn_rnn is not registered to avoid recursion, so it uses PyTorch default
        res_output, res_hidden = lstm(input_tensor)

    # Compare outputs
    gems_assert_close(res_output, ref_output, dtype)
    gems_assert_close(res_hidden[0], ref_hidden[0], dtype)
    gems_assert_close(res_hidden[1], ref_hidden[1], dtype)
