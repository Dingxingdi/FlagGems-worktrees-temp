...
@pytest.mark.parametrize("shape", MESHGRID_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_meshgrid_ij(shape, dtype):
    """Test meshgrid with ij indexing."""
    x = torch.randn(shape[0], dtype=dtype, device=flag_gems.device)
    y = torch.randn(shape[1], dtype=dtype, device=flag_gems.device)
    ...
    with flag_gems.use_gems():
        res_out = torch.meshgrid(x, y, indexing='ij')
...
@pytest.mark.parametrize("shape", MESHGRID_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_meshgrid_xy(shape, dtype):
    ...
    ref_out = torch.meshgrid(ref_x, ref_y, indexing='xy')
    with flag_gems.use_gems():
        res_out = torch.meshgrid(x, y, indexing='xy')
...
@pytest.mark.meshgrid
@pytest.mark.parametrize("shape", MESHGRID_3D_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_meshgrid_3d(shape, dtype):
    """Test 3D meshgrid."""
    x = torch.randn(shape[0], dtype=dtype, device=flag_gems.device)
    y = torch.randn(shape[1], dtype=dtype, device=flag_gems.device)
    z = torch.randn(shape[2], dtype=dtype, device=flag_gems.device)

    ref_x = to_reference(x)
    ref_y = to_reference(y)
    ref_z = to_reference(z)

    ref_out = torch.meshgrid(ref_x, ref_y, ref_z, indexing='ij')
    with flag_gems.use_gems():
        res_out = torch.meshgrid(x, y, z, indexing='ij')
...
@pytest.mark.parametrize("shape", MESHGRID_SHAPES)
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_meshgrid_int(shape, dtype):
    """Test meshgrid with integer types."""
    x = torch.randint(0, 100, (shape[0],), dtype=dtype, device=flag_gems.device)
    y = torch.randint(0, 100, (shape[1],), dtype=dtype, device=flag_gems.device)
...
def test_accuracy_meshgrid_single_input(dtype):
    """Test meshgrid with single input (edge case)."""
    x = torch.arange(5, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)

    ref_out = torch.meshgrid(ref_x, indexing='ij')
    with flag_gems.use_gems():
        res_out = torch.meshgrid(x, indexing='ij')
