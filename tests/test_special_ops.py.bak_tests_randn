...
# DynamicQuantizeLSTM tests
# Note: DynamicQuantizeLSTM doesn't exist in PyTorch, so we use quantize_per_tensor_dynamic as reference


@pytest.mark.DynamicQuantizeLSTM
@pytest.mark.parametrize("shape", [(16,), (32, 64), (128, 256), (1, 2, 128)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_DynamicQuantizeLSTM(shape, dtype):
    """Test DynamicQuantizeLSTM accuracy against PyTorch quantize_per_tensor_dynamic"""
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Reference: PyTorch's dynamic quantization (only supports float32)
    ref_quantized = torch.quantize_per_tensor_dynamic(input_tensor, torch.qint8, False)
    ref_output = ref_quantized.int_repr().to(torch.float32)
    ref_scale = ref_quantized.q_scale()
    ref_zp = ref_quantized.q_zero_point()

    # FlagGems implementation
    with flag_gems.use_gems():
        res_output, res_scale, res_zp = flag_gems.dynamic_quantize_lstm(input_tensor)

    # Compare outputs - quantized values
    gems_assert_close(res_output.float(), ref_output, dtype)

    # Compare scale (use torch.testing directly since ref_scale is a Python float)
    torch.testing.assert_close(res_scale.item(), ref_scale, rtol=1e-5, atol=1e-5)

    # Compare zero_point
    assert res_zp.item() == ref_zp
