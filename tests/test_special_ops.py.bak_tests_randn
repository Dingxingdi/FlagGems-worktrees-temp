...
def test_accuracy_sparse_semi_structured_linear(M, K, dtype):
    N = K  # Output dimension equals input dimension for simplicity

    # Create valid meta tensor: (K//2, K//4)
    # This is required by the API but not used in the dense fallback
    meta = torch.randint(0, 4, (K // 2, K // 4), dtype=torch.int8, device=flag_gems.device)

    input = torch.randn(M, K, dtype=dtype, device=flag_gems.device)
    weight = torch.randn(N, K, dtype=dtype, device=flag_gems.device)

    # Reference: use torch.matmul (dense fallback)
    # Since the sparse meta is ignored in our implementation,
    # we compare against regular matmul
...
@pytest.mark.parametrize("M, K", SPARSE_LINEAR_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_sparse_semi_structured_linear_bias(M, K, dtype):
    N = K
    ...
    ref_out = torch.matmul(input, weight.t()) + bias

    with flag_gems.use_gems():
        res_out = torch._sparse_semi_structured_linear(input, weight, meta, bias=bias)

    gems_assert_close(res_out, ref_out, dtype, reduce_dim=K)


@pytest.mark.sparse_semi_structured_linear
@pytest.mark.parametrize("M, K", SPARSE_LINEAR_SHAPES[:3])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_sparse_semi_structured_linear_activation(M, K, dtype):
    N = K
    ...
    ref_out = torch.nn.functional.relu(torch.matmul(input, weight.t()) + bias)

    with flag_gems.use_gems():
        res_out = torch._sparse_semi_structured_linear(
            input, weight, meta, bias=bias, activation="relu"
        )
