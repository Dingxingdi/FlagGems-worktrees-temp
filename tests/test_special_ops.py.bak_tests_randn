...
# Shapes for LU factorization (square matrices)
# Note: Only 3x3 is currently working due to Triton loop limitations
LU_SHAPES = [(3, 3)]
# Only test float32 (lu_factor doesn't support float16 on CPU)
...
def test_accuracy_linalg_lu_factor(shape, dtype):
    """Test linalg.lu_factor accuracy"""
    ...
    with flag_gems.use_gems():
        res_LU, res_pivots = torch.linalg.lu_factor(A)
...
@pytest.mark.parametrize("shape", LU_SHAPES)
@pytest.mark.parametrize("dtype", LU_DTYPES)
def test_accuracy_linalg_lu_factor_ex(shape, dtype):
    """Test linalg.lu_factor_ex accuracy"""
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_A = to_reference(A)

    ref_LU, ref_pivots, ref_info = torch.ops.aten.linalg_lu_factor_ex(ref_A)
    with flag_gems.use_gems():
        res_LU, res_pivots, res_info = torch.ops.aten.linalg_lu_factor_ex(A)
...
# Batch test is disabled due to kernel issues with batch dimensions
# @pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
# @pytest.mark.linalg_lu_factor
# @pytest.mark.parametrize("shape", LU_SHAPES)
# @pytest.mark.parametrize("dtype", LU_DTYPES)
# def test_accuracy_linalg_lu_factor_batch(shape, dtype):
#     """Test linalg.lu_factor with batch dimensions"""
#     # Add batch dimension: (batch, n, n)
#     batch_size = 4
#     A = torch.randn(batch_size, shape[0], shape[1], dtype=dtype, device=flag_gems.device)
#     ref_A = to_reference(A)
#
#     ref_LU, ref_pivots = torch.linalg.lu_factor(ref_A)
#     with flag_gems.use_gems():
