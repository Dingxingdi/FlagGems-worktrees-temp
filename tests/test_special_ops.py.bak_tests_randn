...
# Test for _padded_dense_to_jagged_forward
@pytest.mark.padded_dense_to_jagged_forward
@pytest.mark.parametrize("batch_size", [4, 8])
@pytest.mark.parametrize("max_seq_len", [8, 16])
@pytest.mark.parametrize("hidden_size", [2, 8])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__padded_dense_to_jagged_forward(batch_size, max_seq_len, hidden_size, dtype):
    # Generate random sequence lengths for each batch
    seq_lengths = torch.randint(1, max_seq_len + 1, (batch_size,), device=device)

    # Create offsets tensor (cumulative sum with leading 0)
    offsets = torch.zeros(batch_size + 1, dtype=torch.int64, device=device)
    offsets[1:] = torch.cumsum(seq_lengths, dim=0)

    # Create padded dense tensor
    dense = torch.randn(batch_size, max_seq_len, hidden_size, dtype=dtype, device=device)

    # Reference implementation
    ref_dense = to_reference(dense)
    ref_offsets = [to_reference(offsets.clone())]

    ref_out = torch.ops.aten._padded_dense_to_jagged_forward(ref_dense, ref_offsets)

    # GEMS implementation
    with flag_gems.use_gems():
        res_out = torch.ops.aten._padded_dense_to_jagged_forward(dense, [offsets])
...
@pytest.mark.padded_dense_to_jagged_forward
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy__padded_dense_to_jagged_forward_edge_cases(dtype):
    # Test edge case: all sequences have same length
    batch_size = 4
    max_seq_len = 8
    hidden_size = 4

    seq_lengths = torch.full((batch_size,), max_seq_len, dtype=torch.int64, device=device)
    offsets = torch.zeros(batch_size + 1, dtype=torch.int64, device=device)
    offsets[1:] = torch.cumsum(seq_lengths, dim=0)
