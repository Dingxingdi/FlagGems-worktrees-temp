...
# FFT ihfftn tests
@pytest.mark.fft_ihfftn
@pytest.mark.parametrize("shape", [(8, 16), (16, 32), (32, 64)])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_ihfftn(shape, dtype):
    """Test accuracy of fft_ihfftn against PyTorch implementation."""
    ...
    # Compute ihfftn
    ref_out = torch.fft.ihfftn(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfftn(inp)

    # For complex outputs, use relative tolerance
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfftn_with_s(shape, dtype):
    """Test accuracy of fft_ihfftn with s parameter."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Test with s parameter
    s = (shape[0], shape[1] // 2)
    ref_out = torch.fft.ihfftn(ref_inp, s=s)
    with flag_gems.use_gems():
        res_out = torch.fft.ihfftn(inp, s=s)

    torch.testing.assert_close(res_out, ref_out, atol=1e-4, rtol=1e-4)


@pytest.mark.fft_ihfftn
@pytest.mark.parametrize("shape", [(8, 16), (16, 32)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_fft_ihfftn_with_norm(shape, dtype):
    """Test accuracy of fft_ihfftn with different normalization modes."""
    for norm in ["forward", "backward", "ortho"]:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
        ref_inp = to_reference(inp)

        ref_out = torch.fft.ihfftn(ref_inp, norm=norm)
        with flag_gems.use_gems():
            res_out = torch.fft.ihfftn(inp, norm=norm)
