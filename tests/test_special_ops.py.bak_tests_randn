...
def test_upsample_bicubic2d_aa_backward(dtype, shape, scale, align_corners):
    ...
    grad_output = torch.rand(shape[0], shape[1], output_size[0], output_size[1], dtype=dtype, device=flag_gems.device)

    ref_grad_output = to_reference(grad_output, True)

    # Call backward using PyTorch reference implementation
    ref_grad = torch.ops.aten._upsample_bicubic2d_aa_backward.default(
        ref_grad_output, output_size, input_shape, align_corners, None, None
    )

    # Call backward using GEMS implementation
    with flag_gems.use_gems():
        res_grad = torch.ops.aten._upsample_bicubic2d_aa_backward.default(
            grad_output, output_size, input_shape, align_corners, None, None
        )
    ...
    if ref_grad.dtype != res_grad.dtype:
        ref_grad = ref_grad.to(res_grad.dtype)

    reduce_dim = span(scale[0]) * span(scale[1])
    gems_assert_close(res_grad, ref_grad, dtype, reduce_dim=reduce_dim)
