...
# KV Cache shapes for transformer inference testing
KV_CACHE_SHAPES = [
    (1, 4, 16, 32),    # (batch, heads, seq_len, head_dim)
    (2, 8, 32, 64),
    (4, 16, 64, 128),
]
...
@pytest.mark.parametrize("shape", KV_CACHE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_kv_cache_update(shape, dtype):
    """Test KV_Cache_Update operator accuracy.

    This operator updates a KV cache tensor with new key/value pairs
    at specified positions, commonly used in transformer inference.
    """
    batch, num_heads, max_seq_len, head_dim = shape
    new_seq_len = max_seq_len // 4  # Update 1/4 of the sequence
    start_position = 0

    # Create cache and new key/value tensors
    k_cache = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    v_cache = torch.zeros(shape, dtype=dtype, device=flag_gems.device)
    new_k = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
    new_v = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=flag_gems.device)

    # Reference implementation using PyTorch tensor indexing
    ref_k_cache = k_cache.clone()
    ref_v_cache = v_cache.clone()
    ref_k_cache[:, :, start_position:start_position + new_seq_len, :] = new_k
    ref_v_cache[:, :, start_position:start_position + new_seq_len, :] = new_v

    # Use GEMS implementation
    with flag_gems.use_gems():
        res_k_cache, res_v_cache = flag_gems.kv_cache_update(
            k_cache.clone(), v_cache.clone(), new_k, new_v, start_position
        )

    gems_assert_close(res_k_cache[:, :, start_position:start_position + new_seq_len, :],
                      ref_k_cache[:, :, start_position:start_position + new_seq_len, :], dtype)
    gems_assert_close(res_v_cache[:, :, start_position:start_position + new_seq_len, :],
                      ref_v_cache[:, :, start_position:start_position + new_seq_len, :], dtype)


@pytest.mark.kv_cache_update
@pytest.mark.parametrize("shape", KV_CACHE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_kv_cache_update_offset(shape, dtype):
    """Test KV_Cache_Update with non-zero start position."""
    batch, num_heads, max_seq_len, head_dim = shape
    new_seq_len = max_seq_len // 4
    start_position = max_seq_len // 2  # Start from middle of cache

    k_cache = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    v_cache = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    new_k = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
    new_v = torch.randn(batch, num_heads, new_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
