...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dist_p2(shape, dtype):
    """Test torch.dist with p=2 (Euclidean distance)."""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.dist(ref_inp1, ref_inp2, p=2)
    with flag_gems.use_gems():
        res_out = torch.dist(inp1, inp2, p=2)
...
@pytest.mark.parametrize("shape", DIST_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dist_p1(shape, dtype):
    """Test torch.dist with p=1 (Manhattan distance)."""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.dist(ref_inp1, ref_inp2, p=1)
    with flag_gems.use_gems():
        res_out = torch.dist(inp1, inp2, p=1)
...
@pytest.mark.parametrize("shape", DIST_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dist_pinf(shape, dtype):
    """Test torch.dist with p=inf (Chebyshev distance)."""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.dist(ref_inp1, ref_inp2, p=float('inf'))
    with flag_gems.use_gems():
        res_out = torch.dist(inp1, inp2, p=float('inf'))
...
@pytest.mark.parametrize("shape", DIST_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dist_p3(shape, dtype):
    """Test torch.dist with p=3."""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.dist(ref_inp1, ref_inp2, p=3)
    with flag_gems.use_gems():
        res_out = torch.dist(inp1, inp2, p=3)
...
@pytest.mark.parametrize("shape", DIST_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_dist_broadcast(shape, dtype):
    """Test torch.dist with broadcast."""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(1, dtype=dtype, device=flag_gems.device).expand(shape)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)
