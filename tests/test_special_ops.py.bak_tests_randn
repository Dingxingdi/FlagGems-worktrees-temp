...
def test_accuracy_cross_attention(shape, dtype):
    ...
    q = torch.randn(batch, num_heads, q_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
    k = torch.randn(batch, num_heads, kv_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
    v = torch.randn(batch, num_heads, kv_seq_len, head_dim, dtype=dtype, device=flag_gems.device)
