...
def test_accuracy_Adaptive_Attention_Span(shape, dtype):
    """Test Adaptive_Attention_Span operator.

    This operator computes adaptive attention span using sigmoid activation,
    which maps input values to (0, 1) range.
    """
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Reference implementation using torch.sigmoid
    ref_out = torch.sigmoid(inp)

    # Test using flag_gems function directly
...
@pytest.mark.inplace
@pytest.mark.Adaptive_Attention_Span_
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Adaptive_Attention_Span_(shape, dtype):
    """Test in-place Adaptive_Attention_Span_ operator."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = inp.clone()

    # Reference implementation using torch.sigmoid_
    ref_out = torch.sigmoid_(ref_inp)

    # Test using flag_gems function directly
    res_out = flag_gems.adaptive_attention_span_(inp)
