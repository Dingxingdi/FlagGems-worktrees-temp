...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_narrow_copy(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_narrow_copy_negative_start(dtype):
...
@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_narrow_copy_full_slice(dtype):
...
@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_narrow_copy_1d(dtype):
    # Test 1D tensor
