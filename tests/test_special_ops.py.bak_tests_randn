...
# Test for _assert_async
@pytest.mark.assert_async
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16, torch.int32, torch.int64])
def test_accuracy__assert_async(dtype):
    """Test _assert_async with a one-element non-zero tensor."""
    # _assert_async requires a one-element tensor
    inp = torch.ones(1, dtype=dtype, device=flag_gems.device)

    ref_inp = to_reference(inp)

    ref_out = torch._assert_async(ref_inp)
    with flag_gems.use_gems():
        res_out = torch._assert_async(inp)

    # _assert_async returns None (void), so we just verify both return None
