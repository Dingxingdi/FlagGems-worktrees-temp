...
# Test shapes for linalg_solve_ex - use smaller shapes for simplicity
LINALG_SOLVE_SHAPES = [
    (2, 2),
    (4, 4),
    (8, 8),
...
# Use float32 only (torch.lu doesn't support bfloat16 on CUDA)
LINALG_SOLVE_DTYPES = [torch.float32]


@pytest.mark.linalg_solve_ex
@pytest.mark.parametrize("shape", LINALG_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", LINALG_SOLVE_DTYPES)
def test_accuracy_linalg_solve_ex(shape, dtype):
    # Create coefficient matrix A (must be square)
    n = shape[0]
    A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
    # Make A invertible by adding to diagonal
    A = A + torch.eye(n, dtype=dtype, device=flag_gems.device) * n

    # Create right-hand side B (vector)
    B = torch.randn(n, dtype=dtype, device=flag_gems.device)

    ref_A = to_reference(A)
    ref_B = to_reference(B)

    # Use torch.linalg.solve as reference (more widely supported)
    ref_out_result = torch.linalg.solve(ref_A, ref_B)

    with flag_gems.use_gems():
        res_out = torch.linalg.solve_ex(A, B)

    # Compare result tensors
    gems_assert_close(res_out.result, ref_out_result, dtype)


@pytest.mark.linalg_solve_ex
@pytest.mark.parametrize("shape", LINALG_SOLVE_SHAPES)
@pytest.mark.parametrize("dtype", LINALG_SOLVE_DTYPES)
def test_accuracy_linalg_solve_ex_multiple_rhs(shape, dtype):
    # Test with multiple right-hand sides
    n = shape[0]
    k = 4  # number of right-hand sides
    A = torch.randn(n, n, dtype=dtype, device=flag_gems.device)
    A = A + torch.eye(n, dtype=dtype, device=flag_gems.device) * n
    ...
    # Use torch.linalg.solve as reference
    ref_out_result = torch.linalg.solve(ref_A, ref_B)
...
@pytest.mark.linalg_solve_ex
@pytest.mark.parametrize("shape", [(2, 2, 2), (4, 4, 2), (8, 8, 4)])
@pytest.mark.parametrize("dtype", LINALG_SOLVE_DTYPES)
def test_accuracy_linalg_solve_ex_batched(shape, dtype):
    ...
    B = torch.randn(batch_size, n, dtype=dtype, device=flag_gems.device)
