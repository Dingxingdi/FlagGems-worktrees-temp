...
# RNN tanh tests
RNN_SHAPES = [
    (4, 2, 3, 5),  # (seq_len, batch, input_size, hidden_size)
    (8, 4, 10, 20),
    (16, 8, 32, 64),
] if not QUICK_MODE else [(4, 2, 3, 5)]


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
@pytest.mark.rnn_tanh
@pytest.mark.parametrize("shape", RNN_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rnn_tanh(shape, dtype):
    """Test rnn_tanh accuracy against PyTorch reference implementation."""
    seq_len, batch, input_size, hidden_size = shape

    # Create RNN model to get weights
    rnn = torch.nn.RNN(
    ...
    # Prepare inputs
    input_tensor = torch.randn(batch, seq_len, input_size, dtype=dtype, device=flag_gems.device)
    hx = torch.randn(1, batch, hidden_size, dtype=dtype, device=flag_gems.device)

    # Get parameters
    params = tuple(p.to(dtype).to(flag_gems.device) for p in rnn.parameters())

    # Reference implementation
    ref_input = to_reference(input_tensor)
    ref_hx = to_reference(hx)
    ref_params = tuple(to_reference(p) for p in params)

    ref_out, ref_hn = torch.rnn_tanh(
        ref_input, ref_hx, ref_params, True, 1, 0.0, False, False, True
    )

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, res_hn = torch.rnn_tanh(
            input_tensor, hx, params, True, 1, 0.0, False, False, True
        )
...
@pytest.mark.parametrize("shape", RNN_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rnn_tanh_no_bias(shape, dtype):
    """Test rnn_tanh without bias."""
    seq_len, batch, input_size, hidden_size = shape

    # Create RNN model without bias
    rnn = torch.nn.RNN(
        input_size=input_size,
        hidden_size=hidden_size,
        num_layers=1,
    ...
    ref_out, ref_hn = torch.rnn_tanh(
        ref_input, ref_hx, ref_params, False, 1, 0.0, False, False, True
    )

    # GEMS implementation
    with flag_gems.use_gems():
        res_out, res_hn = torch.rnn_tanh(
            input_tensor, hx, params, False, 1, 0.0, False, False, True
        )
