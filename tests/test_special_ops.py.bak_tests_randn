...
# FFT FFTShift tests
# Note: Triton does not support complex types, so we only test with real float types
FFT_SHAPES = [(8,), (16,), (32,), (64,), (128,), (256,), (8, 8), (16, 16), (32, 32), (8, 16), (16, 8), (4, 8, 16), (2, 4, 8)]
FFT_DTYPES = FLOAT_DTYPES


@pytest.mark.fft_fftshift
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_fftshift(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fftshift(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.fftshift(inp)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_fftshift_dim0(shape, dtype):
    if len(shape) < 1:
        pytest.skip("Need at least 1D tensor for dim=0")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fftshift(ref_inp, dim=0)
    with flag_gems.use_gems():
        res_out = torch.fft.fftshift(inp, dim=0)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_fftshift_dim1(shape, dtype):
    if len(shape) < 2:
        pytest.skip("Need at least 2D tensor for dim=1")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fftshift(ref_inp, dim=1)
    with flag_gems.use_gems():
        res_out = torch.fft.fftshift(inp, dim=1)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", FFT_DTYPES)
def test_accuracy_fft_fftshift_dims_all(shape, dtype):
    if len(shape) < 2:
        pytest.skip("Need at least 2D tensor for multi-dim shift")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.fft.fftshift(ref_inp, dim=[0, 1])
    with flag_gems.use_gems():
        res_out = torch.fft.fftshift(inp, dim=[0, 1])
