...
# Poisson distribution sampling tests
POISSON_SHAPES = ((64, 64), (128, 128), (32, 32, 32))


@pytest.mark.poisson
@pytest.mark.parametrize("shape", POISSON_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_poisson(shape, dtype):
    if flag_gems.vendor_name == "kunlunxin":
        pytest.skip("Poisson not supported on kunlunxin")

    # Poisson requires non-negative input rates
    # Use uniform random in [0, 10] to get meaningful Poisson samples
    rate = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 10.0
    ref_rate = to_reference(rate)

    ref_out = torch.poisson(ref_rate)
    with flag_gems.use_gems():
        res_out = torch.poisson(rate)

    # Poisson samples should be non-negative integers
    assert res_out.dtype == torch.int64, f"Expected int64, got {res_out.dtype}"
    assert torch.all(res_out >= 0), "Poisson samples should be non-negative"

    # Check statistical properties: mean should be approximately equal to rate
    # Use a relaxed tolerance since there's inherent randomness
    res_out_float = res_out.to(torch.float32)
    ref_out_float = ref_out.to(torch.float32)

    res_mean = res_out_float.mean().item()
    ref_mean = ref_out_float.mean().item()
    rate_mean = ref_rate.to(torch.float32).mean().item()

    # The sample mean should be within 3 standard errors of the rate mean
    # std_error = sqrt(rate_mean / n) where n is total samples
    n = res_out.numel()
    std_error = (rate_mean / n) ** 0.5 * 3  # 3-sigma tolerance

    assert abs(res_mean - rate_mean) < std_error * 5, (
        f"Sample mean {res_mean} differs too much from rate mean {rate_mean}"
    )
    assert abs(ref_mean - rate_mean) < std_error * 5, (
        f"Reference mean {ref_mean} differs too much from rate mean {rate_mean}"
    )
...
def test_accuracy_poisson_large_lambda(shape, dtype):
    ...
    # Test with larger lambda values where normal approximation is used
    rate = torch.rand(shape, dtype=dtype, device=flag_gems.device) * 100.0 + 20.0
    ref_rate = to_reference(rate)
    ...
    # Statistical check for large lambda
    res_out_float = res_out.to(torch.float32)
    ref_out_float = ref_out.to(torch.float32)
