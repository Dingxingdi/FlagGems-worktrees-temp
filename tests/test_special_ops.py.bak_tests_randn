...
# Shapes for sparse semi-structured MM tests
SPARSE_MM_SHAPES = [
    (64, 128, 64),
    (128, 256, 128),
    (256, 512, 256),
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy_sparse_semi_structured_mm(shape, dtype):
    M, K, N = shape

    # Create the input matrices
    mat1 = torch.rand(M, K, dtype=dtype, device=flag_gems.device)
    mat2 = torch.rand(K, N, dtype=dtype, device=flag_gems.device)

    # Create dummy meta (will trigger fallback to dense computation)
    meta = torch.zeros(K//4, M, dtype=torch.int8, device=flag_gems.device)

    # Reference: dense computation
    ref_out = torch.mm(mat1, mat2)

    # GEMS implementation - don't use use_gems() context to avoid recursion
    res_out = flag_gems.sparse_semi_structured_mm(mat1, meta, mat2)
