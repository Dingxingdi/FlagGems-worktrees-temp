...
# Test for unsafe_chunk operator
@pytest.mark.unsafe_chunk
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("chunks", [2, 3, 5])
@pytest.mark.parametrize("dim", [0, -1])
def test_accuracy_unsafe_chunk(shape, dtype, chunks, dim):
    ...
    for res_chunk, ref_chunk in zip(res_out, ref_out):
        res_chunk_cpu = to_reference(res_chunk)
        ref_chunk_cpu = to_reference(ref_chunk)
        gems_assert_close(res_chunk_cpu, ref_chunk_cpu, dtype)
