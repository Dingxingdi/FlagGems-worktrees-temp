...
# Test shapes for unsqueeze operation
UNSQUEEZE_SHAPES = [
    (2, 3),
    (2, 3, 4),
    (2, 3, 4, 5),
...
@pytest.mark.parametrize("shape", UNSQUEEZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_unsqueeze_(shape, dtype):
    """Test functional unsqueeze operation"""
...
@pytest.mark.parametrize("shape", UNSQUEEZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_unsqueeze__inplace(shape, dtype):
    """Test in-place unsqueeze_ operation"""
    for dim in range(-len(shape) - 1, len(shape) + 1):
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
