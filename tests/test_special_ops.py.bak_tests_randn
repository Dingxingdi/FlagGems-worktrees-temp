...
# Squeeze operator tests
# Test shapes with dimensions of size 1
SQUEEZE_SHAPES = [
    (1,),
    (1, 2, 3),
    (2, 1, 3),
...
@pytest.mark.squeeze
@pytest.mark.parametrize("shape", SQUEEZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + ALL_INT_DTYPES)
def test_accuracy_squeeze(shape, dtype):
    ...
    # Test squeeze without dim argument (squeeze all dims of size 1)
    ref_out = torch.squeeze(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.squeeze(inp)
...
@pytest.mark.parametrize("shape", SQUEEZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + ALL_INT_DTYPES)
def test_accuracy_squeeze_dim(shape, dtype):
    if dtype in FLOAT_DTYPES:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    else:
        inp = torch.randint(
    ...
    # Test squeeze with specific dim
    for dim in range(inp.dim()):
        if shape[dim] == 1:
            ref_out = torch.squeeze(ref_inp, dim=dim)
            with flag_gems.use_gems():
                res_out = torch.squeeze(inp, dim=dim)
            gems_assert_equal(res_out, ref_out)
...
@pytest.mark.parametrize("shape", SQUEEZE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + ALL_INT_DTYPES)
def test_accuracy_squeeze_(shape, dtype):
    if dtype in FLOAT_DTYPES:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    else:
        inp = torch.randint(
    ...
    # Test in-place squeeze_
    ref_out = ref_inp.squeeze_()
    with flag_gems.use_gems():
        res_out = inp.squeeze_()
