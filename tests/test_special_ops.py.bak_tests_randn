...
# Position_wise_FFN tests
@pytest.mark.Position_wise_FFN
@pytest.mark.parametrize("shape", [
    (2, 4, 32),    # Small: batch=2, seq=4, hidden=32
    (4, 8, 64),    # Medium: batch=4, seq=8, hidden=64
])
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("activation", ["relu", "gelu"])
def test_accuracy_Position_wise_FFN(shape, dtype, activation):
    """Test Position-wise FFN accuracy against PyTorch reference implementation."""
    ...
    w1 = torch.randn(hidden_dim, intermediate_dim, dtype=dtype, device=flag_gems.device)
    b1 = torch.randn(intermediate_dim, dtype=dtype, device=flag_gems.device)
    w2 = torch.randn(intermediate_dim, hidden_dim, dtype=dtype, device=flag_gems.device)
    b2 = torch.randn(hidden_dim, dtype=dtype, device=flag_gems.device)

    # Reference implementation using PyTorch
    def reference_ffn(input_tensor, weight1, bias1, weight2, bias2, act):
        hidden = torch.matmul(input_tensor, weight1) + bias1
        if act == "relu":
    ...
    ref_x = to_reference(x)
    ref_w1 = to_reference(w1)
    ref_b1 = to_reference(b1)
    ref_w2 = to_reference(w2)
    ref_b2 = to_reference(b2)

    ref_out = reference_ffn(ref_x, ref_w1, ref_b1, ref_w2, ref_b2, activation)

    with flag_gems.use_gems():
        res_out = flag_gems.position_wise_ffn(x, w1, b1, w2, b2, activation)
...
def test_accuracy_Position_wise_FFN_backward(shape, dtype, activation):
    """Test Position-wise FFN backward pass accuracy."""
    batch_size, seq_len, hidden_dim = shape
    intermediate_dim = hidden_dim * 4

    # Create input with requires_grad for gradient computation
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)

    # Create weights and biases with requires_grad
    w1 = torch.randn(hidden_dim, intermediate_dim, dtype=dtype, device=flag_gems.device, requires_grad=True)
    b1 = torch.randn(intermediate_dim, dtype=dtype, device=flag_gems.device, requires_grad=True)
    w2 = torch.randn(intermediate_dim, hidden_dim, dtype=dtype, device=flag_gems.device, requires_grad=True)
    b2 = torch.randn(hidden_dim, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ...
    ref_b1 = b1.detach().clone().requires_grad_(True)
    ref_w2 = w2.detach().clone().requires_grad_(True)
    ref_b2 = b2.detach().clone().requires_grad_(True)

    ref_out = reference_ffn(ref_x, ref_w1, ref_b1, ref_w2, ref_b2, activation)
    ref_out.sum().backward()

    # Compute gems gradients
    x_gems = x.detach().clone().requires_grad_(True)
    w1_gems = w1.detach().clone().requires_grad_(True)
    b1_gems = b1.detach().clone().requires_grad_(True)
    w2_gems = w2.detach().clone().requires_grad_(True)
    b2_gems = b2.detach().clone().requires_grad_(True)

    with flag_gems.use_gems():
        res_out = flag_gems.position_wise_ffn(x_gems, w1_gems, b1_gems, w2_gems, b2_gems, activation)
        res_out.sum().backward()

    # Compare gradients
    gems_assert_close(x_gems.grad, ref_x.grad, dtype)
    gems_assert_close(w1_gems.grad, ref_w1.grad, dtype)
    gems_assert_close(b1_gems.grad, ref_b1.grad, dtype)
    gems_assert_close(w2_gems.grad, ref_w2.grad, dtype)
    gems_assert_close(b2_gems.grad, ref_b2.grad, dtype)
