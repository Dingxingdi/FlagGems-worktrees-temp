...
# Segment reduce backward tests
SEGMENT_REDUCE_SHAPES = [
    (64,),
    (128,),
    (256,),
    (512,),
...
@pytest.mark.parametrize("shape", SEGMENT_REDUCE_SHAPES)
@pytest.mark.parametrize("reduce", ["sum", "mean", "max"])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_segment_reduce_backward(shape, reduce, dtype):
    # Create test data
    data = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Create lengths for 4 segments
    n_segments = 4
    segment_length = shape[0] // n_segments
    lengths = torch.tensor([segment_length] * n_segments, dtype=torch.long, device=flag_gems.device)

    ref_data = to_reference(data)
    ref_lengths = to_reference(lengths)

    # Get forward output using torch.segment_reduce
    ref_output = torch.segment_reduce(ref_data, reduce, lengths=ref_lengths)

    # Create gradient tensor
    grad = torch.ones(n_segments, dtype=dtype, device=flag_gems.device)
    ref_grad = to_reference(grad)

    # Call backward
    ref_out = torch.ops.aten._segment_reduce_backward.default(
        ref_grad, ref_output, ref_data, reduce, lengths=ref_lengths
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten._segment_reduce_backward.default(
            grad, ref_output, data, reduce, lengths=lengths
        )
