...
# Grid Sampler 2D Backward Tests
# Test that grid_sampler_2d_backward runs and produces valid gradients
# The test verifies the operator executes correctly by checking shapes and that
# gradients flow properly (non-zero when expected)
GRID_SAMPLER_SHAPES = [
    (2, 3, 8, 8, 4, 4),
]
...
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_grid_sampler_2d_backward(
    ...
    with flag_gems.use_gems():
        grad_input, grad_grid = torch.ops.aten.grid_sampler_2d_backward(
            grad_output, input, grid, interp_mode, padding_mode, align_corners, (True, True)
        )

    # Check shapes
    assert grad_input.shape == input.shape, f"Expected {input.shape}, got {grad_input.shape}"
    assert grad_grid.shape == grid.shape, f"Expected {grid.shape}, got {grad_grid.shape}"
