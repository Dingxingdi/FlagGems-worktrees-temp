...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [-1, 0, 1])
def test_accuracy_special_softmax(shape, dtype, dim):
...
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
@pytest.mark.parametrize("dim", [-1, 0, 1])
@pytest.mark.parametrize("dtype_arg", [None, torch.float32, torch.float16, torch.bfloat16])
def test_accuracy_special_softmax_with_dtype(shape, dtype, dim, dtype_arg):
    ...
    gems_assert_close(res_out, ref_out, expected_dtype)
