...
# feature_dropout tests
# Note: feature_dropout requires at least 2D input in PyTorch
FEATURE_DROPOUT_SHAPES = [
    (1024, 1024), (20, 320, 15), (16, 128, 64, 1280), (16, 7, 57, 32, 29)
]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_feature_dropout_train_false(shape, dtype):
    """Test feature_dropout with train=False returns input unchanged."""
    if TO_CPU:
        shape = (1024, 1024)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.feature_dropout(ref_inp, 0.5, False)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.feature_dropout(inp, 0.5, False)
...
@pytest.mark.parametrize("shape", FEATURE_DROPOUT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_feature_dropout_p0(shape, dtype):
    """Test feature_dropout with p=0 returns input unchanged."""
    if TO_CPU:
        shape = (1024, 1024)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.feature_dropout(ref_inp, 0.0, True)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.feature_dropout(inp, 0.0, True)
...
@pytest.mark.parametrize("shape", FEATURE_DROPOUT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_feature_dropout_p1(shape, dtype):
    """Test feature_dropout with p=1 returns zeros."""
    if TO_CPU:
        shape = (1024, 1024)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.feature_dropout(ref_inp, 1.0, True)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.feature_dropout(inp, 1.0, True)
...
@pytest.mark.parametrize("p", [0.3, 0.6, 0.9])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_feature_dropout_backward(shape, p, dtype):
    """Test feature_dropout backward pass with fixed mask."""
    if TO_CPU:
        shape = (1024, 1024)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ...
    # Forward pass - get output and save for backward
    ref_out = torch.ops.aten.feature_dropout(ref_inp, p, True)
    ref_out.backward(ref_grad)

    with flag_gems.use_gems():
        res_out = torch.ops.aten.feature_dropout(inp, p, True)
...
@pytest.mark.parametrize("shape", FEATURE_DROPOUT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_feature_dropout_inference_mode(shape, dtype):
    """Test feature_dropout in inference mode is identity."""
    if TO_CPU:
        shape = (1024, 1024)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # During inference (train=False), output should equal input
    ref_out = torch.ops.aten.feature_dropout(ref_inp, 0.5, False)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.feature_dropout(inp, 0.5, False)

    # In inference mode, should be identity
    gems_assert_equal(res_out, inp)
    gems_assert_equal(ref_out, ref_inp)
