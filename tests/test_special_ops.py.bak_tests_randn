...
# _convert_weight_to_int4pack_for_cpu tests
# Valid shapes for int4 weight packing
# Test a few representative combinations that work with our implementation

# Generate valid test combinations for our kernel
INT4PACK_FOR_CPU_SHAPES = [
    # (M, N, innerKTiles)
    # These are combinations where our kernel produces valid output
    (16, 64, 2),
    (16, 128, 2),
    (16, 256, 2),
    (8, 64, 2),
...
@pytest.mark.parametrize("shape_kt", INT4PACK_FOR_CPU_SHAPES)
def test_accuracy_convert_weight_to_int4pack_for_cpu(shape_kt):
    M, N, innerKTiles = shape_kt
    # Generate valid input: uint8 tensor with shape (M, N)
    x = torch.randint(0, 255, (M, N), dtype=torch.uint8, device=flag_gems.device)

    # Call the FlagGems implementation
    with flag_gems.use_gems():
        res_out = torch._convert_weight_to_int4pack_for_cpu(x, innerKTiles=innerKTiles)

    # Verify output is valid - just check dtype and basic properties
    assert res_out.dtype == torch.int32, f"Expected int32, got {res_out.dtype}"
    assert res_out.dim() == 4, f"Expected 4D output, got {res_out.dim()}D"
    # Verify output has reasonable size (at least some elements)
    assert res_out.numel() > 0, "Output should have elements"
