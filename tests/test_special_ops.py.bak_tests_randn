...
@pytest.mark.parametrize("shape", [(2, 5), (8, 16), (32, 64), (1, 1024)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_searchsorted_2d(shape, dtype):
    """Test searchsorted with 2D sorted_sequence and 2D values"""
    sorted_seq, values_shape = shape
    sorted_sequence = torch.sort(torch.randn((sorted_seq, values_shape), dtype=dtype, device=flag_gems.device))[0]
    values = torch.randn((sorted_seq, values_shape // 2), dtype=dtype, device=flag_gems.device)

    ref_sorted = to_reference(sorted_sequence)
    ref_values = to_reference(values)

    ref_out = torch.searchsorted(ref_sorted, ref_values)
    with flag_gems.use_gems():
        res_out = torch.searchsorted(sorted_sequence, values)

    gems_assert_equal(res_out, ref_out)


@pytest.mark.searchsorted
@pytest.mark.parametrize("shape", [(8, 16), (32, 64), (1, 1024)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_searchsorted_2d_right(shape, dtype):
    """Test searchsorted with side='right' (upper bound)"""
    sorted_seq, N = shape
    sorted_sequence = torch.sort(torch.randn((sorted_seq, N), dtype=dtype, device=flag_gems.device))[0]
    values = torch.randn((sorted_seq, N // 2), dtype=dtype, device=flag_gems.device)

    ref_sorted = to_reference(sorted_sequence)
    ref_values = to_reference(values)

    ref_out = torch.searchsorted(ref_sorted, ref_values, side='right')
    with flag_gems.use_gems():
        res_out = torch.searchsorted(sorted_sequence, values, side='right')

    gems_assert_equal(res_out, ref_out)


@pytest.mark.searchsorted
@pytest.mark.parametrize("N", [8, 16, 1024])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_searchsorted_1d(N, dtype):
    """Test searchsorted with 1D sorted_sequence and 1D values"""
    sorted_sequence = torch.sort(torch.randn(N, dtype=dtype, device=flag_gems.device))[0]
    values = torch.randn(N // 2, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("N", [8, 16, 1024])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_searchsorted_scalar_values(N, dtype):
    """Test searchsorted with scalar values (same value searched in 1D sequence)"""
    sorted_sequence = torch.sort(torch.randn(N, dtype=dtype, device=flag_gems.device))[0]
    value = torch.tensor(0.5, dtype=dtype, device=flag_gems.device)

    ref_sorted = to_reference(sorted_sequence)
    ref_value = value.float()  # for comparison

    ref_out = torch.searchsorted(ref_sorted, ref_value)
    with flag_gems.use_gems():
        res_out = torch.searchsorted(sorted_sequence, value)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_searchsorted_out_int32(shape, dtype):
    """Test searchsorted with out_int32=True"""
    sorted_seq, N = shape
    sorted_sequence = torch.sort(torch.randn((sorted_seq, N), dtype=dtype, device=flag_gems.device))[0]
    values = torch.randn((sorted_seq, N // 2), dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.searchsorted(ref_sorted, ref_values, out_int32=True)
    with flag_gems.use_gems():
        res_out = torch.searchsorted(sorted_sequence, values, out_int32=True)
...
@pytest.mark.searchsorted
@pytest.mark.parametrize("shape", [(2, 5), (8, 16)])
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_searchsorted_int(shape, dtype):
    """Test searchsorted with integer types"""
    sorted_seq, N = shape
    sorted_sequence = torch.sort(torch.randint(0, 100, (sorted_seq, N), dtype=dtype, device=flag_gems.device))[0]
    values = torch.randint(0, 100, (sorted_seq, N // 2), dtype=dtype, device=flag_gems.device)
