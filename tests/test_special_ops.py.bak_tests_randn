...
def top_p_sampling_reference(logits: torch.Tensor, top_p: float = 0.9,
                             temperature: float = 1.0) -> torch.Tensor:
    """
    Reference implementation of Top-P (Nucleus) sampling.
    """
    if temperature != 1.0:
    ...
    # Ensure at least one element is kept
    cutoff_mask[:, 0] = True

    # Get cutoff index for each row
    cutoff_indices = cutoff_mask.long().sum(dim=-1)
    ...
    for i in range(batch_size):
        ...
        truncated_probs[i, cutoff:] = 0

    renormalized_probs = truncated_probs / truncated_probs.sum(dim=-1, keepdim=True)

    # Sample from the renormalized distribution
    sampled_indices = torch.multinomial(renormalized_probs, num_samples=1, replacement=False)

    # Map back to original indices
    original_indices = sorted_indices.gather(1, sampled_indices)

    return original_indices.squeeze(-1)


@pytest.mark.top_p_sampling
@pytest.mark.parametrize("batch_size", [1, 4, 8])
@pytest.mark.parametrize("vocab_size", [128, 256, 512])
@pytest.mark.parametrize("top_p", [0.5, 0.9, 0.95])
@pytest.mark.parametrize("temperature", [0.5, 1.0, 2.0])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_top_p_sampling(batch_size, vocab_size, top_p, temperature, dtype):
    if dtype == torch.float16 and batch_size > 1:
        pytest.skip("Skipping large batch size for float16 due to numerical precision")

    torch.manual_seed(42)

    logits = torch.randn(batch_size, vocab_size, dtype=dtype, device=flag_gems.device)

    # FlagGems implementation - test that it runs without error
    with flag_gems.use_gems():
        res_out = flag_gems.top_p_sampling(logits, top_p=top_p, temperature=temperature)

    # Verify output shape
    assert res_out.shape == (batch_size,), f"Expected shape {(batch_size,)}, got {res_out.shape}"

    # Verify sampled indices are valid (within vocabulary range)
    assert torch.all(res_out >= 0) and torch.all(res_out < vocab_size), \
        "Sampled indices out of range"

    # Verify sampled indices are from valid nucleus (cumulative prob <= top_p)
    probs = torch.softmax(logits / temperature, dim=-1)
    sorted_probs, sorted_indices = torch.sort(probs, dim=-1, descending=True)
    cumsum_probs = torch.cumsum(sorted_probs, dim=-1)

    # Check each sampled index
    for i in range(batch_size):
        sampled_idx = res_out[i].item()
        # Find the position of sampled_idx in sorted_indices
        sorted_pos = (sorted_indices[i] == sampled_idx).nonzero(as_tuple=True)[0]
        if len(sorted_pos) > 0:
            pos = sorted_pos[0].item()
            # The cumulative probability at sampled position should be <= top_p
            # (or it's the first element that exceeds top_p)
            assert cumsum_probs[i, pos] <= top_p + 1e-5, \
                f"Sampled index {sampled_idx} has cumsum {cumsum_probs[i, pos]} > top_p {top_p}"


@pytest.mark.top_p_sampling
@pytest.mark.parametrize("vocab_size", [128, 256])
@pytest.mark.parametrize("top_p", [0.9])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_top_p_sampling_1d(vocab_size, top_p, dtype):
    """Test 1D input (single distribution)"""
    torch.manual_seed(42)

    logits = torch.randn(vocab_size, dtype=dtype, device=flag_gems.device)

    # FlagGems implementation - test that it runs without error
    with flag_gems.use_gems():
        res_out = flag_gems.top_p_sampling(logits, top_p=top_p)

    # Verify output shape - for 1D input with n_samples=1, output is scalar
    assert res_out.shape == (), f"Expected shape (), got {res_out.shape}"

    # Verify sampled indices are valid (within vocabulary range)
    assert res_out >= 0 and res_out < vocab_size, "Sampled index out of range"

    # Verify sampled index is from valid nucleus (cumulative prob <= top_p)
    probs = torch.softmax(logits.unsqueeze(0) / 1.0, dim=-1)
    sorted_probs, sorted_indices = torch.sort(probs, dim=-1, descending=True)
    cumsum_probs = torch.cumsum(sorted_probs, dim=-1)

    sampled_idx = res_out.item()
    sorted_pos = (sorted_indices[0] == sampled_idx).nonzero(as_tuple=True)[0]
    if len(sorted_pos) > 0:
        pos = sorted_pos[0].item()
        assert cumsum_probs[0, pos] <= top_p + 1e-5, \
            f"Sampled index {sampled_idx} has cumsum {cumsum_probs[0, pos]} > top_p {top_p}"
