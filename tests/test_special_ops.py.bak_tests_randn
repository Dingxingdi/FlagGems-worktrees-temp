...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_permute(shape, dtype):
    # Test basic permute with various dimension orders
    ndim = len(shape)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Test with reversed dims
    dims = list(reversed(range(ndim)))
    ref_out = torch.permute(ref_inp, dims)
    with flag_gems.use_gems():
        res_out = torch.permute(inp, dims)
...
@pytest.mark.parametrize("shape", PERMUTE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_permute_all_dims(shape, dtype):
    # Test permute with more dimension permutations
    ndim = len(shape)
    if ndim < 2:
    ...
    # Test with cyclic shift: for each ndim, create appropriate permutation
    if ndim == 2:
        dims = [1, 0]
    elif ndim == 3:
        dims = [1, 2, 0]
...
@pytest.mark.permute
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_permute_single_dim(dtype):
    # Test that permute with identity dims works (no-op)
    shape = (2, 3, 4)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    dims = (0, 1, 2)  # identity, should return same tensor
    ref_out = torch.permute(ref_inp, dims)
    with flag_gems.use_gems():
        res_out = torch.permute(inp, dims)
...
@pytest.mark.permute
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_permute_negative_dims(dtype):
    # Test permute with negative dimensions
    shape = (2, 3, 4)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Negative dims should work the same as positive
    dims = (-1, -2, -3)  # equivalent to (2, 1, 0)
    ref_out = torch.permute(ref_inp, dims)
    with flag_gems.use_gems():
        res_out = torch.permute(inp, dims)
