...
CHOLESKY_SHAPES = [(2, 2), (4, 4), (8, 8), (16, 16), (32, 32), (64, 64), (128, 128)]
# Only float32 is supported for Cholesky - PyTorch doesn't support float16/bfloat16
CHOLESKY_DTYPES = [torch.float32]


def make_positive_definite_matrix(n, dtype, device):
    """Create a symmetric positive-definite matrix."""
    # Start with a random matrix
    A = torch.randn(n, n, dtype=dtype, device=device)
    # Make it symmetric positive definite: A = A @ A^T + alpha * I
...
def test_accuracy_cholesky(shape, dtype):
    n = shape[0]
    A = make_positive_definite_matrix(n, dtype, flag_gems.device)
    ref_A = to_reference(A)

    ref_out = torch.linalg.cholesky(ref_A)
    with flag_gems.use_gems():
        res_out = torch.linalg.cholesky(A)
...
@pytest.mark.parametrize("shape", CHOLESKY_SHAPES)
@pytest.mark.parametrize("dtype", CHOLESKY_DTYPES)
def test_accuracy_cholesky_upper(shape, dtype):
    n = shape[0]
    A = make_positive_definite_matrix(n, dtype, flag_gems.device)
    ref_A = to_reference(A)
    ...
    with flag_gems.use_gems():
        res_out = torch.linalg.cholesky(A, upper=True)
...
@pytest.mark.cholesky
@pytest.mark.parametrize("dtype", CHOLESKY_DTYPES)
def test_accuracy_cholesky_batched(dtype):
    batch_size = 4
    n = 16
    A = torch.randn(batch_size, n, n, dtype=dtype, device=flag_gems.device)
    # Make each matrix in batch positive definite
...
@pytest.mark.cholesky_
@pytest.mark.parametrize("shape", CHOLESKY_SHAPES)
@pytest.mark.parametrize("dtype", CHOLESKY_DTYPES)
def test_accuracy_cholesky_(shape, dtype):
    n = shape[0]
    A = make_positive_definite_matrix(n, dtype, flag_gems.device)
    ref_A = to_reference(A.clone())

    ref_out = ref_A.linalg_cholesky_()
    with flag_gems.use_gems():
        res_out = A.linalg_cholesky_()
