...
# Test shapes for _resize_output
_RESIZE_SHAPES = [
    (4,),      # original
    (8,),      # larger
    (2,),      # smaller
...
def _reference_resize_output_(inp, size, device):
    """Reference implementation of _resize_output_ (in-place)."""
    new_numel = 1
    for s in size:
        new_numel *= s
    old_numel = inp.numel()

    # For in-place, resize the tensor
    if inp.device == device:
        inp.resize_(size)
        return inp
    else:
        # Different device - create temp and copy
        out = torch.empty(size, dtype=inp.dtype, device=device)
        copy_numel = min(old_numel, new_numel)
        if copy_numel > 0:
            inp_flat = inp.reshape(-1)[:copy_numel]
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_resize_output_(shape, dtype):
    # Create input tensor with known values for easier debugging
    inp = torch.arange(np.prod(shape), dtype=dtype, device=flag_gems.device)
    inp = inp.reshape(shape)
    ...
    # For in-place, create a copy for reference
    ref_inp = inp.clone()
    ref_out = _reference_resize_output_(ref_inp, target_size, device)

    with flag_gems.use_gems():
        res_out = torch.ops.aten._resize_output_(inp, target_size, device)
