...
@pytest.mark.parametrize(
    "N, C, D, H, W, mode, padding, align_corners",
    [
        (1, 3, 4, 4, 4, 0, 1, False),
    ],
)
...
@pytest.mark.parametrize("seed", [42])
def test_grid_sampler_3d_backward(N, C, D, H, W, mode, padding, align_corners, dtype, seed):
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)

    # Create input and grid
    input = torch.randn(N, C, D, H, W, device=flag_gems.device, dtype=dtype)
    grid = torch.randn(N, D, H, W, 3, device=flag_gems.device, dtype=dtype)

    # Run forward pass to get output
    output = torch.grid_sampler_3d(input, grid, interpolation_mode=mode, padding_mode=padding, align_corners=align_corners)
    ...
    ref_out = torch.ops.aten.grid_sampler_3d_backward.default(
        ref_grad_output, ref_input, ref_grid, mode, padding, align_corners, [True, True]
    )

    with flag_gems.use_gems():
        res_out = torch.ops.aten.grid_sampler_3d_backward.default(
            grad_output, input, grid, mode, padding, align_corners, [True, True]
        )
