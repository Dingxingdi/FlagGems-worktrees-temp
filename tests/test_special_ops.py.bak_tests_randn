...
# Tests for _sparse_coo_tensor_with_dims_and_tensors
@pytest.mark.sparse_coo_tensor_with_dims_and_tensors
@pytest.mark.parametrize(
    "size,nnz,dtype",
    [
        ((10, 10), 5, torch.float32),
        ((10, 10), 5, torch.float16),
        ((10, 10), 5, torch.bfloat16),
        ((10, 20), 10, torch.float32),
        ((100, 100), 50, torch.float32),
    ],
)
def test_accuracy__sparse_coo_tensor_with_dims_and_tensors(size, nnz, dtype):
    """Test creating sparse COO tensors with various configurations."""
    device = flag_gems.device
    sparse_dim = 2
    dense_dim = 0

    # Generate random indices within bounds
    indices = torch.randint(0, size[0], (sparse_dim, nnz), dtype=torch.long, device=device)
    indices[1] = torch.randint(0, size[1], (nnz,), dtype=torch.long, device=device)

    # Generate random values
    values = torch.randn(nnz, dtype=dtype, device=device)

    # Create sparse tensor using FlagGems
    with flag_gems.use_gems():
        res_out = torch.ops.aten._sparse_coo_tensor_with_dims_and_tensors(
            sparse_dim, dense_dim, list(size), indices, values
        )

    # Create reference using torch.sparse_coo_tensor
    ref_out = torch.sparse_coo_tensor(indices, values, size)

    # Compare results
    assert res_out.shape == ref_out.shape
    assert res_out.is_sparse == ref_out.is_sparse
    assert res_out.sparse_dim() == ref_out.sparse_dim()
    assert res_out.dense_dim() == ref_out.dense_dim()

    # Compare indices and values
    res_indices = res_out._indices()
    res_values = res_out._values()
    ref_indices = ref_out._indices()
    ref_values = ref_out._values()

    # For sparse tensors, indices are always long - use gems_assert_equal
    gems_assert_equal(res_indices, ref_indices)

    # For values, compare with appropriate tolerance based on dtype
    if dtype == torch.float32:
        gems_assert_close(res_values, ref_values, dtype)
    elif dtype == torch.float16:
        gems_assert_close(res_values, ref_values, dtype, atol=1e-3)
    else:  # bfloat16
        gems_assert_close(res_values, ref_values, dtype, atol=2e-2)
...
def test_accuracy__sparse_coo_tensor_with_dims_and_tensors_dense(dtype):
    """Test creating sparse COO tensors with dense dimensions."""
    device = flag_gems.device
    sparse_dim = 2
    dense_dim = 2
    size = (10, 10, 4, 4)
    ...
    # Generate random dense values
    dense_size = (nnz, size[2], size[3])
    values = torch.randn(dense_size, dtype=dtype, device=device)
