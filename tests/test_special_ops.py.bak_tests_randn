...
@pytest.mark.parametrize("shape", CHANNEL_SHUFFLE_SHAPES)
@pytest.mark.parametrize("groups", GROUPS_LIST)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_channel_shuffle(shape, groups, dtype):
    if shape[1] % groups != 0:
        pytest.skip("Number of channels must be divisible by groups")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
