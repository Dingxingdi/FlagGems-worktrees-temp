...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("reduction", [0, 1, 2])
def test_accuracy_binary_cross_entropy_backward(shape, dtype, reduction):
    ...
    ref_out = torch.ops.aten.binary_cross_entropy_backward(
        ref_grad_output := torch.ones_like(ref_self),
        ref_self,
        ref_target,
        None,
        reduction,
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("reduction", [0, 1, 2])
def test_accuracy_binary_cross_entropy_backward_weighted(shape, dtype, reduction):
