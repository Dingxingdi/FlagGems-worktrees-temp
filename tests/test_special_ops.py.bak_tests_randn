...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + ALL_INT_DTYPES)
def test_accuracy_pin_memory(shape, dtype):
    # pin_memory only works on CPU tensors
    if dtype in FLOAT_DTYPES:
        inp = torch.randn(shape, dtype=dtype, device="cpu")
    else:
        inp = torch.randint(low=-10000, high=10000, size=shape, dtype=dtype, device="cpu")

    ref_inp = to_reference(inp)
    ref_out = ref_inp.pin_memory()
    with flag_gems.use_gems():
        res_out = inp.pin_memory()

    # Check that the tensor is pinned
    assert res_out.is_pinned() == True
    assert ref_out.is_pinned() == True
    gems_assert_equal(res_out, ref_out)
