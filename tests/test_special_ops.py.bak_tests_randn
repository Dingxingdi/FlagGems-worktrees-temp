...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rrelu_with_noise_functional(shape, dtype):
    ...
    noise = torch.rand(shape, dtype=dtype, device=flag_gems.device) * (upper - lower) + lower
    ...
    gems_assert_close(res_noise_out, ref_noise_out, dtype)
