...
# Shapes for 3D pooling tests (N, C, D, H, W)
POOLING3D_SHAPES = [
    (1, 3, 4, 4, 4),
    (2, 8, 8, 8, 8),
    (4, 16, 16, 16, 16),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__adaptive_avg_pool3d(shape, dtype):
    # Test with output_size being a single integer
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    ...
    with flag_gems.use_gems():
        res_out = torch._adaptive_avg_pool3d(inp, output_size)
...
@pytest.mark.parametrize("shape", POOLING3D_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__adaptive_avg_pool3d_tuple_output_size(shape, dtype):
    # Test with output_size being a tuple of 3 integers
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.adaptive_avg_pool3d
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__adaptive_avg_pool3d_large_input(dtype):
    # Test with larger input and output sizes
    shape = (1, 3, 32, 32, 32)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
