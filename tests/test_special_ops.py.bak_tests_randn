...
def test_accuracy__assert_tensor_metadata(shape, dtype):
    ...
    # Test with no parameters - should pass silently
    ref_out = torch._assert_tensor_metadata(ref_inp)
    with flag_gems.use_gems():
    ...
    ref_out = torch._assert_tensor_metadata(
        ref_inp, size=list(shape), stride=list(inp.stride()), dtype=dtype
    )
    with flag_gems.use_gems():
...
@pytest.mark.assert_tensor_metadata
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__assert_tensor_metadata_errors(dtype):
    inp = torch.randn((2, 3), dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Test with mismatched size
    with pytest.raises(RuntimeError):
        torch._assert_tensor_metadata(ref_inp, size=[3, 2])
    with flag_gems.use_gems():
        with pytest.raises(RuntimeError):
            torch._assert_tensor_metadata(inp, size=[3, 2])

    # Test with mismatched stride
    with pytest.raises(RuntimeError):
        torch._assert_tensor_metadata(ref_inp, stride=[1, 3])
    with flag_gems.use_gems():
        with pytest.raises(RuntimeError):
            torch._assert_tensor_metadata(inp, stride=[1, 3])

    # Test with mismatched dtype
    wrong_dtype = torch.float16 if dtype != torch.float16 else torch.float32
    with pytest.raises(RuntimeError):
        torch._assert_tensor_metadata(ref_inp, dtype=wrong_dtype)
    with flag_gems.use_gems():
        with pytest.raises(RuntimeError):
            torch._assert_tensor_metadata(inp, dtype=wrong_dtype)
