...
from flag_gems.ops.Top_K_Sampling import top_k_sampling as top_k_sampling_ref
...
# Define Top_K_Sampling test shapes and parameters
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_top_k_sampling(batch_size, vocab_size, k, dtype):
    ...
    # Compare - since sampling is stochastic, we compare that the sampling
    # produces valid indices (within range [0, actual_k))
    # The actual sampled value depends on random numbers, so we verify:
    # 1. Output shape is correct
    # 2. All indices are in valid range [0, vocab_size)
    assert res_out.shape == ref_out.shape, f"Shape mismatch: {res_out.shape} vs {ref_out.shape}"
    assert res_out.dtype == ref_out.dtype, f"Dtype mismatch: {res_out.dtype} vs {ref_out.dtype}"

    # Check that all indices are valid
    assert (res_out >= 0).all(), "Sampled indices should be non-negative"
    assert (res_out < vocab_size).all(), "Sampled indices should be less than vocab_size"

    # Also verify using the flag_gems function directly
    with flag_gems.use_gems():
        res_out2 = flag_gems.top_k_sampling(logits, actual_k)

    assert (res_out == res_out2).all(), "Direct call should match method call"


@pytest.mark.Top_K_Sampling
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_top_k_sampling_single_element(dtype):
    ...
    assert (res_out >= 0).all() and (res_out < vocab_size).all(), "Invalid sampled index"
