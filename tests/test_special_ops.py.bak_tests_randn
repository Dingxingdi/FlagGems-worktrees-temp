...
def test_accuracy_randint(shape, dtype):
    high = 100
    with flag_gems.use_gems():
        res_out = torch.randint(high=high, size=shape, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", ALL_INT_DTYPES)
def test_accuracy_randint_like(shape, dtype):
    high = 100
    inp = torch.empty(shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res_out = torch.randint_like(inp, high=high)
    assert res_out.shape == shape
    assert res_out.dtype == dtype
    assert (res_out >= 0).all()
    assert (res_out < high).all()
