...
# GRU operator tests
GRU_SHAPES = [
    (3, 2, 4, 5),  # (seq_len, batch, input_size, hidden_size)
    (5, 1, 8, 16),
    (1, 4, 10, 20),
]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_gru(shape, dtype):
    seq_len, batch, input_size, hidden_size = shape

    # Create GRU module to get parameters - move to GPU with correct dtype
    gru = torch.nn.GRU(
    ...
    input = torch.randn(seq_len, batch, input_size, dtype=dtype, device=flag_gems.device)
    hx = torch.randn(1, batch, hidden_size, dtype=dtype, device=flag_gems.device)

    ref_input = to_reference(input)
    ref_hx = to_reference(hx)
    ref_params = [to_reference(p) for p in params]

    # Call GRU on reference
    ref_out, ref_hidden = torch.ops.aten.gru(
        ref_input, ref_hx, ref_params, True, 1, 0.0, False, False, False
    )

    # Call GRU with FlagGems
    with flag_gems.use_gems():
        res_out, res_hidden = torch.ops.aten.gru(
            input, hx, params, True, 1, 0.0, False, False, False
        )

    # Compare outputs
    gems_assert_close(res_out, ref_out, dtype)
    gems_assert_close(res_hidden, ref_hidden, dtype)
