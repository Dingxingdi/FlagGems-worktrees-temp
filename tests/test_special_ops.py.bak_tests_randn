...
# Shapes for nonzero_numpy tests
NONZERO_NUMPY_SHAPES = [(2, 32), (16, 16), (4, 8, 16), (2, 3, 4, 5)]


@pytest.mark.nonzero_numpy
@pytest.mark.parametrize("shape", NONZERO_NUMPY_SHAPES)
...
def test_accuracy_nonzero_numpy(shape, dtype):
    ...
    ref_out = torch.ops.aten.nonzero_numpy.default(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.nonzero_numpy.default(inp)

    # Compare length of output list
    assert len(res_out) == len(ref_out), f"Length mismatch: {len(res_out)} vs {len(ref_out)}"

    # Compare each index tensor
    for i, (res_idx, ref_idx) in enumerate(zip(res_out, ref_out)):
...
def test_accuracy_nonzero_numpy_1d(dtype):
    shape = (32,)
    if dtype == torch.bool:
        inp = torch.randint(0, 2, shape, dtype=torch.int, device=flag_gems.device).to(dtype)
    elif dtype == torch.int64 or dtype in INT_DTYPES:
        inp = torch.randint(-3, 3, shape, device=flag_gems.device).to(dtype)
    else:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp, False)
    ...
    for res_idx, ref_idx in zip(res_out, ref_out):
        gems_assert_equal(res_idx, ref_idx)
...
@pytest.mark.nonzero_numpy
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_nonzero_numpy_empty(dtype):
    shape = (0,)
    ...
    assert len(res_out) == len(ref_out) == 1
    gems_assert_equal(res_out[0], ref_out[0])
