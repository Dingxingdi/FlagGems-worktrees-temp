...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES + BOOL_TYPES)
def test_accuracy_empty(shape, dtype):
    """Test for torch.empty - verifies shape and dtype are correct."""
    with flag_gems.use_gems():
        res_out = torch.empty(*shape, dtype=dtype)

    ref_out = torch.empty(*shape, dtype=dtype)

    # Empty returns uninitialized data, so we only verify shape and dtype
...
@pytest.mark.empty
def test_accuracy_empty_default_dtype():
    """Test for torch.empty with default dtype."""
    with flag_gems.use_gems():
        res_out = torch.empty(10, 20)

    ref_out = torch.empty(10, 20)

    # Verify shape
    assert res_out.shape == ref_out.shape
    # Verify default dtype matches
