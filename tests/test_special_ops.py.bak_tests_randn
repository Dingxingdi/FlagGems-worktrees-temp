...
def test_accuracy_gather_block_quantized(shape, block_size):
    ...
    quantized_data = torch.randint(-100, 100, shape, dtype=torch.int8, device=flag_gems.device)

    # Generate scale factors per block
    scales = torch.rand(n_blocks, dtype=torch.float32, device=flag_gems.device) * 2 + 0.5  # scales in [0.5, 2.5]

    # Test without indices (dequantize all)
    with flag_gems.use_gems():
        res_out = flag_gems.ops.gather_block_quantized(quantized_data, scales, None, block_size)

    # Reference implementation
    ref_out = torch.empty_like(res_out.cpu().float())
    for i in range(n_elements):
        block_idx = i // block_size
        if block_idx < n_blocks:
            ref_out[i] = quantized_data[i].float() * scales[block_idx].float()
        else:
            ref_out[i] = 0
