...
@pytest.mark.parametrize("shape", UNSAFE_INDEX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__unsafe_index(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Create index tensors - use 2 indices for advanced indexing
    idx1_size = 8
    idx2_size = 8
    idx1 = torch.randint(0, shape[0], (idx1_size,), device=flag_gems.device)
    idx2 = torch.randint(0, shape[1], (idx2_size,), device=flag_gems.device)

    ref_inp = to_reference(inp)
    ref_idx1 = to_reference(idx1)
    ref_idx2 = to_reference(idx2)

    ref_out = torch._unsafe_index(ref_inp, (ref_idx1, ref_idx2))
    with flag_gems.use_gems():
        res_out = torch._unsafe_index(inp, (idx1, idx2))
...
@pytest.mark.parametrize("shape", UNSAFE_INDEX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__unsafe_index_single_index(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Single index tensor
    idx_size = 16
    idx = torch.randint(0, shape[0], (idx_size,), device=flag_gems.device)

    ref_inp = to_reference(inp)
    ref_idx = to_reference(idx)

    ref_out = torch._unsafe_index(ref_inp, (ref_idx,))
    with flag_gems.use_gems():
        res_out = torch._unsafe_index(inp, (idx,))
...
@pytest.mark.parametrize("shape", UNSAFE_INDEX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__unsafe_index_broadcast(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Two indices with shapes (4, 1) and (1, 8) that broadcast to (4, 8)
    idx1 = torch.randint(0, shape[0], (4, 1), device=flag_gems.device)
    idx2 = torch.randint(0, shape[1], (1, 8), device=flag_gems.device)
...
@pytest.mark.parametrize("shape", UNSAFE_INDEX_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__unsafe_index_three_indices(shape, dtype):
    if len(shape) < 3:
        pytest.skip("Need at least 3 dimensions for three indices")
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Three index tensors
    idx1_size = 4
    idx2_size = 4
    ...
    idx1 = torch.randint(0, shape[0], (idx1_size,), device=flag_gems.device)
    idx2 = torch.randint(0, shape[1], (idx2_size,), device=flag_gems.device)
    idx3 = torch.randint(0, shape[2], (idx3_size,), device=flag_gems.device)

    ref_inp = to_reference(inp)
    ref_idx1 = to_reference(idx1)
    ref_idx2 = to_reference(idx2)
    ref_idx3 = to_reference(idx3)

    ref_out = torch._unsafe_index(ref_inp, (ref_idx1, ref_idx2, ref_idx3))
    with flag_gems.use_gems():
        res_out = torch._unsafe_index(inp, (idx1, idx2, idx3))
