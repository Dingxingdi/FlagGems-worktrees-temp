...
# Shapes for linalg_householder_product: (m, n) where m >= n
HOUSEHOLDER_SHAPES = [
    (4, 3),
    (8, 5),
    (16, 8),
...
# Only float32 is supported because torch.linalg.householder_product doesn't support float16/bfloat16 on CUDA
...
def test_accuracy_linalg_householder_product(m, n, dtype):
    # Create input matrix A with Householder vectors in lower triangular part
    # We use geqrf to get a valid (A, tau) pair
    # Note: geqrf is not implemented for float16/bfloat16, so we generate in float32 and convert
    ...
    ref_h = to_reference(h)
    ref_tau = to_reference(tau)
...
@pytest.mark.inplace
@pytest.mark.linalg_householder_product_
@pytest.mark.parametrize("m, n", HOUSEHOLDER_SHAPES)
@pytest.mark.parametrize("dtype", HOUSEHOLDER_DTYPES)
def test_accuracy_linalg_householder_product_(m, n, dtype):
    # Create input matrix A with Householder vectors in lower triangular part
    # Note: geqrf is not implemented for float16/bfloat16, so we generate in float32 and convert
    A_gen = torch.randn(m, n, dtype=torch.float32, device=flag_gems.device)
    h_float, tau_float = torch.geqrf(A_gen)
    ...
    with flag_gems.use_gems():
        res_out = torch.linalg.householder_product(h, tau)
        h.copy_(res_out)  # in-place behavior
...
@pytest.mark.parametrize("dtype", HOUSEHOLDER_DTYPES)
def test_accuracy_linalg_householder_product_batched(shape, dtype):
