...
# Reference implementation for _fused_moving_avg_obs_fq_helper
def _fused_moving_avg_obs_fq_helper_ref(
    self,
    observer_on,
    fake_quant_on,
    running_min,
    ...
    input_vals = self.clone()
    observer_on_vals = observer_on.clone()
    fake_quant_on_vals = fake_quant_on.clone()

    # Get scale and zero_point values
    scale_val = scale.item() if scale.numel() > 0 else 0.1
    zero_point_val = zero_point.item() if zero_point.numel() > 0 else 0.0

    # Compute masks
    observer_mask = observer_on_vals > 0
    fake_quant_mask = fake_quant_on_vals > 0
    ...
    if symmetric_quant:
        ...
        quantized_vals = torch.round(scaled_vals)
        quantized_vals = torch.clamp(quantized_vals, quant_min, quant_max)
        output = torch.where(fake_quant_mask, quantized_vals * scale_val, input_vals)
    else:
        # Asymmetric quantization
        scaled_vals = input_vals / scale_val + zero_point_val
        quantized_vals = torch.round(scaled_vals)
        quantized_vals = torch.clamp(quantized_vals, quant_min, quant_max)
        output = torch.where(fake_quant_mask, (quantized_vals - zero_point_val) * scale_val, input_vals)

    return output, quant_mask


@pytest.mark.fused_moving_avg_obs_fq_helper
@pytest.mark.parametrize("shape", [(2, 4), (4, 8), (16, 32)])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fused_moving_avg_obs_fq_helper(shape, dtype):
    """Test _fused_moving_avg_obs_fq_helper accuracy."""
    torch.manual_seed(42)
    device = flag_gems.device

    # Create input tensors
    self = torch.randn(shape, dtype=dtype, device=device)
    observer_on = torch.ones(shape, dtype=dtype, device=device)
    fake_quant_on = torch.ones(shape, dtype=dtype, device=device)

    # Create scale and zero_point
    running_min = torch.tensor([0.0], dtype=torch.float32, device=device)
    running_max = torch.tensor([1.0], dtype=torch.float32, device=device)
    scale = torch.tensor([0.1], dtype=torch.float32, device=device)
    zero_point = torch.tensor([0.0], dtype=torch.float32, device=device)
    ...
    ref_output, ref_mask = _fused_moving_avg_obs_fq_helper_ref(
        self.cpu().float(),
        observer_on.cpu().float(),
        fake_quant_on.cpu().float(),
        running_min.cpu(),
        running_max.cpu(),
        scale.cpu(),
        zero_point.cpu(),
    ...
    # Compute FlagGems output
    with flag_gems.use_gems():
        res_output, res_mask = torch._fused_moving_avg_obs_fq_helper(
            self,
            observer_on,
            fake_quant_on,
            running_min,
    ...
    assert res_output.shape == ref_output.shape, f"Shape mismatch: {res_output.shape} vs {ref_output.shape}"
    assert res_mask.shape == ref_mask.shape, f"Mask shape mismatch: {res_mask.shape} vs {ref_mask.shape}"

    # Compare outputs - move ref to the same device and dtype
    ref_output_gems = ref_output.to(dtype=dtype, device=res_output.device)
    gems_assert_close(res_output, ref_output_gems, dtype)

    # Compare masks - move to CPU and compare
    ref_mask_cpu = ref_mask.to(device='cpu')
    res_mask_cpu = res_mask.cpu()
    assert torch.all(res_mask_cpu == ref_mask_cpu), f"Mask mismatch"
