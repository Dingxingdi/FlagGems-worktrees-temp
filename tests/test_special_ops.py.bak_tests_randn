...
# Add+Softmax: fused add + softmax operation
@pytest.mark.add_softmax
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, -1])
def test_accuracy_add_softmax(shape, dtype, dim):
    """Test fused add + softmax operation."""
    if len(shape) == 0:
        # Skip scalar tensors for softmax
        pytest.skip("Scalar tensor not supported for softmax")
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    # Reference: add then softmax
    ref_out = torch.nn.functional.softmax(ref_inp1 + ref_inp2, dim=dim)
    with flag_gems.use_gems():
        res_out = flag_gems.add_softmax(inp1, inp2, dim=dim)
