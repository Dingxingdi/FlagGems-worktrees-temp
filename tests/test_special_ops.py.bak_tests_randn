...
# Test shapes for FFT - power of 2 sizes work best
FFT_SHAPES = (
    [(8, 16)]
    if QUICK_MODE
    else [(4, 8), (8, 16), (16, 32), (32, 64), (4, 8, 16)]
)
...
def test_accuracy_fft_hfftn(shape, dtype):
    """Test fft_hfftn accuracy against torch reference."""
    if not bf16_is_supported and dtype == torch.bfloat16:
        pytest.skip("bfloat16 not supported")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Test basic fft_hfftn (all dimensions) - use rfftn as reference since torch.fft_hfftn may not exist
    ref_out = torch.fft.rfftn(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.fft_hfftn(inp)

    # FFT output is complex, use the result's dtype for comparison
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_hfftn_with_norm(shape, dtype):
    """Test fft_hfftn with normalization."""
    if not bf16_is_supported and dtype == torch.bfloat16:
        pytest.skip("bfloat16 not supported")
    ...
    for norm in ["backward", "forward", "ortho"]:
        ...
        with flag_gems.use_gems():
            res_out = torch.ops.aten.fft_hfftn(inp, norm=norm)
...
@pytest.mark.parametrize("shape", FFT_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_fft_hfftn_with_s(shape, dtype):
    """Test fft_hfftn with output shape parameter."""
    if not bf16_is_supported and dtype == torch.bfloat16:
        pytest.skip("bfloat16 not supported")
    ...
    # Test with padding (s parameter larger than input)
    s = tuple(d * 2 for d in shape)
    ref_out = torch.fft.rfftn(ref_inp, s=s)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.fft_hfftn(inp, s=s)
