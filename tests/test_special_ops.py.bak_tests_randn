...
# Matrix shapes for LU factorization tests
LU_FACTOR_SHAPES = [
    (3, 3),
    (4, 4),
    (8, 8),
...
@pytest.mark.parametrize("shape", LU_FACTOR_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("pivot", [True, False])
def test_accuracy_linalg_lu_factor_ex(shape, dtype, pivot):
    # Generate a non-singular matrix for LU factorization
    # A = L @ U where L and U are random, so A is non-singular
    torch.manual_seed(42)
    L = torch.tril(torch.randn(shape, dtype=dtype, device=flag_gems.device) * 0.5 + 0.5)
    U = torch.triu(torch.randn(shape, dtype=dtype, device=flag_gems.device) * 0.5 + 0.5)
    # Ensure diagonal of U is positive to make matrix well-conditioned
    for i in range(min(shape)):
        U[i, i] = torch.abs(U[i, i]) + 0.1
    A = L @ U

    ref_A = to_reference(A)

    ref_LU, ref_pivots, ref_info = torch.linalg.lu_factor_ex(ref_A, pivot=pivot)
    # Call the FlagGems implementation directly to avoid recursion
    from flag_gems.ops.linalg_lu_factor_ex import linalg_lu_factor_ex
    res_LU, res_pivots, res_info = linalg_lu_factor_ex(A, pivot=pivot)

    gems_assert_close(res_LU, ref_LU, dtype)
    gems_assert_equal(res_pivots, ref_pivots)
    gems_assert_equal(res_info, ref_info)


@pytest.mark.linalg_lu_factor_ex
@pytest.mark.parametrize("shape", [(3, 4), (4, 3), (8, 12), (12, 8)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_linalg_lu_factor_ex_non_square(shape, dtype):
    """Test LU factorization for non-square matrices."""
    torch.manual_seed(42)
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    ref_A = to_reference(A)

    ref_LU, ref_pivots, ref_info = torch.linalg.lu_factor_ex(ref_A)
    # Call the FlagGems implementation directly to avoid recursion
    from flag_gems.ops.linalg_lu_factor_ex import linalg_lu_factor_ex
    res_LU, res_pivots, res_info = linalg_lu_factor_ex(A)
