...
# Test shapes for as_strided_scatter
AS_STRIDED_SCATTER_SHAPES = [
    (3, 3),
    (4, 4),
    (5, 5),
    (8, 8),
...
@pytest.mark.parametrize("shape", AS_STRIDED_SCATTER_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_scatter(shape, dtype):
    # Create input tensor with random values
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Create src tensor with smaller size that fits the as_strided view
    src_shape = (shape[0] - 1, shape[1] - 1)
    src = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
    ref_src = to_reference(src)

    # Define as_strided view parameters
    size = (shape[0] - 1, shape[1] - 1)
    stride = (1, shape[1])  # row-major stride within the input

    ref_out = torch.as_strided_scatter(ref_inp, ref_src, size, stride)
    with flag_gems.use_gems():
        res_out = torch.as_strided_scatter(inp, src, size, stride)
...
@pytest.mark.as_strided_scatter
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_scatter_with_storage_offset(dtype):
    ...
    size = (2, 2)
    stride = (1, 4)  # row-major
    storage_offset = 5

    ref_out = torch.as_strided_scatter(ref_inp, ref_src, size, stride, storage_offset)
    with flag_gems.use_gems():
        res_out = torch.as_strided_scatter(inp, src, size, stride, storage_offset)
...
@pytest.mark.as_strided_scatter
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_scatter_3d(dtype):
    # Test with 3D tensor
    shape = (4, 4, 4)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    ...
    size = (2, 2, 2)
    stride = (1, 4, 16)  # row-major within the 3D tensor
