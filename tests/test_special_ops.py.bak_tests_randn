...
@pytest.mark.expand
@pytest.mark.parametrize("in_shape", EXPAND_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_expand(in_shape, dtype):
    # Generate input tensor with given shape
    inp = torch.randn(in_shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Compute target shapes for expansion
    # Test various expansion patterns based on input shape
    if len(in_shape) == 0:
        # Scalar tensor - can expand to any shape
        target_shapes = [(1,), (4,), (1, 4), (2, 3, 4)]
    elif len(in_shape) == 1:
        target_shapes = [
            in_shape,  # no change
            (in_shape[0] * 2,) if in_shape[0] > 0 else (4,),  # expand first dim
            (1, in_shape[0]) if in_shape[0] > 0 else (1, 4),  # add leading dim
        ]
    elif len(in_shape) == 2:
        # Test: expand first dim, expand second dim, expand both
        target_shapes = [
            in_shape,  # no change
            (in_shape[0] * 2, in_shape[1]) if in_shape[0] > 0 else (4, in_shape[1]),
            (in_shape[0], in_shape[1] * 2) if in_shape[1] > 0 else (in_shape[0], 4),
            (in_shape[0] * 2, in_shape[1] * 3) if in_shape[0] > 0 and in_shape[1] > 0 else (4, 6),
            (1, in_shape[1]) if in_shape[1] > 0 else (1, 4),  # expand first dim to 1
        ]
    elif len(in_shape) == 3:
        target_shapes = [
            in_shape,  # no change
            (in_shape[0] * 2, in_shape[1], in_shape[2]),
            (in_shape[0], in_shape[1] * 2, in_shape[2]),
            (in_shape[0], in_shape[1], in_shape[2] * 3),
        ]
    else:
        # For higher dimensions, just test no change and one expansion
        target_shapes = [in_shape]
        if in_shape[0] > 0:
            target_shapes.append((in_shape[0] * 2,) + in_shape[1:])

    for target_shape in target_shapes:
        try:
            ref_out = ref_inp.expand(target_shape)
            with flag_gems.use_gems():
                res_out = inp.expand(target_shape)
            gems_assert_equal(res_out, ref_out)
        except RuntimeError:
            # Some expansions may not be valid, skip them
            pass
...
def test_accuracy_expand_with_minus_one(dtype):
    # Test expand with -1 (keep size) semantic
    inp = torch.randn((3, 1), dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = ref_inp.expand((3, 4))
    with flag_gems.use_gems():
        res_out = inp.expand((3, 4))
...
@pytest.mark.expand
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_expand_add_dimensions(dtype):
    # Test adding leading dimensions
    inp = torch.randn((2, 3), dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = ref_inp.expand((4, 5, 2, 3))
    with flag_gems.use_gems():
        res_out = inp.expand((4, 5, 2, 3))
