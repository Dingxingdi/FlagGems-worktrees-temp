...
# Test sym_stride operator
SYML_STRIDE_SHAPES = [(2, 3), (10, 20, 30), (5, 10), (100,), (1, 2, 3, 4)]


@pytest.mark.sym_stride
@pytest.mark.parametrize("shape", SYML_STRIDE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_sym_stride(shape, dtype):
    ...
    # Compare stride results
    assert res_out == ref_out, f"Stride mismatch: {res_out} vs {ref_out}"
