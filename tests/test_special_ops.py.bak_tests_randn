...
# linalg_qr supports float32, float64 (complex types have Triton issues)
LINALG_QR_DTYPES = [torch.float32, torch.float64]
LINALG_QR_SHAPES = [(4, 3), (8, 6), (16, 12), (32, 24), (64, 48), (128, 96)]


@pytest.mark.linalg_qr
@pytest.mark.parametrize("shape", LINALG_QR_SHAPES)
@pytest.mark.parametrize("dtype", LINALG_QR_DTYPES)
def test_accuracy_linalg_qr(shape, dtype):
    # QR decomposition is not unique - different implementations can produce
    # different Q and R (up to sign changes). Instead of comparing Q and R directly,
    # we verify the reconstruction Q @ R equals the original matrix A.
    A = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_A = to_reference(A)

    with flag_gems.use_gems():
        res_Q, res_R = torch.linalg.qr(A)

    # Verify reconstruction: Q @ R should equal A
    res_reconstruction = res_Q @ res_R
    gems_assert_close(res_reconstruction, ref_A, dtype)

    # Also verify Q has orthonormal columns: Q^T @ Q should be identity
    qoq = res_Q.T @ res_Q
    identity = torch.eye(min(shape[0], shape[1]), dtype=dtype, device=flag_gems.device)
    gems_assert_close(qoq, identity, dtype)
