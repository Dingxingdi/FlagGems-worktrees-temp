...
# Conv_BN_ReLU tests: Conv2d + ReLU fusion
CONV_BN_RELU_SHAPES = [
    (1, 3, 32, 32),  # (N, C_in, H, W)
    (2, 16, 16, 16),
    (4, 8, 8, 8),
    (1, 32, 64, 64),
]

CONV_BN_RELU_KERNELS = [
    (3, 3),  # (kH, kW)
    (1, 1),
]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("kernel_size", CONV_BN_RELU_KERNELS)
@pytest.mark.parametrize("stride", [1])
@pytest.mark.parametrize("padding", [0, 1])
def test_accuracy_conv_bn_relu(shape, dtype, kernel_size, stride, padding):
    ...
    if out_H <= 0 or out_W <= 0:
        pytest.skip("Invalid output size")

    input = torch.randn(N, C_in, H, W, dtype=dtype, device=flag_gems.device)
    weight = torch.randn(C_out, C_in, kH, kW, dtype=dtype, device=flag_gems.device)

    # Reference implementation: use flag_gems conv2d + relu (without calling conv_bn_relu directly)
    with flag_gems.use_gems():
        ref_conv = flag_gems.conv2d(input, weight, stride=stride, padding=padding)
        ref_out = flag_gems.relu(ref_conv)

    # Test implementation
    res_out = flag_gems.conv_bn_relu(input, weight, stride=stride, padding=padding)
