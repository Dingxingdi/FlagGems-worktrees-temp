...
# Test for is_non_overlapping_and_dense operator
# This operator checks if a tensor's storage is non-overlapping and dense


@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_contiguous(dtype):
    """Test contiguous tensor"""
    shape = (4, 8, 16)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.ops.aten.is_non_overlapping_and_dense.default(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.is_non_overlapping_and_dense.default(inp)
...
@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_transpose(dtype):
    """Test transposed tensor"""
    inp = torch.randn(4, 8, dtype=dtype, device=flag_gems.device).t()
    ref_inp = to_reference(inp)
...
@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_strided(dtype):
    """Test strided tensor (slice with step > 1)"""
    inp = torch.randn(4, 8, dtype=dtype, device=flag_gems.device)[:, ::2]
    ref_inp = to_reference(inp)
...
@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_permute(dtype):
    """Test permuted tensor"""
    inp = torch.randn(2, 3, 4, dtype=dtype, device=flag_gems.device).permute(2, 0, 1)
    ref_inp = to_reference(inp)
...
@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_1d(dtype):
    """Test 1D tensor"""
    shape = (1024,)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.is_non_overlapping_and_dense
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_is_non_overlapping_and_dense_view(dtype):
    """Test view tensor"""
    inp = torch.randn(24, dtype=dtype, device=flag_gems.device).view(2, 3, 4)
    ref_inp = to_reference(inp)
