...
HISTC_SHAPES = (
    [(2, 19, 7)]
    if QUICK_MODE
    else [(1,), (1024,), (1024, 1024), (20, 320, 15), (16, 128, 64, 60)]
)


@pytest.mark.histc
@pytest.mark.parametrize("shape", HISTC_SHAPES)
@pytest.mark.parametrize("bins", [10, 100, 256])
def test_accuracy_histc(shape, bins):
    inp = torch.randn(shape, dtype=torch.float32, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.histc(ref_inp, bins=bins)
    with flag_gems.use_gems():
        res_out = torch.histc(inp, bins=bins)

    # Use gems_assert_close with atol=5 because floating-point precision
    # at bin boundaries can cause differences of up to 4-5 counts out of millions
    gems_assert_close(res_out, ref_out, dtype=torch.float32, atol=5)


@pytest.mark.histc
@pytest.mark.parametrize("shape", HISTC_SHAPES)
@pytest.mark.parametrize("bins", [10, 100])
def test_accuracy_histc_with_min_max(shape, bins):
    inp = torch.randn(shape, dtype=torch.float32, device=flag_gems.device) * 10
    ref_inp = to_reference(inp)

    min_val = -5.0
    max_val = 5.0
    ref_out = torch.histc(ref_inp, bins=bins, min=min_val, max=max_val)
    with flag_gems.use_gems():
        res_out = torch.histc(inp, bins=bins, min=min_val, max=max_val)

    gems_assert_close(res_out, ref_out, dtype=torch.float32, atol=1)


@pytest.mark.histc
@pytest.mark.parametrize("shape", HISTC_SHAPES)
@pytest.mark.parametrize("bins", [10, 100])
def test_accuracy_histc_with_nan(shape, bins):
    inp = torch.randn(shape, dtype=torch.float32, device=flag_gems.device)
    # Set some elements to NaN
    inp.view(-1)[::10] = float("nan")
    ref_inp = to_reference(inp)

    # PyTorch's histc throws an error when min/max are both NaN (which happens
    # when all values are NaN). Only test when we have valid values.
    try:
        ref_out = torch.histc(ref_inp, bins=bins)
    except RuntimeError as e:
        if "not finite" in str(e):
            # When all values are NaN, PyTorch can't compute min/max
            # Just verify that GEMS also handles this gracefully
            with flag_gems.use_gems():
                res_out = torch.histc(inp, bins=bins)
            # Both should return zeros or handle gracefully
            return
        raise
...
@pytest.mark.histc
def test_accuracy_histc_edge_cases():
    # Test empty tensor
    inp_empty = torch.tensor([], device=flag_gems.device)
    ref_inp_empty = to_reference(inp_empty)
    ref_out_empty = torch.histc(ref_inp_empty, bins=10)
    with flag_gems.use_gems():
        res_out_empty = torch.histc(inp_empty, bins=10)
    gems_assert_equal(res_out_empty, ref_out_empty)

    # Test tensor with all same values
    inp_same = torch.ones(100, device=flag_gems.device, dtype=torch.float32)
    ref_inp_same = to_reference(inp_same)
    ref_out_same = torch.histc(ref_inp_same, bins=10)
    with flag_gems.use_gems():
        res_out_same = torch.histc(inp_same, bins=10)
    gems_assert_equal(res_out_same, ref_out_same)
