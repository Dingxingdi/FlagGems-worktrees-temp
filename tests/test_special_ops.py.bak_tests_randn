...
# segment_reduce tests
SEGMENT_REDUCE_SHAPES = [(10,), (100,), (1000,)]


@pytest.mark.segment_reduce
@pytest.mark.parametrize("shape", [(100,), (1000,)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_segment_reduce_sum_lengths(shape, dtype):
    # Generate random lengths that sum to exactly shape[0]
    num_segments = shape[0] // 10
    data = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # First generate random lengths
    lengths = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=flag_gems.device)
    # Scale to make sure sum equals shape[0]
    total_len = lengths.sum().item()
    lengths = lengths * shape[0] // total_len
    # Adjust last element to ensure exact sum
    lengths[-1] = shape[0] - lengths[:-1].sum().item()

    ref_inp = to_reference(data)
    ref_lengths = to_reference(lengths)

    ref_out = torch.segment_reduce(ref_inp, "sum", lengths=ref_lengths)
    with flag_gems.use_gems():
        res_out = torch.segment_reduce(data, "sum", lengths=lengths)
...
@pytest.mark.parametrize("shape", [(100,), (1000,)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_segment_reduce_sum_offsets(shape, dtype):
    num_segments = shape[0] // 10
    data = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Generate offsets that sum to shape[0]
    segment_sizes = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=flag_gems.device)
    segment_sizes = segment_sizes * shape[0] // segment_sizes.sum().item()
    segment_sizes[-1] = shape[0] - segment_sizes[:-1].sum().item()
    offsets = torch.zeros(num_segments + 1, dtype=torch.int64, device=flag_gems.device)
    offsets[1:] = segment_sizes.cumsum(dim=0)

    ref_inp = to_reference(data)
    ref_offsets = to_reference(offsets)

    ref_out = torch.segment_reduce(ref_inp, "sum", offsets=ref_offsets)
    with flag_gems.use_gems():
        res_out = torch.segment_reduce(data, "sum", offsets=offsets)
...
@pytest.mark.parametrize("shape", [(100,), (1000,)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_segment_reduce_max(shape, dtype):
    num_segments = shape[0] // 10
    data = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    lengths = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=flag_gems.device)
    total_len = lengths.sum().item()
    ...
    ref_out = torch.segment_reduce(ref_inp, "max", lengths=ref_lengths)
    with flag_gems.use_gems():
        res_out = torch.segment_reduce(data, "max", lengths=lengths)
...
@pytest.mark.parametrize("shape", [(100,), (1000,)])
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_segment_reduce_min(shape, dtype):
    num_segments = shape[0] // 10
    data = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    lengths = torch.randint(1, 5, (num_segments,), dtype=torch.int64, device=flag_gems.device)
    total_len = lengths.sum().item()
    ...
    ref_out = torch.segment_reduce(ref_inp, "min", lengths=ref_lengths)
    with flag_gems.use_gems():
        res_out = torch.segment_reduce(data, "min", lengths=lengths)
