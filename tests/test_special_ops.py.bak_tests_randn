...
# Test shapes for sparse_sampled_addmm
SPARSE_SAMPLED_ADDMM_SHAPES = [
    (16, 16, 8),
    (32, 32, 16),
    (64, 64, 32),
    (128, 64, 32),
...
@pytest.mark.parametrize("shape", SPARSE_SAMPLED_ADDMM_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
@pytest.mark.parametrize("alpha", [0.5, 1.0, 2.0])
@pytest.mark.parametrize("beta", [0.0, 0.5, 1.0])
def test_accuracy_sparse_sampled_addmm(shape, dtype, alpha, beta):
    """Test sparse_sampled_addmm accuracy against PyTorch implementation"""
    M, N, K = shape

    # Create a sparse CSR input tensor
    # Create a random sparse matrix with ~30% density
    density = 0.3
    nnz = int(M * N * density)
    row_indices = torch.randint(0, M, (nnz,), device='cuda')
    col_indices = torch.randint(0, N, (nnz,), device='cuda')
    values = torch.randn(nnz, dtype=dtype, device='cuda')

    # Sort by row to create valid CSR format
    sorted_idx = torch.argsort(row_indices)
    row_indices = row_indices[sorted_idx]
    col_indices = col_indices[sorted_idx]
    values = values[sorted_idx]

    # Create CSR format
    crow_indices = torch.zeros(M + 1, dtype=torch.int32, device='cuda')
    for i in range(M):
        crow_indices[i + 1] = crow_indices[i] + (row_indices == i).sum()

    input_sparse = torch.sparse_csr_tensor(
        crow_indices, col_indices, values, size=(M, N), dtype=dtype, device='cuda'
    )
    ...
    ref_input = to_reference(input_sparse.clone())
    ref_mat1 = to_reference(mat1)
    ref_mat2 = to_reference(mat2)

    ref_out = torch.sparse.sampled_addmm(ref_input, ref_mat1, ref_mat2, alpha=alpha, beta=beta)

    # FlagGems implementation
    with flag_gems.use_gems():
        res_out = torch.sparse.sampled_addmm(input_sparse, mat1, mat2, alpha=alpha, beta=beta)

    # Compare results
    res_values = res_out.values()
...
@pytest.mark.sparse_sampled_addmm
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_sparse_sampled_addmm_identity(dtype):
    """Test sparse_sampled_addmm with identity-like sparse matrix"""
    M, N, K = 32, 32, 16

    # Create identity-like sparse matrix (diagonal)
    eye_dense = torch.eye(M, N, dtype=dtype, device='cuda')
    input_sparse = eye_dense.to_sparse_csr()
    ...
    # FlagGems implementation
    with flag_gems.use_gems():
        res_out = torch.sparse.sampled_addmm(input_sparse, mat1, mat2)
