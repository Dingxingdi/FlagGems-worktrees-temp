...
# QKV_Projection_Fusion test: splits a tensor of shape [..., 3*hidden_dim] into three [..., hidden_dim] tensors
QKV_SHAPES = [(2, 3, 63), (4, 8, 126), (1, 16, 192)] if not QUICK_MODE else [(2, 3, 63)]


@pytest.mark.qkv_projection_fusion
@pytest.mark.parametrize("shape", QKV_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_qkv_projection_fusion(shape, dtype):
    """Test QKV projection fusion: split tensor into Q, K, V"""
    # Ensure the last dimension is divisible by 3
    assert shape[-1] % 3 == 0, "Last dimension must be divisible by 3"

    torch.manual_seed(123)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Reference: use torch.chunk to split into 3 parts
    ref_q, ref_k, ref_v = torch.chunk(ref_inp, 3, dim=-1)

    with flag_gems.use_gems():
        res_q, res_k, res_v = flag_gems.qkv_projection_fusion(inp)

    # Move to CPU for comparison if needed
    res_q = to_cpu(res_q, ref_q)
    res_k = to_cpu(res_k, ref_k)
    res_v = to_cpu(res_v, ref_v)

    # Compare all three outputs
    gems_assert_equal(res_q, ref_q)
    gems_assert_equal(res_k, ref_k)
    gems_assert_equal(res_v, ref_v)
