...
# Only float32 is fully supported by linalg.cond on CUDA
...
def test_accuracy_linalg_cond(shape, dtype):
    # Test with p=None (2-norm using SVD)
...
def test_accuracy_linalg_cond_with_p(shape, dtype, p):
...
def test_accuracy_linalg_cond_svd(shape, dtype, p):
