...
# Shapes for huber_loss_backward tests
HUBER_LOSS_SHAPES = [
    (4,),
    (16,),
    (64,),
...
@pytest.mark.parametrize("shape", HUBER_LOSS_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("reduction", [0, 1, 2])
def test_accuracy_huber_loss_backward(shape, dtype, reduction):
    if dtype == torch.bfloat16:
        pytest.skip("huber_loss_backward bfloat16 has known issues")
    # Create inputs
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    # Compute huber_loss forward to get the loss output
    delta = 1.0

    # The grad_output is d(loss)/d(loss_output) = 1.0 for all reduction modes
    # The backward function handles the reduction internally
    grad_output = torch.tensor(1.0, dtype=dtype, device=flag_gems.device)

    # Compute backward
    ref_out = torch.ops.aten.huber_loss_backward(
        grad_output, ref_inp, ref_target, reduction, delta
    )
    with flag_gems.use_gems():
