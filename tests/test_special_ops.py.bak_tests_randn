...
# Flatten operator tests
@pytest.mark.Flatten
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_flatten_default(shape, dtype):
    """Test flatten with default parameters (flatten all dimensions)."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.flatten(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.flatten(inp)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_flatten_start_dim(shape, dtype):
    """Test flatten with start_dim parameter."""
    if len(shape) < 2:
    ...
    # Flatten from dimension 1 to end
    ref_out = torch.flatten(ref_inp, start_dim=1)
    with flag_gems.use_gems():
        res_out = torch.flatten(inp, start_dim=1)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_flatten_start_end_dim(shape, dtype):
    """Test flatten with start_dim and end_dim parameters."""
    if len(shape) < 3:
        pytest.skip("Requires at least 3 dimensions")

    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Flatten from dimension 1 to 2
    ref_out = torch.flatten(ref_inp, start_dim=1, end_dim=2)
    with flag_gems.use_gems():
        res_out = torch.flatten(inp, start_dim=1, end_dim=2)
...
@pytest.mark.Flatten
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_flatten_non_contiguous(dtype):
    """Test flatten with non-contiguous input tensor."""
    # Create a non-contiguous tensor via transpose
    inp = torch.randn(4, 3, device=flag_gems.device).transpose(0, 1).to(dtype)
    ref_inp = to_reference(inp)

    ref_out = torch.flatten(ref_inp, start_dim=0, end_dim=1)
    with flag_gems.use_gems():
        res_out = torch.flatten(inp, start_dim=0, end_dim=1)
