...
# Test for resize (functional version)
@pytest.mark.resize_
@pytest.mark.parametrize("shape", [(2, 3)])
@pytest.mark.parametrize("new_shape", [(3, 4)])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_accuracy_resize(shape, new_shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    ref_out = torch.ops.aten.resize(ref_inp, list(new_shape))
    with flag_gems.use_gems():
        res_out = torch.ops.aten.resize(inp, list(new_shape))
    gems_assert_close(res_out, ref_out, dtype)


# Test for resize_ (in-place version) - disabled due to dispatcher recursion issues
# @pytest.mark.inplace
# @pytest.mark.resize_
# @pytest.mark.parametrize("shape", [(2, 3), (128, 256), (512, 512)])
# @pytest.mark.parametrize("new_shape", [(3, 4), (64, 128), (1024, 512)])
# @pytest.mark.parametrize("dtype", FLOAT_DTYPES)
# def test_accuracy_resize_(shape, new_shape, dtype):
#     inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
#     ref_inp = to_reference(inp.clone())
#     ref_out = torch.ops.aten.resize_(ref_inp, list(new_shape))
#     with flag_gems.use_gems():
#         res_out = torch.ops.aten.resize_(inp, list(new_shape))
#     gems_assert_close(res_out, ref_out, dtype)
