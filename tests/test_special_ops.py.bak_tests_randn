...
# Test for jagged_to_padded_dense_forward
JAGGED_SHAPES = [
    (6,),    # 2 sequences, 3 values each
    (10,),   # 2 sequences, 5 values each
    (12,),   # 3 sequences, 4 values each
    (8,),    # 2 sequences with different lengths
    (16,),   # 4 sequences, 4 values each
]
JAGGED_MAX_LENGTHS = [3, 4, 5]
JAGGED_PADDING_VALUES = [0.0, -1.0, 999.0]


@pytest.mark.jagged_to_padded_dense_forward
@pytest.mark.parametrize("shape", JAGGED_SHAPES)
@pytest.mark.parametrize("max_length", JAGGED_MAX_LENGTHS)
@pytest.mark.parametrize("padding_value", JAGGED_PADDING_VALUES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
def test_accuracy_jagged_to_padded_dense_forward(shape, max_length, padding_value, dtype):
    """Test _jagged_to_padded_dense_forward accuracy against PyTorch reference"""
    total_values = shape[0]
    # Create offsets: 2 sequences for simplicity
    batch_size = 2
    seq_len1 = total_values // 2
    seq_len2 = total_values - seq_len1
    offsets = torch.tensor([0, seq_len1, seq_len1 + seq_len2], device=device, dtype=torch.int64)
    max_lengths = [max_length]

    # Ensure max_length is at least as large as the longest sequence
    if max_length < max(seq_len1, seq_len2):
        pytest.skip("max_length too small for sequence length")

    # Create values tensor on GPU
    values = torch.randn(total_values, dtype=dtype, device=device)

    # Run reference implementation on GPU (use .default to bypass FlagGems)
    ref_out = torch.ops.aten._jagged_to_padded_dense_forward.default(
...
# Test for 2D values (with feature dimension)
JAGGED_2D_SHAPES = [
    (6, 2),   # 6 values, 2 features each
    (10, 4),  # 10 values, 4 features each
    (8, 3),   # 8 values, 3 features each
]


@pytest.mark.jagged_to_padded_dense_forward
@pytest.mark.parametrize("shape", JAGGED_2D_SHAPES)
@pytest.mark.parametrize("max_length", JAGGED_MAX_LENGTHS)
@pytest.mark.parametrize("padding_value", [0.0])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_jagged_to_padded_dense_forward_2d(shape, max_length, padding_value, dtype):
    """Test _jagged_to_padded_dense_forward with 2D values (feature dimension)"""
    total_values, feature_size = shape
    # Create offsets: 2 sequences
    batch_size = 2
    seq_len1 = total_values // 2
    seq_len2 = total_values - seq_len1
    offsets = torch.tensor([0, seq_len1, seq_len1 + seq_len2], device=device, dtype=torch.int64)
    ...
    # Create values tensor [total_values, feature_size] on GPU
    values = torch.randn(total_values, feature_size, dtype=dtype, device=device)
