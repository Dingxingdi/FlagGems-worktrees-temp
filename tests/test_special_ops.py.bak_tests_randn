...
def test_accuracy_linalg_det(shape, dtype):
    ...
    # Add a large constant to ensure the matrix is well-conditioned
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device) + 10.0

    # For float16/bfloat16 on CUDA, compute reference on CPU since torch.linalg.det
    # doesn't support these dtypes on CUDA ("lu_factor_cusolver" not implemented)
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.linalg.det(inp.cpu().float()).to(dtype)
        ref_out = ref_out.to(flag_gems.device)  # Move to GPU for comparison
    else:
        ref_inp = to_reference(inp)
        ref_out = torch.linalg.det(ref_inp)

    with flag_gems.use_gems():
        res_out = torch.linalg.det(inp)
...
def test_accuracy_linalg_det_batch(shape, dtype):
    # Test batched matrices: (batch, n, n)
    batch_size = 4
    inp = torch.randn(batch_size, shape[0], shape[1], dtype=dtype, device=flag_gems.device) + 10.0

    # For float16/bfloat16 on CUDA, compute reference on CPU
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.linalg.det(inp.cpu().float()).to(dtype)
        ref_out = ref_out.to(flag_gems.device)  # Move to GPU for comparison
    else:
...
@pytest.mark.linalg_det
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_det_1x1(dtype):
    # Test 1x1 matrix
    inp = torch.randn(1, 1, dtype=dtype, device=flag_gems.device) + 10.0
