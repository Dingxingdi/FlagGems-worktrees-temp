...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_(shape, size_stride, dtype):
    ...
    assert res_out.shape == torch.Size(size), f"Shape mismatch: {res_out.shape} vs {size}"
    assert res_out.stride() == tuple(stride), f"Stride mismatch: {res_out.stride()} vs {stride}"
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_with_offset(shape, size_stride, storage_offset, dtype):
...
@pytest.mark.as_strided_
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_as_strided_inplace(dtype):
