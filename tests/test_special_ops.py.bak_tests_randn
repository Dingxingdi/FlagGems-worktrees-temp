...
# Shapes for flash attention backward tests
FLASH_ATTENTION_SHAPES = [
    (2, 4, 32, 64),  # (batch, heads, seq_len, head_dim)
    (1, 2, 16, 32),
    (4, 8, 64, 128),
]
...
def test_accuracy_scaled_dot_product_flash_attention_backward(shape, dtype):
    """Test that _scaled_dot_product_flash_attention_backward runs correctly.

    This test verifies that our implementation of _scaled_dot_product_flash_attention_backward
    can be called and produces tensors of the expected shape. The actual gradient computation
    is tested via the standard scaled_dot_product_attention_backward in test_attention_ops.py.
    """
    ...
    # Create output and logsumexp from forward pass
    with flag_gems.use_gems():
        out, logsumexp, _, _, philox_seed, philox_offset, _, _, _ = (
    ...
    assert dq.shape == query.shape, f"dq shape {dq.shape} != query shape {query.shape}"
    assert dk.shape == key.shape, f"dk shape {dk.shape} != key shape {key.shape}"
    assert dv.shape == value.shape, f"dv shape {dv.shape} != value shape {value.shape}"

    # Verify outputs are not all zeros (gradient computation ran)
    assert not torch.all(dq == 0), "dq is all zeros"
    assert not torch.all(dk == 0), "dk is all zeros"
    assert not torch.all(dv == 0), "dv is all zeros"
