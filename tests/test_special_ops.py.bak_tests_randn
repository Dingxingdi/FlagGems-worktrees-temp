...
# Test shapes for _pack_padded_sequence
PACK_PADDED_SHAPES = [
    (2, 3, 4),   # small
    (4, 8, 16),  # medium
    (8, 16, 32), # larger
...
@pytest.mark.parametrize("shape", PACK_PADDED_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
@pytest.mark.parametrize("batch_first", [True, False])
def test_accuracy__pack_padded_sequence(shape, dtype, batch_first):
    # Create input tensor with random data
    batch_size, max_seq_len, features = shape

    # Generate random lengths sorted in descending order
    lengths = torch.randint(1, max_seq_len + 1, (batch_size,), dtype=torch.int64)
    lengths = lengths.sort(descending=True)[0]

    # Create padded input tensor (with padding at the end of shorter sequences)
    input_tensor = torch.randn(batch_size, max_seq_len, features, dtype=dtype, device=flag_gems.device)

    # Set padding values to 0 for shorter sequences
    for b in range(batch_size):
        seq_len = lengths[b].item()
        if seq_len < max_seq_len:
            input_tensor[b, seq_len:] = 0

    # Convert to reference
    ref_input = to_reference(input_tensor)

    # Pack with batch_first=False (seq_first format)
    if not batch_first:
        # Transpose for seq-first: (seq, batch, features)
        input_tensor = input_tensor.transpose(0, 1)
        ref_input = ref_input.transpose(0, 1)

    # Reference implementation
    ref_data, ref_batch_sizes = torch._pack_padded_sequence(ref_input, lengths, batch_first=batch_first)

    # GEMS implementation
    with flag_gems.use_gems():
        res_data, res_batch_sizes = torch._pack_padded_sequence(input_tensor, lengths, batch_first=batch_first)

    # Compare
    gems_assert_close(res_data, ref_data, dtype)
    gems_assert_equal(res_batch_sizes.cpu(), ref_batch_sizes)
