...
# Test shapes for flash attention backward
FLASH_ATTENTION_SHAPES = [
    (1, 1, 16, 32),   # batch, heads, seq_len, head_dim
    (1, 2, 32, 64),
    (2, 4, 64, 64),
    (1, 1, 128, 128),
]
...
@pytest.mark.parametrize("shape", FLASH_ATTENTION_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_accuracy_flash_attention_backward(shape, dtype):
    """Test _flash_attention_backward accuracy against PyTorch reference."""
    ...
    # Get reference (PyTorch) implementation results
    ref_query = to_reference(query)
    ref_key = to_reference(key)
    ref_value = to_reference(value)

    # Run forward pass to get necessary tensors for backward
    forward_result = torch.ops.aten._flash_attention_forward(
        ref_query, ref_key, ref_value, None, None, seq_len, seq_len, 0.0, False, False
    )
    ref_out, ref_logsumexp, ref_philox_seed, ref_philox_offset, _ = forward_result

    # Create grad_out
    grad_out = torch.randn_like(ref_out)

    # Compute backward using PyTorch reference
    ...
    # Run with GEMS
    query_gems = query.clone()
    key_gems = key.clone()
    value_gems = value.clone()

    forward_result_gems = torch.ops.aten._flash_attention_forward(
        query_gems, key_gems, value_gems, None, None, seq_len, seq_len, 0.0, False, False
    )
    gems_out, gems_logsumexp, gems_philox_seed, gems_philox_offset, _ = forward_result_gems

    grad_out_gems = grad_out.clone()

    with flag_gems.use_gems():
        dq_gems, dk_gems, dv_gems = torch.ops.aten._flash_attention_backward(
            grad_out_gems,
            query_gems,
            key_gems,
            value_gems,
    ...
    # Compare results - use a larger atol for bfloat16
    atol = 1e-2 if dtype == torch.bfloat16 else 1e-3
    gems_assert_close(dq_gems, ref_dq, dtype, atol=atol)
    gems_assert_close(dk_gems, ref_dk, dtype, atol=atol)
    gems_assert_close(dv_gems, ref_dv, dtype, atol=atol)
