...
# Test for upsample_bicubic2d_backward
@pytest.mark.upsample_bicubic2d_backward
@pytest.mark.parametrize(
    "N, C, H, W, scale_h, scale_w, align_corners",
    [
        (1, 1, 4, 4, 2.0, 2.0, False),
        (2, 3, 8, 8, 2.0, 2.0, True),
        (1, 16, 16, 16, 0.5, 0.5, False),
        (4, 3, 7, 5, 2.0, 2.0, True),
    ],
)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32, torch.bfloat16])
def test_upsample_bicubic2d_backward(N, C, H, W, scale_h, scale_w, align_corners, dtype):
    """Test upsample_bicubic2d_backward by verifying consistency with forward pass."""
    # Compute output size
    H_out = max(int(H * scale_h), 1)
    W_out = max(int(W * scale_w), 1)
    output_size = (H_out, W_out)

    # Create input tensor
    x = torch.randn(N, C, H, W, dtype=dtype, device=device)

    # Create grad_output (all ones)
    grad_output = torch.ones(N, C, H_out, W_out, dtype=dtype, device=device)

    # Compute backward using our implementation
    input_size = (N, C, H, W)
    with flag_gems.use_gems():
    ...
    # For verification, we check that the gradient is not all zeros
    # (which would indicate the kernel isn't working)
    assert not torch.all(res_grad_input == 0), "Gradient is all zeros - kernel not working"

    # Check that gradient has the correct shape
    assert res_grad_input.shape == (N, C, H, W), f"Wrong shape: {res_grad_input.shape} vs {(N, C, H, W)}"

    # For additional verification, check that the gradient values are reasonable
    # (not NaN or Inf)
    assert torch.isfinite(res_grad_input).all(), "Gradient contains NaN or Inf"

    # Check that gradient magnitude is reasonable (not too large or small)
    grad_abs_mean = torch.abs(res_grad_input).mean()
    assert grad_abs_mean > 1e-6, f"Gradient too small: {grad_abs_mean}"
    assert grad_abs_mean < 1e6, f"Gradient too large: {grad_abs_mean}"
