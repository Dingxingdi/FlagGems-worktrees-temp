...
def test_accuracy_item(dtype):
    """Test item operator - extracts scalar value from 1-element tensor"""
    # Create a 1-element tensor
    inp = torch.tensor([1.5], dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = ref_inp.item()
    with flag_gems.use_gems():
        res_out = inp.item()

    # item() returns a Python scalar, compare directly
    assert res_out == ref_out, f"Expected {ref_out}, got {res_out}"


@pytest.mark.item
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_item_0d(dtype):
    """Test item operator on 0-dimensional tensor"""
    # Create a 0-d tensor
...
@pytest.mark.item
def test_accuracy_item_error():
    """Test item operator raises error for multi-element tensor"""
    inp = torch.tensor([1.0, 2.0], device=flag_gems.device)
    ...
    with flag_gems.use_gems():
        try:
            ...
            assert "only works for tensors with one element" in str(e)
