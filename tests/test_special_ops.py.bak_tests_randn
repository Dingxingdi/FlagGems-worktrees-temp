...
# Shapes for _fused_adam tests
_FUSED_ADAM_SHAPES = [(1024,), (2048,), (4096,), (1024, 1024)]


@pytest.mark.fused_adam
@pytest.mark.parametrize("shape", _FUSED_ADAM_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__fused_adam(shape, dtype):
    """Test _fused_adam operator accuracy."""
    torch.manual_seed(42)
    ...
    state_step = torch.tensor([1.0], device=flag_gems.device)
    
    ref_param = to_reference(param)
    ref_grad = to_reference(grad)
    ref_exp_avg = to_reference(exp_avg)
    ref_exp_avg_sq = to_reference(exp_avg_sq)
    ref_state_step = to_reference(state_step)
    
    # Reference output
    ref_out = torch.ops.aten._fused_adam(
        [ref_param],
        [ref_grad],
        [ref_exp_avg],
        [ref_exp_avg_sq],
    ...
    # FlagGems output
    with flag_gems.use_gems():
        gems_out = torch.ops.aten._fused_adam(
            [param],
            [grad],
            [exp_avg],
            [exp_avg_sq],
    ...
    # Compare outputs
    ref_params, ref_grads, ref_exp_avgs, ref_exp_avg_sqs, ref_max = ref_out
    gems_params, gems_grads, gems_exp_avgs, gems_exp_avg_sqs, gems_max = gems_out
    
    gems_assert_close(gems_params[0], ref_params[0], dtype)
    gems_assert_close(gems_grads[0], ref_grads[0], dtype)
    gems_assert_close(gems_exp_avgs[0], ref_exp_avgs[0], dtype)
    gems_assert_close(gems_exp_avg_sqs[0], ref_exp_avg_sqs[0], dtype)
...
def test_accuracy__fused_adam_amsgrad(shape, dtype):
    """Test _fused_adam operator with amsgrad=True."""
    torch.manual_seed(42)
    ...
    ref_exp_avg = to_reference(exp_avg)
    ref_exp_avg_sq = to_reference(exp_avg_sq)
    ref_max_exp_avg_sq = to_reference(max_exp_avg_sq)
    ref_state_step = to_reference(state_step)
    ...
    gems_assert_close(gems_exp_avgs[0], ref_exp_avgs[0], dtype)
    gems_assert_close(gems_exp_avg_sqs[0], ref_exp_avg_sqs[0], dtype)
    gems_assert_close(gems_max[0], ref_max[0], dtype)
...
def test_accuracy__fused_adam_(shape, dtype):
    """Test _fused_adam_ (in-place) operator accuracy."""
    torch.manual_seed(42)
    
    # Create tensors
    param_ref = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    grad_ref = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    exp_avg_ref = torch.zeros_like(param_ref)
    exp_avg_sq_ref = torch.zeros_like(param_ref)
    state_step_ref = torch.tensor([1.0], device=flag_gems.device)
    
    param = param_ref.clone()
    grad = grad_ref.clone()
    exp_avg = exp_avg_ref.clone()
    exp_avg_sq = exp_avg_sq_ref.clone()
    state_step = state_step_ref.clone()
    
    # Reference in-place update
    ...
    # Compare in-place updated tensors
    gems_assert_close(param, param_ref, dtype)
    gems_assert_close(exp_avg, exp_avg_ref, dtype)
    gems_assert_close(exp_avg_sq, exp_avg_sq_ref, dtype)


@pytest.mark.fused_adam_
@pytest.mark.parametrize("shape", _FUSED_ADAM_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__fused_adam__amsgrad(shape, dtype):
    """Test _fused_adam_ (in-place) with amsgrad=True."""
    torch.manual_seed(42)
    ...
    exp_avg_ref = torch.zeros_like(param_ref)
    exp_avg_sq_ref = torch.zeros_like(param_ref)
    max_exp_avg_sq_ref = torch.zeros_like(param_ref)
    state_step_ref = torch.tensor([1.0], device=flag_gems.device)
    ...
    exp_avg = exp_avg_ref.clone()
    exp_avg_sq = exp_avg_sq_ref.clone()
    max_exp_avg_sq = max_exp_avg_sq_ref.clone()
    state_step = state_step_ref.clone()
    ...
    gems_assert_close(exp_avg, exp_avg_ref, dtype)
    gems_assert_close(exp_avg_sq, exp_avg_sq_ref, dtype)
    gems_assert_close(max_exp_avg_sq, max_exp_avg_sq_ref, dtype)
