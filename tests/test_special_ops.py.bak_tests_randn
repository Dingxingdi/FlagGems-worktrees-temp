...
# Test for _nested_tensor_from_tensor_list operator
NESTED_TENSOR_LISTS = [
    [torch.Size([3]), torch.Size([4]), torch.Size([2])],
    [torch.Size([1]), torch.Size([1]), torch.Size([1])],
    [torch.Size([5, 3]), torch.Size([3, 4]), torch.Size([2, 5])],
]


@pytest.mark.nested_tensor_from_tensor_list
@pytest.mark.parametrize("shapes", NESTED_TENSOR_LISTS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nested_tensor_from_tensor_list(shapes, dtype):
    """Test _nested_tensor_from_tensor_list accuracy against PyTorch reference."""
    # Create list of tensors with the specified shapes
    tensors = [torch.randn(shape, dtype=dtype, device=flag_gems.device) for shape in shapes]
    ref_tensors = [to_reference(t.clone()) for t in tensors]

    # Create nested tensors
    ref_out = torch._nested_tensor_from_tensor_list(ref_tensors)
    with flag_gems.use_gems():
        res_out = torch._nested_tensor_from_tensor_list(tensors)

    # Verify the nested tensor structure by unbinding
    ref_unbound = ref_out.unbind()
    res_unbound = res_out.unbind()

    # Check that we have the same number of tensors
    assert len(ref_unbound) == len(res_unbound), "Number of tensors mismatch"

    # Compare each tensor in the nested tensor
    for ref_t, res_t in zip(ref_unbound, res_unbound):
        gems_assert_close(res_t, ref_t, dtype)
