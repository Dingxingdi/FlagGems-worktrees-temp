...
@pytest.mark.amp_foreach_non_finite_check_and_unscale
@pytest.mark.parametrize("shape", _AMP_FOREACH_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy__amp_foreach_non_finite_check_and_unscale_(shape, dtype):
    """Test _amp_foreach_non_finite_check_and_unscale_ accuracy"""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp1 = inp1.clone()
    ref_inp2 = inp2.clone()
    found_inf = torch.tensor(0.0, device=flag_gems.device)
    ref_found_inf = torch.tensor(0.0, device=flag_gems.device)
    inv_scale = torch.tensor(0.5, device=flag_gems.device)

    torch._amp_foreach_non_finite_check_and_unscale_(
        [ref_inp1, ref_inp2], ref_found_inf, inv_scale
    )
    with flag_gems.use_gems():
        torch._amp_foreach_non_finite_check_and_unscale_(
            [inp1, inp2], found_inf, inv_scale
        )

    gems_assert_equal(inp1, ref_inp1)
    gems_assert_equal(inp2, ref_inp2)
    assert found_inf.item() == ref_found_inf.item(), "found_inf mismatch"


@pytest.mark.amp_foreach_non_finite_check_and_unscale
@pytest.mark.parametrize("shape", _AMP_FOREACH_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_accuracy__amp_foreach_non_finite_check_and_unscale__with_inf(shape, dtype):
    """Test _amp_foreach_non_finite_check_and_unscale_ with inf/nan"""
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Add inf to inp1 and nan to inp2
    inp1.view(-1)[0] = float("inf")
    inp2.view(-1)[1] = float("nan")
    ...
    gems_assert_equal(inp1, ref_inp1, equal_nan=True)
    gems_assert_equal(inp2, ref_inp2, equal_nan=True)
    assert found_inf.item() == ref_found_inf.item() == 1.0, "found_inf should be 1.0"
