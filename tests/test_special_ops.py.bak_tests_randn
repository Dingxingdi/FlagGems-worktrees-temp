...
@pytest.mark.parametrize("shape", REPLICATION_PAD3D_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_replication_pad3d_backward(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # padding = (pad_l, pad_r, pad_t, pad_b, pad_f, pad_ba)
    padding = (1, 1, 1, 1, 1, 1)

    # Compute forward with replication padding
    ref_out = torch.nn.functional.pad(ref_inp, padding, mode="replicate")
    res_out = torch.nn.functional.pad(inp, padding, mode="replicate")

    # Compute backward - gradient of ones (sum of output)
    grad_output = torch.ones_like(res_out)

    ref_grad_input = torch.ops.aten.replication_pad3d_backward(
        grad_output, ref_inp, padding
    )
    with flag_gems.use_gems():
