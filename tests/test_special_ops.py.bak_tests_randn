...
@pytest.mark.stft
@pytest.mark.parametrize("shape", STFT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_stft(shape, dtype):
    """Test STFT accuracy against PyTorch reference."""
    if dtype not in [torch.float32, torch.float16]:
        pytest.skip("STFT only supports float32 and float16")
    ...
    # Use rectangular window for testing
    window = torch.ones(win_length, dtype=dtype, device=flag_gems.device)

    # Reference implementation
    ref_out = torch.stft(ref_inp, n_fft=n_fft, hop_length=hop_length,
                         win_length=win_length, window=window,
                         center=False, return_complex=True)

    # GEMS implementation - call directly
    res_out = flag_gems.stft(inp, n_fft=n_fft, hop_length=hop_length,
                            win_length=win_length, window=window,
                            center=False, return_complex=True)

    # Output is complex, use the corresponding dtype for comparison
    out_dtype = torch.complex64 if dtype == torch.float32 else torch.complex32
    # Use higher tolerance for float16
    atol = 1e-2 if dtype == torch.float32 else 1e-1
    gems_assert_close(res_out, ref_out, out_dtype, atol=atol)


@pytest.mark.stft
@pytest.mark.parametrize("shape", STFT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_stft_center(shape, dtype):
    """Test STFT with center=True."""
    if dtype not in [torch.float32, torch.float16]:
        pytest.skip("STFT only supports float32 and float16")
    ...
    # Reference with center=True
    ref_out = torch.stft(ref_inp, n_fft=n_fft, hop_length=hop_length,
                         win_length=win_length, window=window,
                         center=True, return_complex=True)

    # GEMS with center=True
    res_out = flag_gems.stft(inp, n_fft=n_fft, hop_length=hop_length,
                            win_length=win_length, window=window,
                            center=True, return_complex=True)
...
@pytest.mark.parametrize("shape", STFT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_stft_normalized(shape, dtype):
    """Test STFT with normalized=True."""
    if dtype not in [torch.float32, torch.float16]:
        pytest.skip("STFT only supports float32 and float16")
    ...
    # Reference with normalized=True
    ref_out = torch.stft(ref_inp, n_fft=n_fft, hop_length=hop_length,
                         win_length=win_length, window=window,
                         center=False, normalized=True, return_complex=True)

    # GEMS with normalized=True
    res_out = flag_gems.stft(inp, n_fft=n_fft, hop_length=hop_length,
                            win_length=win_length, window=window,
                            center=False, normalized=True, return_complex=True)
