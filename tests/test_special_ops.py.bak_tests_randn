...
def test_accuracy__list_to_tensor(input_list):
    """Test _list_to_tensor accuracy against PyTorch reference implementation."""
    # Get reference result using PyTorch's implementation
    ref_out = torch.ops.aten._list_to_tensor(input_list)

    # Get GEMS result
    with flag_gems.use_gems():
        res_out = flag_gems._list_to_tensor(input_list)

    # The output should be on GPU, move reference to GPU for comparison
    ref_out_gpu = ref_out.to(flag_gems.device)

    # Compare results - using gems_assert_equal for exact integer comparison
    gems_assert_equal(res_out, ref_out_gpu)


@pytest.mark.list_to_tensor
def test_accuracy__list_to_tensor_empty():
    """Test _list_to_tensor with empty list."""
    input_list = []
