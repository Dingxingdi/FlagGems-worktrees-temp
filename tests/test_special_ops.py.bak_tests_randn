...
def get_linalg_diagonal_shapes():
    """Generate shapes for testing linalg.diagonal."""
    shapes = [
        (1024, 1024),
        (512, 1024),
        (1024, 512),
    ...
    result = []
    for s in shapes:
        # Test with different offsets (dim1=-2, dim2=-1 is default for linalg.diagonal)
        result.append((s, -2, -1, 0))  # default
        result.append((s, -2, -1, 1))  # offset=1
        result.append((s, -2, -1, -1))  # offset=-1
    return result


@pytest.mark.linalg_diagonal
@pytest.mark.parametrize("shape, dim1, dim2, offset", get_linalg_diagonal_shapes())
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_linalg_diagonal(shape, dtype, dim1, dim2, offset):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    ref_out = torch.linalg.diagonal(ref_inp, offset=offset, dim1=dim1, dim2=dim2)
    with flag_gems.use_gems():
        res_out = torch.linalg.diagonal(inp, offset=offset, dim1=dim1, dim2=dim2)
