...
# Reflection pad 1d backward tests
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_reflection_pad1d_backward(shape, padding, dtype):
    ...
    W_out = padded_out.shape[-1]

    # Create gradient of ones
    grad_output = torch.ones_like(padded_out)

    ref_out = torch.ops.aten.reflection_pad1d_backward(
        grad_output, ref_inp, padding
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten.reflection_pad1d_backward(
