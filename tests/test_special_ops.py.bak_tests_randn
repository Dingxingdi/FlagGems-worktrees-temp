...
# Shapes for cholesky_solve_helper tests
# (batch, n, nrhs) - we need square matrices for Cholesky
CHOLESKY_SOLVE_SHAPES = [
    (1, 4, 1),      # Single batch, small matrix
    (2, 4, 2),      # Multiple right-hand sides
]
...
def test_accuracy_cholesky_solve_helper(shape, dtype, upper):
    """Test _cholesky_solve_helper accuracy against PyTorch reference."""
    batch, n, nrhs = shape

    # Create a valid Cholesky factor directly (lower triangular with positive diagonal)
    # This avoids calling torch.linalg.cholesky which doesn't support float16/bfloat16
    if batch == 1:
        # Create lower triangular L with positive diagonal
        L = torch.zeros(n, n, dtype=dtype, device=flag_gems.device)
        for i in range(n):
            L[i, i] = torch.rand(1, dtype=dtype, device=flag_gems.device).abs() + 0.1
            for j in range(i):
                L[i, j] = torch.randn(1, dtype=dtype, device=flag_gems.device)
    else:
        L = torch.zeros(batch, n, n, dtype=dtype, device=flag_gems.device)
        for b in range(batch):
            for i in range(n):
                L[b, i, i] = torch.rand(1, dtype=dtype, device=flag_gems.device).abs() + 0.1
                for j in range(i):
                    L[b, i, j] = torch.randn(1, dtype=dtype, device=flag_gems.device)

    # If upper=True, convert to upper triangular
    if upper:
        A_factor = L.transpose(-2, -1)
    else:
        A_factor = L
    ...
    ref_A = to_reference(A_factor)

    ref_out = torch.ops.aten._cholesky_solve_helper(ref_b, ref_A, upper)

    # Get result from FlagGems
    with flag_gems.use_gems():
        res_out = torch.ops.aten._cholesky_solve_helper(b, A_factor, upper)
