...
def test_accuracy_special_shifted_chebyshev_polynomial_t(shape, dtype):
    ...
    inp2 = torch.randint(0, 5, shape, dtype=torch.long, device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    # PyTorch reference only supports float32/float64 for this op
    ref_inp1_f32 = ref_inp1.to(torch.float32) if dtype != torch.float32 else ref_inp1
    ref_inp2_f32 = ref_inp2.to(torch.int64)
    ref_out = torch.ops.aten.special_shifted_chebyshev_polynomial_t(ref_inp1_f32, ref_inp2_f32)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_shifted_chebyshev_polynomial_t_(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    inp2 = torch.randint(0, 5, shape, dtype=torch.long, device=flag_gems.device)
    ...
    ref_inp1_f32 = ref_inp1.to(torch.float32) if dtype != torch.float32 else ref_inp1
    ref_inp2_f32 = ref_inp2.to(torch.int64)
    ref_out = ref_inp1_f32.special_shifted_chebyshev_polynomial_t_(ref_inp2_f32)
    if dtype != torch.float32:
        ref_out = ref_out.to(dtype)

    with flag_gems.use_gems():
        res_out = inp1.special_shifted_chebyshev_polynomial_t_(inp2)
