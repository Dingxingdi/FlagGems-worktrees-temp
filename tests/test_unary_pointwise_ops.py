...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnnconvfilter(shape, dtype):
    # cudnnconvfilter is an identity/passthrough operator
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    # Since torch.cudnnconvfilter doesn't exist in PyTorch,
    # we directly call flag_gems implementation and compare to input
    with flag_gems.use_gems():
        res_out = flag_gems.cudnnconvfilter(inp)

    # Output should equal input (identity operation)
    gems_assert_equal(res_out, ref_inp)


@pytest.mark.inplace
@pytest.mark.cudnnconvfilter_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnnconvfilter_(shape, dtype):
    # cudnnconvfilter_ is an in-place identity/passthrough operator
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())

    # Since torch.cudnnconvfilter_ doesn't exist in PyTorch,
    # we directly call flag_gems implementation and compare to input
    with flag_gems.use_gems():
        res_out = flag_gems.cudnnconvfilter_(inp)
