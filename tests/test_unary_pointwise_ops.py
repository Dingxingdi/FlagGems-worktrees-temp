...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_log_sigmoid_forward(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    gems_assert_close(res_out[0], ref_out[0].to(res_out[0].device), dtype)
    gems_assert_close(res_out[1], ref_out[1].to(res_out[1].device), dtype)
