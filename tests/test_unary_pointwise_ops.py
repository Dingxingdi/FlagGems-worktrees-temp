...
# Test shapes for reshape - different source and target shapes
RESHAPE_SHAPES = [
    ((4, 6), (2, 12)),
    ((4, 6), (3, 8)),
    ((4, 6), (24,)),
...
def test_accuracy_reshape(shapes, dtype):
    ...
    ref_out = torch.reshape(ref_inp, dst_shape)
    with flag_gems.use_gems():
        res_out = torch.reshape(inp, dst_shape)
...
def test_accuracy_reshape_int(shapes, dtype):
    src_shape, dst_shape = shapes
    inp = torch.randint(-1000, 1000, src_shape, device=flag_gems.device).to(dtype)
    ref_inp = to_reference(inp)
