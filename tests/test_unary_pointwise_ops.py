...
# Test for special_scaled_modified_bessel_k0
# Note: PyTorch only supports float32 for this operator
# Using larger tolerance (atol=0.02) because approximation error can be up to ~0.01
@pytest.mark.special_scaled_modified_bessel_k0
@pytest.mark.parametrize("shape", [(2, 3), (128, 256), (512, 512)])
def test_accuracy_special_scaled_modified_bessel_k0(shape):
    # Only test float32 since PyTorch doesn't support float16/bfloat16
    dtype = torch.float32
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x)
    ...
    gems_assert_close(act_out, ref_out, dtype, equal_nan=True, atol=0.02)
