...
@pytest.mark.inplace
@pytest.mark.special_ndtr_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_ndtr_(shape, dtype):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_x = to_reference(x.clone())
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.ops.aten.special_ndtr_(ref_x.float()).to(dtype)
    else:
        ref_out = torch.ops.aten.special_ndtr_(ref_x)
    with flag_gems.use_gems():
        act_out = torch.ops.aten.special_ndtr_(x)
    gems_assert_close(act_out, ref_out, dtype)
