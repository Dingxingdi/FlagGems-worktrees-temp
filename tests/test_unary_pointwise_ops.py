...
# mvlgamma_ is an in-place unary operator with an integer parameter p
P_LIST = [1, 2, 3, 5, 8, 12]
...
def test_accuracy_mvlgamma_(shape, dtype, p):
    ...
    if dtype in (torch.float16, torch.bfloat16) and numel > 1024:
        pytest.skip("float16/bfloat16 have precision issues with lgamma at poles for large shapes")
