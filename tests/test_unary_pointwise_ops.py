...
def test_accuracy_special_bessel_y0(shape, dtype):
    # Bessel Y0 is only defined for x > 0
    # Use abs and add small offset to avoid x <= 0
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device).abs() + 0.1
    ref_x = to_reference(x)
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.special.bessel_y0(ref_x.float()).to(dtype)
    else:
        ref_out = torch.special.bessel_y0(ref_x)
    with flag_gems.use_gems():
        act_out = torch.special.bessel_y0(x)
    gems_assert_close(act_out, ref_out, dtype)
...
@pytest.mark.parametrize("shape", [(2, 3), (128, 256), (512, 512)])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_bessel_y0_out(shape, dtype):
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device).abs() + 0.1
    ref_x = to_reference(x)
    if dtype in (torch.float16, torch.bfloat16):
        out_ref = torch.empty_like(ref_x, dtype=torch.float32)
        ref_out = torch.special.bessel_y0(ref_x.float(), out=out_ref)
        out_ref = out_ref.to(dtype)
        ref_out = out_ref
    else:
        out_ref = torch.empty_like(ref_x)
        ref_out = torch.special.bessel_y0(ref_x, out=out_ref)
    out_act = torch.empty_like(x)
    with flag_gems.use_gems():
        act_out = torch.special.bessel_y0(x, out=out_act)
    gems_assert_close(act_out, ref_out, dtype)
    gems_assert_close(out_act, out_ref, dtype)
