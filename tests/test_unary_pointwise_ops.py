...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy__prelu_kernel_backward(shape, dtype):
    # The _prelu_kernel_backward expects weight to match the LAST dimension
    # For 1D: weight shape = (N,)
    # For 2D: weight shape = (C,) where C is the last dim
    # For 3D+: weight shape = (last_dim,)
    if len(shape) == 0:
        # Scalar: use scalar weight
        weight = torch.tensor(0.25, dtype=dtype, device=flag_gems.device)
    else:
        # Weight shape = last dimension
        last_dim = shape[-1]
        weight = torch.randn(last_dim, dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.ops.aten._prelu_kernel_backward.default(
        ref_grad_output, ref_x, ref_weight
    )
    with flag_gems.use_gems():
        res_out = torch.ops.aten._prelu_kernel_backward.default(
