...
def test_accuracy_special_log_ndtr(shape, dtype):
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Reference: call outside use_gems to get PyTorch's implementation
    ref_out = torch.ops.aten.special_log_ndtr(inp)
    with flag_gems.use_gems():
    ...
    gems_assert_close(res_out, ref_out, dtype, atol=0.2)


@pytest.mark.inplace
@pytest.mark.special_log_ndtr_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_log_ndtr_(shape, dtype):
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = inp.clone()
    # Reference: call outside use_gems to get PyTorch's implementation
