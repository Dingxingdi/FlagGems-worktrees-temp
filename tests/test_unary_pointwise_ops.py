...
# fft_ifftshift tests
# Test dimensions for fft_ifftshift - similar to flip but with ifftshift semantics
# Note: fft_ifftshift does not support scalar (0-D) tensors, so we exclude () from shapes
# Only use valid, non-conflicting dimension combinations
IFFTSHIFT_DIMS = [(0,), (1,), (0, 1), (1, 0)]

# Valid shapes for fft_ifftshift (exclude scalar and single element)
IFFTSHIFT_SHAPES = [
    (2, 19, 7),
    (1024, 1024),
    (20, 320, 15),
...
@pytest.mark.parametrize("shape", IFFTSHIFT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
@pytest.mark.parametrize("dims", IFFTSHIFT_DIMS)
def test_accuracy_fft_ifftshift(shape, dtype, dims):
    # For fft_ifftshift, we need to handle different dim sizes properly
    # Only test valid dimension combinations
    max_ndim = len(shape)
    # dims is a tuple like (0,) or (0, 1) - check all dims are valid
    if not all(abs(di) < max_ndim for di in dims):
        pytest.skip("Invalid dims for shape")
    ...
    ref_out = torch.fft.ifftshift(ref_inp, dim=dims)
    with flag_gems.use_gems():
        res_out = torch.fft.ifftshift(inp, dim=dims)
    gems_assert_equal(res_out, ref_out)
...
@pytest.mark.parametrize("shape", IFFTSHIFT_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
def test_accuracy_fft_ifftshift_all_dims(shape, dtype):
    # Test with dim=None (shift all dimensions)
    if dtype in ALL_FLOAT_DTYPES:
        inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    else:
        inp = torch.randint(-1000, 1000, shape, device=flag_gems.device).to(dtype)
    ...
    ref_out = torch.fft.ifftshift(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.fft.ifftshift(inp)
