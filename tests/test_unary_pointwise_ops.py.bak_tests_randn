...
def test_accuracy_conj_physical_(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.conj_physical_(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.conj_physical_(inp)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", COMPLEX_DTYPES)
def test_accuracy_conj_physical__inplace(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())

    ref_out = ref_inp.conj_physical_()
    with flag_gems.use_gems():
        res_out = inp.conj_physical_()
