...
def test_accuracy_LeakyReLU(shape, dtype):
    ...
    with flag_gems.use_gems():
        res_out = torch.nn.functional.leaky_relu(res_inp)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.inplace
@pytest.mark.LeakyReLU_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_LeakyReLU_(shape, dtype):
    res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(res_inp.clone(), True)

    ref_out = torch.nn.functional.leaky_relu_(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.nn.functional.leaky_relu_(res_inp)
