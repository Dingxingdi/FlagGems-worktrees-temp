...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_erfinv(shape, dtype):
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_erfinv_out(shape, dtype):
    x = torch.empty(shape, dtype=dtype, device=flag_gems.device).uniform_(-0.9, 0.9)
    ref_x = to_reference(x)
    if dtype in (torch.float16, torch.bfloat16):
        out_ref = torch.empty_like(ref_x, dtype=torch.float32)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_erfinv_(shape, dtype):
    x = torch.empty(shape, dtype=dtype, device=flag_gems.device).uniform_(-0.9, 0.9)
    ref_x = to_reference(x.clone())
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.ops.aten.special_erfinv_(ref_x.float()).to(dtype)
    else:
        ref_out = torch.ops.aten.special_erfinv_(ref_x)
    with flag_gems.use_gems():
        act_out = torch.ops.aten.special_erfinv_(x)
    gems_assert_close(act_out, ref_out, dtype)
