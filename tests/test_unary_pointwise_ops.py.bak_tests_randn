...
def test_accuracy_quantize(shape, dtype):
    # Test quantize operator against torch.quantize_per_tensor reference
    # Note: PyTorch's quantize_per_tensor only supports float32
    ...
    # Reference: use torch.quantize_per_tensor and then dequantize
    ref_quantized = torch.quantize_per_tensor(ref_inp, scale, zero_point, torch.quint8)
    ref_out = ref_quantized.dequantize()

    # GEMS implementation: directly call our quantize function
    from flag_gems.ops.quantize import quantize
    with flag_gems.use_gems():
        res_out = quantize(inp, scale, zero_point)

    # Compare the quantized values (not dequantized) since our implementation
    # returns the raw quantized values as float
    # For fair comparison, we compare with the reference's int_repr dequantized
    # Use a more relaxed tolerance for quantization (atol=2 to handle integer rounding differences)
    ref_quantized_values = ref_quantized.int_repr().float()
    gems_assert_close(res_out, ref_quantized_values, dtype, atol=2)


@pytest.mark.inplace
@pytest.mark.quantize_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_quantize_(shape, dtype):
    # Test in-place quantize operator
    # Note: PyTorch's quantize_per_tensor only supports float32
    scale = 0.1
    zero_point = 10
    ...
    # GEMS implementation: in-place version
    from flag_gems.ops.quantize import quantize_
    with flag_gems.use_gems():
        res_out = quantize_(inp, scale, zero_point)
