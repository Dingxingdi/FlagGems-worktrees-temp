...
def test_accuracy_cudnnActivateionbwd(shape, dtype):
    # cudnnActivateionbwd is the backward of ReLU activation
    # Compute reference using autograd
    res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=True)
    ref_inp = res_inp.clone().detach().requires_grad_(True)
    ...
    # Compute using flag_gems
    grad_output = torch.ones_like(res_inp)
    with flag_gems.use_gems():
        res_grad = flag_gems.cudnnActivateionbwd(grad_output, res_inp)
