...
def test_accuracy_special_bessel_j0(shape, dtype):
    ...
    ref_inp = to_reference(inp)
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = torch.special.bessel_j0(ref_inp.float())
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_bessel_j0_(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())
    if dtype in (torch.float16, torch.bfloat16):
        ref_inp_f = ref_inp.float()
        ref_out = ref_inp_f.special_bessel_j0_()
        ref_out = ref_out.to(dtype)
    else:
        ref_out = ref_inp.special_bessel_j0_()
    with flag_gems.use_gems():
        res_out = inp.special_bessel_j0_()
    gems_assert_close(res_out, ref_out, dtype)
