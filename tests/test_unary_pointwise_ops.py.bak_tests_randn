...
def test_accuracy_view_as_real(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.view_as_real(ref_inp)
    with flag_gems.use_gems():
        res_out = torch.view_as_real(inp)

    # Output is float32 for complex64, float16 for complex32
