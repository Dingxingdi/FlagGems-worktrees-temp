...
def test_accuracy_special_ndtri(shape, dtype):
    # ndtri is defined for input in (0, 1), generate values in that range
    inp = torch.rand(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
    # PyTorch's ndtri only supports float32, convert to float32 for reference
    if dtype in (torch.float16, torch.bfloat16):
...
@pytest.mark.inplace
@pytest.mark.special_ndtri_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_ndtri_(shape, dtype):
    inp = torch.rand(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())
    # PyTorch's ndtri only supports float32, convert to float32 for reference
    if dtype in (torch.float16, torch.bfloat16):
        ref_out = ref_inp.float().special_ndtri_().to(dtype)
    else:
        ref_out = ref_inp.special_ndtri_()
    with flag_gems.use_gems():
        res_out = inp.special_ndtri_()
