...
@pytest.mark.Fused_SwiGLU
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Fused_SwiGLU(shape, dtype):
    if flag_gems.vendor_name == "tsingmicro":
        res_inp = torch.randn(shape, dtype=dtype, device="cpu")
    else:
        res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    for dim in range(len(shape)):
        if shape[dim] % 2 != 0:
            continue
        # Reference: split along dim, apply silu to first half, multiply with second half
        a, b = torch.chunk(ref_inp, 2, dim=dim)
        ref_out = a * torch.nn.functional.silu(b)
        with flag_gems.use_gems():
            if flag_gems.vendor_name == "tsingmicro":
                res_out = flag_gems.Fused_SwiGLU(
                    res_inp.to(device=flag_gems.device), dim=dim
                )
            else:
                res_out = flag_gems.Fused_SwiGLU(res_inp, dim=dim)
        gems_assert_close(res_out, ref_out, dtype)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Fused_SwiGLU_backward(shape, dtype):
    res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    for dim in range(len(shape)):
        if shape[dim] == 0 or shape[dim] % 2 != 0:
            continue
        # Compute reference backward using autograd (without upcast to keep dtype)
        ref_inp = res_inp.clone()
        ref_inp.requires_grad = True
        # Fused_SwiGLU: split along dim, apply silu to first half, multiply with second half
        a, b = torch.chunk(ref_inp, 2, dim=dim)
        ref_out = a * torch.nn.functional.silu(b)
        ref_out.sum().backward()
        ref_in_grad = ref_inp.grad.clone()

        # Compute FlagGems backward using gradient from forward pass
        # We compute forward pass to get output, then use ones_like for grad_output
        res_inp2 = res_inp.clone()
        res_inp2.requires_grad = True
        with flag_gems.use_gems():
            res_out = flag_gems.Fused_SwiGLU(res_inp2, dim=dim)
            # Use ones_like as gradient (equivalent to sum().backward())
            grad_output = torch.ones_like(res_out)
            res_in_grad = flag_gems.Fused_SwiGLU_backward(grad_output, res_inp2, dim=dim)
