...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rrelu_with_noise(shape, dtype):
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ...
    ref_out = torch.ops.aten.rrelu_with_noise(ref_inp, ref_noise, training=False)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.rrelu_with_noise(inp, noise, training=False)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rrelu_with_noise_training(shape, dtype):
    # Training mode uses random values, so we cannot directly compare with PyTorch.
    # Instead, we verify that the kernel runs without error.
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    noise = torch.zeros_like(inp)

    with flag_gems.use_gems():
        res_out = torch.ops.aten.rrelu_with_noise(inp, noise, training=True)

    # Basic sanity checks
    assert res_out.shape == inp.shape
    assert res_out.dtype == dtype
    # For non-negative values, output should equal input
    positive_mask = inp >= 0
    torch.testing.assert_close(res_out[positive_mask], inp[positive_mask])
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_rrelu_with_noise_(shape, dtype):
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    noise = torch.zeros_like(inp)
    ref_inp = to_reference(inp.clone())

    ref_out = torch.ops.aten.rrelu_with_noise_(ref_inp, to_reference(noise), training=False)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.rrelu_with_noise_(inp, noise, training=False)
