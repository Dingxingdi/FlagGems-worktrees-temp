...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Constant(shape, dtype):
    value = 5.0
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)

    ref_out = torch.full_like(ref_inp, value)
    with flag_gems.use_gems():
        res_out = flag_gems.ops.constant(inp, value)

    gems_assert_equal(res_out, ref_out)


@pytest.mark.inplace
@pytest.mark.Constant_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_Constant_(shape, dtype):
    value = 7.0
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())

    ref_out = ref_inp.fill_(value)
    with flag_gems.use_gems():
        res_out = flag_gems.ops.constant_(inp, value)
