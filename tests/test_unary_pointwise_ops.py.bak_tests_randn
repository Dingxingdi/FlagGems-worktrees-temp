...
# Test shapes for resize (source shape -> target shape with same total elements)
RESIZE_SHAPES = [
    ((6,), (2, 3)),
    ((12,), (3, 4)),
    ((24,), (4, 6)),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_resize(src_shape, dst_shape, dtype):
...
# Note: resize_ (in-place) is difficult to implement due to recursion issues
# The in-place resize operation triggers dispatch to our own implementation
# Skipping resize_ tests for now
# @pytest.mark.resize_
# @pytest.mark.parametrize("src_shape, dst_shape", RESIZE_SHAPES)
# @pytest.mark.parametrize("dtype", FLOAT_DTYPES)
# def test_accuracy_resize_(src_shape, dst_shape, dtype):
#     inp = torch.randn(src_shape, dtype=dtype, device=flag_gems.device)
#     ref_inp = to_reference(inp.clone())
#
#     ref_out = ref_inp.resize_(*dst_shape)
#     with flag_gems.use_gems():
#         res_out = inp.resize_(*dst_shape)
#
#     gems_assert_close(res_out, ref_out, dtype)
