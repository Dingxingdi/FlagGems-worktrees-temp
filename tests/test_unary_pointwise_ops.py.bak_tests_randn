...
@pytest.mark.cudnnActivateion
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnnActivateion(shape, dtype):
    # cudnnActivateion is implemented as ReLU-like: max(0, x)
    # Using torch.relu as reference since there's no standard torch.cudnnActivateion
    res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(res_inp, True)

    ref_out = torch.relu(ref_inp)
    with flag_gems.use_gems():
        res_out = flag_gems.cudnnActivateion(res_inp)

    gems_assert_close(res_out, ref_out, dtype)


@pytest.mark.inplace
@pytest.mark.cudnnActivateion_
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_cudnnActivateion_(shape, dtype):
    # cudnnActivateion_ is the in-place version: max(0, x) applied in-place
    res_inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(res_inp.clone(), True)

    ref_out = torch.relu_(ref_inp)
    with flag_gems.use_gems():
        res_out = flag_gems.cudnnActivateion_(res_inp)
