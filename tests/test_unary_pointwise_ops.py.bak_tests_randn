...
# Test p values for special_multigammaln
...
def _get_multigammaln_input(shape, dtype, device, p):
    ...
    return torch.rand(shape, dtype=dtype, device=device) + min_val
...
def test_accuracy_special_multigammaln(shape, dtype, p):
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("p", MULTIGAMMALN_P_VALUES)
def test_accuracy_special_multigammaln_out(shape, dtype, p):
    x = _get_multigammaln_input(shape, dtype, flag_gems.device, p)
    ref_x = to_reference(x)
    if dtype in (torch.float16, torch.bfloat16):
        out_ref = torch.empty_like(ref_x, dtype=torch.float32)
