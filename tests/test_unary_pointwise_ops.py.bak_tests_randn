...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_round(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
# Note: torch.special.round_ does not exist as a native PyTorch method.
# Testing in-place behavior via out= parameter
@pytest.mark.inplace
@pytest.mark.special_round_out
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_special_round_out(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp.clone())
    out = torch.empty_like(inp)
    ref_out = torch.empty_like(ref_inp)

    torch.special.round(ref_inp, out=ref_out)
    with flag_gems.use_gems():
        torch.special.round(inp, out=out)
