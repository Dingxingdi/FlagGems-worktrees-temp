...
# as_strided_copy test configurations
AS_STRIDED_COPY_CONFIGS = [
    # (input_shape, size, stride, storage_offset)
    ((3, 4), (3, 4), (4, 1), None),  # same shape/stride (identity-like)
    ((3, 4), (4, 3), (1, 4), None),  # transpose view
    ((3, 4), (2, 3), (4, 1), 1),  # with storage_offset
    ((12,), (3, 4), (1, 1), 0),  # 1D to 2D view
    ((3, 4), (12,), (1,), None),  # flatten
]
...
@pytest.mark.parametrize("config", AS_STRIDED_COPY_CONFIGS)
def test_accuracy_as_strided_copy(shape, dtype, config):
    inp_shape, size, stride, storage_offset = config
    # Generate input tensor with the base shape
    if dtype in ALL_FLOAT_DTYPES:
        inp = torch.randn(inp_shape, dtype=dtype, device=flag_gems.device)
    else:
        inp = torch.randint(-1000, 1000, inp_shape, device=flag_gems.device).to(dtype)
    ref_inp = to_reference(inp)

    ref_out = torch.as_strided_copy(ref_inp, size, stride, storage_offset)
    with flag_gems.use_gems():
        res_out = torch.as_strided_copy(inp, size, stride, storage_offset)
