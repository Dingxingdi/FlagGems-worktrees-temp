...
# Roll operation test parameters
ROLL_SHIFTS = [1, 2, -1, -2]
ROLL_DIMS_SINGLE = [(0,), (1,), (2,), (-1,)]
ROLL_DIMS_MULTI = [(0, 1), (1, 2)]


@pytest.mark.roll
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shifts", ROLL_SHIFTS)
@pytest.mark.parametrize("dims", ROLL_DIMS_SINGLE)
def test_accuracy_roll(shape, dtype, shifts, dims):
    # Filter out invalid dim/shape combinations
    max_ndim = len(shape)
    # Check that all dimensions in dims are valid for this shape
    invalid_dims = [d for d in dims if d < -max_ndim or d >= max_ndim]
    if invalid_dims:
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shifts", [(1, 2), (-1, 1), (2, -2)])
@pytest.mark.parametrize("dims", ROLL_DIMS_MULTI)
def test_accuracy_roll_multi(shape, dtype, shifts, dims):
    """Test multi-dimensional roll with matching shifts tuple."""
    # Filter out invalid dim/shape combinations
    max_ndim = len(shape)
    # Check that all dimensions in dims are valid for this shape
    invalid_dims = [d for d in dims if d < -max_ndim or d >= max_ndim]
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_roll_flattened(shape, dtype):
    """Test roll without specifying dims (flattened roll)."""
    shifts = 3
