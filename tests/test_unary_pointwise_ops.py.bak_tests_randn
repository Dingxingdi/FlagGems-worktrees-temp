...
def test_accuracy_special_scaled_modified_bessel_k1(shape, dtype):
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_scaled_modified_bessel_k1_out(shape, dtype):
    inp = torch.rand(shape, dtype=dtype, device=flag_gems.device) + 0.1
    ref_inp = to_reference(inp)
    ...
    gems_assert_close(out, out_ref, dtype)
