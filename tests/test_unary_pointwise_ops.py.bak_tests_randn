...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nan_to_num(shape, dtype):
    # Create input with NaN, Inf, -Inf values
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Add some NaN, Inf, -Inf values
    if inp.numel() > 0:
    ...
    ref_out = torch.nan_to_num_(ref_inp.clone())
    with flag_gems.use_gems():
        res_out = torch.nan_to_num_(inp)
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_nan_to_num_(shape, dtype):
    # Create input with NaN, Inf, -Inf values
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    # Add some NaN, Inf, -Inf values
    if inp.numel() > 0:
    ...
    ref_out = ref_inp.nan_to_num_()
    with flag_gems.use_gems():
        res_out = inp.nan_to_num_()
