...
def test_accuracy__functional_assert_async(shape, dtype):
    torch.manual_seed(0)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    dep_token = torch.zeros(1, dtype=dtype, device=flag_gems.device)
    # Since aten::_functional_assert_async is not available on CUDA backend,
    # we directly call flag_gems implementation and verify it returns a tensor
    # with the same data as input (pass-through behavior)
    res_out = flag_gems._functional_assert_async(inp, "test assert", dep_token)
    # Verify the output is a tensor with expected properties
    assert isinstance(res_out, torch.Tensor)
    assert res_out.device == inp.device
    # Verify pass-through: output should match input
    assert torch.allclose(res_out, inp, atol=1e-5)
